...
public sealed class StockMovementBranchOwnershipIntegrationTests : IAsyncLifetime
{
    ...
    public async Task Transferencia_inter_sucursal_cada_movimiento_toma_el_BranchId_de_su_propia_bodega_no_el_del_operador()
    {
        ...
        exitMovement
            .BranchId.Should()
            .Be(_branchAId, "el movimiento de salida pertenece a la bodega origen");
        entryMovement
            .BranchId.Should()
            .Be(
                _branchBId,
                "el movimiento de entrada pertenece a la bodega destino, no a la sucursal del operador"
            );
