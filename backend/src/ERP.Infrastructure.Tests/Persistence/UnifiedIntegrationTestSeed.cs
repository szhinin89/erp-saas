...
internal static class UnifiedIntegrationTestSeed
{
    ...
    {
        if (_seeded && await ctx.Tenants.AnyAsync(t => t.Id == UnifiedDocumentSyncIntegrationTests.TenantId, ct))
            return;

        var now = DateTime.UtcNow;
        var tid = UnifiedDocumentSyncIntegrationTests.TenantId;
        ...
        await ctx.Database.ExecuteSqlRawAsync(
            """
            INSERT INTO tenants (id, name, slug, is_active, password_reset_mode, display_order, priority, electronic_billing_trial_enabled, "TenantId", created_at, created_by)
            VALUES ({0}, 'Test Tenant', 'test-tenant', true, 0, 0, 0, false, {0}, {1}, {2})
            ON CONFLICT (id) DO NOTHING;
            INSERT INTO company (id, tenant_id, ruc, legal_name, main_address, country_code, created_at, updated_at)
            ...
            ON CONFLICT (id) DO NOTHING;
            INSERT INTO branches (id, name, address, is_main_branch, tenant_id, created_at, created_by, is_active)
            VALUES ({5}, 'Sucursal Test', 'Quito', true, {0}, {1}, {2}, true)
            ON CONFLICT (id) DO NOTHING;
            INSERT INTO warehouse (id, establishment_id, name, tenant_id, created_at, created_by, is_active)
            VALUES ({6}, {4}, 'Bodega Test', {0}, {1}, {2}, true)
            ON CONFLICT (id) DO NOTHING;
            INSERT INTO customers (id, identification_type, identification_number, legal_name, payment_days, tenant_id, created_at, created_by, is_active)
