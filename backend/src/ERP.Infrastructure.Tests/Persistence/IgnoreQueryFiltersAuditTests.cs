...
public sealed class IgnoreQueryFiltersAuditTests
{
    ...
    )
    {
        ...
        "src/ERP.Infrastructure/Seeding/PaymentMethodSriMappingBackfillService.cs", // SALES-PAYMENT-METHOD-SRI-MAPPING-BACKFILL-ARCHITECTURE-GUARD-01: backfill de una sola vez (dotnet run -- backfill-payment-method-sri-mapping, no HTTP, no IGlobalBootstrapStep) para PaymentMethod.SriPaymentMethodCode en tenants creados antes de SALES-PAYMENT-METHOD-SRI-MAPPING-SSOT-01 — mismo motivo que MasterDataClassificationBackfillService/DocumentFlowPolicyBackfillService: corre fuera de cualquier contexto de tenant HTTP ambiente, necesita ver TODAS las filas de TODOS los tenants para decidir cuáles siguen con SriPaymentMethodCode null. Cada fila conserva su propio TenantId (nunca se reasigna entre tenants); PaymentMethod.BackfillSriPaymentMethodCode() es un no-op si el campo ya tiene valor (nunca pisa un mapeo manual) y el código sugerido se valida contra ISriCatalogLookupRepository.GetActivePaymentMethodsAsync antes de escribirse.
        "src/ERP.Infrastructure/Seeding/PaymentMethodAccountBackfillService.cs", // SALES-TRANSFER-ACCOUNTING-CASH-VS-BANK-01: hallazgo pre-existente (no relacionado a BANK-CATALOG-01) — backfill de una sola vez (dotnet run -- backfill-payment-method-account) para companies creadas antes de ese ticket; faltaba en esta allowlist desde que se introdujo. Mismo motivo que PaymentMethodSriMappingBackfillService: corre fuera de cualquier contexto de tenant HTTP ambiente.
        "src/ERP.Infrastructure/Seeding/BankCatalogSeeder.cs", // BANK-CATALOG-01: reutilizado por bootstrap step (request-scoped) y backfill (multi-tenant, sin contexto HTTP ambiente) — mismo motivo que MasterDataClassificationSeeder; filtro explícito TenantId+CountryCode, fail-closed.
        "src/ERP.Infrastructure/Seeding/BankCatalogBackfillService.cs", // BANK-CATALOG-01: itera todos los TenantId existentes para el backfill de tenants ya creados — mismo motivo que MasterDataClassificationBackfillService/PaymentMethodSriMappingBackfillService. Invocación: dotnet run -- backfill-bank-catalog (no HTTP, no IGlobalBootstrapStep).
    };
