...
public sealed class IgnoreQueryFiltersAuditTests
{
    ...
    )
    {
        ...
        "src/ERP.Infrastructure/Seeding/CashRegisterAccountingAccountBackfillService.cs", // SALES-COLLECTION-ACCOUNT-SSOT-CLEANUP-01: backfill de una sola vez (dotnet run -- backfill-cash-register-accounting-account, no HTTP, no IGlobalBootstrapStep) para CashRegister.AccountingAccountId en companies creadas antes de este ticket — mismo motivo que PaymentMethodSriMappingBackfillService: corre fuera de cualquier contexto de tenant HTTP ambiente.
