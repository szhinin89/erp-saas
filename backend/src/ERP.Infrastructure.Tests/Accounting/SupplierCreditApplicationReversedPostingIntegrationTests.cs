...
/// <summary>
/// PURCHASE-SUPPLIER-CREDIT-APPLICATION-REVERSED-POSTING-RULE-01 — suite de integración
/// (PostgreSQL 16 real vía Testcontainers) para el consumidor real del reverso contable del hecho
/// de §19.1: SupplierCredit.ReverseApplication() → SupplierCreditApplicationReversedEvent →
/// SupplierCreditApplicationReversedPostingTranslator → IPostingEngine → JournalEntry Posted. Mismo
/// patrón exacto que <see cref="SupplierCreditAppliedPostingIntegrationTests"/> — deliberadamente
/// NO comparte fixture con esa suite. Requiere Docker.
/// </summary>
[Trait("Category", "PostgreSql")]
public sealed class SupplierCreditApplicationReversedPostingIntegrationTests : IAsyncLifetime
{
    private readonly PostgreSqlContainer _postgres = new PostgreSqlBuilder()
        .WithImage("postgres:16-alpine")
        .WithDatabase("erp_supplier_credit_application_reversed_posting_test")
        .WithUsername("erp")
    ...
    /// <summary>Mismo mecanismo que SupplierCreditAppliedPostingIntegrationTests: contenedor DI real, mismo ErpDbContext para los traductores y para el caller.</summary>
    ...
    )
    {
        ...
        // SupplierCreditAuditHandler también escucha SupplierCreditApplicationReversedEvent
        // (ADR-022, Entity Audit) — mismo evento, otro consumidor real registrado en producción; sin
        // estos dos el MediatR.Publish falla en tiempo de resolución de DI, no en el traductor bajo
        // prueba.
        services.AddScoped<
    ...
    /// <summary>
    /// Siembra las DOS PostingRule reales de MinimalPostingRules — "Purchases"/"SupplierCreditApplied"
    ...
    /// es exclusivamente el de ApplyToPayable/ReverseApplication, no el de la devolución.
    /// </summary>
    ...
    [Fact]
    public async Task Reversar_aplicacion_de_SupplierCredit_genera_JournalEntry_reverso_balanceado()
    {
        ...
        var payable = BuildPayable(issueDate, "001-001-000000021", total: 100m);
        ...
        totalDebit.Should().Be(totalCredit, because: "el reverso también debe quedar balanceado");
        totalDebit.Should().Be(30m);

        // Verifica la inversión Debe↔Haber real (no solo el total): el mismo monto que fue Debe de
        // CxP al aplicar debe volver a ser Haber de CxP al reversar.
        var debitLine = reversalEntry.Lines.Should().ContainSingle(l => l.Debit > 0).Which;
        debitLine.AccountId.Should().Be(supplierCreditAccountId);
        debitLine.Debit.Should().Be(30m);

        var creditLine = reversalEntry.Lines.Should().ContainSingle(l => l.Credit > 0).Which;
        creditLine.AccountId.Should().Be(payableAccountId);
    ...
    [Fact]
    public async Task Reversar_el_mismo_movimiento_dos_veces_no_duplica_el_JournalEntry_reverso()
    {
        // Idempotencia del Posting Engine (SourceModule, FactType, SourceEventId) — publicar el
        // mismo SupplierCreditApplicationReversedEvent (mismo movimiento reverso) una segunda vez
