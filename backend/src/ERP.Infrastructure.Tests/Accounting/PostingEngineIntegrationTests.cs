...
public sealed class PostingEngineIntegrationTests : IAsyncLifetime
{
    ...
    public async Task PostAsync_persiste_el_JournalEntry_ya_publicado_Posted()
    {
        ...
        await using var verifyDb = CreateContext();
        var loaded = await verifyDb.JournalEntries.FirstAsync(x => x.SourceEventId == fact.SourceEventId);

        loaded.Status.Should().Be(JournalEntryStatus.Posted, because: "Fase 5.2: el Posting Engine publica el asiento antes de persistirlo");
    ...
    public async Task PostAsync_asigna_EntryNumber_y_lo_persiste_junto_con_el_JournalEntry()
    {
        ...
        loaded.EntryNumber.Should().Be(1, because: "es el primer asiento del ejercicio fiscal 2026 para esta empresa");

        var sequence = await verifyDb.JournalEntrySequences
            .FirstAsync(s => s.CompanyId == _companyId && s.FiscalYear == 2026);
    ...
    {
        var factA = new PostingFact(_tenantId, _companyId, "Sales", "InvoiceIssued", Guid.NewGuid(), new DateOnly(2026, 7, 15), 100m, 15m, 0m, 0m, 115m);
        var factB = new PostingFact(_tenantId, _companyId, "Sales", "InvoiceIssued", Guid.NewGuid(), new DateOnly(2026, 7, 16), 200m, 30m, 0m, 0m, 230m);

        await using (var seedDb = CreateContext())
        {
            await SeedRuleAndPeriodAsync(seedDb, factA.SourceModule, factA.FactType, factA.EntryDate);
        }
        ...
        await using var verifyDb = CreateContext();
        var entryA = await verifyDb.JournalEntries.FirstAsync(x => x.SourceEventId == factA.SourceEventId);
        var entryB = await verifyDb.JournalEntries.FirstAsync(x => x.SourceEventId == factB.SourceEventId);

        entryA.EntryNumber.Should().Be(1);
        entryB.EntryNumber.Should().Be(2, because: "el segundo asiento del mismo (CompanyId, FiscalYear) debe continuar el correlativo");
    }
    ...
    public async Task Indice_unico_company_fiscal_year_entry_number_rechaza_duplicados()
    {
        ...
        var entryA = JournalEntry.Create(
            _tenantId, _companyId, new DateOnly(2026, 7, 15), period.Id, 2026,
            "Sales", "InvoiceIssued", Guid.NewGuid(), "Asiento A", _createdBy);
        ...
        var entryB = JournalEntry.Create(
            _tenantId, _companyId, new DateOnly(2026, 7, 16), period.Id, 2026,
            "Sales", "InvoiceIssued", Guid.NewGuid(), "Asiento B", _createdBy);
