...
{
    private static PriceList CreateList(PricingRuleType? ruleType = null, decimal? ruleValue = null) =>
        PriceList.Create(
            Guid.NewGuid(), Guid.NewGuid(), "MAYORISTA", "Mayorista", "USD",
            isDefault: false, createdBy: Guid.NewGuid(), ruleType: ruleType, ruleValue: ruleValue);
    ...
    {
        var act = () => PriceList.Create(
            Guid.NewGuid(), Guid.NewGuid(), "MAYORISTA", "Mayorista", "USD",
            false, Guid.NewGuid(), ruleType: null, ruleValue: 10m);
        act.Should().Throw<ArgumentException>();
    }
    ...
    public void IsValidOn_fuera_del_rango_de_vigencia_es_falso()
    {
        var list = PriceList.Create(
            Guid.NewGuid(), Guid.NewGuid(), "PROMO", "Promo", "USD", false, Guid.NewGuid(),
            validFrom: new DateOnly(2020, 1, 1), validUntil: new DateOnly(2020, 1, 31));
