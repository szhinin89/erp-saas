...
public sealed record RidePaymentInfo
{
    ...
    public static RidePaymentInfo Create(string paymentMethodCode, decimal amount, int? term = null, string? timeUnit = null)
    {
        if (string.IsNullOrWhiteSpace(paymentMethodCode))
            throw new ArgumentException("El código de forma de pago es obligatorio.", nameof(paymentMethodCode));
        if (amount < 0)
            throw new ArgumentException("El monto pagado no puede ser negativo.", nameof(amount));
        if (term is <= 0)
            throw new ArgumentException("El plazo debe ser mayor a cero cuando se especifica.", nameof(term));

        return new RidePaymentInfo(
            paymentMethodCode.Trim(), amount, term,
