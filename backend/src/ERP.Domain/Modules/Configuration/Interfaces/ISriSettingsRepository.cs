...
{
    Task<SriSettings?> GetByCompanyIdAsync(Guid companyId, CancellationToken cancellationToken = default);
    /// <summary>Carga SriSettings con SELECT FOR UPDATE. Llamar dentro de una transacción activa.</summary>
    Task<SriSettings?> GetByCompanyIdForUpdateAsync(Guid companyId, CancellationToken cancellationToken = default);
