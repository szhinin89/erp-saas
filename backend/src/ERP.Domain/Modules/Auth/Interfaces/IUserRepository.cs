...
{
    Task<User?> GetByIdAsync(Guid id, Guid subscriberId, CancellationToken ct = default);
    Task<User?> GetByIdSystemAsync(Guid id, CancellationToken ct = default);
    Task<User?> GetByEmailAsync(string email, Guid subscriberId, CancellationToken ct = default);
    Task<User?> GetByEmailSystemAsync(string email, Guid subscriberId, CancellationToken ct = default);
    Task<IReadOnlyList<User>> GetAllByTenantAsync(Guid subscriberId, CancellationToken ct = default);
    ...
    Task<bool> ExistsAsync(string email, Guid subscriberId, CancellationToken ct = default);
    ...
    /// Usado para recuperación de contraseña sin <c>subscriberId</c> en el formulario.
    /// </summary>
