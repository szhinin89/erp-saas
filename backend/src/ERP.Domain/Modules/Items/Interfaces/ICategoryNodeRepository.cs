...
public interface ICategoryNodeRepository
{
    ...
    Task<IReadOnlyList<ItemCategoryNode>> GetAllAsync(Guid tenantId, bool includeInactive = false, CancellationToken ct = default);
    Task<IReadOnlyList<ItemCategoryNode>> GetChildrenAsync(Guid parentId, CancellationToken ct = default);
    Task<IReadOnlyList<ItemCategoryNode>> GetDescendantsByPathAsync(Guid tenantId, string pathPrefix, CancellationToken ct = default);
    Task<bool> CodeExistsAsync(string code, Guid tenantId, Guid? excludeId = null, CancellationToken ct = default);
