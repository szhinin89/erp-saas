...
public interface ISerialRepository
{
    ...
    Task<SerialNumber?> GetBySerialAsync(
        Guid itemId, string serial, CancellationToken cancellationToken = default);
    ...
    Task AddRangeAsync(IEnumerable<SerialNumber> serials, CancellationToken cancellationToken = default);
