...
/// <summary>
/// Catálogo persistido de clasificación de BusinessPartner (CLASS-BP-CATALOGS-01): El segmento de proveedor.
/// Tenant+company-scoped, con bootstrap por empresa (CreateSystemSeeded) y backfill idempotente
/// para empresas existentes. Reemplaza el HashSet fijo SupplierClassificationConfig.ValidSegments.
/// Solo lectura desde el frontend en este bloque — CRUD administrativo queda fuera de alcance.
/// </summary>
public sealed class SupplierSegment : MasterEntity, ITenantScopedEntity, ICompanyOperationalEntity, IClassificationCatalogEntity
{
    ...
    )
    {
        ...
        var entity = new SupplierSegment
        {
            TenantId = tenantId,
            CompanyId = companyId,
            Code = code.Trim(),
    ...
    /// </summary>
    public static SupplierSegment CreateSystemSeeded(
        Guid tenantId,
        Guid companyId,
        string code,
        string name,
    ...
    {
        this.EnsureEditable("El segmento de proveedor", "actualizarse");
        if (string.IsNullOrWhiteSpace(name))
            throw new ArgumentException("El nombre es obligatorio.", nameof(name));
        if (name.Trim().Length > NameMaxLength)
            throw new ArgumentException(
    ...
    {
        this.EnsureEditable("El segmento de proveedor", "deshabilitarse");
