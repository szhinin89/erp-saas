...
/// <summary>
/// Catálogo persistido de clasificación de BusinessPartner (CLASS-BP-CATALOGS-01): El formato de factura preferido.
/// Tenant+company-scoped, con bootstrap por empresa (CreateSystemSeeded) y backfill idempotente
/// para empresas existentes. Reemplaza el HashSet fijo CustomerRoleConfig.ValidInvoiceFormats.
/// Solo lectura desde el frontend en este bloque — CRUD administrativo queda fuera de alcance.
/// </summary>
public sealed class CustomerInvoiceFormat : MasterEntity, ITenantScopedEntity, ICompanyOperationalEntity, IClassificationCatalogEntity
{
    ...
    private CustomerInvoiceFormat() { }

    public static CustomerInvoiceFormat Create(
        Guid tenantId,
        Guid companyId,
        string code,
        string name,
    ...
    )
    {
        ...
        var entity = new CustomerInvoiceFormat
        {
            TenantId = tenantId,
            CompanyId = companyId,
            Code = code.Trim(),
    ...
    /// </summary>
    public static CustomerInvoiceFormat CreateSystemSeeded(
        Guid tenantId,
        Guid companyId,
        string code,
        string name,
    ...
    {
        this.EnsureEditable("El formato de factura preferido", "actualizarse");
        if (string.IsNullOrWhiteSpace(name))
            throw new ArgumentException("El nombre es obligatorio.", nameof(name));
        if (name.Trim().Length > NameMaxLength)
            throw new ArgumentException(
    ...
    {
        this.EnsureEditable("El formato de factura preferido", "deshabilitarse");
