...
/// <summary>
/// Catálogo persistido de clasificación de BusinessPartner (CLASS-BP-CATALOGS-01): El tipo de bien principal.
/// Tenant+company-scoped, con bootstrap por empresa (CreateSystemSeeded) y backfill idempotente
/// para empresas existentes. Reemplaza el HashSet fijo SupplierClassificationConfig.ValidGoodTypes.
/// Solo lectura desde el frontend en este bloque — CRUD administrativo queda fuera de alcance.
/// </summary>
public sealed class PrimaryGoodType : MasterEntity, ITenantScopedEntity, ICompanyOperationalEntity, IClassificationCatalogEntity
{
    ...
    private PrimaryGoodType() { }

    public static PrimaryGoodType Create(
        Guid tenantId,
        Guid companyId,
        string code,
        string name,
    ...
    )
    {
        ...
        var entity = new PrimaryGoodType
        {
            TenantId = tenantId,
            CompanyId = companyId,
            Code = code.Trim(),
    ...
    /// </summary>
    public static PrimaryGoodType CreateSystemSeeded(
        Guid tenantId,
        Guid companyId,
        string code,
        string name,
    ...
    {
        this.EnsureEditable("El tipo de bien principal", "actualizarse");
        if (string.IsNullOrWhiteSpace(name))
            throw new ArgumentException("El nombre es obligatorio.", nameof(name));
        if (name.Trim().Length > NameMaxLength)
            throw new ArgumentException(
    ...
    {
        this.EnsureEditable("El tipo de bien principal", "deshabilitarse");
