...
/// <summary>
/// Catálogo persistido de clasificación de BusinessPartner (CLASS-BP-CATALOGS-01): El nivel de fidelización.
...
public sealed class LoyaltyTier : MasterEntity, ITenantScopedEntity, ICompanyOperationalEntity, IClassificationCatalogEntity
{
    ...
    /// </summary>
    public static LoyaltyTier CreateSystemSeeded(
        Guid tenantId,
        Guid companyId,
        string code,
        string name,
    ...
    {
        this.EnsureEditable("El nivel de fidelización", "actualizarse");
        if (string.IsNullOrWhiteSpace(name))
            throw new ArgumentException("El nombre es obligatorio.", nameof(name));
        if (name.Trim().Length > NameMaxLength)
            throw new ArgumentException(
    ...
    {
        this.EnsureEditable("El nivel de fidelización", "deshabilitarse");
