...
/// <summary>
/// Catálogo persistido de clasificación de BusinessPartner (CLASS-BP-CATALOGS-01): La categoría de cliente.
/// Tenant+company-scoped, con bootstrap por empresa (CreateSystemSeeded) y backfill idempotente
/// para empresas existentes. Reemplaza el HashSet fijo CustomerRoleConfig.ValidCategories.
/// Solo lectura desde el frontend en este bloque — CRUD administrativo queda fuera de alcance.
/// </summary>
public sealed class CustomerCategory : MasterEntity, ITenantScopedEntity, ICompanyOperationalEntity, IClassificationCatalogEntity
{
    ...
    /// </summary>
    public static CustomerCategory CreateSystemSeeded(
        Guid tenantId,
        Guid companyId,
        string code,
        string name,
    ...
    {
        this.EnsureEditable("La categoría de cliente", "actualizarse");
        if (string.IsNullOrWhiteSpace(name))
            throw new ArgumentException("El nombre es obligatorio.", nameof(name));
        if (name.Trim().Length > NameMaxLength)
            throw new ArgumentException(
    ...
    {
        this.EnsureEditable("La categoría de cliente", "deshabilitarse");
