...
/// <summary>
/// Catálogo persistido de clasificación de BusinessPartner (CLASS-BP-CATALOGS-01): El riesgo de proveedor.
/// Tenant+company-scoped, con bootstrap por empresa (CreateSystemSeeded) y backfill idempotente
/// para empresas existentes. Reemplaza el HashSet fijo SupplierClassificationConfig.ValidRisks.
/// Solo lectura desde el frontend en este bloque — CRUD administrativo queda fuera de alcance.
/// </summary>
public sealed class SupplierRisk : MasterEntity, ITenantScopedEntity, ICompanyOperationalEntity, IClassificationCatalogEntity
{
    ...
    /// </summary>
    public static SupplierRisk CreateSystemSeeded(
        Guid tenantId,
        Guid companyId,
        string code,
        string name,
    ...
    {
        this.EnsureEditable("El riesgo de proveedor", "actualizarse");
        if (string.IsNullOrWhiteSpace(name))
            throw new ArgumentException("El nombre es obligatorio.", nameof(name));
        if (name.Trim().Length > NameMaxLength)
            throw new ArgumentException(
    ...
    {
        this.EnsureEditable("El riesgo de proveedor", "deshabilitarse");
