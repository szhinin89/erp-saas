...
/// </summary>
public sealed class PurchaseOrdersEndToEndTests
{
    [Fact]
    ...
    public async Task Enviar_y_Aprobar_cambia_estado_correctamente()
    {
        ...
        var enviar = await mediator.Send(
            new SendPurchaseOrderCommand(crear.Value!.Id), CancellationToken.None);
        enviar.IsSuccess.Should().BeTrue(enviar.Error);
        enviar.Value!.Status.Should().Be("Sent");

        var aprobar = await mediator.Send(
            new ApprovePurchaseOrderCommand(crear.Value.Id), CancellationToken.None);
    ...
    public async Task Cancelar_OC_en_Borrador_cambia_estado_a_Cancelada()
    {
        ...
        var cancelar = await mediator.Send(
            new CancelPurchaseOrderCommand(crear.Value!.Id), CancellationToken.None);
    ...
    public async Task Vincular_factura_completa_cierra_la_OC()
    {
        ...
        var crear = await mediator.Send(new CreatePurchaseOrderCommand(
            ...
            CancellationToken.None);
        await mediator.Send(new ApprovePurchaseOrderCommand(crear.Value!.Id), CancellationToken.None);

        // Crear salesBill de compra Aprobada con 5 unidades del mismo producto
        var salesBill = BuildFacturaAprobada(subscriberId, proveedorId, productoId, quantity: 5m, userId, db);

        var vincular = await mediator.Send(
            new LinkInvoiceToPurchaseOrderCommand(crear.Value.Id, salesBill.Id),
            CancellationToken.None);
    ...
    public async Task Vincular_factura_parcial_pone_OC_en_RecibidaParcial()
    {
        ...
        // salesBill solo trae 4 unidades
        var salesBill = BuildFacturaAprobada(subscriberId, proveedorId, productoId, quantity: 4m, userId, db);
