...
/// </summary>
public sealed class TransfersEndToEndTests
{
    [Fact]
    public async Task ConfirmTransfer_mueve_stock_de_origen_a_destino_y_registra_movimientos()
    {
        ...
        var stockOrigenResult = await mediator.Send(
            new GetCurrentStockByWarehouseQuery(origenId, productoId), CancellationToken.None);
        ...
        var stockDestinoResult = await mediator.Send(
            new GetCurrentStockByWarehouseQuery(destinoId, productoId), CancellationToken.None);
