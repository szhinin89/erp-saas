...
public sealed class SalesNotesAndRetentionsEndToEndTests
{
    ...
    public async Task Nota_credito_autorizada_genera_xml_simulado_y_reingresa_stock()
    {
        ...
        var crearNota = await mediator.Send(
            new CreateSalesNoteCommand(
                ...
                new[] { new SalesNoteItemDto(seed.ProductId, 1m, 25m) }),
            CancellationToken.None);
        ...
        var note = await db.SalesNotes.FindAsync(new object[] { crearNota.Value }, CancellationToken.None);
        note.Should().NotBeNull();
        note!.Status.Should().Be("Authorized");
        note.XmlSignedPath.Should().NotBeNullOrEmpty();
    ...
    public async Task Nota_debito_autorizada_crea_asiento_con_credito_a_ventas()
    {
        ...
        var note = await db.SalesNotes.FindAsync(new object[] { crearNd.Value }, CancellationToken.None);
        note!.JournalEntryId.Should().NotBeNull();

        var lineas = db.JournalEntryLines.Where(l => l.JournalEntryId == note.JournalEntryId).ToList();
        lineas.Should().NotBeEmpty();
        lineas.Should().Contain(l =>
            l.AccountId == cuentaVentas.Id
            && l.Credit.Amount == note.Total
            && l.Debit.Amount == 0m);
    }
    ...
    public async Task Nota_credito_proveedor_en_compra_aprobada_reingresa_stock_y_crea_asiento_inverso()
    {
        ...
        var claveNota = AccessKey49TestFactory.FromPrefix48(new string('1', 48));
        ...
        var note = db.PurchNotes.First(n => n.Id == importar.Value.Id);
        note.Status.Should().Be("Approved");
        note.JournalEntryId.Should().NotBeNull();
        ...
        var lineasNota = db.JournalEntryLines.Where(l => l.JournalEntryId == note.JournalEntryId).ToList();
        lineasNota.Should().Contain(l => l.AccountId == cuentaPasivo.Id && l.Debit.Amount == note.Total && l.Credit.Amount == 0m);
    }
    ...
    public async Task Nota_debito_proveedor_en_compra_disminuye_stock_y_aumenta_deuda()
    {
        ...
        var claveNota = AccessKey49TestFactory.FromPrefix48(new string('2', 48));
        var xmlNota = BuildNotaProveedorXml(
            "DEBIT",
            claveNota,
            seed.ProveedorRuc,
        ...
        var lineasNota = db.JournalEntryLines.Where(l => l.JournalEntryId == note.JournalEntryId).ToList();
        lineasNota.Should().Contain(l => l.AccountId == cuentaPasivo.Id && l.Credit.Amount == note.Total && l.Debit.Amount == 0m);
    }
    ...
    public async Task Nota_credito_en_gasto_ajusta_gasto_sin_afectar_inventario()
    {
        ...
        var claveNota = AccessKey49TestFactory.FromPrefix48(new string('3', 48));
        var xmlNota = BuildNotaProveedorXml(
            "CREDIT",
            claveNota,
            seed.ProveedorRuc,
        ...
        gasto.TotalNotesApplied.Should().Be(note.Total);
        db.CurrentStocks.First(s =>
                s.SubscriberId == seed.SubscriberId && s.ProductId == seed.ProductId && s.WarehouseId == seed.WarehouseId)
    ...
    public async Task Compra_aprobada_retencion_emitida_enviada_simulada_crea_asiento()
    {
        ...
        var seed = await IntegrationSeedData.SeedAsync(db, factory.MutableSubscriber, factory.MutableUser, CancellationToken.None, factory.MutableCompany);
        await SalesEndToEndHelpers.SeedVentasPrerequisitesAsync(db, seed, stockInicial: 0m, crearStockActual: false, ct: CancellationToken.None);
    ...
    public async Task Retencion_recibida_registrada_con_xml_vincula_factura_y_crea_asiento()
    {
        ...
        var claveRet = AccessKey49TestFactory.FromPrefix48(new string('8', 48));
