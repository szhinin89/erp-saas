...
public sealed class DocumentSequenceConcurrencyTests : IClassFixture<DocSeqConcurrencyFixture>
{
    ...
    public DocumentSequenceConcurrencyTests(
        DocSeqConcurrencyFixture fixture,
    ...
    public async Task CaptureNext_repeated_20_iterations_never_produces_duplicates()
    {
        ...
        allSeqs
            .Should()
            .OnlyHaveUniqueItems(because: "los secuenciales no se reutilizan entre iteraciones");

        await AssertFinalSeqAsync(epId, iterations * requestsPerIt);
        _out.WriteLine(
            $"Repetición: {iterations} iter × {requestsPerIt} req = {allSeqs.Count} núms únicos."
        );
    }
    ...
    public async Task CaptureNext_two_tenants_concurrent_are_isolated()
    {
        ...
        _out.WriteLine(
            $"Multi-tenant: EP-A y EP-B generaron {n} secuenciales independientes cada uno."
        );
    }
    ...
    public async Task CaptureNext_multi_doctype_same_ep_sequences_are_independent()
    {
        ...
        dtResults[0]
            .Numbers.Should()
            .OnlyHaveUniqueItems(because: "FACTURA no debe tener duplicados");
        dtResults[1]
            .Numbers.Should()
            .OnlyHaveUniqueItems(because: "RETENCION no debe tener duplicados");
        ...
        _out.WriteLine(
            $"Multi-doctype: FACTURA y RETENCION generaron {n} secuenciales independientes cada uno."
        );
    }
    ...
    public void Domain_CaptureAndIncrement_throws_when_current_seq_below_1()
    {
        ...
        var seq = (DocumentSequence)
            Activator.CreateInstance(typeof(DocumentSequence), nonPublic: true)!;
        ...
        act.Should().Throw<InvalidOperationException>().WithMessage("*CurrentSeq*1*");
    }
            ...
            .Select(i =>
                Task.Run(
                    async () =>
                    {
                        ...
                        var repo =
                            scope.ServiceProvider.GetRequiredService<IDocumentSequenceRepository>();
    ...
    {
        r.Numbers.Should()
            .HaveCount(expected, because: "todos los requests deben completarse exitosamente");
        r.Numbers.Should()
            .OnlyHaveUniqueItems(because: "el advisory lock debe garantizar unicidad absoluta");
        r.Metrics.Should()
            .NotContain(-1L, because: "no deben existir excepciones en ningún request concurrente");
    }
    ...
    private async Task AssertFinalSeqAsync(Guid epId, int expectedCount, string docType = DocType)
    {
        ...
        seq.Should()
            .NotBeNull(
                because: $"debe existir una fila en document_sequence para EP={epId}, docType={docType}"
            );
        seq!
            .CurrentSeq.Should()
            .Be(
    ...
    private void PrintMetrics(int n, long[] metrics)
    {
        ...
        if (valid.Length == 0)
        {
            _out.WriteLine($"[{n} req] TODOS fallaron.");
            return;
        }
        ...
        _out.WriteLine(
            $"[{n,4} req] avg={avgMs:F1}ms  max={maxMs}ms  min={minMs}ms  " + $"errors={errors}"
        );
    }
