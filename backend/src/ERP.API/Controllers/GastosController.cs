...
using ERP.Application.Modules.Gastos.UseCases.GetGastoById;
...
public sealed class GastosController : ControllerBase
{
    ...
    [ProducesResponseType(typeof(ApiResponse<IReadOnlyList<GastoFacturaDto>>), StatusCodes.Status200OK)]
    public async Task<IActionResult> GetAll(CancellationToken ct = default)
    {
        EstadoGasto? estado = null;
        if (Request.Query.TryGetValue("estado", out var ev) &&
            Enum.TryParse<EstadoGasto>(ev, out var ep)) estado = ep;
        ...
        return this.ToOkOrBadRequest(result, "OK", () => Array.Empty<GastoFacturaDto>());
    }

    /// <summary>Obtiene un gasto por id (tenant actual).</summary>
    [HttpGet("{id:guid}")]
    [Authorize(Policy = "perm:gastos.facturas.view")]
    [ProducesResponseType(typeof(ApiResponse<GastoFacturaDto?>), StatusCodes.Status200OK)]
    ...
    [ProducesResponseType(typeof(ApiResponse<GastoFacturaDto?>), StatusCodes.Status201Created)]
    ...
    /// <response code="400">XML inválido o clave duplicada.</response>
    [HttpPost("xml")]
    [Authorize(Policy = "perm:gastos.facturas.create")]
