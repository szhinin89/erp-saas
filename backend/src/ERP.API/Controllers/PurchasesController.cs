...
public sealed class PurchasesController : ControllerBase
{
    ...
    public async Task<IActionResult> CreateDraft(
        [FromBody] CreatePurchaseDraftCommand command, CancellationToken ct)
        => this.ToCreatedOrBadRequest(await _mediator.Send(command, ct));

    [HttpPut("{id:guid}")]
    [Authorize(Policy = $"perm:{PurchasePermissions.Update}")]
    public async Task<IActionResult> UpdateDraft(
        Guid id, [FromBody] UpdatePurchaseDraftCommand command, CancellationToken ct)
    {
    ...
    public async Task<IActionResult> GetList(
        ...
        => this.ToOkOrBadRequest(await _mediator.Send(
            new GetPurchaseListQuery(search, status, pageNumber, pageSize), ct), "OK");
    ...
    public async Task<IActionResult> ApplyGlobalDiscount(
        ...
        => this.ToOkOrBadRequest(await _mediator.Send(new ApplyGlobalDiscountCommand(id, request.DiscountPct), ct));
    ...
    public async Task<IActionResult> ConfirmPurchase(
        Guid id, [FromBody] ConfirmPurchaseRequest? request, CancellationToken ct)
        => this.ToOkOrBadRequest(await _mediator.Send(
            new ConfirmPurchaseCommand(id, request?.Schedule), ct));

    [HttpPost("{id:guid}/cancel")]
    [Authorize(Policy = $"perm:{PurchasePermissions.Update}")]
    public async Task<IActionResult> CancelPurchase(
        Guid id, [FromBody] CancelPurchaseRequest request, CancellationToken ct)
        => this.ToOkOrBadRequest(await _mediator.Send(new CancelPurchaseCommand(id, request.Reason), ct));

    /// <summary>Contexto completo de un ítem para el detalle de compra (1 request SSOT).</summary>
    [HttpGet("items/context")]
    [Authorize(Policy = $"perm:{PurchasePermissions.View}")]
    [ProducesResponseType(typeof(Contracts.ApiResponse<PurchaseItemContextDto>), StatusCodes.Status200OK)]
    public async Task<IActionResult> GetItemContext(
        [FromQuery] Guid itemId, [FromQuery] Guid warehouseId, [FromQuery] Guid? supplierId, CancellationToken ct)
        => this.ToOkOrBadRequest(await _mediator.Send(
            new GetPurchaseItemContextQuery(itemId, warehouseId, supplierId), ct));

    [HttpPost("{id:guid}/lines/{lineId:guid}/update-pvp")]
    [Authorize(Policy = $"perm:{PurchasePermissions.Update}")]
    public async Task<IActionResult> UpdateLinePvp(
        Guid id, Guid lineId, [FromBody] UpdatePvpRequest request, CancellationToken ct)
        => this.ToOkOrBadRequest(await _mediator.Send(new UpdateLinePvpCommand(id, lineId, request.NewPvp), ct));
    ...
    public async Task<IActionResult> IssueWithholding(
        Guid id, [FromBody] IssueWithholdingRequest request, CancellationToken ct)
        => this.ToCreatedOrBadRequest(await _mediator.Send(
            new IssueWithholdingCommand(id, request.EmissionPointId, request.IssueDate), ct));
    ...
    public async Task<IActionResult> CancelWithholding(
        Guid whId, [FromBody] CancelWithholdingRequest request, CancellationToken ct)
        => this.ToOkOrBadRequest(await _mediator.Send(new CancelWithholdingCommand(whId, request.Reason), ct));
}
