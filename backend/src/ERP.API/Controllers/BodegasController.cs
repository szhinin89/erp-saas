...
public sealed class BodegasController : ControllerBase
{
    ...
    public BodegasController(IMediator mediator) => _mediator = mediator;

    /// <summary>Lista bodegas con filtros opcionales.</summary>
    [HttpGet]
    [Authorize(Policy = "perm:inventario.bodegas.view")]
    [ProducesResponseType(typeof(ApiResponse<IReadOnlyList<BodegaDto>>), StatusCodes.Status200OK)]
    ...
    public async Task<IActionResult> GetAll(CancellationToken ct = default)
    {
        ...
        var result = await _mediator.Send(new GetBodegasQuery(activeFilter, search, sucursalId), ct);
        return this.ToOkOrBadRequest(result, "OK", () => Array.Empty<BodegaDto>());
    }
    ...
    [HttpGet("{id:guid}")]
    [Authorize(Policy = "perm:inventario.bodegas.view")]
    [ProducesResponseType(typeof(ApiResponse<BodegaDetailDto?>), StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    public async Task<IActionResult> GetById(Guid id, CancellationToken ct = default)
    {
        var result = await _mediator.Send(new GetBodegaByIdQuery(id), ct);
    ...
    [HttpPost]
    [Authorize(Policy = "perm:inventario.bodegas.create")]
    [ProducesResponseType(typeof(ApiResponse<BodegaDto?>), StatusCodes.Status201Created)]
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    [ProducesResponseType(StatusCodes.Status422UnprocessableEntity)]
    public async Task<IActionResult> Create(
        [FromBody] CreateBodegaCommand command, CancellationToken ct = default)
    {
    ...
    /// <response code="422">Datos inválidos.</response>
    [HttpPut("{id:guid}")]
    [Authorize(Policy = "perm:inventario.bodegas.update")]
    [ProducesResponseType(typeof(ApiResponse<BodegaDto?>), StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    [ProducesResponseType(StatusCodes.Status422UnprocessableEntity)]
    public async Task<IActionResult> Update(
        Guid id, [FromBody] UpdateBodegaCommand command, CancellationToken ct = default)
    {
        if (id != command.Id)
    ...
    /// <response code="400">La bodega ya está deshabilitada o no existe.</response>
    [HttpPatch("{id:guid}/disable")]
    [Authorize(Policy = "perm:inventario.bodegas.delete")]
    [ProducesResponseType(typeof(ApiResponse<BodegaDto>), StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    public async Task<IActionResult> Disable(Guid id, CancellationToken ct = default)
    {
        var result = await _mediator.Send(new DisableBodegaCommand(id), ct);
    ...
    /// <response code="400">La bodega ya está activa o no existe.</response>
    ...
    {
        var result = await _mediator.Send(new EnableBodegaCommand(id), ct);
