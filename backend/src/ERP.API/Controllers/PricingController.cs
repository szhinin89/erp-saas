...
public sealed class PricingController : ControllerBase
{
    ...
    public async Task<IActionResult> GetPriceLists(
        ...
        => this.ToOkOrBadRequest(await _mediator.Send(new GetPriceListsQuery(isActive, search), ct), "OK");
    ...
    public async Task<IActionResult> UpdatePriceList(
        Guid id, [FromBody] UpdatePriceListCommand command, CancellationToken ct)
    {
    ...
    public async Task<IActionResult> GetAssignedItems(Guid id, CancellationToken ct)
        => this.ToOkOrBadRequest(await _mediator.Send(new GetItemsAssignedToPriceListQuery(id), ct), "OK");
    ...
    public async Task<IActionResult> GetPricingRules(
        [FromQuery] Guid? priceListId = null, [FromQuery] Guid? itemId = null,
        CancellationToken ct = default)
        => this.ToOkOrBadRequest(await _mediator.Send(new GetPricingRulesQuery(priceListId, itemId), ct), "OK");
    ...
    public async Task<IActionResult> SetPricingRule(
        [FromBody] SetPricingRuleCommand command, CancellationToken ct)
        => this.ToCreatedOrBadRequest(await _mediator.Send(command, ct));
    ...
    public async Task<IActionResult> EnablePricingRule(
        [FromBody] EnablePricingRuleCommand command, CancellationToken ct)
