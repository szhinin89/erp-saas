...
/// </summary>
[ApiController]
[Route("api/master/business-partners/{bpId:guid}/trading-settings")]
...
public sealed class CompanyBpTradingSettingsController : ControllerBase
{
    ...
    /// Devuelve 404 si no existe configuración — crear con PUT primero.
    /// </summary>
    [HttpGet]
    ...
    public async Task<IActionResult> GetSettings(
        [FromRoute] Guid bpId, CancellationToken ct = default)
    {
        var result = await _mediator.Send(new GetCompanyBpTradingSettingsQuery(bpId), ct);
    ...
    public async Task<IActionResult> Block(
        ...
        CancellationToken ct = default)
    {
        var result = await _mediator.Send(new BlockBusinessPartnerCommand(bpId, body.Reason), ct);
        return this.ToOkOrBadRequest(result);
    }
    ...
    public async Task<IActionResult> Unblock(
        [FromRoute] Guid bpId, CancellationToken ct = default)
    {
        var result = await _mediator.Send(new UnblockBusinessPartnerCommand(bpId), ct);
        return this.ToOkOrBadRequest(result);
    }
}
