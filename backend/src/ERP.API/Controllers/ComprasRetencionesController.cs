...
public sealed class ComprasRetencionesController : ControllerBase
{
    ...
    [ProducesResponseType(typeof(ApiResponse<IReadOnlyList<CompraRetencionEmitidaListItemDto>>), StatusCodes.Status200OK)]
    public async Task<IActionResult> Listar([FromQuery] Guid? proveedorId, CancellationToken ct = default)
    {
        ...
        return this.ToOkOrBadRequest(result, "OK", () => Array.Empty<CompraRetencionEmitidaListItemDto>());
    }

    [HttpPost]
    [Authorize(Policy = "perm:compras.retenciones.create")]
    [ProducesResponseType(typeof(ApiResponse<Guid>), StatusCodes.Status201Created)]
    public async Task<IActionResult> Generar([FromBody] GenerarCompraRetencionEmitidaCommand command, CancellationToken ct = default)
    {
    ...
    {
        var result = await _mediator.Send(new EnviarCompraRetencionEmitidaCommand(id), ct);
