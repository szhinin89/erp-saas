...
public sealed class CashRegisterController : ControllerBase
{
    ...
    public async Task<IActionResult> GetByCurrentBranch(
        ...
        => this.ToOkOrBadRequest(await _mediator.Send(new GetCashRegistersByCurrentBranchQuery(activeOnly), ct), "OK");
    ...
    public async Task<IActionResult> GetAll(
        ...
        => this.ToOkOrBadRequest(await _mediator.Send(new GetAllCashRegistersQuery(activeOnly, search), ct), "OK");
    ...
    public async Task<IActionResult> GetEmissionPointLookups(
        ...
        => this.ToOkOrBadRequest(await _mediator.Send(new GetEmissionPointLookupsByBranchQuery(branchId), ct), "OK");
    ...
    [HttpPut("{id:guid}")]
    [Authorize(Policy = $"perm:{CajaPermissions.Manage}")]
    public async Task<IActionResult> Update(Guid id, [FromBody] UpdateCashRegisterCommand command, CancellationToken ct)
    {
        if (id != command.Id)
