...
public sealed class BusinessPartnersController : ControllerBase
{
        ...
        CancellationToken ct = default)
    {
        var result = await _mediator.Send(
            new SearchBusinessPartnersQuery(q, isActive, roles, skip, take), ct);
    ...
    public async Task<IActionResult> Create(
        [FromBody] CreateBusinessPartnerRequest body, CancellationToken ct = default)
    {
        ...
        return result.IsSuccess
            ? this.ApiCreated(result.Value!, "BusinessPartner creado.")
    ...
    {
        var result = await _mediator.Send(new DeactivateBusinessPartnerCommand(id), ct);
        return this.ToOkOrBadRequest(result);
    }
}
