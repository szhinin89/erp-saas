...
/// <summary>
/// Cookie httpOnly del refresh token. Path <c>/api</c> cubre login platform
/// (<c>/api/platform/auth/login</c>) y refresh (<c>/api/auth/refresh</c>).
/// Path <c>/api/auth</c> no es prefijo válido del login platform y el navegador ignora Set-Cookie.
/// </summary>
