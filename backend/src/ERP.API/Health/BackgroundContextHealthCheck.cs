...
public sealed class BackgroundContextHealthCheck : IHealthCheck
{
    ...
    {
        if (JobSubscriberContext.Current != Guid.Empty || JobCompanyContext.Current != Guid.Empty)
        {
            _metrics.RecordBackgroundContextLeakDetected();
            return Task.FromResult(HealthCheckResult.Unhealthy(
                "Job AsyncLocal context leak: subscriber or company set outside job execution."));
        }
