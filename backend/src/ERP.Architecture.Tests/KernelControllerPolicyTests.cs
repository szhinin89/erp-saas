...
public sealed class KernelControllerPolicyTests
{
    ...
    public void Controller_perm_policies_exist_in_KernelRegistry_Permissions()
    {
        ...
        foreach (var type in controllerTypes)
        {
            ...
            {
                if (
                    attr.Policy is not { } policy
                    || !policy.StartsWith(PermPrefix, StringComparison.Ordinal)
                )
                    continue;
        ...
        orphanPolicies
            .Should()
            .BeEmpty(
                "toda policy 'perm:X' debe corresponder a una constante en ERP.Domain.Kernel.Permissions.*"
            );
    }

    private static Assembly ResolveControllersAssembly() =>
        AppDomain.CurrentDomain.GetAssemblies().FirstOrDefault(a => a.GetName().Name == "ERP.API")
