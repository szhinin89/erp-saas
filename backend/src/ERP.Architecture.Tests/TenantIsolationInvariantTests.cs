...
public sealed class TenantIsolationInvariantTests
{
    private static readonly Assembly DomainAssembly =
        typeof(ISubscriberScopedEntity).Assembly;
    ...
    // ── INVARIANT B — SubscriberId consistente ────────────────────────────────

    /// <summary>
    /// INV-B1: Ninguna entidad operacional (COMPANY scope) puede tener SubscriberId con setter público.
    /// SubscriberId es parte de la identidad del tenant — no es editable.
    /// </summary>
    [Fact]
    public void INV_B1_subscriber_id_must_not_have_public_setter_on_operational_entities()
    {
        var violators = DomainAssembly.GetTypes()
            .Where(t => t.IsClass && !t.IsAbstract)
            .Where(t => typeof(ICompanyOperationalEntity).IsAssignableFrom(t)
            ...
            {
                Type = t,
                Prop = t.GetProperty("SubscriberId", BindingFlags.Public | BindingFlags.Instance)
            })
            .Where(x => x.Prop is not null)
            .Where(x =>
            {
        ...
        violators.Should().BeEmpty(
            "SubscriberId es un invariante de tenancy — no puede tener setter público. " +
            "Cambiar SubscriberId permitiría mover datos entre tenants.");
    }

    // ── INVARIANT C — Ownership Company ↔ Subscriber ─────────────────────────

    /// <summary>
    /// INV-C1: ICompanyScopedEntity también debe implementar ISubscriberScopedEntity.
    /// Una company siempre pertenece a un subscriber — el scope de empresa implica el de tenant.
    /// </summary>
    [Fact]
    public void INV_C1_company_scoped_entity_must_also_be_subscriber_scoped()
    {
        var violators = DomainAssembly.GetTypes()
            .Where(t => t.IsClass && !t.IsAbstract)
            .Where(t => typeof(ICompanyScopedEntity).IsAssignableFrom(t))
            .Where(t => !typeof(ISubscriberScopedEntity).IsAssignableFrom(t))
            .Select(t => t.FullName)
            .ToList();

        violators.Should().BeEmpty(
            "Toda entidad company-scoped debe también ser subscriber-scoped. " +
            "Una empresa siempre pertenece a un suscriptor.");
    }

    /// <summary>
    /// INV-C2: ICompanyOperationalEntity (que extiende ISubscriberScopedEntity)
    /// no puede existir sin SubscriberId.
    /// </summary>
    [Fact]
    public void INV_C2_company_operational_entity_inherits_subscriber_scope()
    {
        // ICompanyOperationalEntity extends ISubscriberScopedEntity — verified at compile time
        // This test verifies the runtime type hierarchy is correct
        typeof(ICompanyOperationalEntity).GetInterfaces()
            .Should().Contain(typeof(ISubscriberScopedEntity),
            "ICompanyOperationalEntity debe extender ISubscriberScopedEntity. " +
    ...
    public void INV_E2_company_module_requests_must_declare_explicit_scope()
    {
        ...
        var violators = AppAssembly.GetTypes()
            ...
            .Where(t =>
                ...
                !typeof(ERP.Application.Common.ISubscriberScopedRequest).IsAssignableFrom(t) &&
