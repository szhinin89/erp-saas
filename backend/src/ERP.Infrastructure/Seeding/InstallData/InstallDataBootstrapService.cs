...
public sealed partial class InstallDataBootstrapService : IInstallDataBootstrapService
{
    ...
    public async Task ApplyPendingAsync(CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            foreach (var resource in resources)
            {
                ...
                var effectiveScript = await ResolveSourceResourceIfAnyAsync(scriptText, cancellationToken);
                ...
                var existingChecksum = await GetExistingChecksumAsync(conn, resource, cancellationToken);
                ...
                try
                {
                    ...
                    await RegisterAppliedAsync(conn, tx, resource, checksum, isDestructive, cancellationToken);
    ...
    private static async Task EnsureMetadataTableAsync(NpgsqlConnection conn, CancellationToken cancellationToken)
    {
        const string sql = """
    ...
    private async Task<string> ResolveSourceResourceIfAnyAsync(string scriptText, CancellationToken cancellationToken)
    {
    ...
    [LoggerMessage(Level = LogLevel.Information, Message = "InstallData deshabilitado por configuración.")]
    private partial void LogInstallDataDisabled();

    [LoggerMessage(Level = LogLevel.Information, Message = "InstallData omitido: Supplier actual '{Provider}' no es PostgreSQL/Npgsql.")]
    private partial void LogInstallDataSkippedProvider(string provider);

    [LoggerMessage(Level = LogLevel.Information, Message = "No se encontraron scripts en {Prefix}.")]
    private partial void LogNoScriptsFound(string prefix);

    [LoggerMessage(Level = LogLevel.Information, Message = "InstallData skip (ya aplicado): {Script}")]
    ...
    [LoggerMessage(Level = LogLevel.Warning, Message = "InstallData checksum cambió, re-aplicando: {Script}")]
    private partial void LogScriptChecksumChanged(string script);

    [LoggerMessage(Level = LogLevel.Information, Message = "InstallData script utiliza source-resource: {Resource}")]
