...
///   1. Unicidad (subscriber_id, business_partner_id, role_type) — índice UNIQUE en BD.
...
public sealed class BusinessPartnerRoleRepository : IBusinessPartnerRoleRepository
{
    ...
    /// </summary>
    public Task<BusinessPartnerRole?> GetByIdAsync(Guid id, CancellationToken ct = default)
    ...
    public async Task AddAsync(BusinessPartnerRole role, CancellationToken ct = default)
        => await _db.BusinessPartnerRoles.AddAsync(role, ct);
