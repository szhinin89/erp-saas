...
/// Scope: ISubscriberScopedEntity — el global query filter agrega subscriber_id en cada query.
...
public sealed class BusinessPartnerLocationRepository : IBusinessPartnerLocationRepository
{
    ...
    /// El global query filter en BusinessPartnerContacts filtra por subscriber_id.
    /// </summary>
    public Task<bool> HasActiveContactsAsync(Guid locationId, CancellationToken ct = default)
        => _db.BusinessPartnerContacts
              .AnyAsync(c => c.LocationId == locationId && c.IsActive, ct);
