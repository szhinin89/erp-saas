...
public sealed class PaymentTermRepository : IPaymentTermRepository
{
    ...
    public async Task<IReadOnlyList<PaymentTerm>> ListAsync(Guid tenantId, string? search = null, CancellationToken cancellationToken = default)
    {
    ...
    public Task<PaymentTerm?> GetByIdAsync(Guid tenantId, Guid id, CancellationToken cancellationToken = default)
        => _db.PaymentTerms.FirstOrDefaultAsync(x => x.TenantId == tenantId && x.Id == id, cancellationToken);
    ...
    public void Update(PaymentTerm entity)
        => _db.PaymentTerms.Update(entity);
}
