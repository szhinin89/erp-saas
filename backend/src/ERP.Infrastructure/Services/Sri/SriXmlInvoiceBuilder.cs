...
/// </summary>
public static class SriXmlInvoiceBuilder
{
    // ── Constantes SRI ────────────────────────────────────────────────────────
        ...
        BusinessPartner?     buyer = null)
    {
        ...
        var doc = new XDocument(
            ...
            new XElement("salesBill",
                ...
                BuildInfoTributaria(cfg, company, salesBill.AccessKey, salesBill.DocType,
                    salesBill.EstabCode, salesBill.EmPointCode, salesBill.Sequential),

                new XElement("infoFactura",
                    new XElement("fechaEmision",        salesBill.IssueDate.ToString("dd/MM/yyyy")),
                    ...
                    new XElement("totalSinImpuestos",   F2(salesBill.Subtotal)),
                    new XElement("totalDescuento",      F2(salesBill.TotalDiscount)),
                    new XElement("totalConImpuestos",   gruposIva),
                    new XElement("propina",  "0.00"),
                    new XElement("importeTotal", F2(salesBill.Total)),
                    new XElement("moneda", Currency),
                    new XElement("pagos",
                        new XElement("pago",
                            new XElement("formaPago",   salesBill.PaymentMethodCode),
                            new XElement("total",       F2(salesBill.Total)),
                            new XElement("plazo",       salesBill.PaymentDays.ToString()),
                            new XElement("unidadTiempo","dias")))),

                new XElement("lines", detElements),

                BuildInfoAdicional(buyer, salesBill.Notes)));

        return ToUtf8String(doc);
    }

    /// <summary>Genera el XML de una note de crédito (tipo 04) o débito (tipo 05).</summary>
        ...
        BusinessPartner?    buyer = null)
    {
        ...
        var lines = lineas.Select(l => BuildDetalleNota(l, ref totalDesc)).ToList();

        // totalConImpuestos agrupado por código IVA (igual que salesBill)
        var gruposIva = lineas
            .GroupBy(l => l.VatCode)
        ...
        var doc = new XDocument(
            ...
            new XElement(rootName,
                ...
                BuildInfoTributaria(cfg, company, note.AccessKey, note.DocType,
                    note.EstabCode, note.EmPointCode, note.Sequential),

                new XElement(infoName,
                    new XElement("fechaEmision",            note.IssueDate.ToString("dd/MM/yyyy")),
                    new XElement("dirEstablecimiento",      company.MainAddress),
                    new XElement("tipoIdentificacionComprador", SriIdTypeCode(buyer)),
                    new XElement("razonSocialComprador",    buyer?.Name.LegalName ?? "CONSUMIDOR FINAL"),
                    new XElement("identificacionComprador", buyer?.Identification.Number ?? "9999999999"),
                    ...
                    new XElement("totalSinImpuestos",       F2(note.Subtotal)),
                    new XElement("valorModificacion",       F2(note.Total)),
                    new XElement("totalConImpuestos",       gruposIva),
                    new XElement("moneda", Currency)),
    ...
    private static XElement BuildInfoAdicional(BusinessPartner? buyer, string? notes)
    {
        ...
        if (!string.IsNullOrWhiteSpace(notes))
            campos.Add(CampoAdicional("notes", notes));
