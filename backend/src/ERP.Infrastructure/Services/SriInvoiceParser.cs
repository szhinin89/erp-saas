...
public sealed class SriInvoiceParser : IXmlInvoiceParser
{
    ...
    public Task<InvoiceParseResult> ParseAsync(Stream xmlStream, CancellationToken ct = default)
    {
        ...
        // El elemento raíz puede ser <salesBill>, <notaCredito> u otro comprobante.
        ...
        var detallesEl = RequireElement(root, "lines");
        ...
        // ── lines ──────────────────────────────────────────────────────
        var items = detallesEl.Elements("line").Select(ParseDetalle).ToList();

        if (items.Count == 0)
            throw new XmlParseException("El comprobante no contiene ningún line (<line>).");
    ...
    public Task<SupplierNoteParseResult> ParseSupplierNoteAsync(Stream xmlStream, CancellationToken ct = default)
    {
        ...
        var total =
            ...
            ?? throw new XmlParseException(
                "No se encontró importeTotal ni valorModificacion en la note.");

        var items = detallesEl.Elements("line").Select(ParseDetalleNota).ToList();
        if (items.Count == 0)
            throw new XmlParseException("El comprobante no contiene ningún line (<line>).");
    ...
    /// 1. XML directo con raíz &lt;salesBill&gt; (o notaCredito, etc.)
    ...
    private static InvoiceLineItem ParseDetalle(XElement det)
    {
        ...
        return new InvoiceLineItem(codigo, desc, cantidad, precioUnitario, descuento, subtotal);
    }
