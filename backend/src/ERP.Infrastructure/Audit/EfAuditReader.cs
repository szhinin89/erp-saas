...
/// </summary>
public sealed class EfAuditReader<TAudit> : IAuditReader<TAudit>
    where TAudit : AuditRecordBase
{
    ...
    {
        if (take <= 0)
            return Array.Empty<TAudit>();
        if (take > 200)
            take = 200;

        return await _db.Set<TAudit>()
            .AsNoTracking()
    ...
    )
    {
        if (entityIds.Count == 0)
            return new Dictionary<Guid, TAudit>();
        var ids = entityIds.Distinct().ToArray();

        var rows = await _db.Set<TAudit>()
            .AsNoTracking()
