...
{
    public static void Compose(IContainer container, InvoiceRideDocumentLayout layout, byte[]? logoBytes, byte[] barcodeImageBytes)
    {
        ...
        container.Row(row =>
        {
            row.RelativeItem(6).Column(left =>
            {
                ...
                left.Item().RideBox().Height(40).Padding(4).AlignMiddle().Element(logo =>
                {
                    if (logoBytes is not null)
                        logo.AlignLeft().Image(logoBytes).FitArea();
                    else
                        logo.AlignLeft().Text("NO TIENE LOGO").Bold().FontColor(Colors.Red.Medium).FontSize(14);
                });

                left.Item().RideBox().Padding(8).Column(info =>
                {
                    info.Spacing(2);
            ...
            row.RelativeItem(5).RideBox().Padding(8).Column(right =>
            {
                ...
                right.Item().Text(text =>
                {
                    ...
                    text.Span(header.EmissionType == "1" ? "NORMAL" : "INDISPONIBILIDAD DEL SISTEMA");
                });
