...
public sealed class CommercialCatalogQuery : ICommercialCatalogQuery, IPublicCommercialPlansQuery
{
    ...
    public async Task<IReadOnlyList<PublicCommercialPlanDto>> GetVisiblePlansAsync(CancellationToken ct = default)
    {
        var plans = await _db.CommercialPlans.AsNoTracking()
        ...
        if (plans.Count == 0)
            return Array.Empty<PublicCommercialPlanDto>();
        ...
        var list = new List<PublicCommercialPlanDto>(plans.Count);
        foreach (var plan in plans)
        {
            var feats = links
                ...
                .Select(l =>
                {
                    ...
                    return new PublicCommercialPlanFeatureDto(
                        d.Code,
                        d.Name,
                        d.Description,
                        d.IsMetered,
            ...
            list.Add(new PublicCommercialPlanDto(
                plan.Id,
                plan.Code,
                plan.Name,
                plan.ShortLabel,
