...
public sealed class StockAdjustmentRepository : IStockAdjustmentRepository
{
    ...
    public async Task<(IReadOnlyList<StockAdjustment> Items, int TotalCount)> GetPagedAsync(
        ...
        Guid? warehouseId, Guid? productId, string? status,
        DateTime? startDate, DateTime? endDate, CancellationToken ct = default)
    {
        ...
        if (startDate.HasValue) q = q.Where(a => a.AdjustmentDate >= startDate.Value);
        if (endDate.HasValue) q = q.Where(a => a.AdjustmentDate <= endDate.Value);
