...
public sealed class PurchaseReceptionDocumentRepository : IPurchaseReceptionDocumentRepository
{
    ...
    public Task<bool> ExistsByAccessKeyAsync(Guid tenantId, string accessKey, CancellationToken ct = default)
        => Scoped(tenantId).AnyAsync(x => x.AccessKey == accessKey, ct);

    public Task<PurchaseReceptionDocument?> GetByAccessKeyAsync(Guid tenantId, string accessKey, CancellationToken ct = default)
