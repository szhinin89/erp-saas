...
public sealed class UserSessionRepository : IUserSessionRepository
{
    ...
    {
        var query = _db.UserSessions.IgnoreQueryFilters()
            .Where(x => x.TenantId == tenantId);

        if (identityUserId is Guid uid) query = query.Where(x => x.IdentityUserId == uid);
        if (companyId is Guid cid) query = query.Where(x => x.CompanyId == cid);
        if (status is UserSessionStatus st) query = query.Where(x => x.Status == st);
        if (fromUtc is DateTime from) query = query.Where(x => x.StartedAt >= from);
        if (toUtc is DateTime to) query = query.Where(x => x.StartedAt <= to);
    ...
    public Task<UserSession?> GetByRefreshTokenIdAsync(Guid refreshTokenId, CancellationToken cancellationToken = default)
        => _db.UserSessions.IgnoreQueryFilters()
