...
public class AccessRepository : IAccessRepository
{
    ...
    public Task<IdentityUser?> GetUserByIdAsync(Guid userId, CancellationToken cancellationToken = default)
    ...
    {
        if (userIds.Count == 0) return Array.Empty<IdentityUser>();
        return await _db.IdentityUsers.Where(u => userIds.Contains(u.Id)).ToListAsync(cancellationToken);
    }

    public Task<IdentityUser?> GetUserByEmailAsync(string email, CancellationToken cancellationToken = default)
    {
        var normalized = NormalizeEmail(email);
        return _db.IdentityUsers.FirstOrDefaultAsync(u => u.EmailNormalized == normalized, cancellationToken);
    }

    public Task<bool> AnyUserWithEmailAsync(string email, CancellationToken cancellationToken = default)
    {
        var normalized = NormalizeEmail(email);
        return _db.IdentityUsers.AnyAsync(u => u.EmailNormalized == normalized, cancellationToken);
    }

    public Task<IdentityUser?> GetUserByUsernameAsync(string username, CancellationToken cancellationToken = default)
    {
        var normalized = NormalizeUsername(username);
        return _db.IdentityUsers.FirstOrDefaultAsync(u => u.UsernameNormalized == normalized, cancellationToken);
    }

    public Task<bool> AnyUserWithUsernameAsync(string username, CancellationToken cancellationToken = default)
    {
        var normalized = NormalizeUsername(username);
        return _db.IdentityUsers.AnyAsync(u => u.UsernameNormalized == normalized, cancellationToken);
    }
    ...
    public async Task<IReadOnlyList<CompanyUserMembership>> GetActiveCompanyUserMembershipsForUserSystemAsync(
        Guid identityUserId, CancellationToken cancellationToken = default)
        => await _db.CompanyUserMemberships.IgnoreQueryFilters()
            .Where(m => m.IdentityUserId == identityUserId && m.IsActive)
            .ToListAsync(cancellationToken);

    public Task<CompanyUserMembership?> GetCompanyUserMembershipAsync(
        Guid companyId, Guid identityUserId, CancellationToken cancellationToken = default)
        => _db.CompanyUserMemberships.IgnoreQueryFilters()
            .FirstOrDefaultAsync(m => m.CompanyId == companyId && m.IdentityUserId == identityUserId, cancellationToken);
    ...
    public Task AddCompanyUserMembershipAsync(CompanyUserMembership membership, CancellationToken cancellationToken = default)
    ...
    public async Task<IReadOnlyList<CompanyUserMembership>> GetCompanyUserMembershipsByCompanyAsync(
        Guid companyId, bool onlyActive = true, CancellationToken cancellationToken = default)
    {
        var q = _db.CompanyUserMemberships.IgnoreQueryFilters()
    ...
    public Task<int> CountActiveCompanyUserMembershipsByCompanyAsync(Guid companyId, CancellationToken cancellationToken = default)
        => _db.CompanyUserMemberships.IgnoreQueryFilters()
            .CountAsync(m => m.CompanyId == companyId && m.IsActive, cancellationToken);

    public Task<int> CountActiveCompanyUserMembershipsByTenantAsync(Guid tenantId, CancellationToken cancellationToken = default)
        => (
            from m in _db.CompanyUserMemberships.IgnoreQueryFilters()
            join c in _db.Companies on m.CompanyId equals c.Id
    ...
    public async Task<IReadOnlyList<AccessProfile>> GetProfilesByTenantAsync(Guid tenantId, bool onlyActive = true, CancellationToken cancellationToken = default)
    {
    ...
    public Task<AccessProfile?> GetProfileByIdAsync(Guid tenantId, Guid profileId, CancellationToken cancellationToken = default)
        => _db.AccessProfiles.FirstOrDefaultAsync(p => p.TenantId == tenantId && p.Id == profileId, cancellationToken);

    public Task AddProfileAsync(AccessProfile profile, CancellationToken cancellationToken = default)
    ...
    public async Task<IReadOnlyList<AccessProfilePermission>> GetProfilePermissionsAsync(
        Guid tenantId, Guid profileId, CancellationToken cancellationToken = default)
    ...
    public Task<AccessProfilePermission?> GetProfilePermissionAsync(
        Guid tenantId, Guid profileId, string permissionKey, CancellationToken cancellationToken = default)
        => _db.AccessProfilePermissions
            .FirstOrDefaultAsync(p => p.TenantId == tenantId && p.ProfileId == profileId && p.PermissionKey == permissionKey, cancellationToken);

    public Task AddProfilePermissionAsync(AccessProfilePermission permission, CancellationToken cancellationToken = default)
