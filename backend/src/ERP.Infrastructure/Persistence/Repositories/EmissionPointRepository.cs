...
public sealed class EmissionPointRepository : IEmissionPointRepository
{
    ...
    public Task<IReadOnlyList<EmissionPoint>> GetByEstablishmentAsync(Guid tenantId, Guid establishmentId, CancellationToken cancellationToken = default)
        => _db.EmissionPoints
            .AsNoTracking()
    ...
    {
        var q = _db.EmissionPoints
            .AsNoTracking()
            .Include(ep => ep.Establishment).ThenInclude(e => e.Branch)
        ...
        var list = await q.OrderBy(ep => ep.Establishment.Code).ThenBy(ep => ep.Code).ToListAsync(cancellationToken);
        return list;
    }

    public Task<EmissionPoint?> GetByIdAsync(Guid id, Guid tenantId, CancellationToken cancellationToken = default)
        => _db.EmissionPoints
            .Include(ep => ep.Establishment)
            .FirstOrDefaultAsync(ep => ep.Id == id && ep.TenantId == tenantId && ep.IsActive, cancellationToken);
    ...
    public async Task<IReadOnlyList<EmissionPoint>> GetActiveByBranchAsync(
        ...
        => await _db.EmissionPoints
            .AsNoTracking()
            .Include(ep => ep.Establishment)
            .Where(ep => ep.TenantId == tenantId
                      && ep.Establishment.BranchId == branchId
                      && ep.IsActive)
            .OrderBy(ep => ep.Establishment.Code).ThenBy(ep => ep.Code)
            .ToListAsync(cancellationToken);

    public async Task<EmissionPoint?> GetDefaultForBranchAsync(Guid tenantId, Guid branchId, CancellationToken cancellationToken = default)
    {
    ...
    /// <inheritdoc/>
    public async Task<EmissionPoint?> GetDefaultForCompanyAsync(Guid tenantId, Guid companyId, CancellationToken cancellationToken = default)
    {
        var establishmentId = await _db.Establishments
            .Where(e => e.TenantId == tenantId
