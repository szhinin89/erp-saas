...
public sealed class PriceListItemRepository : IPriceListItemRepository
{
    ...
    public async Task<IReadOnlyList<PriceListItem>> GetByItemAsync(
        Guid tenantId,
        Guid itemId,
        CancellationToken ct = default
    ) => await Scoped(tenantId).Where(x => x.ItemId == itemId).ToListAsync(ct);

    public async Task<IReadOnlyList<PriceListItem>> GetByPriceListAsync(
        Guid tenantId,
        Guid priceListId,
        CancellationToken ct = default
    ) =>
    ...
    public Task<PriceListItem?> FindByKeyAsync(
        Guid tenantId,
        Guid priceListId,
        Guid itemId,
        CancellationToken ct = default
        ...
        Scoped(tenantId)
            .FirstOrDefaultAsync(x => x.PriceListId == priceListId && x.ItemId == itemId, ct);
