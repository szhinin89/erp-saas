...
public sealed class DocumentSequenceRepository : IDocumentSequenceRepository
{
        ...
        string docTypeCode, CancellationToken ct = default)
    {
        ...
        var existing = await _db.DocumentSequences
            .IgnoreQueryFilters()
            .FirstOrDefaultAsync(
    ...
    public async Task<DocumentSequence?> GetForUpdateAsync(
        Guid emissionPointId, string docTypeCode, CancellationToken cancellationToken = default)
    {
        return await _db.DocumentSequences
            .FromSqlInterpolated(
                $"SELECT * FROM document_sequence WHERE emission_point_id = {emissionPointId} AND doc_type_code = {docTypeCode} FOR UPDATE")
            .FirstOrDefaultAsync(cancellationToken);
    }

    public Task<bool> ExistsAsync(Guid emissionPointId, string docTypeCode, CancellationToken cancellationToken = default)
        => _db.DocumentSequences
            .AnyAsync(s => s.EmissionPointId == emissionPointId && s.DocTypeCode == docTypeCode, cancellationToken);

    public Task AddAsync(DocumentSequence sequence, CancellationToken cancellationToken = default)
