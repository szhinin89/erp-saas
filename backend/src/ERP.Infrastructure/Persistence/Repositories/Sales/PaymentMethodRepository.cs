...
public sealed class PaymentMethodRepository : IPaymentMethodRepository
{
    ...
    {
        var q = _db.PaymentMethods.Where(x => x.TenantId == tenantId);
    ...
    public async Task<PaymentMethod?> GetByIdAsync(
        ...
        await _db.PaymentMethods.FirstOrDefaultAsync(x => x.TenantId == tenantId && x.Id == id, ct);

    public async Task<PaymentMethod?> GetByCodeAsync(
        Guid tenantId,
        string code,
        CancellationToken ct = default
    ) =>
        await _db.PaymentMethods.FirstOrDefaultAsync(
            x => x.TenantId == tenantId && x.Code == code.ToUpperInvariant(),
            ct
        );
    ...
    {
        var q = _db.PaymentMethods.Where(x =>
            x.TenantId == tenantId && x.Code == code.ToUpperInvariant()
        );
