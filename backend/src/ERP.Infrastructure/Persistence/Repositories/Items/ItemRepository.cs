...
public sealed class ItemRepository : IItemRepository
{
    ...
    public async Task<Item?> ResolveByAnyCodeAsync(
        string code,
        Guid tenantId,
        CancellationToken cancellationToken = default
    )
    {
        ...
        return await Full().FirstOrDefaultAsync(x => x.Code.SKU == upper, cancellationToken)
            ?? await Full()
                .FirstOrDefaultAsync(
                    x => x.Variants.Any(v => v.SKU == upper && v.IsActive),
                    cancellationToken
                )
            ?? await Full()
                .FirstOrDefaultAsync(
                    x => x.Variants.Any(v => v.Barcodes.Any(b => b.Code == c && b.IsActive)),
                    cancellationToken
                )
            ?? await Full()
                .FirstOrDefaultAsync(
                    x => x.PackagingLevels.Any(p => p.Barcode == c && p.IsActive),
                    cancellationToken
                );
    }
    ...
    public async Task<bool> VariantSkuExistsAsync(
        string sku,
        Guid tenantId,
        CancellationToken cancellationToken = default
    )
    {
        ...
        return await _context
            .Set<ItemVariant>()
            .AnyAsync(
                v => v.TenantId == tenantId && v.SKU == normalized && v.IsActive,
                cancellationToken
            );
    }

    public async Task<string?> GetSupplierCodeAsync(
        Guid itemId,
        Guid supplierId,
        Guid tenantId,
        CancellationToken cancellationToken = default
    ...
    public async Task<Guid?> FindItemIdBySupplierCodeAsync(
        Guid supplierId,
        string code,
        Guid tenantId,
        CancellationToken cancellationToken = default
    )
    {
        ...
        var supplierItemId = await _context
            .Set<ItemSupplierCode>()
            .Where(s =>
                s.TenantId == tenantId
                && s.SupplierId == supplierId
    ...
    )
    {
        var results = await Scoped(tenantId)
            ...
            .Select(x => new
            {
                ...
                x.Code.ShortName,
                x.Code.Description,
                Score = EF.Functions.TrigramsSimilarity(x.Code.Description, text)
        ...
        return results
            .Select(x => new ItemSimilarityMatch(
                x.Id,
                x.SKU,
                x.ShortName,
                x.Description,
    ...
    public async Task ReplaceImagesAsync(
        Guid itemId,
        IEnumerable<ItemImage> newImages,
        CancellationToken cancellationToken = default
    )
    {
    ...
    public async Task ReplaceUnitConversionsAsync(
        Guid itemId,
        IEnumerable<ItemUnitConversion> newConversions,
        CancellationToken cancellationToken = default
    )
    {
        var existing = await _context
    ...
    public async Task ReplaceSubstitutesAsync(
        Guid itemId,
        IEnumerable<ItemSubstitute> newSubstitutes,
        CancellationToken cancellationToken = default
    )
    {
        var existing = await _context
    ...
    public async Task ReplacePackagingLevelsAsync(
        Guid itemId,
        IEnumerable<ItemPackagingLevel> newLevels,
        CancellationToken cancellationToken = default
    )
    {
