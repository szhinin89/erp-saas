...
public sealed class ItemTypeRepository : IItemTypeRepository
{
    ...
    public async Task<ItemTypeDefinition?> GetByIdAsync(Guid tenantId, Guid id, CancellationToken ct = default)
        => await _db.ItemTypes
    ...
    public async Task<ItemTypeDefinition?> GetByCodeAsync(Guid tenantId, string code, CancellationToken ct = default)
    {
        var trimmed = code.Trim();
        return await _db.ItemTypes
            .FirstOrDefaultAsync(x => x.TenantId == tenantId && x.Code == trimmed, ct);
    }
    ...
    public async Task<bool> ExistsByCodeAsync(Guid tenantId, string code, Guid? excludeId = null, CancellationToken ct = default)
    {
        var trimmed = code.Trim();
        var q = _db.ItemTypes
            .Where(x => x.TenantId == tenantId && x.Code == trimmed);
