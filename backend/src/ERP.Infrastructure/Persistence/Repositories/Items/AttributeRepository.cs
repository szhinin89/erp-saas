...
public sealed class AttributeGroupRepository : IAttributeGroupRepository
{
    ...
    public async Task<IReadOnlyList<AttributeGroup>> GetAllAsync(Guid tenantId, CancellationToken cancellationToken)
        => await Scoped(tenantId).OrderBy(x => x.SortOrder).ThenBy(x => x.Name).ToListAsync(cancellationToken);

    public async Task<AttributeGroup?> GetByIdAsync(Guid id, Guid tenantId, CancellationToken cancellationToken)
        => await Scoped(tenantId).FirstOrDefaultAsync(x => x.Id == id, cancellationToken);

    public async Task<bool> ExistsByCodeAsync(string code, Guid tenantId, CancellationToken cancellationToken)
    {
...
public sealed class AttributeDefinitionRepository : IAttributeDefinitionRepository
{
    ...
    public async Task<IReadOnlyList<AttributeDefinition>> GetAllAsync(Guid tenantId, Guid? groupId, CancellationToken cancellationToken)
    {
    ...
    public async Task<AttributeDefinition?> GetByIdAsync(Guid id, Guid tenantId, CancellationToken cancellationToken)
        => await Scoped(tenantId).FirstOrDefaultAsync(x => x.Id == id, cancellationToken);

    public async Task<bool> ExistsByCodeAsync(Guid groupId, string code, Guid tenantId, CancellationToken cancellationToken)
    {
        var normalized = code.Trim().ToUpperInvariant();
        return await Scoped(tenantId).AnyAsync(x => x.GroupId == groupId && x.Code == normalized, cancellationToken);
    }
