...
public sealed class CategoryNodeRepository : ICategoryNodeRepository
{
    ...
    public async Task<IReadOnlyList<ItemCategoryNode>> GetAllAsync(Guid tenantId, bool includeInactive = false, CancellationToken ct = default)
    {
        var q = _ctx.Set<ItemCategoryNode>().Where(n => n.TenantId == tenantId);
        if (!includeInactive) q = q.Where(n => n.IsActive);
        return await q.OrderBy(n => n.Path).ThenBy(n => n.SortOrder).ToListAsync(ct);
    }

    public async Task<IReadOnlyList<ItemCategoryNode>> GetChildrenAsync(Guid parentId, CancellationToken ct = default)
        => await _ctx.Set<ItemCategoryNode>().Where(n => n.ParentId == parentId).OrderBy(n => n.SortOrder).ToListAsync(ct);

    public async Task<IReadOnlyList<ItemCategoryNode>> GetDescendantsByPathAsync(Guid tenantId, string pathPrefix, CancellationToken ct = default)
        => await _ctx.Set<ItemCategoryNode>()
            .Where(n => n.TenantId == tenantId && n.Path.StartsWith(pathPrefix))
            .OrderBy(n => n.Path).ToListAsync(ct);

    public async Task<bool> CodeExistsAsync(string code, Guid tenantId, Guid? excludeId = null, CancellationToken ct = default)
    {
        var normalized = code.Trim().ToUpperInvariant();
        var q = _ctx.Set<ItemCategoryNode>().Where(n => n.TenantId == tenantId && n.Code == normalized);
        if (excludeId.HasValue) q = q.Where(n => n.Id != excludeId.Value);
    ...
    {
        var node = await _ctx.Set<ItemCategoryNode>().AsNoTracking().FirstOrDefaultAsync(n => n.Id == nodeId, ct);
