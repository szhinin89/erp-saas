...
public sealed class SalesInvoiceConfiguration : IEntityTypeConfiguration<SalesInvoice>
{
    public void Configure(EntityTypeBuilder<SalesInvoice> builder)
    {
        ...
        builder.Property(x => x.DocTypeCode).HasColumnName("doc_type_code")
            .HasMaxLength(SalesInvoice.DocTypeCodeMaxLen).IsRequired();
        builder.Property(x => x.InvoiceNumber).HasColumnName("invoice_number")
            .HasMaxLength(SalesInvoice.InvoiceNumberMaxLen).IsRequired();
        ...
        builder.OwnsOne(x => x.Customer, cs =>
        {
            cs.Property(c => c.Name).HasColumnName("customer_name")
                .HasMaxLength(CustomerSnapshot.NameMaxLen).IsRequired();
            cs.Property(c => c.TaxId).HasColumnName("customer_tax_id")
                .HasMaxLength(CustomerSnapshot.TaxIdMaxLen).IsRequired();
            cs.Property(c => c.IdentificationType).HasColumnName("customer_id_type")
                .HasMaxLength(CustomerSnapshot.IdTypeMaxLen).IsRequired();
            cs.Property(c => c.Email).HasColumnName("customer_email")
                .HasMaxLength(CustomerSnapshot.EmailMaxLen);
            cs.Property(c => c.Address).HasColumnName("customer_address")
                .HasMaxLength(CustomerSnapshot.AddressMaxLen);
        });

        // ── Moneda ──────────────────────────────────────────────────
        builder.Property(x => x.CurrencyCode).HasColumnName("currency_code")
            .HasMaxLength(SalesInvoice.CurrencyCodeMaxLen).IsRequired();
        ...
        builder.OwnsOne(x => x.PaymentTerm, pt =>
        {
            pt.Property(p => p.Id).HasColumnName("payment_term_id").IsRequired();
            pt.Property(p => p.Name).HasColumnName("payment_term_name")
                .HasMaxLength(PaymentTermSnapshot.NameMaxLen).IsRequired();
            pt.Property(p => p.Installments).HasColumnName("payment_term_installments").IsRequired();
            pt.Property(p => p.DaysBetween).HasColumnName("payment_term_days_between").IsRequired();
        ...
        // ── Totales snapshot ────────────────────────────────────────
        builder.Property(x => x.AuthorizedSubtotal).HasColumnName("authorized_subtotal")
            .HasColumnType("numeric(18,2)");
        builder.Property(x => x.AuthorizedTotalTax).HasColumnName("authorized_total_tax")
            .HasColumnType("numeric(18,2)");
        builder.Property(x => x.AuthorizedTotalDiscount).HasColumnName("authorized_total_discount")
            .HasColumnType("numeric(18,2)");
        builder.Property(x => x.AuthorizedGrandTotal).HasColumnName("authorized_grand_total")
            .HasColumnType("numeric(18,2)");
        ...
        builder.HasIndex(x => new { x.TenantId, x.CompanyId, x.InvoiceNumber })
            .IsUnique()
