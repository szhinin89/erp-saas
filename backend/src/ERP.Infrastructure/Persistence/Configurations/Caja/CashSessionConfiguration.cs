...
public sealed class CashSessionConfiguration : IEntityTypeConfiguration<CashSession>
{
    public void Configure(EntityTypeBuilder<CashSession> builder)
    {
        ...
        builder.Property(x => x.CashRegisterCodeSnapshot).HasColumnName("cash_register_code_snapshot")
            .HasMaxLength(CashSession.CashRegisterCodeSnapshotMaxLen).IsRequired();
        builder.Property(x => x.CashRegisterNameSnapshot).HasColumnName("cash_register_name_snapshot")
            .HasMaxLength(CashSession.CashRegisterNameSnapshotMaxLen).IsRequired();

        builder.Property(x => x.EmissionPointId).HasColumnName("emission_point_id").IsRequired();
        builder.Property(x => x.EmissionPointCodeSnapshot).HasColumnName("emission_point_code_snapshot")
            .HasMaxLength(CashSession.EmissionPointCodeSnapshotMaxLen).IsRequired();

        builder.Property(x => x.OpenedAt).HasColumnName("opened_at").IsRequired();
        builder.Property(x => x.OpeningAmount).HasColumnName("opening_amount")
        ...
        builder.Property(x => x.Notes).HasColumnName("notes")
            .HasMaxLength(CashSession.NotesMaxLen);

        // ── Cierre ──────────────────────────────────────────────────
        builder.Property(x => x.ClosedAt).HasColumnName("closed_at");
        builder.Property(x => x.ClosedBy).HasColumnName("closed_by");
        builder.Property(x => x.CloseNotes).HasColumnName("close_notes")
            .HasMaxLength(CashSession.CloseNotesMaxLen);

        builder.Property(x => x.ExpectedAmount).HasColumnName("expected_amount")
            .HasColumnType("numeric(18,2)");
        builder.Property(x => x.CountedAmount).HasColumnName("counted_amount")
            .HasColumnType("numeric(18,2)");
        builder.Property(x => x.Difference).HasColumnName("difference")
            .HasColumnType("numeric(18,2)");
        ...
        builder.HasMany(x => x.ClosingCounts)
            .WithOne()
            .HasForeignKey(x => x.CashSessionId)
            .OnDelete(DeleteBehavior.Cascade);
        ...
        builder.HasIndex(x => new { x.TenantId, x.CashRegisterId, x.Status })
            .HasDatabaseName("ix_cash_sessions_tenant_register_status");

        builder.HasIndex(x => new { x.TenantId, x.EmissionPointId })
