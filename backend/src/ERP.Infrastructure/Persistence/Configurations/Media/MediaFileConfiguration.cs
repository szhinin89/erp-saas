...
public sealed class MediaFileConfiguration : IEntityTypeConfiguration<MediaFile>
{
    public void Configure(EntityTypeBuilder<MediaFile> builder)
    {
        ...
        builder.Property(x => x.FileName).HasColumnName("file_name")
            .HasMaxLength(MediaFile.MaxFileNameLength).IsRequired();
        ...
        builder.Property(x => x.ContentType).HasColumnName("content_type")
            .HasMaxLength(MediaFile.MaxContentTypeLength).IsRequired();
        builder.Property(x => x.SizeBytes).HasColumnName("size_bytes").IsRequired();

        builder.Property(x => x.StorageProvider).HasColumnName("storage_provider")
            .HasMaxLength(MediaFile.MaxStorageProviderLength).IsRequired();
        builder.Property(x => x.StoragePath).HasColumnName("storage_path")
            .HasMaxLength(MediaFile.MaxStoragePathLength).IsRequired();
        builder.Property(x => x.PublicUrl).HasColumnName("public_url")
        ...
        builder.Property(x => x.Checksum).HasColumnName("checksum").HasMaxLength(MediaFile.MaxChecksumLength);
        builder.Property(x => x.AltText).HasColumnName("alt_text").HasMaxLength(MediaFile.MaxAltTextLength);
        ...
        builder.HasIndex(x => new { x.TenantId, x.CompanyId, x.OwnerType, x.OwnerId, x.Role, x.IsPrimary, x.IsActive })
