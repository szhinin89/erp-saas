...
public sealed class StockTransferConfiguration : IEntityTypeConfiguration<StockTransfer>
{
    public void Configure(EntityTypeBuilder<StockTransfer> builder)
    {
        ...
        builder.Property(x => x.TransferNumber).HasColumnName("transfer_number")
            .HasMaxLength(StockTransfer.NumberMaxLen).IsRequired();
        builder.Property(x => x.OperationBranchId).HasColumnName("operation_branch_id").IsRequired();
        builder.Property(x => x.SourceWarehouseId).HasColumnName("source_warehouse_id").IsRequired();
        builder.Property(x => x.TargetWarehouseId).HasColumnName("target_warehouse_id").IsRequired();
        builder.Property(x => x.TransferDate).HasColumnName("transfer_date").IsRequired();
        builder.Property(x => x.Status).HasColumnName("status")
            .HasMaxLength(StockTransfer.StatusMaxLen).IsRequired();
        ...
        builder.HasOne(x => x.SourceWarehouse).WithMany()
            .HasForeignKey(x => x.SourceWarehouseId).OnDelete(DeleteBehavior.Restrict);
        builder.HasOne(x => x.TargetWarehouse).WithMany()
        ...
        builder.HasIndex(x => new { x.TenantId, x.CompanyId, x.TransferNumber })
            .IsUnique()
