...
public sealed class StockAdjustmentConfiguration : IEntityTypeConfiguration<StockAdjustment>
{
    public void Configure(EntityTypeBuilder<StockAdjustment> builder)
    {
        ...
        builder.Property(x => x.AdjustmentNumber).HasColumnName("adjustment_number")
            .HasMaxLength(StockAdjustment.NumberMaxLen).IsRequired();
        builder.Property(x => x.WarehouseId).HasColumnName("warehouse_id").IsRequired();
        builder.Property(x => x.WarehouseName).HasColumnName("warehouse_name")
            .HasMaxLength(StockAdjustment.NameSnapshotMaxLen).IsRequired();
        builder.Property(x => x.ProductId).HasColumnName("product_id").IsRequired();
        builder.Property(x => x.ProductName).HasColumnName("product_name")
            .HasMaxLength(StockAdjustment.NameSnapshotMaxLen).IsRequired();
        builder.Property(x => x.AdjustmentQty).HasColumnName("adjustment_qty")
            .HasColumnType("numeric(18,4)").IsRequired();
        builder.Property(x => x.AdjustmentType).HasColumnName("adjustment_type")
            .HasMaxLength(StockAdjustment.AdjustmentTypeMaxLen).IsRequired();
        builder.Property(x => x.Reason).HasColumnName("reason")
            .HasMaxLength(StockAdjustment.ReasonMaxLen).IsRequired();
        ...
        builder.Property(x => x.Status).HasColumnName("status")
            .HasMaxLength(StockAdjustment.StatusMaxLen).IsRequired();
