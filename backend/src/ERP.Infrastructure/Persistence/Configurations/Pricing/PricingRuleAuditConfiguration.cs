...
public sealed class PricingRuleAuditConfiguration : IEntityTypeConfiguration<PricingRuleAudit>
{
    public void Configure(EntityTypeBuilder<PricingRuleAudit> builder)
    {
        ...
        builder.Property(x => x.OldRuleType).HasColumnName("old_rule_type").HasConversion<int>().IsRequired();
        builder.Property(x => x.OldRuleValue).HasColumnName("old_rule_value").HasColumnType("numeric(18,6)").IsRequired();
        builder.Property(x => x.NewRuleType).HasColumnName("new_rule_type").HasConversion<int>().IsRequired();
        builder.Property(x => x.NewRuleValue).HasColumnName("new_rule_value").HasColumnType("numeric(18,6)").IsRequired();
