...
public sealed class RidePdfDocumentConfiguration : IEntityTypeConfiguration<RidePdfDocument>
{
    public void Configure(EntityTypeBuilder<RidePdfDocument> builder)
    {
        ...
        builder.Property(x => x.ElectronicDocumentId).HasColumnName("electronic_document_id").IsRequired();

        builder.Property(x => x.DocumentType).HasColumnName("document_type")
            .HasConversion<int>().IsRequired();

        builder.Property(x => x.SourceXmlHash).HasColumnName("source_xml_hash")
        ...
        builder.Property(x => x.TemplateId).HasColumnName("template_id")
            .HasMaxLength(RidePdfDocument.TemplateIdMaxLen).IsRequired();
        builder.Property(x => x.TemplateVersion).HasColumnName("template_version")
            .HasMaxLength(RidePdfDocument.VersionMaxLen).IsRequired();
        builder.Property(x => x.BrandingVersion).HasColumnName("branding_version")
            .HasMaxLength(RidePdfDocument.VersionMaxLen).IsRequired();
        builder.Property(x => x.RendererVersion).HasColumnName("renderer_version")
            .HasMaxLength(RidePdfDocument.VersionMaxLen).IsRequired();
        builder.Property(x => x.RideSpecificationVersion).HasColumnName("ride_specification_version")
            .HasMaxLength(RidePdfDocument.VersionMaxLen).IsRequired();

        builder.Property(x => x.State).HasColumnName("state")
            .HasConversion<int>().IsRequired();

        builder.Property(x => x.StoragePath).HasColumnName("storage_path").HasMaxLength(RidePdfDocument.PathMaxLen);
        builder.Property(x => x.LastError).HasColumnName("last_error").HasMaxLength(RidePdfDocument.ReasonMaxLen);
