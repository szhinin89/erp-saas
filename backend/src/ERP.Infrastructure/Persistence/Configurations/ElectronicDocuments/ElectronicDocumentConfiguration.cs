...
public sealed class ElectronicDocumentConfiguration : IEntityTypeConfiguration<ElectronicDocument>
{
    public void Configure(EntityTypeBuilder<ElectronicDocument> builder)
    {
        ...
        builder.Property(x => x.SourceModule).HasColumnName("source_module")
            .HasMaxLength(ElectronicDocument.SourceModuleMaxLen).IsRequired();
        ...
        builder.Property(x => x.XmlVersion).HasColumnName("xml_version").HasMaxLength(ElectronicDocument.VersionMaxLen);
        builder.Property(x => x.SchemaVersion).HasColumnName("schema_version").HasMaxLength(ElectronicDocument.VersionMaxLen);

        builder.Property(x => x.XmlDraftPath).HasColumnName("xml_draft_path").HasMaxLength(ElectronicDocument.PathMaxLen);
        builder.Property(x => x.SignedXmlPath).HasColumnName("signed_xml_path").HasMaxLength(ElectronicDocument.PathMaxLen);
        builder.Property(x => x.AuthorizedXmlPath).HasColumnName("authorized_xml_path").HasMaxLength(ElectronicDocument.PathMaxLen);
        ...
        builder.Property(x => x.PreDeadLetterState).HasColumnName("pre_dead_letter_state").HasConversion<int?>();
        ...
        builder.HasIndex(x => new { x.TenantId, x.SourceModule, x.SourceEntityId })
            .IsUnique()
