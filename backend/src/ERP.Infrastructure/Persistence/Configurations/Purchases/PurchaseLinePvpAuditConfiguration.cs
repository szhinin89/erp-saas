...
public sealed class PurchaseLinePvpAuditConfiguration : IEntityTypeConfiguration<PurchaseLinePvpAudit>
{
    public void Configure(EntityTypeBuilder<PurchaseLinePvpAudit> builder)
    {
        ...
        builder.Property(x => x.OldPvp).HasColumnName("old_pvp").HasColumnType("numeric(18,6)").IsRequired();
        builder.Property(x => x.NewPvp).HasColumnName("new_pvp").HasColumnType("numeric(18,6)").IsRequired();

        builder.HasIndex(x => new { x.TenantId, x.EntityId, x.OccurredAtUtc })
            .HasDatabaseName("ix_purchase_line_pvp_audit_item_occurred_at");
        builder.HasIndex(x => new { x.TenantId, x.PurchaseInvoiceId, x.OccurredAtUtc })
