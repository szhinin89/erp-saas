...
public sealed class IssuedWithholdingConfiguration : IEntityTypeConfiguration<IssuedWithholding>
{
    public void Configure(EntityTypeBuilder<IssuedWithholding> builder)
    {
        ...
        builder.Property(x => x.WithholdingNumber).HasColumnName("withholding_number")
            .HasMaxLength(IssuedWithholding.NumberMaxLen).IsRequired();
        builder.Property(x => x.IssueDate).HasColumnName("issue_date").IsRequired();

        // ── Totals ──────────────────────────────────────────────────────
        builder.Property(x => x.TotalRetainedVat).HasColumnName("total_retained_vat")
            .HasColumnType("numeric(18,2)").IsRequired();
        builder.Property(x => x.TotalRetainedIncome).HasColumnName("total_retained_income")
            .HasColumnType("numeric(18,2)").IsRequired();
        builder.Property(x => x.TotalRetainedIsd).HasColumnName("total_retained_isd")
            .HasColumnType("numeric(18,2)").IsRequired();
        builder.Property(x => x.TotalRetained).HasColumnName("total_retained")
            .HasColumnType("numeric(18,2)").IsRequired();
        ...
        builder.Property(x => x.XmlPath).HasColumnName("xml_path")
            .HasMaxLength(IssuedWithholding.PathMaxLen);
        builder.Property(x => x.SignedXmlPath).HasColumnName("signed_xml_path")
            .HasMaxLength(IssuedWithholding.PathMaxLen);
        builder.Property(x => x.PdfPath).HasColumnName("pdf_path")
            .HasMaxLength(IssuedWithholding.PathMaxLen);
        builder.Property(x => x.SriStatus).HasColumnName("sri_status")
            .HasConversion<int>().HasDefaultValue(SriDocumentStatus.None).IsRequired();
        builder.Property(x => x.SriReceiptNumber).HasColumnName("sri_receipt_number")
            .HasMaxLength(IssuedWithholding.SriReceiptMaxLen);
        builder.Property(x => x.SriAuthorizationNumber).HasColumnName("sri_authorization_number")
            .HasMaxLength(IssuedWithholding.AccessKeyLen);
        builder.Property(x => x.SriAuthorizationDate).HasColumnName("sri_authorization_date");
        builder.Property(x => x.SriMessage).HasColumnName("sri_message")
        ...
        // ── Relationships ───────────────────────────────────────────────
        builder.HasMany(x => x.Details).WithOne().HasForeignKey(x => x.WithholdingId)
            .OnDelete(DeleteBehavior.Cascade);

        builder.HasOne<PurchaseInvoice>().WithMany().HasForeignKey(x => x.PurchaseInvoiceId)
            .OnDelete(DeleteBehavior.Restrict);
        builder.HasOne<BusinessPartner>().WithMany().HasForeignKey(x => x.SupplierId)
            .OnDelete(DeleteBehavior.Restrict);
        builder.HasOne<EmissionPoint>().WithMany().HasForeignKey(x => x.EmissionPointId)
            .OnDelete(DeleteBehavior.Restrict);

        // ── Indexes ─────────────────────────────────────────────────────
        builder.HasIndex(x => x.PurchaseInvoiceId).IsUnique()
        ...
        builder.HasIndex(x => new { x.TenantId, x.CompanyId, x.WithholdingNumber }).IsUnique()
