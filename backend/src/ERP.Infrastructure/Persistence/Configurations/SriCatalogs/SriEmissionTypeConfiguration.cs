...
public class SriEmissionTypeConfiguration : IEntityTypeConfiguration<SriEmissionType>
{
    ...
    {
        builder.ToTable("sri_emission_type");
        builder.HasKey(x => x.Code);
        builder.Property(x => x.Code).HasColumnName("code");
        builder.Property(x => x.Name).HasColumnName("name").HasMaxLength(60).IsRequired();

        builder.HasData(
            new SriEmissionType { Code = 1, Name = "Emisión Normal" },
            new SriEmissionType { Code = 2, Name = "Emisión por Indisponibilidad del Sistema" }
        );
    }
}
