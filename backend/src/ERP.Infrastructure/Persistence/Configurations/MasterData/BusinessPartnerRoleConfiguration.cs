...
///
/// FK COMPUESTO (ADR-BP-13):
///   (business_partner_id, subscriber_id) → master_business_partners(id, subscriber_id)
///   Garantiza cross-tenant safety a nivel BD. Requiere uq_mbp_id_subscriber en BusinessPartner.
///
...
///
/// QUERY FILTER:
///   Aplicado automáticamente (ISubscriberScopedEntity → subscriber fail-closed).
///
/// ÍNDICE ÚNICO DE ROL:
...
public sealed class BusinessPartnerRoleConfiguration : IEntityTypeConfiguration<BusinessPartnerRole>
{
    public void Configure(EntityTypeBuilder<BusinessPartnerRole> builder)
    {
        ...
        builder.HasAlternateKey(r => new { r.Id, r.SubscriberId })
        ...
        // Composite FK (ADR-BP-13) delegada a migration SQL raw — EF API limitation.
        // Cross-tenant safety garantizada por global query filter (fail-closed).
