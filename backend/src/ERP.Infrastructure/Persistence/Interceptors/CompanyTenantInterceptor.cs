...
/// tenga SubscriberId y CompanyId válidos (no Guid.Empty).
...
public sealed class CompanyTenantInterceptor : SaveChangesInterceptor
{
    ...
    private static void Validate(DbContext? ctx)
    {
        ...
        foreach (var entry in ctx.ChangeTracker.Entries())
        {
            ...
            if (entry.Entity is ISubscriberScopedEntity sub && sub.SubscriberId == Guid.Empty)
                throw new InvalidOperationException(
                    $"[TenantGuard] {entry.Entity.GetType().Name}: SubscriberId is Guid.Empty. " +
                    "All tenant-scoped entities require a valid SubscriberId.");
