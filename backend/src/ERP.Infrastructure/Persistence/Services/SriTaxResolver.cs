...
public sealed class SriTaxResolver : ISriTaxResolver, PurchasesTaxResolver
{
    ...
    public async Task<TaxRateResult?> GetVatRateWithNameAsync(string vatCode, CancellationToken ct = default)
    {
        var rate = await _db.SriVatRates.AsNoTracking()
            .FirstOrDefaultAsync(r => r.Code == vatCode && r.IsActive, ct);
    ...
    public async Task<TaxRateResult?> GetIceRateWithNameAsync(string iceCode, CancellationToken ct = default)
    {
        var rate = await _db.SriIceRates.AsNoTracking()
            .FirstOrDefaultAsync(r => r.Code == iceCode && r.IsActive, ct);
        return rate?.Percentage is null ? null : new TaxRateResult(rate.Percentage.Value, rate.Name);
    }

    public async Task<string?> GetPaymentMethodNameAsync(string code, CancellationToken ct = default)
    {
        var pm = await _db.SriPaymentMethods.AsNoTracking()
