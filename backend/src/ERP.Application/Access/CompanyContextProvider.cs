...
public sealed class CompanyContextProvider : ICompanyContextProvider
{
    ...
    public async Task<Guid?> ResolveDefaultCompanyIdAsync(Guid tenantId, CancellationToken cancellationToken = default)
    {
    ...
    public Task<int> CountActiveCompaniesAsync(Guid tenantId, CancellationToken cancellationToken = default)
        => _companyRepository.CountActiveByTenantIdAsync(tenantId, cancellationToken);

    public Task<OperationalCompanyContext?> ResolveOperationalForCurrentUserAsync(CancellationToken cancellationToken = default)
    {
    ...
    public async Task<OperationalCompanyContext?> ResolveOperationalForUserAsync(Guid userId, CancellationToken cancellationToken = default)
    {
        ...
        var membership = await _repo.GetCompanyUserMembershipAsync(companyId, userId, cancellationToken);
    ...
    private async Task<Guid> ResolveOperationalCompanyIdAsync(Guid userId, CancellationToken cancellationToken)
    {
        if (_currentCompany.HasCompanyContext && _currentCompany.CompanyId != Guid.Empty)
            return _currentCompany.CompanyId;

        var memberships = await _repo.GetActiveCompanyUserMembershipsForUserSystemAsync(userId, cancellationToken);
