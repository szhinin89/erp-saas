...
public sealed class CreateInitialAdminHandler : IRequestHandler<CreateInitialAdminCommand, Result<string>>
{
    ...
    public async Task<Result<string>> Handle(CreateInitialAdminCommand cmd, CancellationToken cancellationToken)
    {
        ...
        if (state is null)
            return Result<string>.Failure("No hay un token de instalación activo. Reinicie el backend para generar uno.");
        ...
        if (state.SetupTokenExpiryUtc.Value <= DateTime.UtcNow)
            return Result<string>.Failure("Token de inicialización expirado. Reinicie el backend para generar uno nuevo.");
        ...
        await _unitOfWork.ExecuteInTransactionAsync(async ct =>
        {
            ...
            var membership = CompanyUserMembership.Create(
                company.Id, user.Id, role: SecurityRoles.Admin, profileId: null, createdBy: bootstrapId);
