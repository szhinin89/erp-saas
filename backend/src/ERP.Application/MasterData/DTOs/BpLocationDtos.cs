...
{
    public static BpLocationDto From(BusinessPartnerLocation l) =>
        new(
            l.Id,
            l.BusinessPartnerId,
            l.Name,
...
{
    public static BpContactDto From(BusinessPartnerContact c) =>
        new(
            c.Id,
            c.BusinessPartnerId,
            c.LocationId,
...
{
    internal static string LocationTypeLabel(LocationType t) =>
        t switch
        {
    ...
    internal static string[] ParsePurposes(LocationPurpose p)
    {
        ...
        if (p.HasFlag(LocationPurpose.Billing))
            list.Add("Facturación");
        if (p.HasFlag(LocationPurpose.Delivery))
            list.Add("Entrega");
        if (p.HasFlag(LocationPurpose.Fiscal))
            list.Add("Fiscal");
        if (p.HasFlag(LocationPurpose.Correspondence))
            list.Add("Correspondencia");
        return list.ToArray();
    }

    internal static string ContactRoleLabel(ContactRole r) =>
        r switch
        {
