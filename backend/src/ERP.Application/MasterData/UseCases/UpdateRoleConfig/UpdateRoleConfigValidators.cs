    ...
    public UpdateSupplierRoleConfigValidator()
    {
        ...
        When(x => x.Config is not null, () =>
        {
            RuleFor(x => x.Config.PaymentTermId)
                .NotEmpty().WithMessage("El proveedor debe tener una condición de pago obligatoria.");
            ...
            RuleFor(x => x.Config.RefundProviderTypeCode)
                .MaximumLength(SupplierRoleConfig.SriCodeMaxLen).When(x => x.Config.RefundProviderTypeCode is not null);
        });
    }
}
