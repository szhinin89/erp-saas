...
/// <summary>
/// Crea un contacto del BP. Subscriber-scoped (scope cambiado en Fase 4 — ADR-BP-02).
...
public sealed record CreateBpContactCommand(
    ...
    : IRequest<Result<BpContactDto>>, ISubscriberScopedRequest;
...
public sealed record GetBpContactsQuery(
    ...
    : IRequest<Result<IReadOnlyList<BpContactDto>>>, ISubscriberScopedRequest;
    ...
    : IRequestHandler<CreateBpContactCommand, Result<BpContactDto>>
{
    ...
    {
        if (cmd.IsPrimary)
            await _contactRepo.ClearPrimaryAsync(cmd.BusinessPartnerId, ct);
    ...
    : IRequestHandler<UpdateBpContactCommand, Result<BpContactDto>>
{
    ...
    {
        var contact = await _contactRepo.GetByIdAsync(cmd.ContactId, ct);
    ...
    : IRequestHandler<SetPrimaryBpContactCommand, Result<bool>>
{
    ...
    public async Task<Result<bool>> Handle(SetPrimaryBpContactCommand cmd, CancellationToken ct)
    {
        ...
        await _contactRepo.ClearPrimaryAsync(contact.BusinessPartnerId, ct);
    ...
    : IRequestHandler<ActivateBpContactCommand, Result<bool>>
{
    ...
    public async Task<Result<bool>> Handle(ActivateBpContactCommand cmd, CancellationToken ct)
    {
        var contact = await _contactRepo.GetByIdAsync(cmd.ContactId, ct);
        if (contact is null) return Result<bool>.NotFound("Contacto no encontrado.");
    ...
    : IRequestHandler<GetBpContactsQuery, Result<IReadOnlyList<BpContactDto>>>
{
    ...
    {
        var contacts = await _contactRepo.GetByBusinessPartnerAsync(q.BusinessPartnerId, q.OnlyActive, ct);
    ...
    : IRequestHandler<GetBpContactByIdQuery, Result<BpContactDto>>
{
    ...
    {
        var contact = await _contactRepo.GetByIdAsync(q.ContactId, ct);
