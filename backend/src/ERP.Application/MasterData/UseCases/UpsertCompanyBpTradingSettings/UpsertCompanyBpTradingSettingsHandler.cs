    ...
    : IRequestHandler<UpsertCompanyBpTradingSettingsCommand, Result<CompanyBpTradingSettingsDto>>
{
    ...
    public async Task<Result<CompanyBpTradingSettingsDto>> Handle(
        UpsertCompanyBpTradingSettingsCommand cmd, CancellationToken ct)
    {
        if (!_ctx.HasCompany)
            return Result<CompanyBpTradingSettingsDto>.Failure("Contexto de empresa no establecido.");

        var existing = await _settingsRepo.GetByBusinessPartnerAsync(cmd.BusinessPartnerId, ct);
        ...
        else
        {
            try
            {
                settings = CompanyBpTradingSettings.Create(
                    _ctx.SubscriberId, _ctx.CompanyId, cmd.BusinessPartnerId,
