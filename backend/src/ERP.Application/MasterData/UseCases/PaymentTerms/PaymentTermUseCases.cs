...
{
    public static PaymentTermDto From(PaymentTerm e) =>
        new(
            e.Id,
            e.Code,
            e.Name,
            e.Installments,
            e.DaysBetweenInstallments,
            e.TotalDays,
            e.Summary,
            e.IsActive
        );
...
{
    public async Task<Result<PaymentTermDto>> Handle(
...
{
    public async Task<Result<PaymentTermDto>> Handle(
        CreatePaymentTermCommand cmd,
        CancellationToken ct
    )
    {
        if (await repo.ExistsByCodeAsync(tenant.TenantId, cmd.Code, cancellationToken: ct))
            return Result<PaymentTermDto>.Conflict(
                $"Ya existe una condición de pago con código '{cmd.Code.Trim().ToUpperInvariant()}'."
            );
...
{
    public async Task<Result<PaymentTermDto>> Handle(
        UpdatePaymentTermCommand cmd,
        CancellationToken ct
    )
    {
        var entity = await repo.GetByIdAsync(tenant.TenantId, cmd.Id, ct);
        if (entity is null)
            return Result<PaymentTermDto>.NotFound("Condición de pago no encontrada.");
...
) : IRequestHandler<EnablePaymentTermCommand, Result<bool>>
{
    public async Task<Result<bool>> Handle(EnablePaymentTermCommand cmd, CancellationToken ct)
    {
        var entity = await repo.GetByIdAsync(tenant.TenantId, cmd.Id, ct);
        if (entity is null)
            return Result<bool>.NotFound("Condición de pago no encontrada.");
