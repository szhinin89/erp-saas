...
/// </summary>
public sealed class PurchaseInvoiceConfirmedPostingTranslator : INotificationHandler<PurchaseInvoiceConfirmedEvent>
{
    ...
    public PurchaseInvoiceConfirmedPostingTranslator(
        IPostingEngine postingEngine, ILogger<PurchaseInvoiceConfirmedPostingTranslator> logger)
    {
    ...
    public async Task Handle(PurchaseInvoiceConfirmedEvent e, CancellationToken ct)
    {
        var fact = new PostingFact(
            e.TenantId!.Value, e.CompanyId, SourceModuleName, FactTypeName, e.InvoiceId, e.IssueDate,
            e.Subtotal, e.TotalVat, e.TotalIce, e.TotalDiscount, e.GrandTotal);
        ...
        if (!result.IsSuccess)
        {
            _logger.LogWarning(
                ...
                e.InvoiceId, e.InvoiceNumber, result.Code, result.Error);
        }
    }
}
