    ...
    : IRequestHandler<ApproveExpenseCommand, Result<ExpenseInvoiceDto>>
{
    ...
    public async Task<Result<ExpenseInvoiceDto>> Handle(ApproveExpenseCommand command, CancellationToken ct)
    {
        ...
        try
        {
            ...
            var journalEntryResult = await _accounting.CreateExpenseJournalEntryAsync(
                gasto.Id,
                gasto.Category,
            ...
            if (!journalEntryResult.IsSuccess)
            {
                ...
                return Result<ExpenseInvoiceDto>.Failure(
                    journalEntryResult.Error ?? "No se pudo registrar el asiento contable del gasto.");
            }

            gasto.Approve(userId, journalEntryResult.Value);
            ...
            await _activity.AddAsync(UserActivity.Create(
                ...
                description: $"{gasto.Concept} — asiento: {journalEntryResult.Value}"), ct);
