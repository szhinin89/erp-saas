    ...
    : IRequestHandler<ValidateSriConfigurationQuery, Result<SriConfigurationValidationDto>>
{
    ...
    public async Task<Result<SriConfigurationValidationDto>> Handle(
        ValidateSriConfigurationQuery request, CancellationToken cancellationToken)
    {
        ...
        if (settings is null)
        {
            checks.Add(new SriConfigurationCheckDto(
                "configuration", false, "No existe configuración SRI registrada para esta empresa."));
            return Result<SriConfigurationValidationDto>.Success(
                new SriConfigurationValidationDto(false, checks, certificateInfo));
        }
        checks.Add(new SriConfigurationCheckDto("configuration", true, "Existe configuración SRI registrada."));

        var environmentValid = settings.Environment is 1 or 2;
        checks.Add(new SriConfigurationCheckDto(
            "environment", environmentValid,
            environmentValid ? "El ambiente configurado es válido." : "El ambiente configurado no es válido."));

        var certStatus = await _certStatusResolver.ResolveAsync(settings, cancellationToken);

        checks.Add(new SriConfigurationCheckDto(
            "certificateFile", certStatus.Installed,
        ...
        {
            checks.Add(new SriConfigurationCheckDto(
                "certificatePassword", certStatus.PasswordCorrect,
            ...
            {
                daysRemaining = (int)Math.Ceiling((certStatus.NotAfterUtc.Value - DateTime.UtcNow).TotalDays);
                var notExpired = certStatus.NotAfterUtc.Value > DateTime.UtcNow;
                checks.Add(new SriConfigurationCheckDto(
                    "certificateExpiry", notExpired,
                    notExpired
                ...
                var company = await _companyRepo.GetByIdForTenantAsync(companyId, _currentTenant.TenantId, cancellationToken);
                if (company is not null && !string.IsNullOrWhiteSpace(certStatus.Subject))
                {
                    var rucDigits = new string(company.TaxIdentificationNumber.Where(char.IsDigit).ToArray());
                    var cedulaPrefix = rucDigits.Length >= 10 ? rucDigits[..10] : rucDigits;
                    var subjectMatches = cedulaPrefix.Length > 0
                        && certStatus.Subject.Contains(cedulaPrefix, StringComparison.Ordinal);

                    checks.Add(new SriConfigurationCheckDto(
                        "certificateOwner", subjectMatches,
                        subjectMatches
            ...
            certificateInfo = new SriCertificateInfoDto(
                certStatus.PasswordCorrect, certStatus.PasswordCorrect, certStatus.NotAfterUtc, daysRemaining,
                certStatus.Subject, certStatus.Issuer, certStatus.ErrorMessage);
        }
        ...
        checks.Add(new SriConfigurationCheckDto(
            "wsdlUrl", urlValid,
            urlValid ? "La URL del webservice SRI tiene un formato válido." : "La URL del webservice SRI no es válida."));

        if (urlValid)
        {
            var reachable = await _connectivityChecker.PingAsync(settings.WsdlUrl, cancellationToken);
            checks.Add(new SriConfigurationCheckDto(
                "wsdlReachable", reachable,
                reachable
