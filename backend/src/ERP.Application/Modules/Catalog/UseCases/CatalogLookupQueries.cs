...
public sealed record SriRetentionCodeDto(
    Guid Id,
    string TaxType,
    string Code,
    string Name,
...
public sealed record GetSriUomsQuery
    : IRequest<Result<IReadOnlyList<SriUomDto>>>,
        IPlatformScopedRequest;

public sealed record GetSriVatRatesQuery
    : IRequest<Result<IReadOnlyList<SriVatRateDto>>>,
        IPlatformScopedRequest;

public sealed record GetSriIceRatesQuery
    : IRequest<Result<IReadOnlyList<SriIceRateDto>>>,
        IPlatformScopedRequest;

public sealed record GetSriRetentionCodesQuery(string? TaxType = null)
    : IRequest<Result<IReadOnlyList<SriRetentionCodeDto>>>,
        IPlatformScopedRequest;

public sealed record GetSriTaxSupportCodesQuery
    : IRequest<Result<IReadOnlyList<SriTaxSupportDto>>>,
        IPlatformScopedRequest;

public sealed record GetSriDocTypesQuery
    : IRequest<Result<IReadOnlyList<SriDocTypeDto>>>,
        IPlatformScopedRequest;

public sealed record GetSriPaymentMethodsQuery
    : IRequest<Result<IReadOnlyList<SriPaymentMethodDto>>>,
        IPlatformScopedRequest;

public sealed record GetSriSupplierTypesQuery
    : IRequest<Result<IReadOnlyList<SriSupplierTypeDto>>>,
        IPlatformScopedRequest;

public sealed record GetSriTaxRegimesQuery
...
public sealed record GetCatalogBarcodeTypesQuery
    : IRequest<Result<IReadOnlyList<CatalogBarcodeTypeDto>>>,
        IPlatformScopedRequest;

public sealed record GetCatalogItemMarginStatusesQuery
    : IRequest<Result<IReadOnlyList<CatalogItemMarginStatusDto>>>,
        IPlatformScopedRequest;

public sealed record GetSriIdTypesQuery
    : IRequest<Result<IReadOnlyList<SriIdTypeDto>>>,
        IPlatformScopedRequest;
...
public sealed class GetSriUomsQueryHandler
    : IRequestHandler<GetSriUomsQuery, Result<IReadOnlyList<SriUomDto>>>
{
    ...
    public async Task<Result<IReadOnlyList<SriUomDto>>> Handle(
        GetSriUomsQuery request,
...
public sealed class GetSriVatRatesQueryHandler
    : IRequestHandler<GetSriVatRatesQuery, Result<IReadOnlyList<SriVatRateDto>>>
{
    private readonly ISriCatalogLookupRepository _repo;

    public GetSriVatRatesQueryHandler(ISriCatalogLookupRepository repo) => _repo = repo;

    public async Task<Result<IReadOnlyList<SriVatRateDto>>> Handle(
        GetSriVatRatesQuery request,
        CancellationToken cancellationToken
    )
    {
        // DATETIME-COMPANY-CLOCK-GLOBAL-FIX-01: catálogo SRI es platform-scoped (sin
        // tenant/empresa en contexto — ver comentario de la sección de queries), así que no puede
...
public sealed class GetSriIceRatesQueryHandler
    : IRequestHandler<GetSriIceRatesQuery, Result<IReadOnlyList<SriIceRateDto>>>
{
    private readonly ISriCatalogLookupRepository _repo;

    public GetSriIceRatesQueryHandler(ISriCatalogLookupRepository repo) => _repo = repo;

    public async Task<Result<IReadOnlyList<SriIceRateDto>>> Handle(
        GetSriIceRatesQuery request,
        CancellationToken cancellationToken
    )
    {
...
public sealed class GetSriRetentionCodesQueryHandler
    : IRequestHandler<GetSriRetentionCodesQuery, Result<IReadOnlyList<SriRetentionCodeDto>>>
{
    private readonly ISriCatalogLookupRepository _repo;

    public GetSriRetentionCodesQueryHandler(ISriCatalogLookupRepository repo) => _repo = repo;

    public async Task<Result<IReadOnlyList<SriRetentionCodeDto>>> Handle(
        GetSriRetentionCodesQuery request,
        CancellationToken cancellationToken
    )
    {
        ...
        return Result<IReadOnlyList<SriRetentionCodeDto>>.Success(
            items
                .Select(r => new SriRetentionCodeDto(
                    r.Id,
                    r.TaxType,
                    r.Code,
                    r.Name,
...
public sealed class GetSriTaxSupportCodesQueryHandler
    : IRequestHandler<GetSriTaxSupportCodesQuery, Result<IReadOnlyList<SriTaxSupportDto>>>
{
    private readonly ISriCatalogLookupRepository _repo;

    public GetSriTaxSupportCodesQueryHandler(ISriCatalogLookupRepository repo) => _repo = repo;

    public async Task<Result<IReadOnlyList<SriTaxSupportDto>>> Handle(
        GetSriTaxSupportCodesQuery request,
        CancellationToken cancellationToken
    )
    {
...
public sealed class GetSriDocTypesQueryHandler
    : IRequestHandler<GetSriDocTypesQuery, Result<IReadOnlyList<SriDocTypeDto>>>
{
    private readonly ISriCatalogLookupRepository _repo;

    public GetSriDocTypesQueryHandler(ISriCatalogLookupRepository repo) => _repo = repo;

    public async Task<Result<IReadOnlyList<SriDocTypeDto>>> Handle(
        GetSriDocTypesQuery request,
        CancellationToken cancellationToken
    )
    {
        ...
        return Result<IReadOnlyList<SriDocTypeDto>>.Success(
            items
                .Select(d => new SriDocTypeDto(d.Code, d.Name, d.ShortName, d.IsElectronic))
                .ToList()
        );
    }
}

public sealed class GetSriPaymentMethodsQueryHandler
    : IRequestHandler<GetSriPaymentMethodsQuery, Result<IReadOnlyList<SriPaymentMethodDto>>>
{
    private readonly ISriCatalogLookupRepository _repo;

    public GetSriPaymentMethodsQueryHandler(ISriCatalogLookupRepository repo) => _repo = repo;

    public async Task<Result<IReadOnlyList<SriPaymentMethodDto>>> Handle(
        GetSriPaymentMethodsQuery request,
        CancellationToken cancellationToken
    )
    {
...
public sealed class GetSriSupplierTypesQueryHandler
    : IRequestHandler<GetSriSupplierTypesQuery, Result<IReadOnlyList<SriSupplierTypeDto>>>
{
    private readonly ISriCatalogLookupRepository _repo;

    public GetSriSupplierTypesQueryHandler(ISriCatalogLookupRepository repo) => _repo = repo;

    public async Task<Result<IReadOnlyList<SriSupplierTypeDto>>> Handle(
        GetSriSupplierTypesQuery request,
        CancellationToken cancellationToken
    )
    {
...
public sealed class GetSriTaxRegimesQueryHandler
    : IRequestHandler<GetSriTaxRegimesQuery, Result<IReadOnlyList<SriTaxRegimeDto>>>
{
    private readonly ISriCatalogLookupRepository _repo;

    public GetSriTaxRegimesQueryHandler(ISriCatalogLookupRepository repo) => _repo = repo;

    public async Task<Result<IReadOnlyList<SriTaxRegimeDto>>> Handle(
        GetSriTaxRegimesQuery request,
        CancellationToken cancellationToken
    )
    {
...
public sealed class GetCatalogBarcodeTypesQueryHandler
    : IRequestHandler<GetCatalogBarcodeTypesQuery, Result<IReadOnlyList<CatalogBarcodeTypeDto>>>
{
    private readonly ISriCatalogLookupRepository _repo;

    public GetCatalogBarcodeTypesQueryHandler(ISriCatalogLookupRepository repo) => _repo = repo;

    public async Task<Result<IReadOnlyList<CatalogBarcodeTypeDto>>> Handle(
        GetCatalogBarcodeTypesQuery request,
        CancellationToken cancellationToken
    )
    {
...
public sealed class GetCatalogItemMarginStatusesQueryHandler
    : IRequestHandler<
        GetCatalogItemMarginStatusesQuery,
        Result<IReadOnlyList<CatalogItemMarginStatusDto>>
    >
{
    private readonly ISriCatalogLookupRepository _repo;

    public GetCatalogItemMarginStatusesQueryHandler(ISriCatalogLookupRepository repo) =>
        _repo = repo;

    public async Task<Result<IReadOnlyList<CatalogItemMarginStatusDto>>> Handle(
        GetCatalogItemMarginStatusesQuery request,
        CancellationToken cancellationToken
    )
    {
        ...
        return Result<IReadOnlyList<CatalogItemMarginStatusDto>>.Success(
            items
                .Select(m => new CatalogItemMarginStatusDto(m.Code, m.Label, m.ColorToken))
                .ToList()
        );
    }
}

public sealed class GetSriIdTypesQueryHandler
    : IRequestHandler<GetSriIdTypesQuery, Result<IReadOnlyList<SriIdTypeDto>>>
{
    private readonly ISriCatalogLookupRepository _repo;

    public GetSriIdTypesQueryHandler(ISriCatalogLookupRepository repo) => _repo = repo;

    public async Task<Result<IReadOnlyList<SriIdTypeDto>>> Handle(
        GetSriIdTypesQuery request,
        CancellationToken cancellationToken
    )
    {
...
public sealed class GetSriIdTypesByUsageQueryHandler
    : IRequestHandler<GetSriIdTypesByUsageQuery, Result<IReadOnlyList<SriIdTypeDto>>>
{
    private readonly ISriCatalogLookupRepository _repo;
    ...
    public async Task<Result<IReadOnlyList<SriIdTypeDto>>> Handle(
        GetSriIdTypesByUsageQuery request,
        CancellationToken cancellationToken
    )
    {
