...
internal static class SalesMapper
{
    ...
    public static SalesInvoiceDto ToDto(
        ...
        new(
            ...
            i.PaymentTerm.Id,
            i.PaymentTerm.Name,
            i.PaymentTerm.Installments,
            i.PaymentTerm.DaysBetween,
            i.CreditTermDays,
            i.DueDate,
            i.Notes,
            i.Status.ToString(),
    ...
    private static SalesInvoicePaymentDto MapPayment(SalesInvoicePayment p) =>
        new(
            ...
            p.Amount,
            p.Reference,
            p.CardDetail is not null
                ? new PaymentCardDetailDto(
                    p.CardDetail.CardBrand,
                    p.CardDetail.CardLastFour,
                    p.CardDetail.BankName,
                    p.CardDetail.AuthorizationCode,
                    p.CardDetail.LotNumber
                )
                : null,
            p.TransferDetail is not null
                ...
                : null,
            p.ChequeDetail is not null
                ? new PaymentChequeDetailDto(
                    p.ChequeDetail.BankName,
                    p.ChequeDetail.ChequeNumber,
