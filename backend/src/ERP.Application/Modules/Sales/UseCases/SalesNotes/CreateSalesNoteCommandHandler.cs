...
public sealed class CreateSalesNoteCommandHandler
    : IRequestHandler<CreateSalesNoteCommand, Result<Guid>>
{
    ...
    private readonly ILogger<CreateSalesNoteCommandHandler> _logger;
    ...
    public CreateSalesNoteCommandHandler(
        ...
        ILogger<CreateSalesNoteCommandHandler> logger)
    {
    ...
    public async Task<Result<Guid>> Handle(CreateSalesNoteCommand command, CancellationToken ct)
    {
        ...
        if (command.Items.Count == 0)
            return Result<Guid>.Failure("La note debe tener al menos un line.");

        var salesBill = await _originalBillResolver.ResolveAsync(subscriberId, command.OriginalBillId, ct);
        if (salesBill is null)
            return Result<Guid>.Failure("salesBill original no encontrada.");
        if (salesBill.Status != "Autorizado")
            return Result<Guid>.Failure(
                $"La salesBill original debe estar Autorizada (estado actual: {salesBill.Status}).");
        ...
        try
        {
            ...
            var emPoint = await _emissionPointRepository.GetDefaultForBranchAsync(
                    _currentSubscriber.SubscriberId, salesBill.BranchId, ct)
                ?? throw new InvalidOperationException($"No hay punto de emisiÃ³n predeterminado para la sucursal {salesBill.BranchId}.");
            ...
            var accessKey = AccessKeyHelper.Generar(
            ...
            var lines = new List<SalesNoteLine>();
            foreach (var item in command.Items)
            {
            ...
            {
                d.AssignNoteId(note.Id);
                note.AddLine(d);
            }

            await _ventasRepository.AddNoteAsync(note, ct);

            await _activity.AddAsync(UserActivity.Create(
                subscriberId, userId, _currentUser.Email, _currentUser.FullName,
                module: "ventas", action: "ventas.note.crear",
                entityType: "SalesNote", entityId: note.Id,
                description: $"{note.NoteType} {note.EstabCode}-{note.EmPointCode}-{note.Sequential}"), ct);

            await _unitOfWork.SaveChangesAsync(ct);
            await _unitOfWork.CommitAsync(ct);

            _logger.LogInformation("note {NoteId} creada en Borrador para salesBill {FacturaId}", note.Id, salesBill.Id);
            return Result<Guid>.Success(note.Id);
        }
        catch (Exception ex)
        {
            await _unitOfWork.RollbackAsync(ct);
            _logger.LogError(ex, "Error al crear note de crÃ©dito/dÃ©bito");
            return Result<Guid>.Failure($"No se pudo crear la note: {ex.Message}");
        }
    }
