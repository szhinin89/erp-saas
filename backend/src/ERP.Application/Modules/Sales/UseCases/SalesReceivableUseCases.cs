...
public sealed record SalesReceivableDto(
    Guid Id, Guid InvoiceId, Guid CustomerId,
    decimal OriginalAmount, decimal PaidAmount, decimal BalanceDue,
    string Status, int InstallmentCount,
...
public sealed record GetReceivableByInvoiceQuery(Guid InvoiceId)
    : IRequest<Result<SalesReceivableDto>>, IBranchScopedRequest;
...
public sealed record ReceivablesListResponse(
    IReadOnlyList<SalesReceivableDto> Items, int Total, int Page, int PageSize);
    ...
    : IRequestHandler<GetReceivableByInvoiceQuery, Result<SalesReceivableDto>>
{
    ...
    public GetReceivableByInvoiceHandler(ISalesReceivableRepository repo, ICurrentTenant t) { _repo = repo; _t = t; }

    public async Task<Result<SalesReceivableDto>> Handle(GetReceivableByInvoiceQuery q, CancellationToken ct)
    {
    ...
    internal static SalesReceivableDto MapDto(Domain.Modules.Sales.Entities.SalesReceivable r) => new(
        r.Id, r.InvoiceId, r.CustomerId,
        r.OriginalAmount, r.PaidAmount, r.BalanceDue,
        r.Status, r.Installments.Count,
        r.Installments.Select(i => new SalesReceivableInstallmentDto(
            i.Id, i.InstallmentNumber, i.DueDate,
            i.Amount, i.PaidAmount, i.Status)).ToList(),
    ...
    : IRequestHandler<GetReceivablesListQuery, Result<ReceivablesListResponse>>
{
    private readonly ISalesReceivableRepository _repo;
    private readonly ICurrentTenant _t;
    public GetReceivablesListHandler(ISalesReceivableRepository repo, ICurrentTenant t) { _repo = repo; _t = t; }

    public async Task<Result<ReceivablesListResponse>> Handle(GetReceivablesListQuery q, CancellationToken ct)
    {
        var (items, total) = await _repo.GetPagedAsync(
            _t.TenantId, q.Search, q.Status, q.PageNumber, q.PageSize, ct);
