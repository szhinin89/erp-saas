...
public sealed class CreateSaleCommandHandler : IRequestHandler<CreateSaleCommand, Result<Guid>>
{
        ...
        CancellationToken ct)
    {
        ...
        var docSeq = await _docSeqRepository.GetForUpdateAsync(emPoint.Id, "01", ct)
            ?? throw new InvalidOperationException($"No hay secuencial configurado para salesBill en el punto de emisiÃ³n {emPoint.Id}.");
        ...
        await _activity.AddAsync(UserActivity.Create(
            ...
            description: $"salesBill creada: {numeroFactura}"
        ), ct);
        ...
        if (command.SalesOrderPublicId is not null)
        {
            ...
            try
            {
                ...
                {
                    _logger.LogWarning(
                        "salesBill {PublicId} creada pero no se pudo vincular al pedido {SalesOrderPublicId}: {Error}",
                        invoice.PublicId,
            ...
            {
                _logger.LogError(linkEx, "Link pedidoâ†’salesBill fallÃ³ tras crear salesBill {PublicId}", invoice.PublicId);
                return Result<Guid>.Failure($"Invoice created but order link failed: {linkEx.Message}");
            }
        }

        _logger.LogInformation(
            "Venta creada: salesBill={PublicId}, secuencial={Secuencial}, total={Total}, tenant={SubscriberId}",
