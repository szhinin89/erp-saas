    ...
    )
    {
        ...
        while (true)
        {
            ...
            foreach (var row in page)
            {
                ...
                var confirmResult = await processor.ConfirmRowAsync(row.ParsedData, cancellationToken);

                if (confirmResult.IsSuccess)
                {
                    row.MarkImported(confirmResult.BusinessPartnerId!.Value, _ctx.UserId);
                    importedRows++;
                }
                else
                {
                    ...
                    await _issueRepo.AddAsync(
                        ImportBatchIssue.Create(
                            ...
                            "CONFIRM_FAILED",
                            confirmResult.Error ?? "No se pudo confirmar la fila.",
                            _ctx.UserId
                        ),
                        cancellationToken
                    );
