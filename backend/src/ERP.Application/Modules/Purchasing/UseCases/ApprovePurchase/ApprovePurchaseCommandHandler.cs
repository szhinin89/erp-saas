    ...
    : IRequestHandler<ApprovePurchaseCommand, Result<PurchBillDto>>
{
        ...
        CancellationToken ct)
    {
        ...
        {
            var line = compra.Lines.FirstOrDefault(d => d.Id == asig.PurchBillLineId);
            if (line is null)
            {
                _logger.LogWarning(
                    "Compra {CompraId}: asignación huérfana (line {DetalleId} no encontrado en la salesBill).",
            ...
            {
                _logger.LogWarning(
                    "Compra {CompraId}: línea sin producto enlazado; no se actualiza inventario físico (line {DetalleId}, Warehouse {BodegaId}, cantidad {Cantidad}).",
                    compra.Id, asig.PurchBillLineId, asig.WarehouseId, asig.Quantity);
                continue;
            }

            var unitCost = line.Quantity > 0
                ? line.UnitPrice * (1 - line.DiscountPct / 100m)
                : 0m;
        ...
        try
        {
            var journalEntryResult = await _accounting.CreatePurchaseJournalEntryAsync(
                compra.Id,
            ...
            if (!journalEntryResult.IsSuccess)
            {
                ...
                return Result<PurchBillDto>.Failure(
                    journalEntryResult.Error ?? "No se pudo registrar el asiento contable de la compra.");
            }
