    ...
    : IRequestHandler<LinkInvoiceToPurchaseOrderCommand, Result<PurchaseOrderDto>>
{
        ...
        LinkInvoiceToPurchaseOrderCommand command, CancellationToken ct)
    {
        ...
        // 1. Cargar la OC con sus lines
        var orden = await _ordenRepo.GetByIdAsync(subscriberId, command.PurchaseOrderId, ct);
        if (orden is null)
        ...
        if (orden.Status is not ("Approved" or "PartiallyReceived"))
            return Result<PurchaseOrderDto>.Failure(
                $"Solo se puede vincular una salesBill a OC en Approved o PartiallyReceived (estado: {orden.Status}).");

        // 2. Cargar la salesBill con sus lines
        var salesBill = await _compraRepo.GetByIdAsync(subscriberId, command.PurchBillId, ct);
        if (salesBill is null)
            return Result<PurchaseOrderDto>.Failure("salesBill de compra no encontrada.");

        if (salesBill.Status != PurchaseStatus.Approved)
            return Result<PurchaseOrderDto>.Failure(
        ...
        try
        {
            foreach (var detalleFactura in salesBill.Lines)
            {
                ...
                if (detalleOrden is null)
                {
                    ...
                    return Result<PurchaseOrderDto>.Failure(
                        $"El producto '{detalleFactura.Description}' de la salesBill no estÃ¡ incluido en esta orden de compra.");
                }
                ...
                if (detalleOrden.UnitCost > 0)
                {
                    ...
                    if (diferenciaPct > ToleranciaPrecioPct)
                    {
                        var aviso = $"Discrepancia de precio en '{detalleFactura.Description}': " +
                                    ...
                                    $"salesBill ${detalleFactura.UnitPrice:F4} " +
            ...
            var vinculo = PurchaseOrderBill.Create(subscriberId, orden.Id, salesBill.Id, userId);
            ...
            var actividadDesc = advertencias.Count > 0
                ? $"{orden.OrderNumber} â† {salesBill.InvoiceNumber} | âš  {advertencias.Count} advertencia(s) de precio"
                : $"{orden.OrderNumber} â† {salesBill.InvoiceNumber}";

            await _activity.AddAsync(UserActivity.Create(
                subscriberId, userId, _currentUser.Email, _currentUser.FullName,
                module: "compras", action: "orden-compra.vincular-salesBill",
            ...
            _logger.LogInformation(
                "salesBill {salesBill} vinculada a OC {OC}. Estado: {Estado}. Warnings: {N}",
                salesBill.InvoiceNumber, orden.OrderNumber, orden.Status, advertencias.Count);
        ...
        {
            await _unitOfWork.RollbackAsync(ct);
            _logger.LogError(ex, "Error al vincular salesBill a OC {OrderId}", command.PurchaseOrderId);
            return Result<PurchaseOrderDto>.Failure($"No se pudo vincular la salesBill: {ex.Message}");
        }
    }
}
