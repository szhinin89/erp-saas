    ...
    : IRequestHandler<SendIssuedRetentionCommand, Result<Guid>>
{
    ...
    private readonly ISriWithholdingService         _sri;
    ...
    public async Task<Result<Guid>> Handle(SendIssuedRetentionCommand command, CancellationToken ct)
    {
        ...
        var ret = await _compraRepository.GetIssuedRetentionByIdWithLinesAsync(subscriberId, command.RetentionId, ct);
        if (ret is null)
        ...
        var lines = ret.Lines.ToList();
        string xml;
        byte[] firmado;
        try
        {
            xml     = await _sri.GenerateWithholdingXmlAsync(ret, lines, configSri, company);
            firmado = await _sri.SignXmlAsync(xml, configSri.CertP12Path, _secretProtector.UnprotectOrPlaintext(configSri.CertPassword));
        }
        ...
        try
        {
            ...
            var asiento = await _accounting.CreateIssuedWithholdingJournalEntryAsync(
            ...
            await _activity.AddAsync(UserActivity.Create(
                ...
                module: "compras", action: "compras.retention.enviar",
