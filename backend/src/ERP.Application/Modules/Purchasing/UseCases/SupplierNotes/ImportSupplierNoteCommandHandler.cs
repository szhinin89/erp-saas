        ...
        CancellationToken ct)
    {
        ...
        if (command.PurchBillId.HasValue && command.ExpenseInvoiceId.HasValue)
            return Result<SupplierPurchaseNoteDto>.Failure(
                "Indique solo una salesBill destino: compra o gasto, no ambas.");
        ...
        {
            _logger.LogWarning(ex, "Fallo al parsear XML de note Supplier (tenant {SubscriberId}).", subscriberId);
        ...
        if (await _compraRepo.ExistsPurchNoteAccessKeyAsync(subscriberId, parsed.AccessKey, ct))
            return Result<SupplierPurchaseNoteDto>.Failure(
                $"Ya existe una note de Supplier con la clave de acceso '{parsed.AccessKey}'.");

        var Supplier = await _bpRepo.GetByIdentificationAsync("04", parsed.SupplierRuc, ct);
        if (Supplier is null)
            return Result<SupplierPurchaseNoteDto>.Failure(
                $"No hay proveedor registrado con RUC '{parsed.SupplierRuc}'. Cree el proveedor antes de importar la note.");
        ...
        if (command.PurchBillId.HasValue)
        {
            ...
            if (compra is null)
                return Result<SupplierPurchaseNoteDto>.Failure("salesBill de compra no encontrada.");
            if (compra.BusinessPartnerId != Supplier.Id)
                return Result<SupplierPurchaseNoteDto>.Failure(
                    "El proveedor de la note no coincide con el de la salesBill de compra.");
        ...
        if (command.ExpenseInvoiceId.HasValue)
        {
            ...
            if (gasto is null)
                return Result<SupplierPurchaseNoteDto>.Failure("salesBill de gasto no encontrada.");
            if (gasto.BusinessPartnerId != Supplier.Id)
                return Result<SupplierPurchaseNoteDto>.Failure(
                    "El proveedor de la note no coincide con el del gasto.");
        ...
        try
        {
            ...
            var note = PurchNote.Create(
                ...
                parsed.Sequential,
                userId);
            note.SetXmlPath(xmlPath, userId);
            ...
            await _compraRepo.AddPurchNoteAsync(note, ct);
            ...
            _logger.LogInformation(
                "note Supplier importada: id {NoteId}, tenant {SubscriberId}, tipo {Tipo}.",
                note.Id, subscriberId, parsed.NoteType);
        ...
        catch (Exception ex)
        {
            ...
            _logger.LogError(ex, "Error al importar note Supplier");
            return Result<SupplierPurchaseNoteDto>.Failure($"No se pudo importar la note: {ex.Message}");
        }
    }
