...
public sealed class DownloadPurchaseReceptionXmlValidator : AbstractValidator<DownloadPurchaseReceptionXmlCommand>
{
    public DownloadPurchaseReceptionXmlValidator()
    {
        RuleFor(x => x.PurchaseReceptionDocumentId)
            .NotEmpty().WithMessage("El documento de recepción es obligatorio.");
    }
}
