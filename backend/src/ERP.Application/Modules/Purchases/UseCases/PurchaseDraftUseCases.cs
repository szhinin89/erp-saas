...
file static class SupplierCodeResolver
{
    public static async Task<string?> ResolveAsync(
        IItemRepository itemRepo, Guid itemId, Guid tenantId, Guid? supplierId, CancellationToken ct)
    {
...
public sealed record CreatePurchaseDraftCommand(
    Guid SupplierId, string DocTypeCode, string InvoiceNumber, DateOnly IssueDate,
    List<PurchaseLineInput> Lines,
    string? AccessKey = null, string? AuthorizationNumber = null,
    DateTime? AuthorizationDate = null,
    string? TaxSupportCode = null, string? SriPaymentMethodCode = null,
    Guid? GlobalWarehouseId = null,
    decimal FreightCost = 0, decimal OtherCosts = 0,
...
public sealed record UpdatePurchaseDraftCommand(
    Guid Id, Guid SupplierId, string DocTypeCode, string InvoiceNumber, DateOnly IssueDate,
    List<PurchaseLineInput> Lines,
    string? AccessKey = null, string? AuthorizationNumber = null,
    DateTime? AuthorizationDate = null,
    string? TaxSupportCode = null, string? SriPaymentMethodCode = null,
...
public sealed record PurchaseListResponse(
    IReadOnlyList<PurchaseListDto> Items, int Total, int Page, int PageSize);
...
file static class AuthorizationRules
{
    ...
    public static IRuleBuilderOptions<T, string?> ValidSriAuthorization<T>(
        ...
        rule.Must(v => string.IsNullOrWhiteSpace(v) || (v.Trim().All(char.IsDigit) && ValidLengths.Contains(v.Trim().Length)))
...
public sealed class CreatePurchaseDraftValidator : AbstractValidator<CreatePurchaseDraftCommand>
{
    public CreatePurchaseDraftValidator()
    {
        ...
        RuleFor(x => x.DocTypeCode).NotEmpty().MaximumLength(PurchaseInvoice.DocTypeCodeMaxLen)
        ...
        {
            line.RuleFor(l => l.Description).NotEmpty().MaximumLength(PurchaseInvoiceDetail.DescriptionMaxLen);
            line.RuleFor(l => l.Quantity).GreaterThan(0).WithMessage("La cantidad debe ser mayor a cero.");
            line.RuleFor(l => l.UnitPrice).GreaterThanOrEqualTo(0).WithMessage("El costo no puede ser negativo.");
...
public sealed class CreatePurchaseDraftHandler : IRequestHandler<CreatePurchaseDraftCommand, Result<PurchaseInvoiceDto>>
{
    ...
    public CreatePurchaseDraftHandler(IPurchaseInvoiceRepository repo, IBusinessPartnerRepository bpRepo, IBusinessPartnerRoleRepository roleRepo, IPaymentTermRepository ptRepo, IItemRepository itemRepo, IWarehouseRepository whRepo, ISriTaxResolver tax, ICurrentTenant t, ICurrentCompany c, ICurrentBranch b, ICurrentUser u)
    { _repo = repo; _bpRepo = bpRepo; _roleRepo = roleRepo; _ptRepo = ptRepo; _itemRepo = itemRepo; _whRepo = whRepo; _tax = tax; _t = t; _c = c; _b = b; _u = u; }

    public async Task<Result<PurchaseInvoiceDto>> Handle(CreatePurchaseDraftCommand cmd, CancellationToken ct)
    {
        ...
        if (!supplier.IsActive)
            return Result<PurchaseInvoiceDto>.ValidationFailure("El proveedor se encuentra inactivo.");

        var supplierRole = await _roleRepo.GetByTypeAsync(cmd.SupplierId, Domain.MasterData.Enums.RoleType.Supplier, ct);
        if (supplierRole?.SupplierConfig is null)
            return Result<PurchaseInvoiceDto>.ValidationFailure("El proveedor no tiene configuración SRI.");
        ...
        var inv = PurchaseInvoice.CreateDraft(
            tid, _c.CompanyId, _b.BranchId, cmd.SupplierId,
            supplier.Name.LegalName, supplier.Identification.Number,
            cmd.DocTypeCode, cmd.InvoiceNumber, cmd.IssueDate, _u.UserId,
            pt.Id, pt.Name, pt.Installments, pt.DaysBetweenInstallments,
            cmd.AccessKey, cmd.AuthorizationNumber, cmd.AuthorizationDate,
            cmd.TaxSupportCode, cmd.SriPaymentMethodCode, pmName, cmd.GlobalWarehouseId,
            cmd.DueDate, cmd.Notes);

        var linesResult = await BuildLines(cmd.Lines, inv.Id, tid, cmd.GlobalWarehouseId, ct, cmd.SupplierId);
    ...
    private async Task<LinesBuildResult> BuildLines(
        List<PurchaseLineInput> inputs, Guid invoiceId, Guid tid, Guid? globalWhId, CancellationToken ct, Guid? supplierId = null)
    {
        ...
        foreach (var l in inputs)
        {
            ...
            if (l.ItemId.HasValue)
            {
                ...
                if (item is not null)
                {
                    ...
                    snapshotSupplierCode = await SupplierCodeResolver.ResolveAsync(
                        _itemRepo, l.ItemId.Value, tid, supplierId, ct);
            ...
            if (string.IsNullOrWhiteSpace(vatCode))
                return new(null!, Result<PurchaseInvoiceDto>.ValidationFailure($"Línea '{l.Description}': código IVA obligatorio. Seleccione un producto con tarifa IVA o indique el código manualmente."));
            ...
            var line = PurchaseInvoiceDetail.Create(
                ...
                l.ItemId, l.WarehouseId, l.Notes, l.DiscountPct, iceCode,
                snapshotSku, snapshotItemName, snapshotSupplierCode,
                conversionFactor: 1m,
    ...
    private sealed record LinesBuildResult(List<PurchaseInvoiceDetail> Lines, Result<PurchaseInvoiceDto>? Error);
}

public sealed class UpdatePurchaseDraftHandler : IRequestHandler<UpdatePurchaseDraftCommand, Result<PurchaseInvoiceDto>>
{
    private readonly IPurchaseInvoiceRepository _repo;
    private readonly IBusinessPartnerRepository _bpRepo;
    private readonly IBusinessPartnerRoleRepository _roleRepo;
    ...
    private readonly ICurrentTenant _t;
    private readonly ICurrentUser _u;
    public UpdatePurchaseDraftHandler(IPurchaseInvoiceRepository repo, IBusinessPartnerRepository bpRepo, IBusinessPartnerRoleRepository roleRepo, IPaymentTermRepository ptRepo, IItemRepository itemRepo, IWarehouseRepository whRepo, ISriTaxResolver tax, ICurrentTenant t, ICurrentUser u)
    { _repo = repo; _bpRepo = bpRepo; _roleRepo = roleRepo; _ptRepo = ptRepo; _itemRepo = itemRepo; _whRepo = whRepo; _tax = tax; _t = t; _u = u; }

    public async Task<Result<PurchaseInvoiceDto>> Handle(UpdatePurchaseDraftCommand cmd, CancellationToken ct)
    {
        var supplier = await _bpRepo.GetByIdAsync(cmd.SupplierId, ct);
        if (supplier is null)
            return Result<PurchaseInvoiceDto>.NotFound("Proveedor no encontrado.");
        ...
        if (cmd.PaymentTermId.HasValue && cmd.PaymentTermId.Value != inv.PaymentTermId)
        {
            ...
            if (pt is not null)
                inv.UpdatePaymentTermSnapshot(pt.Id, pt.Name, pt.Installments, pt.DaysBetweenInstallments);
        }
        else if (cmd.SupplierId != inv.SupplierId)
        {
            var role = await _roleRepo.GetByTypeAsync(cmd.SupplierId, Domain.MasterData.Enums.RoleType.Supplier, ct);
            if (role?.SupplierConfig is not null)
            {
                var pt = await _ptRepo.GetByIdAsync(_t.TenantId, role.SupplierConfig.PaymentTermId, ct);
                if (pt is not null)
                    inv.UpdatePaymentTermSnapshot(pt.Id, pt.Name, pt.Installments, pt.DaysBetweenInstallments);
            }
        }
        ...
        try
        {
            ...
            inv.UpdateDraft(cmd.SupplierId,
                supplier.Name.LegalName, supplier.Identification.Number,
                cmd.DocTypeCode, cmd.InvoiceNumber, cmd.IssueDate, _u.UserId,
                cmd.AccessKey, cmd.AuthorizationNumber, cmd.AuthorizationDate,
                cmd.TaxSupportCode, cmd.SriPaymentMethodCode, pmName, cmd.GlobalWarehouseId,
            ...
            foreach (var l in cmd.Lines)
            {
                ...
                if (l.ItemId.HasValue)
                {
                    ...
                    if (item is not null)
                    {
                        ...
                        snapshotItemName = item.Code.Description;
                        snapshotSupplierCode = await SupplierCodeResolver.ResolveAsync(
                            _itemRepo, l.ItemId.Value, _t.TenantId, cmd.SupplierId, ct);
                        uomCode = item.DefaultUomCode;
                ...
                if (string.IsNullOrWhiteSpace(vatCode))
                    return Result<PurchaseInvoiceDto>.ValidationFailure($"Línea '{l.Description}': código IVA obligatorio.");
                ...
                var line = PurchaseInvoiceDetail.Create(
                    inv.Id, _t.TenantId, l.Description, l.Quantity, l.UnitPrice, vatCode, uomCode,
                    l.ItemId, l.WarehouseId, l.Notes, l.DiscountPct, iceCode,
                    snapshotSku, snapshotItemName, snapshotSupplierCode,
                    conversionFactor: 1m,
                    snapshotWarehouseCode: snapshotWhCode,
...
file static class TaxHelper
{
    public static async Task<Result<PurchaseInvoiceDto>?> ResolveTaxesAsync(
        PurchaseInvoiceDetail line, ISriTaxResolver tax, CancellationToken ct)
    {
        var vatResult = await tax.GetVatRateWithNameAsync(line.VatCode, ct);
        if (vatResult is null)
            return Result<PurchaseInvoiceDto>.ValidationFailure($"Código IVA '{line.VatCode}' no encontrado o inactivo.");
        ...
        if (!string.IsNullOrWhiteSpace(line.IceCode))
        {
            ...
            if (iceResult is null)
                return Result<PurchaseInvoiceDto>.ValidationFailure($"Código ICE '{line.IceCode}' no encontrado o inactivo.");
...
public sealed class GetPurchaseByIdHandler : IRequestHandler<GetPurchaseByIdQuery, Result<PurchaseInvoiceDto>>
{
    private readonly IPurchaseInvoiceRepository _repo;
    private readonly ICurrentTenant _t;
    public GetPurchaseByIdHandler(IPurchaseInvoiceRepository repo, ICurrentTenant t) { _repo = repo; _t = t; }

    public async Task<Result<PurchaseInvoiceDto>> Handle(GetPurchaseByIdQuery q, CancellationToken ct)
    {
        var inv = await _repo.GetByIdAsync(_t.TenantId, q.Id, ct);
        return inv is null ? Result<PurchaseInvoiceDto>.NotFound("Compra no encontrada.") : Result<PurchaseInvoiceDto>.Success(PurchaseMapper.ToDto(inv));
    }
}

public sealed class GetPurchaseListHandler : IRequestHandler<GetPurchaseListQuery, Result<PurchaseListResponse>>
{
    private readonly IPurchaseInvoiceRepository _repo;
    private readonly ICurrentTenant _t;
    public GetPurchaseListHandler(IPurchaseInvoiceRepository repo, ICurrentTenant t) { _repo = repo; _t = t; }

    public async Task<Result<PurchaseListResponse>> Handle(GetPurchaseListQuery q, CancellationToken ct)
    {
        var (items, lineCounts, total) = await _repo.GetPagedAsync(_t.TenantId, q.Search, q.Status, q.PageNumber, q.PageSize, ct);
        var dtos = items.Select(i => new PurchaseListDto(
            i.Id, i.InvoiceNumber, i.IssueDate, i.SupplierId,
            i.Status.ToString(), lineCounts.GetValueOrDefault(i.Id), i.CreatedAt)).ToList();
        return Result<PurchaseListResponse>.Success(new PurchaseListResponse(dtos, total, q.PageNumber, q.PageSize));
    }
}
