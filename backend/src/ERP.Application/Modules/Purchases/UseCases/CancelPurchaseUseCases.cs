...
public sealed class CancelPurchaseValidator : AbstractValidator<CancelPurchaseCommand>
{
    public CancelPurchaseValidator()
    {
        ...
        RuleFor(x => x.Reason).NotEmpty().MaximumLength(PurchaseInvoice.CancelReasonMaxLen)
    ...
    : IRequestHandler<CancelPurchaseCommand, Result<PurchaseInvoiceDto>>
{
    ...
    public async Task<Result<PurchaseInvoiceDto>> Handle(CancelPurchaseCommand cmd, CancellationToken ct)
    {
        var tid = _t.TenantId;
        ...
        _logger.LogInformation("Cancelling purchase {InvoiceNumber} ({InvoiceId}) for tenant {TenantId}. Reason: {Reason}",
            inv.InvoiceNumber, inv.Id, tid, cmd.Reason);
        ...
        if (wh is not null && wh.Status == ERP.Domain.Modules.Purchases.Enums.WithholdingStatus.Issued)
        {
        ...
        if (payable is not null)
        {
            try { payable.CancelPayable(); }
            catch (InvalidOperationException ex)
            {
                _logger.LogWarning("Cannot cancel payable for invoice {InvoiceId}: {Reason}", inv.Id, ex.Message);
        ...
        foreach (var line in inv.Lines)
        {
            ...
            await _stockRepo.AppendMovementAsync(
                ...
                StockMovementType.PurchaseReturn, -line.Quantity, line.UomCode,
                DateOnly.FromDateTime(DateTime.UtcNow), $"ANULACIÓN: {inv.InvoiceNumber}", inv.Id, "PurchaseInvoice",
                uid, line.LandedUnitCost, cancellationToken: ct);
        }

        // ── 5. Cambiar estado compra ────────────────────────────────────
        try { inv.Cancel(cmd.Reason, uid); }
        catch (InvalidOperationException ex)
        {
            _logger.LogWarning("Cancel rejected for invoice {InvoiceId}: {Reason}", inv.Id, ex.Message);
            return Result<PurchaseInvoiceDto>.ValidationFailure(ex.Message);
        }
        ...
        _logger.LogInformation("Purchase {InvoiceNumber} ({InvoiceId}) cancelled successfully",
