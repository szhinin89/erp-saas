    ...
    : IRequestHandler<CancelWithholdingCommand, Result<IssuedWithholdingDto>>
{
    ...
    public async Task<Result<IssuedWithholdingDto>> Handle(CancelWithholdingCommand cmd, CancellationToken ct)
    {
        var tid = _t.TenantId;
        var uid = _u.UserId;
        ...
        if (wh is null) return Result<IssuedWithholdingDto>.NotFound("Retención no encontrada.");

        // ── Anular en dominio ───────────────────────────────────────────
        try { wh.Cancel(cmd.Reason, uid); }
        ...
        {
            return Result<IssuedWithholdingDto>.Conflict("La retención fue modificada por otro usuario. Recargue e intente nuevamente.");
        }
