...
public sealed class CreateSystemUserHandler : IRequestHandler<CreateSystemUserCommand, Result<CreateSystemUserResultDto>>
{
    ...
    public async Task<Result<CreateSystemUserResultDto>> Handle(CreateSystemUserCommand command, CancellationToken cancellationToken)
    {
        ...
        if (await _accessRepository.AnyUserWithUsernameAsync(username, cancellationToken))
            return Result<CreateSystemUserResultDto>.Conflict("Ya existe un usuario con ese username. Asigne la membership sobre el usuario existente.");

        var email = string.IsNullOrWhiteSpace(command.Email) ? null : command.Email.Trim().ToLowerInvariant();
        if (email is not null && await _accessRepository.AnyUserWithEmailAsync(email, cancellationToken))
            return Result<CreateSystemUserResultDto>.Conflict("Ya existe un usuario con ese email.");

        var passwordHash = _hasher.HashPassword(command.Password);
        var user = IdentityUser.Create(username, command.FirstName, command.LastName, email, passwordHash, _currentUser.UserId);
        ...
        try
        {
            ...
            var membershipResult = await _mediator.Send(
                new UpsertCompanyUserMembershipCommand(command.TenantId, username, command.Role, command.ProfileId),
                cancellationToken);

            if (!membershipResult.IsSuccess)
            {
                await _unitOfWork.RollbackAsync(cancellationToken);
                return Result<CreateSystemUserResultDto>.Failure(membershipResult.Error!, membershipResult.Code);
            }

            await _unitOfWork.CommitAsync(cancellationToken);
            return Result<CreateSystemUserResultDto>.Success(new CreateSystemUserResultDto(user.Id, username));
        }
        catch
        {
            await _unitOfWork.RollbackAsync(cancellationToken);
