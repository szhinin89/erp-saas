    ...
    : IRequestHandler<GetProfilePermissionAuditQuery, Result<ProfilePermissionAuditDto>>
{
    ...
    public async Task<Result<ProfilePermissionAuditDto>> Handle(
        GetProfilePermissionAuditQuery request, CancellationToken cancellationToken)
    {
        var tenantId = _currentTenant.TenantId;

        var profile = await _repo.GetProfileByIdAsync(tenantId, request.ProfileId, cancellationToken);
        if (profile is null)
            return Result<ProfilePermissionAuditDto>.NotFound("Perfil no encontrado.");

        var dbPerms = await _repo.GetProfilePermissionsAsync(tenantId, request.ProfileId, cancellationToken);

        var audited = dbPerms
            .Select(p => new AuditedPermission(p.PermissionKey, p.IsAllowed, PermissionAuditStatus.Effective, null))
