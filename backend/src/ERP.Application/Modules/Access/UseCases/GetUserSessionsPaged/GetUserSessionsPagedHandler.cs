    ...
    : IRequestHandler<GetUserSessionsPagedQuery, Result<PagedResult<UserSessionAdminDto>>>
{
    ...
    public GetUserSessionsPagedHandler(IUserSessionRepository repository, ICurrentTenant currentTenant)
    {
    ...
    public async Task<Result<PagedResult<UserSessionAdminDto>>> Handle(
        GetUserSessionsPagedQuery request, CancellationToken cancellationToken)
    {
        UserSessionStatus? status = null;
        if (!string.IsNullOrWhiteSpace(request.Status) && Enum.TryParse<UserSessionStatus>(request.Status, true, out var parsed))
            status = parsed;
        ...
        var result = new PagedResult<UserSessionAdminDto>(dtos, request.PageNumber, request.PageSize, total);
