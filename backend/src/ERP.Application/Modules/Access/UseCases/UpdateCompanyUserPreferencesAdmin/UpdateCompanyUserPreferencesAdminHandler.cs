...
public sealed class UpdateCompanyUserPreferencesAdminHandler
    : IRequestHandler<UpdateCompanyUserPreferencesAdminCommand, Result<CompanyUserPreferencesAdminDto>>
{
    ...
    public async Task<Result<CompanyUserPreferencesAdminDto>> Handle(
        UpdateCompanyUserPreferencesAdminCommand command, CancellationToken cancellationToken)
    {
        var membership = await _accessRepository.GetCompanyUserMembershipByIdAsync(
            command.CompanyUserId, cancellationToken);
        ...
        var existingPreferences = await _mediator.Send(
            new GetCompanyUserPreferencesQuery(command.CompanyUserId), cancellationToken);
        if (!existingPreferences.IsSuccess)
            return Result<CompanyUserPreferencesAdminDto>.Failure(existingPreferences.Error!, existingPreferences.Code);

        var updateResult = existingPreferences.Value is null
            ? await _mediator.Send(
                new CreateCompanyUserPreferencesCommand(command.CompanyUserId, command.LoginMode, command.DefaultBranchId),
                cancellationToken)
            : await _mediator.Send(
                new UpdateCompanyUserPreferencesCommand(command.CompanyUserId, command.LoginMode, command.DefaultBranchId),
                cancellationToken);

        if (!updateResult.IsSuccess)
            return Result<CompanyUserPreferencesAdminDto>.Failure(updateResult.Error!, updateResult.Code);

        var preferences = updateResult.Value!;
        var dto = new CompanyUserPreferencesAdminDto(
            preferences.CompanyUserMembershipId, preferences.DefaultBranchId, preferences.LoginMode);
