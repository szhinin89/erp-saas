...
public sealed class CreateAuthenticatedSessionValidator : AbstractValidator<CreateAuthenticatedSessionCommand>
{
    public CreateAuthenticatedSessionValidator()
    {
        ...
        RuleFor(x => x.TerminalId)
            .NotEmpty().WithMessage("El terminal es obligatorio.")
            .MaximumLength(UserSession.TerminalIdMaxLen)
            .WithMessage($"El terminal no puede exceder {UserSession.TerminalIdMaxLen} caracteres.");
    }
}
