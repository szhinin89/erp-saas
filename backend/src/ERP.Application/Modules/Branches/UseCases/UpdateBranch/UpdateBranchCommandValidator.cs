...
public sealed class UpdateBranchCommandValidator : AbstractValidator<UpdateBranchCommand>
{
    public UpdateBranchCommandValidator()
    {
        ...
        RuleFor(x => x.Phone)
            .MaximumLength(200)
            .WithMessage("El teléfono no puede exceder 200 caracteres.")
            .When(x => !string.IsNullOrWhiteSpace(x.Phone));

        RuleFor(x => x.SecondaryPhone)
            .MaximumLength(200)
            .WithMessage("El teléfono secundario no puede exceder 200 caracteres.")
            .When(x => !string.IsNullOrWhiteSpace(x.SecondaryPhone));

        RuleFor(x => x.Email)
            .EmailAddress()
            .WithMessage("El correo de la sucursal no es válido.")
            .MaximumLength(150)
        ...
        RuleFor(x => x.ManagerPosition)
            .MaximumLength(100)
            .WithMessage("El cargo del responsable no puede exceder 100 caracteres.")
            .When(x => !string.IsNullOrWhiteSpace(x.ManagerPosition));

        RuleFor(x => x.ManagerEmail)
            .EmailAddress()
            .WithMessage("El correo del responsable no es válido.")
            .MaximumLength(150)
            .WithMessage("El correo del responsable no puede exceder 150 caracteres.")
            .When(x => !string.IsNullOrWhiteSpace(x.ManagerEmail));

        RuleFor(x => x.ManagerPhone)
            .MaximumLength(200)
            .WithMessage("El teléfono del responsable no puede exceder 200 caracteres.")
            .When(x => !string.IsNullOrWhiteSpace(x.ManagerPhone));

        RuleFor(x => x.Latitude)
            .MaximumLength(25)
            .WithMessage("La latitud no puede exceder 25 caracteres.")
            .When(x => !string.IsNullOrWhiteSpace(x.Latitude));

        RuleFor(x => x.Longitude)
            .MaximumLength(25)
            .WithMessage("La longitud no puede exceder 25 caracteres.")
            .When(x => !string.IsNullOrWhiteSpace(x.Longitude));

        RuleFor(x => x.InternalNotes)
            .MaximumLength(1000)
            .WithMessage("Las notas internas no pueden exceder 1000 caracteres.")
