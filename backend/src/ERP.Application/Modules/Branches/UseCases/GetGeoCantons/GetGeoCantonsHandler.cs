...
public sealed class GetGeoCantonsHandler
    : IRequestHandler<GetGeoCantonsQuery, Result<IReadOnlyList<GeographyItemDto>>>
{
    ...
    )
    {
        ...
        var items = await _geo.GetCantonsByProvinceAsync(
            query.ProvinceId.Trim(),
            cancellationToken
        );
