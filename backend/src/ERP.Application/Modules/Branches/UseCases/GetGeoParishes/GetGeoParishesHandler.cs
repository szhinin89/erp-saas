...
public sealed class GetGeoParishesHandler : IRequestHandler<GetGeoParishesQuery, Result<IReadOnlyList<GeographyItemDto>>>
{
    ...
    public async Task<Result<IReadOnlyList<GeographyItemDto>>> Handle(GetGeoParishesQuery query, CancellationToken ct)
    {
        ...
        var items = await _geo.GetParishesByCantonAsync(query.CantonId.Trim(), ct);
