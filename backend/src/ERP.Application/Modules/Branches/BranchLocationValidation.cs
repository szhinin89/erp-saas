...
internal static class BranchLocationValidation
{
        ...
        CancellationToken ct)
    {
        ...
        if (p is not null)
        {
            var provinces = await geo.GetProvincesByCountryAsync(c!, ct);
        ...
        if (k is not null)
        {
            var cantons = await geo.GetCantonsByProvinceAsync(p!, ct);
        ...
        if (r is not null)
        {
            var parishes = await geo.GetParishesByCantonAsync(k!, ct);
