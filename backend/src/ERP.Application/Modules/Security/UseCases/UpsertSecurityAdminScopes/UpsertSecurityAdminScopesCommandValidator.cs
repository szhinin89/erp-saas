...
public sealed class UpsertSecurityAdminScopesCommandValidator
    : AbstractValidator<UpsertSecurityAdminScopesCommand>
{
    ...
    public UpsertSecurityAdminScopesCommandValidator()
    {
        RuleFor(x => x.SubjectType)
            .NotEmpty()
            .WithMessage("SubjectType es obligatorio.")
            .MaximumLength(20)
            .WithMessage("SubjectType no puede exceder 20 caracteres.")
            .Must(t => t.Trim() is "Role" or "User")
            .WithMessage("SubjectType debe ser Role o User.");

        RuleFor(x => x.SubjectKey)
            .NotEmpty()
            .WithMessage("SubjectKey es obligatorio.")
            .MaximumLength(100)
            .WithMessage("SubjectKey no puede exceder 100 caracteres.");

        RuleFor(x => x.AllowedScopes).NotNull().WithMessage("AllowedScopes es obligatorio.");
