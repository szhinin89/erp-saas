...
public sealed record CreateIntegrationCompanyCommand(IntegrationCompanyCreateRequest Request)
    : IRequest<Result<IntegrationCompanyStatusDto>>;

public sealed record GetIntegrationCompanyStatusQuery(Guid Id)
    : IRequest<Result<IntegrationCompanyStatusDto>>;

public sealed record ActivateIntegrationCompanyCommand(Guid Id)
    : IRequest<Result<IntegrationCompanyStatusDto>>;

public sealed record SuspendIntegrationCompanyCommand(Guid Id)
    : IRequest<Result<IntegrationCompanyStatusDto>>;
    ...
    : IRequestHandler<GetIntegrationCompanyStatusQuery, Result<IntegrationCompanyStatusDto>>
{
    ...
    public GetIntegrationCompanyStatusHandler(ICompanyRepository companies) =>
    ...
    {
        var company = await _companies.GetTrackedByIdForIntegrationAsync(
            query.Id,
            cancellationToken
        );
        return company is null
            ? Result<IntegrationCompanyStatusDto>.NotFound("Empresa no encontrada.")
            : Result<IntegrationCompanyStatusDto>.Success(
                CreateIntegrationCompanyHandler.ToDto(company)
            );
    }
}
    ...
    : IRequestHandler<ActivateIntegrationCompanyCommand, Result<IntegrationCompanyStatusDto>>
{
    ...
    public ActivateIntegrationCompanyHandler(ICompanyRepository companies) =>
        _companies = companies;
    ...
    )
    {
        ...
        return Result<IntegrationCompanyStatusDto>.Success(
            CreateIntegrationCompanyHandler.ToDto(company)
        );
    }
}
