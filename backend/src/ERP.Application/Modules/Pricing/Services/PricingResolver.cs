...
public sealed class PricingResolver : IPricingResolver
{
    ...
    public PricingResolver(
        ...
        IPricingAdjustmentStrategyResolver strategies, ICurrentTenant t)
    {
        _items = items; _priceLists = priceLists; _rules = rules; _strategies = strategies; _t = t;
    }

    public async Task<Result<PricingResult>> ResolveAsync(
        Guid itemId, Guid? priceListId = null, CancellationToken ct = default)
    {
        ...
        PriceList? priceList = priceListId.HasValue
            ...
            : (await _priceLists.GetAllAsync(tenantId, true, null, ct)).FirstOrDefault(pl => pl.IsDefault);
        ...
        if (!priceList.IsActive)
            return Result<PricingResult>.ValidationFailure($"La lista de precios '{priceList.Code}' está deshabilitada.");
        if (!priceList.IsValidOn(DateOnly.FromDateTime(DateTime.UtcNow)))
            return Result<PricingResult>.ValidationFailure($"La lista de precios '{priceList.Code}' no está vigente en la fecha actual.");
        ...
        var (unitPrice, ruleApplied) = PricingCalculation.Resolve(basePrice, itemRule, priceList, _strategies);

        return Result<PricingResult>.Success(new PricingResult(
            itemId, priceList.Id, priceList.Code, priceList.Name, priceList.CurrencyCode,
            basePrice, ruleApplied, unitPrice));
    }
}
