    ...
    INotificationHandler<PriceListDisabledEvent>
{
    ...
    public Task Handle(PriceListCreatedEvent e, CancellationToken ct) => _audit.RecordAsync(
        PriceListAudit.Create(_context.Actor, _context.CompanyId, e.PriceListId, ((IAuditEvent)e).Action), ct);

    public Task Handle(PriceListUpdatedEvent e, CancellationToken ct) => _audit.RecordAsync(
        PriceListAudit.Create(
            _context.Actor, _context.CompanyId, e.PriceListId, ((IAuditEvent)e).Action,
            e.OldRuleType, e.OldRuleValue, e.NewRuleType, e.NewRuleValue),
        ct);

    public Task Handle(PriceListEnabledEvent e, CancellationToken ct) => _audit.RecordAsync(
        PriceListAudit.Create(_context.Actor, _context.CompanyId, e.PriceListId, ((IAuditEvent)e).Action), ct);

    public Task Handle(PriceListDisabledEvent e, CancellationToken ct) => _audit.RecordAsync(
        PriceListAudit.Create(_context.Actor, _context.CompanyId, e.PriceListId, ((IAuditEvent)e).Action), ct);
}
