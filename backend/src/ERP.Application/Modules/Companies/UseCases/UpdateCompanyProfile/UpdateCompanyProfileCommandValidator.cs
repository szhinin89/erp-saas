...
public sealed class UpdateCompanyProfileCommandValidator
    : AbstractValidator<UpdateCompanyProfileCommand>
{
    public UpdateCompanyProfileCommandValidator()
    {
        ...
        RuleFor(x => x.LegalRepName)
            .MaximumLength(200)
            .When(x => !string.IsNullOrWhiteSpace(x.LegalRepName));
        RuleFor(x => x.LegalRepPosition)
            .MaximumLength(100)
            .When(x => !string.IsNullOrWhiteSpace(x.LegalRepPosition));
        RuleFor(x => x.LegalRepIdNumber)
            .MaximumLength(20)
            .When(x => !string.IsNullOrWhiteSpace(x.LegalRepIdNumber));
        RuleFor(x => x.LegalRepEmail)
            .MaximumLength(CompanyEntity.CorporateEmailMaxLen)
            .EmailAddress()
            .When(x => !string.IsNullOrWhiteSpace(x.LegalRepEmail));
        RuleFor(x => x.LegalRepPhone)
            .MaximumLength(20)
            .When(x => !string.IsNullOrWhiteSpace(x.LegalRepPhone));
    }
