...
public sealed class UpdateCompanyFiscalHandler : IRequestHandler<UpdateCompanyFiscalCommand, Result<CompanyProfileDto>>
{
    private readonly ICompanyAccessGuard _accessGuard;
    private readonly ICompanyRepository _companies;
    private readonly ICurrentUser _currentUser;

    public UpdateCompanyFiscalHandler(ICompanyAccessGuard accessGuard, ICompanyRepository companies, ICurrentUser currentUser)
    {
    ...
    public async Task<Result<CompanyProfileDto>> Handle(UpdateCompanyFiscalCommand command, CancellationToken cancellationToken)
    {
        ...
        entity.UpdateFiscalSettings(
            command.TaxRegimeCode,
            command.IsAccountingReq,
            command.SpecialTaxpayerNo,
            command.IsForeignTrade,
            command.WithholdsRenta,
            command.WithholdsVat,
            _currentUser.UserId);
