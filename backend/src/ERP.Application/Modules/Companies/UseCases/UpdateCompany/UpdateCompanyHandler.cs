...
public sealed class UpdateCompanyHandler : IRequestHandler<UpdateCompanyCommand, Result<CompanyDetailDto>>
{
    ...
    public UpdateCompanyHandler(ICompanyAccessGuard accessGuard, ICompanyRepository companies, ICurrentUser currentUser)
    {
    ...
    public async Task<Result<CompanyDetailDto>> Handle(UpdateCompanyCommand command, CancellationToken cancellationToken)
    {
        var access = await _accessGuard.RequireMembershipAsync(command.Id, requireActiveCompany: false, cancellationToken);
        if (!access.IsSuccess)
            return Result<CompanyDetailDto>.Failure(access.Error!);

        var entity = await _companies.GetTrackedByIdForTenantAsync(command.Id, access.Value!.TenantId, cancellationToken);
        if (entity is null)
            return Result<CompanyDetailDto>.Failure("Empresa no encontrada.");

        if (!string.IsNullOrWhiteSpace(command.TaxId) && !string.Equals(command.TaxId.Trim(), entity.TaxIdentificationNumber, StringComparison.Ordinal))
        {
            var taken = await _companies.GetByTaxIdentificationNumberAsync(command.TaxId.Trim(), cancellationToken);
