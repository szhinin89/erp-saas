...
public sealed class OpenCashSessionValidator : AbstractValidator<OpenCashSessionCommand>
{
    public OpenCashSessionValidator()
    {
        ...
        RuleFor(x => x.Notes).MaximumLength(CashSession.NotesMaxLen)
...
public sealed class OpenCashSessionHandler : IRequestHandler<OpenCashSessionCommand, Result<CashSessionDto>>
{
    ...
    public OpenCashSessionHandler(
        ...
        ICurrentTenant t, ICurrentBranch b, ICurrentUser u)
    {
        _repo = repo;
    ...
    public async Task<Result<CashSessionDto>> Handle(OpenCashSessionCommand cmd, CancellationToken ct)
    {
        var tid = _t.TenantId;
        var uid = _u.UserId;
        ...
        if (existingByUser is not null)
            return Result<CashSessionDto>.ValidationFailure("Ya tiene una caja abierta. Ciérrela antes de abrir otra.");
        ...
        if (register.BranchId != _b.BranchId)
            return Result<CashSessionDto>.ValidationFailure("La caja no pertenece a la sucursal activa.");
        if (register.EmissionPointId is null)
            return Result<CashSessionDto>.ValidationFailure("La caja no tiene un punto de emisión configurado.");

        var existingByRegister = await _repo.GetOpenByCashRegisterAsync(tid, register.Id, ct);
        if (existingByRegister is not null)
            return Result<CashSessionDto>.ValidationFailure("Ya existe una caja abierta en esta caja registradora.");

        var emissionPoint = await _emissionPointRepo.GetByIdAsync(register.EmissionPointId.Value, tid, ct);
        if (emissionPoint is null)
            return Result<CashSessionDto>.ValidationFailure("El punto de emisión configurado en la caja ya no existe.");

        var session = CashSession.Open(
            tid, register.CompanyId, _b.BranchId, uid,
            register.Id, register.Code, register.Name,
            register.EmissionPointId.Value, emissionPoint.Code,
            cmd.OpeningAmount, uid, cmd.Notes);

        await _repo.AddAsync(session, ct);
        await _repo.SaveChangesAsync(ct);

        return Result<CashSessionDto>.Success(CajaMapper.ToDto(
            session, emissionPoint.EmissionType.ToString(),
