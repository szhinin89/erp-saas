...
public sealed partial class ElectronicDocumentIssuer : IElectronicDocumentIssuer
{
    ...
    {
        LogRegisterStarted(request.TenantId, request.CompanyId, request.DocumentType, request.SourceModule, request.SourceEntityId);

        var existing = await _repository.GetBySourceAsync(
            request.TenantId, request.SourceModule, request.SourceEntityId, ct);
        if (existing is not null)
        {
            ...
            if (existing.CurrentState is ElectronicDocumentState.Draft or ElectronicDocumentState.Failed)
    ...
    private async Task<Result<ElectronicDocumentDto>> RunPipelineAsync(
        ElectronicDocument document, RegisterElectronicDocumentRequest request, CancellationToken ct)
    {
        ...
        var reference = new ElectronicDocumentSourceReference(
            request.TenantId, request.CompanyId, request.SourceEntityId);
        ...
        try
        {
            ...
            {
                var reason = dataResult.Error ?? "No se pudo obtener el modelo común del documento de origen.";
            ...
            {
                var reason = xmlResult.Error ?? "No se pudo generar el XML del documento electrónico.";
            ...
            if (!validation.IsValid)
            {
                ...
                LogStageFailed("SchemaValidator", request.SourceModule, request.SourceEntityId, reason);
                return await FailAsync(document, request.UserId, ct, reason, isValidationFailure: true);
            }
            ...
            var signResult = await _signingService.SignAsync(
                request.TenantId, request.CompanyId, xmlResult.Value!, ct);
            ...
            var storageResult = await _xmlStorageService.StoreAsync(
                request.TenantId, request.DocumentType, document.Id,
                xmlResult.Value!, signResult.Value!, ct);
            if (!storageResult.IsSuccess)
            {
                var reason = storageResult.Error ?? "No se pudo almacenar el XML del documento electrónico.";
            ...
            document.MarkXmlGenerated(
                storageResult.Value!.DraftXmlPath, xmlResult.Value!.Version, validation.SchemaVersion ?? string.Empty, request.UserId);
            document.MarkSigned(
                storageResult.Value.SignedXmlPath, AccessKey.Create(signResult.Value!.AccessKey), request.UserId);

            await _repository.SaveChangesAsync(ct);
            LogDocumentSigned(document.Id, request.SourceModule, request.SourceEntityId, document.AccessKey!.Value);
        ...
        catch (Exception ex)
        {
            ...
            return await FailAsync(document, request.UserId, ct, $"Error inesperado en el pipeline: {ex.Message}");
        }
        ...
        try
        {
            var (reachedReceived, _) = await TrySendToReceptionAsync(
                document, request.CompanyId, request.UserId, Encoding.UTF8.GetBytes(signedXml.SignedXml), ct);
            if (reachedReceived)
                await AuthorizeAsync(document, request.TenantId, request.CompanyId, request.UserId, ct);
        }
        catch (Exception ex)
        {
    ...
    private async Task<Result<ElectronicDocumentDto>> FailAsync(
        ElectronicDocument document, Guid userId, CancellationToken ct,
        string reason, string? code = null, bool isValidationFailure = false)
    {
        ...
        Guid tenantId, Guid electronicDocumentId, Guid userId, CancellationToken ct = default)
    {
        ...
        if (document.CurrentState is ElectronicDocumentState.Draft or ElectronicDocumentState.Failed)
        {
            var request = new RegisterElectronicDocumentRequest(
                tenantId, document.CompanyId, document.DocumentType, document.SourceModule, document.SourceEntityId, userId);
            await RunPipelineAsync(document, request, ct);
            await ApplyDeadLetterIfExhaustedAsync(document, userId, document.LastError, ct);
            return Result<ElectronicDocumentDto>.Success(ElectronicDocumentMapper.ToDto(document));
        }

        if (document.CurrentState is not (ElectronicDocumentState.Signed or ElectronicDocumentState.Received))
        ...
        try
        {
            if (document.CurrentState == ElectronicDocumentState.Signed)
            {
                ...
                {
                    var (reachedReceived, receptionFailureReason) =
                        await TrySendToReceptionAsync(document, document.CompanyId, userId, signedXmlBytes, ct);
                    lastFailureReason = receptionFailureReason;
                    if (reachedReceived)
                        lastFailureReason = await AuthorizeAsync(document, tenantId, document.CompanyId, userId, ct);
                }
            }
            else
            {
    ...
    private async Task ApplyDeadLetterIfExhaustedAsync(
        ElectronicDocument document, Guid userId, string? lastFailureReason, CancellationToken ct)
    {
        if (document.CurrentState is not (ElectronicDocumentState.Failed
            or ElectronicDocumentState.Signed or ElectronicDocumentState.Received))
            return;
    ...
    private async Task<byte[]?> ReadSignedXmlAsync(ElectronicDocument document, CancellationToken ct)
    {
    ...
    private async Task<(bool ReachedReceived, string? FailureReason)> TrySendToReceptionAsync(
        ElectronicDocument document, Guid companyId, Guid userId, byte[] signedXmlBytes, CancellationToken ct)
    {
        ...
        var reason = recepcion.Errors.Count > 0
    ...
    private async Task<string?> AuthorizeAsync(
        ElectronicDocument document, Guid tenantId, Guid companyId, Guid userId, CancellationToken ct)
    {
        var authResult = await _authorizationService.CheckAsync(companyId, document.AccessKey!.Value, ct);
        ...
        if (auth.Authorized)
        {
            ...
            if (!string.IsNullOrWhiteSpace(auth.DocumentXml))
            {
                var storeResult = await _xmlStorageService.StoreAuthorizedAsync(
                    tenantId, document.DocumentType, document.Id, auth.DocumentXml, ct);
            ...
            if (!string.IsNullOrWhiteSpace(auth.AuthorizationNumber) &&
                !string.Equals(auth.AuthorizationNumber, document.AccessKey!.Value, StringComparison.Ordinal))
            {
                LogAuthorizationNumberMismatch(document.Id, auth.AuthorizationNumber, document.AccessKey.Value);
            }
        ...
        else if (auth.Status.Equals("TIMEOUT", StringComparison.OrdinalIgnoreCase))
        {
            ...
            var reason = auth.ErrorMessage ?? "El SRI no respondió a la consulta de autorización tras varios reintentos.";
    ...
    private partial void LogRegisterStarted(Guid tenantId, Guid companyId, ElectronicDocumentType documentType, string sourceModule, Guid sourceEntityId);
    ...
    private partial void LogRegisterConflict(string sourceModule, Guid sourceEntityId, Guid existingId);

    [LoggerMessage(Level = LogLevel.Warning,
        Message = "[ElectronicDocuments] Etapa '{Stage}' falló para origen {SourceModule}/{SourceEntityId}: {Reason}")]
    private partial void LogStageFailed(string stage, string sourceModule, Guid sourceEntityId, string? reason);
    ...
    private partial void LogDocumentSigned(Guid electronicDocumentId, string sourceModule, Guid sourceEntityId, string accessKey);
    ...
    private partial void LogDocumentRejectedAtReception(Guid electronicDocumentId, string status, string reason);
    ...
    private partial void LogDocumentAlreadyProcessing(Guid electronicDocumentId, string code, string status);
    ...
    private partial void LogDocumentAuthorized(Guid electronicDocumentId, string authorizationNumber);
    ...
    private partial void LogAuthorizationNumberMismatch(Guid electronicDocumentId, string reportedAuthorizationNumber, string accessKey);

    [LoggerMessage(Level = LogLevel.Warning,
        Message = "[ElectronicDocuments] Documento {ElectronicDocumentId} rechazado en autorización: {Reason}")]
    private partial void LogDocumentRejectedAtAuthorization(Guid electronicDocumentId, string reason);
    ...
    private partial void LogDocumentReactivated(Guid electronicDocumentId, ElectronicDocumentState restoredState);

    [LoggerMessage(Level = LogLevel.Information,
        Message = "[ElectronicDocuments] Reintento #{RetryCount} para documento {ElectronicDocumentId} (estado {State})")]
    private partial void LogRetryAttempted(Guid electronicDocumentId, ElectronicDocumentState state, int retryCount);
    ...
    private partial void LogPipelineStageThrew(Guid electronicDocumentId, string stage, Exception ex);
}
