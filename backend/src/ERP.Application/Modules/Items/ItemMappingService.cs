...
internal static class ItemMappingService
{
    public static ItemDto ToDto(
        ...
        IReadOnlyDictionary<Guid, string> itemTypeNames) => new(
            item.Id,
            item.Code.SKU,
            item.Code.ShortName,
            item.Code.Description,
            item.ItemTypeId,
            itemTypeNames.TryGetValue(item.ItemTypeId, out var itemTypeName) ? itemTypeName : string.Empty,
            item.CategoryNodeId,
            item.BrandId,
            item.DefaultUomCode,
    ...
    public static async Task<ItemDetailDto> ToDetailDtoAsync(
        Item item, ISriCatalogResolver sri, IItemTypeRepository itemTypeRepo, Guid tenantId, CancellationToken ct)
    {
    ...
    private static ItemTaxConfigDto MapTaxConfig(
        Item item,
        IReadOnlyDictionary<string, SriVatInfo> vatMap,
        IReadOnlyDictionary<string, SriIceInfo> iceMap) => new(
            item.TaxConfig.SaleVatCode,
            !string.IsNullOrWhiteSpace(item.TaxConfig.SaleVatCode) && vatMap.TryGetValue(item.TaxConfig.SaleVatCode, out var sv)
                ? sv.Name : null,
            item.TaxConfig.PurchaseVatCode,
            !string.IsNullOrWhiteSpace(item.TaxConfig.PurchaseVatCode) && vatMap.TryGetValue(item.TaxConfig.PurchaseVatCode, out var pv)
                ? pv.Name : null,
            item.TaxConfig.ExciseTaxCode,
            !string.IsNullOrWhiteSpace(item.TaxConfig.ExciseTaxCode) && iceMap.TryGetValue(item.TaxConfig.ExciseTaxCode, out var ice)
                ? ice.Name : null);
    ...
    private static ItemVariantDto MapVariant(ItemVariant v) => new(
        ...
        v.Barcodes.Where(b => b.IsActive).Select(b => new VariantBarcodeDto(b.Id, b.Code, b.BarcodeType, b.IsPrimary)).ToList());
    ...
    private static ItemUnitConversionDto MapConversion(
        ...
        IReadOnlyDictionary<string, SriUomInfo> uomMap) => new(
            c.Id,
            c.FromUomCode,
    ...
    private static ItemPackagingLevelDto MapPackaging(
        ...
        IReadOnlyDictionary<string, SriUomInfo> uomMap) => new(
            p.Id, p.Name, p.Level, p.BaseQuantity,
            p.UomCode,
    ...
    private static ItemSupplierCodeDto MapSupplierCode(ItemSupplierCode s) => new(
        s.Id, s.SupplierId, s.Code, s.IsPrimary, s.IsActive);
}
