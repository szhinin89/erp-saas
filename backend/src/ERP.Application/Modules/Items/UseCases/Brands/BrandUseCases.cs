...
public sealed class CreateBrandCommandHandler : IRequestHandler<CreateBrandCommand, Result<BrandDto>>
{
    ...
    {
        var sid = _s.TenantId;
        if (await _c.BrandCodeExistsAsync(cmd.Code, sid, cancellationToken)) return Result<BrandDto>.Conflict($"Código '{cmd.Code}' duplicado.");
        var b = Brand.Create(sid, cmd.Code, cmd.Name, _u.UserId, cmd.Manufacturer, cmd.CountryOfOrigin);
        await _c.AddBrandAsync(b, cancellationToken); await _c.SaveChangesAsync(cancellationToken);
...
public sealed class UpdateBrandCommandHandler : IRequestHandler<UpdateBrandCommand, Result<BrandDto>>
{
    ...
    public async Task<Result<BrandDto>> Handle(UpdateBrandCommand cmd, CancellationToken cancellationToken)
    {
        ...
        if (nc != b.Code && await _c.BrandCodeExistsAsync(nc, b.TenantId, cancellationToken)) return Result<BrandDto>.Conflict($"Código '{nc}' duplicado.");
...
public sealed class EnableBrandCommandHandler : IRequestHandler<EnableBrandCommand, Result<bool>>
{
    ...
    public async Task<Result<bool>> Handle(EnableBrandCommand cmd, CancellationToken cancellationToken)
    {
        ...
        await _c.SaveChangesAsync(cancellationToken); return Result<bool>.Success(true);
    }
}
...
public sealed class GetBrandByIdQueryHandler : IRequestHandler<GetBrandByIdQuery, Result<BrandDto>>
{
    ...
    public async Task<Result<BrandDto>> Handle(GetBrandByIdQuery q, CancellationToken cancellationToken)
    {
