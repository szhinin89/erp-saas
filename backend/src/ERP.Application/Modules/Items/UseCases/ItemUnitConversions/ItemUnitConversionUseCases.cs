    ...
    : IRequestHandler<ReplaceItemUnitConversionsCommand, Result<ItemDetailDto>>
{
    ...
    public async Task<Result<ItemDetailDto>> Handle(ReplaceItemUnitConversionsCommand cmd, CancellationToken cancellationToken)
    {
        ...
        var newConversions = cmd.Conversions
            .Select(c => ItemUnitConversion.Create(
                ...
                c.FromUomCode, c.ToUomCode, c.Factor, _user.UserId))
            .ToList();

        try { await _repository.ReplaceUnitConversionsAsync(cmd.Id, newConversions, cancellationToken); }
