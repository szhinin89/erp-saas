...
public sealed class CreateItemCommandValidator : AbstractValidator<CreateItemCommand>
{
    public CreateItemCommandValidator()
    {
        ...
        RuleFor(x => x.SaleVatCode)
            .MaximumLength(10).WithMessage("El código de IVA de venta no puede exceder 10 caracteres.")
            .When(x => !string.IsNullOrWhiteSpace(x.SaleVatCode));

        RuleFor(x => x.PurchaseVatCode)
            .MaximumLength(10).WithMessage("El código de IVA de compra no puede exceder 10 caracteres.")
            .When(x => !string.IsNullOrWhiteSpace(x.PurchaseVatCode));

        RuleFor(x => x.ExciseTaxCode)
            .MaximumLength(10).WithMessage("El código de ICE no puede exceder 10 caracteres.")
            .When(x => !string.IsNullOrWhiteSpace(x.ExciseTaxCode));

        RuleFor(x => x.Barcodes)
            .NotEmpty().WithMessage("Debe registrar al menos un código de barras.");
        ...
        RuleFor(x => x.Barcodes)
            .Must(list => list.Select(b => b.Code.Trim().ToUpperInvariant()).Distinct().Count() == list.Count)
            .WithMessage("Los códigos de barras no pueden repetirse.")
            .When(x => x.Barcodes is { Count: > 0 });

        RuleForEach(x => x.SupplierCodes).ChildRules(s =>
        {
            s.RuleFor(x => x.SupplierId)
                .NotEmpty().WithMessage("El proveedor es obligatorio.");
            s.RuleFor(x => x.Code)
                .NotEmpty().WithMessage("El código de proveedor es obligatorio.")
                .MaximumLength(100).WithMessage("El código de proveedor no puede exceder 100 caracteres.");
        });
