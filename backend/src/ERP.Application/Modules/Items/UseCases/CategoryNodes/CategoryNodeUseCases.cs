...
public sealed record CreateCategoryNodeCommand(
    Guid? ParentId, string Code, string Name, string? Description, string Level, int SortOrder = 0)
...
public sealed record GetCategoryTreeQuery(bool IncludeInactive = false)
    : IRequest<Result<CategoryTreeDto>>, ICompanyScopedRequest;
...
public sealed class CreateCategoryNodeCommandValidator : AbstractValidator<CreateCategoryNodeCommand>
{
...
public sealed class UpdateCategoryNodeCommandValidator : AbstractValidator<UpdateCategoryNodeCommand>
{
    ...
    : IRequestHandler<CreateCategoryNodeCommand, Result<CategoryNodeDto>>
{
    ...
    public CreateCategoryNodeCommandHandler(
        ICategoryNodeRepository repo, IOrgSettingsRepository orgRepo,
        ICurrentTenant tenant, ICurrentCompany company, ICurrentUser user)
    { _repo = repo; _orgRepo = orgRepo; _tenant = tenant; _company = company; _user = user; }

    public async Task<Result<CategoryNodeDto>> Handle(CreateCategoryNodeCommand cmd, CancellationToken ct)
    {
        ...
        if (cmd.ParentId.HasValue)
        {
            ...
            if (!parent.IsActive)
                return Result<CategoryNodeDto>.ValidationFailure("No se puede crear un hijo en un nodo inactivo.");
        }

        var newDepth = CategoryDepthResolver.DepthOf(parent?.Path) + 1;
        var maxDepth = await CategoryDepthResolver.ResolveMaxDepthAsync(_orgRepo, tenantId, _company.CompanyId, ct);
        ...
        var node = ItemCategoryNode.Create(tenantId, cmd.Code, cmd.Name, level, userId,
            cmd.ParentId, cmd.Description, cmd.SortOrder);
...
internal static class CategoryDepthResolver
{
    ...
    public static async Task<int> ResolveMaxDepthAsync(
        IOrgSettingsRepository orgRepo, Guid tenantId, Guid companyId, CancellationToken ct)
    {
        var setting = await orgRepo.GetAsync(
            tenantId, companyId, OrgScope.Company, companyId, OrgSettingKeys.Catalog.MaxCategoryDepth, ct);
    ...
    : IRequestHandler<UpdateCategoryNodeCommand, Result<CategoryNodeDto>>
{
    ...
    public UpdateCategoryNodeCommandHandler(ICategoryNodeRepository repo, ICurrentTenant tenant, ICurrentUser user)
    { _repo = repo; _tenant = tenant; _user = user; }

    public async Task<Result<CategoryNodeDto>> Handle(UpdateCategoryNodeCommand cmd, CancellationToken ct)
    {
        var node = await _repo.GetByIdAsync(cmd.Id, ct);
        if (node is null) return Result<CategoryNodeDto>.NotFound("Nodo no encontrado.");
    ...
    : IRequestHandler<DisableCategoryNodeCommand, Result<bool>>
{
    ...
    {
        var node = await _repo.GetByIdAsync(cmd.Id, ct);
        if (node is null) return Result<bool>.NotFound("Nodo no encontrado.");

        var items = await _repo.CountItemsByNodeAsync(cmd.Id, ct);
        if (items > 0)
            return Result<bool>.ValidationFailure($"Tiene {items} ítem(s) asignado(s). Reasígnelos primero.");

        try { node.Disable(_user.UserId); }
    ...
    public async Task<Result<bool>> Handle(EnableCategoryNodeCommand cmd, CancellationToken ct)
    {
        ...
        if (node.ParentId.HasValue)
        {
            ...
            if (parent is not null && !parent.IsActive)
                return Result<bool>.ValidationFailure("No se puede activar un nodo cuyo padre está inactivo.");
        }

        try { node.Enable(_user.UserId); }
        catch (InvalidOperationException ex) { return Result<bool>.ValidationFailure(ex.Message); }
    ...
    : IRequestHandler<GetCategoryTreeQuery, Result<CategoryTreeDto>>
{
    ...
    { _repo = repo; _orgRepo = orgRepo; _tenant = tenant; _company = company; }

    public async Task<Result<CategoryTreeDto>> Handle(GetCategoryTreeQuery query, CancellationToken ct)
    {
        var tenantId = _tenant.TenantId;
    ...
    : IRequestHandler<GetCategoryNodeByIdQuery, Result<CategoryNodeDto>>
{
    ...
    public async Task<Result<CategoryNodeDto>> Handle(GetCategoryNodeByIdQuery query, CancellationToken ct)
    {
...
{
    public static CategoryNodeDto ToDto(ItemCategoryNode n) => new(
        n.Id, n.ParentId, n.Code, n.Name, n.Description,
        n.Level.ToString(), n.Path, CategoryDepthResolver.DepthOf(n.Path), n.SortOrder,
