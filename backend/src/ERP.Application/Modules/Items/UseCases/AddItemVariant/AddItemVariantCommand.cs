...
public sealed class AddItemVariantCommandValidator : AbstractValidator<AddItemVariantCommand>
{
    public AddItemVariantCommandValidator()
    {
        ...
        RuleFor(x => x.Attributes)
            .NotEmpty()
            .WithMessage("Debe especificar al menos un atributo de variante.")
        ...
        RuleFor(x => x.SkuOverride)
            .MaximumLength(80)
            .WithMessage("El SKU override no puede exceder 80 caracteres.")
    ...
    : IRequestHandler<AddItemVariantCommand, Result<ItemVariantDto>>
{
    ...
    public async Task<Result<ItemVariantDto>> Handle(
        AddItemVariantCommand cmd,
    ...
    )
    {
        ...
        var axisAttributes = cmd
            .Attributes.Select(a => (a.AttributeDefinitionId, a.Value))
            .ToList()
            .AsReadOnly();
        ...
        try
        {
            ...
            if (
                await _repository.VariantSkuExistsAsync(
                    variant.SKU,
                    _currentTenant.TenantId,
                    cancellationToken
                )
            )
                return Result<ItemVariantDto>.Conflict(
                    $"Ya existe una variante con SKU '{variant.SKU}' en otro ítem."
                );
            ...
            return Result<ItemVariantDto>.Success(
                new ItemVariantDto(
                    variant.Id,
                    variant.SKU,
                    variant.Name,
                    ...
                    variant.IsActive,
                    variant
                        .Attributes.Select(a => new VariantAttributeDto(
                            a.AttributeDefinitionId,
                            a.Value
                        ))
                        .ToList(),
                    variant
                        .Barcodes.Select(b => new VariantBarcodeDto(
                            b.Id,
                            b.Code,
                            b.BarcodeType,
                            b.IsPrimary
