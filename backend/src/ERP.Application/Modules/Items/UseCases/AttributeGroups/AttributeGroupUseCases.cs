...
{
    Task<IReadOnlyList<AttributeGroup>> GetAllAsync(Guid tenantId, CancellationToken cancellationToken = default);
    Task<AttributeGroup?> GetByIdAsync(Guid id, Guid tenantId, CancellationToken cancellationToken = default);
...
public sealed class CreateAttributeGroupCommandHandler : IRequestHandler<CreateAttributeGroupCommand, Result<AttributeGroupDto>>
{
    ...
    {
        var sid = _s.TenantId;
        if (await _r.ExistsByCodeAsync(cmd.Code, sid, cancellationToken)) return Result<AttributeGroupDto>.Conflict($"Código '{cmd.Code}' duplicado.");
        var g = AttributeGroup.Create(sid, cmd.Code, cmd.Name, cmd.SortOrder);
        await _r.AddAsync(g, cancellationToken); await _r.SaveChangesAsync(cancellationToken);
...
public sealed class UpdateAttributeGroupCommandHandler : IRequestHandler<UpdateAttributeGroupCommand, Result<AttributeGroupDto>>
{
    ...
    public async Task<Result<AttributeGroupDto>> Handle(UpdateAttributeGroupCommand cmd, CancellationToken cancellationToken)
    {
        ...
        g.Update(cmd.Name, cmd.SortOrder, _u.UserId); await _r.SaveChangesAsync(cancellationToken);
        return Result<AttributeGroupDto>.Success(new(g.Id, g.Code, g.Name, g.SortOrder, g.IsActive));
    }
}
...
public sealed class EnableAttributeGroupCommandHandler : IRequestHandler<EnableAttributeGroupCommand, Result<bool>>
{
    ...
    public async Task<Result<bool>> Handle(EnableAttributeGroupCommand cmd, CancellationToken cancellationToken)
    {
        ...
        await _r.SaveChangesAsync(cancellationToken); return Result<bool>.Success(true);
    }
}
