...
public sealed record AddBarcodeCommand(Guid ItemId, Guid VariantId, string Code, string BarcodeType)
    : IRequest<Result<VariantBarcodeDto>>, ICompanyScopedRequest;
...
public sealed class AddBarcodeCommandValidator : AbstractValidator<AddBarcodeCommand>
{
    public AddBarcodeCommandValidator()
    {
        ...
        RuleFor(x => x.BarcodeType).NotEmpty().WithMessage("El tipo de código de barras es obligatorio.");
    }
}

// ── Handlers ────────────────────────────────────────────────────────────

public sealed class AddBarcodeHandler : IRequestHandler<AddBarcodeCommand, Result<VariantBarcodeDto>>
{
    private readonly IItemRepository _repo;
    private readonly IItemCatalogRepository _catalogRepo;
    private readonly ICurrentTenant _t;
    private readonly ICurrentUser _u;
    public AddBarcodeHandler(IItemRepository repo, IItemCatalogRepository catalogRepo, ICurrentTenant t, ICurrentUser u)
    { _repo = repo; _catalogRepo = catalogRepo; _t = t; _u = u; }
    ...
    public async Task<Result<VariantBarcodeDto>> Handle(AddBarcodeCommand cmd, CancellationToken ct)
    {
        ...
        if (item is null) return Result<VariantBarcodeDto>.NotFound("Ítem no encontrado.");

        var variant = item.Variants.FirstOrDefault(v => v.Id == cmd.VariantId);
        if (variant is null) return Result<VariantBarcodeDto>.NotFound("Variante no encontrada.");

        if (!await _catalogRepo.BarcodeTypeExistsAndActiveAsync(cmd.BarcodeType, ct))
            return Result<VariantBarcodeDto>.ValidationFailure($"El tipo de código de barras '{cmd.BarcodeType}' no es válido.");

        var code = cmd.Code.Trim();
        if (await _repo.BarcodeExistsAsync(code, _t.TenantId, ct))
            return Result<VariantBarcodeDto>.Conflict($"El código de barras '{code}' ya está asignado a otro ítem.");
        ...
        return Result<VariantBarcodeDto>.Success(new VariantBarcodeDto(bc.Id, bc.Code, bc.BarcodeType, bc.IsPrimary));
    }
}
...
public sealed class DisableBarcodeHandler : IRequestHandler<DisableBarcodeCommand, Result<bool>>
{
    ...
    private readonly ICurrentTenant _t;
    private readonly ICurrentUser _u;
    public DisableBarcodeHandler(IItemRepository repo, ICurrentTenant t, ICurrentUser u) { _repo = repo; _t = t; _u = u; }
    ...
    public async Task<Result<bool>> Handle(DisableBarcodeCommand cmd, CancellationToken ct)
    {
        ...
        var variant = item.Variants.FirstOrDefault(v => v.Id == cmd.VariantId);
        if (variant is null) return Result<bool>.NotFound("Variante no encontrada.");
