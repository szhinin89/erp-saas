...
{
    Task<IReadOnlyList<AttributeDefinition>> GetAllAsync(Guid tenantId, Guid? groupId, CancellationToken cancellationToken = default);
    Task<AttributeDefinition?> GetByIdAsync(Guid id, Guid tenantId, CancellationToken cancellationToken = default);
    Task<bool> ExistsByCodeAsync(Guid groupId, string code, Guid tenantId, CancellationToken cancellationToken = default);
    ...
    : IRequestHandler<CreateAttributeDefinitionCommand, Result<AttributeDefinitionDto>>
{
    ...
    public CreateAttributeDefinitionCommandHandler(
        IAttributeDefinitionRepository repo, ICurrentTenant tenant, ICurrentUser user)
    { _repo = repo; _currentTenant = tenant; _user = user; }

    public async Task<Result<AttributeDefinitionDto>> Handle(CreateAttributeDefinitionCommand cmd, CancellationToken cancellationToken)
    {
    ...
    : IRequestHandler<UpdateAttributeDefinitionCommand, Result<AttributeDefinitionDto>>
{
    ...
    public async Task<Result<AttributeDefinitionDto>> Handle(UpdateAttributeDefinitionCommand cmd, CancellationToken cancellationToken)
    {
    ...
    : IRequestHandler<EnableAttributeDefinitionCommand, Result<bool>>
{
    ...
    private readonly ICurrentTenant _currentTenant;
    private readonly ICurrentUser _user;
    public EnableAttributeDefinitionCommandHandler(IAttributeDefinitionRepository repo, ICurrentTenant tenant, ICurrentUser user)
    { _repo = repo; _currentTenant = tenant; _user = user; }
    public async Task<Result<bool>> Handle(EnableAttributeDefinitionCommand cmd, CancellationToken cancellationToken)
    {
        var def = await _repo.GetByIdAsync(cmd.Id, _currentTenant.TenantId, cancellationToken);
    ...
    : IRequestHandler<DisableAttributeDefinitionCommand, Result<bool>>
{
    ...
    private readonly ICurrentTenant _currentTenant;
    private readonly ICurrentUser _user;
    public DisableAttributeDefinitionCommandHandler(IAttributeDefinitionRepository repo, ICurrentTenant tenant, ICurrentUser user)
    { _repo = repo; _currentTenant = tenant; _user = user; }
    public async Task<Result<bool>> Handle(DisableAttributeDefinitionCommand cmd, CancellationToken cancellationToken)
    {
        var def = await _repo.GetByIdAsync(cmd.Id, _currentTenant.TenantId, cancellationToken);
        if (def is null) return Result<bool>.NotFound("Atributo no encontrado.");
    ...
    : IRequestHandler<GetAttributeDefinitionsQuery, Result<IReadOnlyList<AttributeDefinitionDto>>>
{
    private readonly IAttributeDefinitionRepository _repo;
    private readonly ICurrentTenant _currentTenant;
    public GetAttributeDefinitionsQueryHandler(IAttributeDefinitionRepository repo, ICurrentTenant tenant)
    { _repo = repo; _currentTenant = tenant; }
    public async Task<Result<IReadOnlyList<AttributeDefinitionDto>>> Handle(GetAttributeDefinitionsQuery q, CancellationToken cancellationToken)
    {
    ...
    : IRequestHandler<GetAttributeDefinitionByIdQuery, Result<AttributeDefinitionDto>>
{
    private readonly IAttributeDefinitionRepository _repo;
    private readonly ICurrentTenant _currentTenant;
    public GetAttributeDefinitionByIdQueryHandler(IAttributeDefinitionRepository repo, ICurrentTenant tenant)
    { _repo = repo; _currentTenant = tenant; }
    public async Task<Result<AttributeDefinitionDto>> Handle(GetAttributeDefinitionByIdQuery q, CancellationToken cancellationToken)
    {
