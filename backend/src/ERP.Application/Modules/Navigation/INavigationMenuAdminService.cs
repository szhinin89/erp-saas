...
{
    Task<AdminNavigationMenuResponse> GetMenuTreeAsync(
    ...
    Task<(bool Ok, string? Error)> ReorderGroupsAsync(
    ...
    /// </summary>
    Task<(bool Ok, string? Error)> ReorderItemLevelsAsync(
    ...
    Task<(bool Ok, Guid? NewId, string? Error)> CreateNavItemAsync(
        CreateNavItemRequest request,
        CancellationToken cancellationToken = default
    );

    Task<(bool Ok, string? Error)> UpdateNavItemAsync(
        Guid itemId,
    ...
    Task<(bool Ok, string? Error)> DeleteNavItemAsync(
        Guid itemId,
        CancellationToken cancellationToken = default
    );
}
