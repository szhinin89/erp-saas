    ...
    : IRequestHandler<GetStockQuery, Result<IReadOnlyList<CurrentStockDto>>>
{
    ...
    public async Task<Result<IReadOnlyList<CurrentStockDto>>> Handle(
        GetStockQuery request, CancellationToken ct)
    {
        ...
        if (request.WarehouseId.HasValue)
        {
            stocks = await _repo.GetStockByWarehouseAsync(
                _tenant.TenantId, request.WarehouseId.Value, request.ItemId, ct);
        }
        else
        {
            stocks = await _repo.GetStockByProductAsync(
                _tenant.TenantId, request.ItemId!.Value, ct);
        }
    ...
    internal static CurrentStockDto ToDto(CurrentStock s) => new(
        s.Id, s.ProductId, s.WarehouseId,
        s.Quantity, s.ReservedQuantity, s.AvailableQuantity,
        s.TotalStockValue, s.AverageCost, s.LastUpdatedAt);
}
