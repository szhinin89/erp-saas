    ...
    : IRequestHandler<GetItemWarehouseAvailabilityQuery, Result<IReadOnlyList<ItemWarehouseAvailabilityDto>>>
{
    ...
    public async Task<Result<IReadOnlyList<ItemWarehouseAvailabilityDto>>> Handle(
        GetItemWarehouseAvailabilityQuery request, CancellationToken ct)
    {
        var warehouses = await _warehouseRepo.GetAsync(
            _tenant.TenantId, activeFilter: true, search: null, branchId: null, ct);
        ...
        var dtos = warehouses.Select(w =>
        {
            ...
            return new ItemWarehouseAvailabilityDto(
                w.Id, w.Name, available, stock?.ReservedQuantity ?? 0m, available > 0);
        }).ToList();
