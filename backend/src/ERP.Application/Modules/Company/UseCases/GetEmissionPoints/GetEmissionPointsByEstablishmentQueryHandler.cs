...
public sealed class GetEmissionPointsByEstablishmentQueryHandler
    : IRequestHandler<
        GetEmissionPointsByEstablishmentQuery,
        Result<IReadOnlyList<EmissionPointDto>>
    >
{
    private readonly IEmissionPointRepository _repo;
    private readonly ICurrentTenant _currentTenant;

    public GetEmissionPointsByEstablishmentQueryHandler(
    ...
    public async Task<Result<IReadOnlyList<EmissionPointDto>>> Handle(
        GetEmissionPointsByEstablishmentQuery request,
        CancellationToken cancellationToken
    )
    {
        var items = await _repo.GetByEstablishmentAsync(
    ...
    internal static EmissionPointDto ToDto(Domain.Modules.Company.Entities.EmissionPoint ep) =>
        new(
            ep.Id,
            ep.EstablishmentId,
            ep.CompanyId,
