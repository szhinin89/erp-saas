...
public sealed class UpsertCompanyBpSalesSettingsHandlerTests
{
    ...
    {
        public Mock<ICompanyBpSalesSettingsRepository> SettingsRepo { get; } = new();
        ...
        public BusinessPartner Customer { get; } =
            BusinessPartner.Create(TenantId, "04", "1791352688001", 2, "Cliente Demo", UserId);
    ...
    public async Task Crea_configuracion_cuando_no_existe()
    {
        ...
        var result = await handler.Handle(
            new UpsertCompanyBpSalesSettingsCommand(CustomerId, f.ActivePaymentTerm.Id),
        ...
        f.SettingsRepo.Verify(
            r => r.AddAsync(It.IsAny<CompanyBpSalesSettings>(), It.IsAny<CancellationToken>()),
            Times.Once
        );
    }
    ...
    public async Task PaymentTermId_null_limpia_el_default()
    {
        ...
        var handler = f.BuildHandler();
        var result = await handler.Handle(
            new UpsertCompanyBpSalesSettingsCommand(CustomerId, null),
            CancellationToken.None
        );
    ...
    [Fact]
    public async Task Rechaza_cliente_inactivo()
    {
        var f = new Fixture();
        f.Customer.Deactivate(UserId);
    ...
    public async Task Rechaza_sin_rol_Customer_activo()
    {
        ...
        result.IsSuccess.Should().BeFalse();
        result.Error.Should().Contain("rol de Cliente");
    }
    ...
    public async Task Rechaza_PaymentTerm_inexistente()
    {
        ...
        var handler = f.BuildHandler();
        var result = await handler.Handle(
            new UpsertCompanyBpSalesSettingsCommand(CustomerId, missingId),
            CancellationToken.None
        );
    ...
    public async Task Aislamiento_empresa_A_vs_empresa_B_no_se_mezclan()
    {
        ...
        // CompanyBpSalesSettingsRepositoryTests (Fase 3a). Aquí se confirma que el handler usa
