...
public sealed class ImportPurchaseReceptionHandlerTests
{
    ...
    private static PurchaseReceptionRecord SampleRecord(string accessKey = "0107202601179135268800120150270001617400016174011") => new(
        ...
        "015-027-000161740", accessKey,
        new DateOnly(2026, 7, 1), new DateTime(2026, 7, 1, 21, 6, 55), "0350016432",
        15.96m, 2.4m, 18.35m, null);

    private static (ImportPurchaseReceptionHandler handler, Mock<IPurchaseReceptionParser> parser,
        Mock<IPurchaseReceptionVerifier> verifier, Mock<IPurchaseReceptionDocumentRepository> repo) BuildHandler()
    {
        ...
        var handler = new ImportPurchaseReceptionHandler(
            parser.Object, verifier.Object, repo.Object, tenant.Object, company.Object, branch.Object, user.Object);
    ...
    public async Task Handle_persists_a_new_document_and_maps_it_into_the_response_dto()
    {
        ...
        verifier.Setup(v => v.VerifyAsync(It.IsAny<IReadOnlyList<PurchaseReceptionRecord>>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync([new PurchaseReceptionVerifiedItem(record, SupplierExists: true, PurchaseExists: false, PurchaseReceptionStatus.Pending)]);
        repo.Setup(r => r.GetByAccessKeyAsync(TenantId, record.AccessKey, It.IsAny<CancellationToken>()))
            .ReturnsAsync((PurchaseReceptionDocument?)null);

        PurchaseReceptionDocument? added = null;
        repo.Setup(r => r.AddAsync(It.IsAny<PurchaseReceptionDocument>(), It.IsAny<CancellationToken>()))
            .Callback<PurchaseReceptionDocument, CancellationToken>((d, _) => added = d)
            .Returns(Task.CompletedTask);

        var command = new ImportPurchaseReceptionCommand(new MediaUploadContent(new MemoryStream(), "reception.txt", "text/plain", 10));
    ...
    public async Task Handle_reuses_existing_document_when_access_key_was_already_imported()
    {
        ...
        var existing = PurchaseReceptionDocument.Create(
            ...
            record.SupplierRuc, record.SupplierName, supplierId: null,
            record.AccessKey, record.InvoiceNumber, record.IssueDate, record.AuthorizationDate,
            record.Subtotal, record.VatAmount, record.Total, UserId);

        var (handler, parser, verifier, repo) = BuildHandler();
        parser.Setup(p => p.ParseAsync(It.IsAny<Stream>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(new PurchaseReceptionParseResult([record], [], 0));
        verifier.Setup(v => v.VerifyAsync(It.IsAny<IReadOnlyList<PurchaseReceptionRecord>>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync([new PurchaseReceptionVerifiedItem(record, SupplierExists: true, PurchaseExists: true, PurchaseReceptionStatus.Imported)]);
        repo.Setup(r => r.GetByAccessKeyAsync(TenantId, record.AccessKey, It.IsAny<CancellationToken>()))
        ...
        repo.Verify(r => r.AddAsync(It.IsAny<PurchaseReceptionDocument>(), It.IsAny<CancellationToken>()), Times.Never);
    ...
    public async Task Handle_deduplicates_repeated_access_key_within_the_same_file()
    {
        ...
        verifier.Setup(v => v.VerifyAsync(It.IsAny<IReadOnlyList<PurchaseReceptionRecord>>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync([
                new PurchaseReceptionVerifiedItem(record1, SupplierExists: false, PurchaseExists: false, PurchaseReceptionStatus.NewSupplier),
                new PurchaseReceptionVerifiedItem(record2, SupplierExists: false, PurchaseExists: false, PurchaseReceptionStatus.NewSupplier),
            ]);
        repo.Setup(r => r.GetByAccessKeyAsync(TenantId, record1.AccessKey, It.IsAny<CancellationToken>()))
            .ReturnsAsync((PurchaseReceptionDocument?)null);
        repo.Setup(r => r.AddAsync(It.IsAny<PurchaseReceptionDocument>(), It.IsAny<CancellationToken>()))
            .Returns(Task.CompletedTask);
        ...
        repo.Verify(r => r.AddAsync(It.IsAny<PurchaseReceptionDocument>(), It.IsAny<CancellationToken>()), Times.Once);
        repo.Verify(r => r.SaveChangesAsync(It.IsAny<CancellationToken>()), Times.Once);
    }
