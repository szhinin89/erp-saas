...
public sealed class LoginHandlerTests
{
    ...
    private sealed class Fixture
    {
        ...
        public LoginHandler BuildHandler() => new(
            TenantRepo.Object, CompanyRepo.Object, AccessRepo.Object, TokenService.Object,
            PasswordHasher.Object, RefreshTokenService.Object, CompanyProvisioning.Object,
            BranchRepo.Object, Mediator.Object, PasswordResetTokenRepo.Object, PasswordResetOptions);
    }

    private static Fixture BuildValidLoginFixture(IdentityUser user, Tenant tenant, Company company, CompanyUserMembership membership)
    {
        var f = new Fixture();
        f.AccessRepo.Setup(r => r.GetUserByUsernameAsync(Username, It.IsAny<CancellationToken>())).ReturnsAsync(user);
        f.AccessRepo.Setup(r => r.GetActiveCompanyUserMembershipsForUserSystemAsync(user.Id, It.IsAny<CancellationToken>()))
            .ReturnsAsync(new[] { membership });
        f.CompanyRepo.Setup(r => r.GetByIdsAsync(It.Is<IReadOnlyCollection<Guid>>(c => c.Contains(company.Id)), It.IsAny<CancellationToken>()))
    ...
    public async Task Login_con_multiples_empresas_no_intenta_crear_UserSession_regresion()
    {
        ...
        var companyB = Company.CreateManaged(tenant.Id, "1790012345002", "Otra S.A.", createdBy: CreatedBy);
        var membershipA = CompanyUserMembership.Create(companyA.Id, user.Id, "Admin", null, CreatedBy);
        var membershipB = CompanyUserMembership.Create(companyB.Id, user.Id, "Admin", null, CreatedBy);

        var f = new Fixture();
        f.AccessRepo.Setup(r => r.GetUserByUsernameAsync(Username, It.IsAny<CancellationToken>())).ReturnsAsync(user);
        f.AccessRepo.Setup(r => r.GetActiveCompanyUserMembershipsForUserSystemAsync(user.Id, It.IsAny<CancellationToken>()))
            .ReturnsAsync(new[] { membershipA, membershipB });
        f.CompanyRepo.Setup(r => r.GetByIdsAsync(It.IsAny<IReadOnlyCollection<Guid>>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(new[] { companyA, companyB });
        f.TenantRepo.Setup(r => r.GetByIdAsync(tenant.Id, It.IsAny<CancellationToken>())).ReturnsAsync(tenant);
        f.PasswordHasher.Setup(h => h.VerifyPassword(Password, PasswordHash)).Returns(true);
        f.TokenService.Setup(s => s.GenerateSessionToken(user, tenant.Id, It.IsAny<string>())).Returns("bootstrap-jwt");
        f.RefreshTokenService.Setup(s => s.CreateAsync(
                user.Id, tenant.Id, null, RefreshUserType.Identity, It.IsAny<CancellationToken>()))
    ...
    {
        var user = IdentityUser.Create(Username, "Ana", "Perez", "ana@test.com", PasswordHash, CreatedBy);
        ...
        f.PasswordResetTokenRepo.Verify(r => r.AddAsync(It.IsAny<ERP.Domain.Auth.Entities.PasswordResetToken>(), It.IsAny<CancellationToken>()), Times.Once);
        f.TokenService.Verify(s => s.GenerateSessionToken(It.IsAny<IdentityUser>(), It.IsAny<Guid>(), It.IsAny<string>()), Times.Never);
        f.RefreshTokenService.Verify(s => s.CreateAsync(
    ...
    public async Task Usuario_con_RequirePasswordReset_pero_credenciales_invalidas_no_revela_el_estado_ni_emite_token()
    {
        ...
        result.IsSuccess.Should().BeFalse();
        f.PasswordResetTokenRepo.Verify(r => r.AddAsync(It.IsAny<ERP.Domain.Auth.Entities.PasswordResetToken>(), It.IsAny<CancellationToken>()), Times.Never);
    }
}
