...
public sealed class ElectronicDocumentRetryPolicyTests
{
    ...
    public void Not_eligible_when_max_attempts_reached()
    {
        ElectronicDocumentRetryPolicy.IsEligibleForAutomaticRetry(
            ElectronicDocumentRetryPolicy.MaxAttempts, Now.AddMinutes(-30), Now).Should().BeFalse();
    }
    ...
    public void Not_eligible_within_backoff_window()
    {
        ...
        ElectronicDocumentRetryPolicy.IsEligibleForAutomaticRetry(1, Now.AddSeconds(-30), Now).Should().BeFalse();
    }

    [Fact]
    public void Eligible_once_backoff_window_elapsed()
    {
        ElectronicDocumentRetryPolicy.IsEligibleForAutomaticRetry(1, Now.AddMinutes(-3), Now).Should().BeTrue();
    }

    [Fact]
    public void Eligible_exactly_at_backoff_boundary()
    {
        ElectronicDocumentRetryPolicy.IsEligibleForAutomaticRetry(0, Now.AddMinutes(-1), Now).Should().BeTrue();
    }
    ...
    public void Uses_last_backoff_step_for_retry_counts_beyond_schedule_length()
    {
        ...
        ElectronicDocumentRetryPolicy.IsEligibleForAutomaticRetry(4, Now.AddMinutes(-10), Now).Should().BeFalse();
        ElectronicDocumentRetryPolicy.IsEligibleForAutomaticRetry(4, Now.AddMinutes(-17), Now).Should().BeTrue();
    }
}
