...
public sealed class ElectronicDocumentReceptionServiceTests
{
    ...
    private sealed class FakeReceptionClient : ISriReceptionClient
    {
        ...
        public FakeReceptionClient(Func<byte[], string, SriReceptionResult> behavior) => _behavior = behavior;

        public Task<SriReceptionResult> SendAsync(byte[] signedXmlBytes, string wsdlUrl, CancellationToken ct = default)
        {
    ...
    public async Task SendAsync_without_sri_settings_fails_with_clear_message_not_exception()
    {
        var service = new ElectronicDocumentReceptionService(
            ...
            new FakeReceptionClient((_, _) => throw new InvalidOperationException("no debería invocarse")),
    ...
    public async Task SendAsync_when_client_reports_connection_error_translates_to_failure()
    {
        ...
        var client = new FakeReceptionClient((_, _) => new SriReceptionResult
        {
            Status = "ERROR_CONEXION",
            Errors = ["No se pudo contactar al servicio de recepción del SRI."],
        });
        var service = new ElectronicDocumentReceptionService(
            new FakeSriSettingsRepository(settings), client, NullLogger<ElectronicDocumentReceptionService>.Instance);
    ...
    {
        var settings = ValidSriSettings("https://celcer.sri.gob.ec/comprobantes-electronicos-ws/RecepcionComprobantesOffline?wsdl");
        var client = new FakeReceptionClient((_, _) => new SriReceptionResult { Status = "RECIBIDA" });
        var service = new ElectronicDocumentReceptionService(
            new FakeSriSettingsRepository(settings), client, NullLogger<ElectronicDocumentReceptionService>.Instance);
