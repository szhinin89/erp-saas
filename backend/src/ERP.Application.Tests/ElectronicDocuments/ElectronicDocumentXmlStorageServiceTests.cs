...
public sealed class ElectronicDocumentXmlStorageServiceTests
{
    private sealed class FakeFileStorage : IFileStorage
    {
        ...
        public Task<Stream?> GetAsync(
            ...
            Task.FromResult<Stream?>(
                Saved.TryGetValue(storedPath, out var bytes) ? new MemoryStream(bytes) : null
            );
    ...
    private static ElectronicDocumentXml DraftXml() =>
        new(
    ...
    private static SignedElectronicDocumentXml SignedXml() =>
        new(
    ...
    public async Task StoreAsync_happy_path_saves_both_files_via_file_storage()
    {
        ...
        var service = new ElectronicDocumentXmlStorageService(
            fileStorage,
            new ElectronicDocumentStorageNamingStrategy(),
            NullLogger<ElectronicDocumentXmlStorageService>.Instance
        );
        ...
        fileStorage
            .Saved[result.Value!.DraftXmlPath]
            .Should()
            .Equal(System.Text.Encoding.UTF8.GetBytes("<factura>draft</factura>"));
        fileStorage
            .Saved[result.Value.SignedXmlPath]
            .Should()
            .Equal(System.Text.Encoding.UTF8.GetBytes("<factura>signed</factura>"));
    }
    ...
    public async Task StoreAsync_when_draft_save_fails_returns_failure_without_saving_signed()
    {
        var fileStorage = new FakeFileStorage
        {
            FailWhenPathContains = p => p.EndsWith("draft.xml"),
        };
        var service = new ElectronicDocumentXmlStorageService(
            fileStorage,
            new ElectronicDocumentStorageNamingStrategy(),
    ...
    public async Task StoreAsync_when_signed_save_fails_deletes_already_saved_draft_and_returns_failure()
    {
        var fileStorage = new FakeFileStorage
        {
            FailWhenPathContains = p => p.EndsWith("signed.xml"),
        };
        var service = new ElectronicDocumentXmlStorageService(
            fileStorage,
            new ElectronicDocumentStorageNamingStrategy(),
        ...
        fileStorage
            .Saved.Should()
            .BeEmpty("el borrador huérfano debe eliminarse (compensación best-effort)");
