...
public sealed class ElectronicDocumentSigningServiceTests
{
    ...
    {
        public string Protect(string plaintext) => plaintext;
        public string UnprotectOrPlaintext(string storedValue) => throw new CryptographicException("clave de protección inválida");
    ...
    private sealed class FakeSigner : IElectronicDocumentSigner
    {
        ...
        public byte[] Sign(string xmlUtf8, string p12FilePath, string p12Password) => _behavior(xmlUtf8, [], p12Password);
        public byte[] Sign(string xmlUtf8, byte[] p12Bytes, string p12Password) => _behavior(xmlUtf8, p12Bytes, p12Password);
    }

    private static ElectronicDocumentXml SampleXml() => new(
        Xml: "<factura/>",
        Encoding: "UTF-8",
        Version: "1.1.0",
    ...
    private static SriSettings ValidSriSettings()
    {
        ...
        s.AttachCertificate("certificates/x/certificate.p12", "certificate.p12", 128, DateTime.UtcNow, Guid.NewGuid());
        s.UpdateConfiguration(1, 1, "https://celcer.sri.gob.ec/wsdl", "plaintext-password", Guid.NewGuid());
        return s;
    }
    ...
    public async Task SignAsync_without_sri_settings_fails_with_clear_message_not_exception()
    {
        var service = new ElectronicDocumentSigningService(
            ...
            new FakeSigner((_, _, _) => throw new InvalidOperationException("no debería invocarse")),
    ...
    public async Task SignAsync_when_certificate_has_no_private_key_fails_with_certificate_error()
    {
        ...
        var service = new ElectronicDocumentSigningService(
            new FakeSriSettingsRepository(settings),
            new PassthroughSecretProtector(),
            new FakeSigner((_, _, _) => throw new InvalidOperationException("El certificado no contiene clave privada RSA.")),
            new FakeFileStorage([1, 2, 3]),
            NullLogger<ElectronicDocumentSigningService>.Instance);
    ...
    public async Task SignAsync_happy_path_delegates_to_signer_and_returns_signed_xml()
    {
        ...
        string? capturedXml = null, capturedPassword = null;
        byte[]? capturedBytes = null;

        var service = new ElectronicDocumentSigningService(
            new FakeSriSettingsRepository(settings),
            new PassthroughSecretProtector(),
            new FakeSigner((xmlUtf8, p12Bytes, p12Password) =>
            {
                capturedXml = xmlUtf8;
