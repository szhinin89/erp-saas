...
public sealed class LinkInvoiceToPurchaseOrderCommandHandlerTests
{
    ...
    {
        // Pedimos 10 unidades, salesBill solo trae 5
    ...
    // ── salesBill no encontrada / no aprobada ──────────────────────────────
    ...
    {
        var ctx     = new TestContext();
        var salesBill = ctx.BuildFactura(aprobar: false); // estado Borrador
        ctx.CompraRepo.Setup(x => x.GetByIdAsync(
                ctx.SubscriberId, It.IsAny<Guid>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(salesBill);
    ...
    {
        // Pedimos 5, salesBill trae 10 → excede
    ...
    {
        // OC a $10, salesBill cobra $12 → diferencia 20% > 1% → advertencia
    ...
    {
        // OC a $10, salesBill $10.05 → diferencia 0.5% < 1% → sin advertencia
    ...
    private sealed class TestContext
    {
            ...
            decimal precioFactura = 10m)
        {
            ...
            var salesBill = BuildFactura(aprobar: true);

            _ordenId   = orden.Id;
            _facturaId = salesBill.Id;
            ...
            CompraRepo.Setup(x => x.GetByIdAsync(SubscriberId, salesBill.Id, It.IsAny<CancellationToken>()))
                .ReturnsAsync(salesBill);
