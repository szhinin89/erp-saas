...
public sealed class UnmatchPurchaseReceptionItemHandlerTests
{
    ...
    private static PurchaseReceptionDocument CreateDocumentWithLine(
        out PurchaseReceptionLine line, string supplierCode = "PROV-001",
    ...
    private static (Mock<IPurchaseReceptionDocumentRepository> DocRepo, Mock<IItemRepository> ItemRepo, UnmatchPurchaseReceptionItemHandler Handler)
        BuildHandler(PurchaseReceptionDocument document, PurchaseReceptionLine line, Item? item)
    {
        ...
        documentRepo.Setup(r => r.GetByLineIdAsync(TenantId, line.Id, It.IsAny<CancellationToken>()))
            .ReturnsAsync(document);

        var itemRepo = new Mock<IItemRepository>();
        if (item is not null)
            itemRepo.Setup(r => r.GetByIdAsync(item.Id, TenantId, It.IsAny<CancellationToken>())).ReturnsAsync(item);
        ...
        var handler = new UnmatchPurchaseReceptionItemHandler(
            documentRepo.Object, new ItemMatchConfirmationService(itemRepo.Object), tenant.Object, user.Object);
    ...
    public async Task Handle_reverts_a_manually_matched_line_to_pending_and_persists()
    {
        ...
        var result = await handler.Handle(new UnmatchPurchaseReceptionItemCommand(line.Id), CancellationToken.None);
    ...
    public async Task Handle_disables_the_supplier_code_that_was_auto_created_by_the_match()
    {
        var document = CreateDocumentWithLine(out var line);
        var item = CreateItem();
        var supplierCode = item.AddSupplierCode(line.SupplierCode!, isPrimary: false, SupplierId, UserId);
        line.ManualMatch(item.Id, UserId, DateTime.UtcNow);

        var (_, itemRepo, handler) = BuildHandler(document, line, item);

        await handler.Handle(new UnmatchPurchaseReceptionItemCommand(line.Id), CancellationToken.None);
    ...
    {
        var document = CreateDocumentWithLine(out var line, status: PurchaseReceptionDocumentStatus.Cancelled);
        var item = CreateItem();
        line.ManualMatch(item.Id, UserId, DateTime.UtcNow);
    ...
    {
        var documentRepo = new Mock<IPurchaseReceptionDocumentRepository>();
        documentRepo.Setup(r => r.GetByLineIdAsync(TenantId, It.IsAny<Guid>(), It.IsAny<CancellationToken>()))
        ...
        var result = await handler.Handle(new UnmatchPurchaseReceptionItemCommand(Guid.NewGuid()), CancellationToken.None);
