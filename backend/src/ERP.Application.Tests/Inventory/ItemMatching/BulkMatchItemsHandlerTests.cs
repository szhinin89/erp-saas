...
public sealed class BulkMatchItemsHandlerTests
{
    ...
    public async Task Handle_reports_a_missing_line_without_aborting_the_rest_of_the_batch()
    {
        ...
        documentRepo
            .Setup(r => r.GetByLineIdAsync(TenantId, lineOk.Id, It.IsAny<CancellationToken>()))
            .ReturnsAsync(documentOk);
        documentRepo
            .Setup(r => r.GetByLineIdAsync(TenantId, missingLineId, It.IsAny<CancellationToken>()))
            .ReturnsAsync((PurchaseReceptionDocument?)null);

        var itemRepo = new Mock<IItemRepository>();
        itemRepo
            .Setup(r => r.GetByIdLightAsync(itemOk.Id, TenantId, It.IsAny<CancellationToken>()))
            .ReturnsAsync(itemOk);
        itemRepo
            .Setup(r =>
        ...
        result
            ...
            .ContainSingle(r => r.PurchaseReceptionLineId == lineOk.Id && r.Success);
        result
            .Value.Results.Should()
            .ContainSingle(r => r.PurchaseReceptionLineId == missingLineId && !r.Success);
