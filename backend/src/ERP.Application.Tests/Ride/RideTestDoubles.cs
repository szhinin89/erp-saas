...
/// </summary>
internal sealed class FakeRideXmlParser(RideDocumentType documentType, RideModel? model = null) : IRideXmlParser
{
...
internal sealed class FakeRideTemplate(RideDocumentType documentType) : IRideTemplate
{
    ...
    public IRideDocumentLayout Compose(RideModel model, RideBranding branding) => new FakeRideDocumentLayout();
}
...
internal static class RideTestModelBuilder
{
    public static RideModel Build()
    {
        ...
        var issuer = RideParty.Create(
            identificationType: null, identificationNumber: "1790012345001", legalName: "ZH Technologies S.A.");
        var receiver = RideParty.Create(
            identificationType: "05", identificationNumber: "1712345678", legalName: "Juan Pérez");

        var line = RideLine.Create(
            code: "PROD-001", description: "Producto de prueba", quantity: 2m, unitPrice: 50m,
            discount: 0m, subtotal: 100m,
            taxes: [RideTaxSummary.Create("2", "2", 100m, 15m, rate: 15m)]);

        return RideModel.Create(
            header, issuer, receiver,
            lines: [line],
