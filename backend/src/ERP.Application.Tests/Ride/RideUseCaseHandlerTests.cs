...
public sealed class RideUseCaseHandlerTests
{
    ...
    private static readonly RideGenerationResultDto ExpectedResult =
        new(RideOutcome.Generated, "ride/path.pdf", null, null);
    ...
    public async Task GetOrGenerateRideQueryHandler_delegates_only_to_IRideDocumentService_GetOrGenerateAsync()
    {
        ...
        service.Setup(s => s.GetOrGenerateAsync(TenantId, CompanyId, SourceModule, SourceEntityId, It.IsAny<CancellationToken>()))
        ...
        var handler = new GetOrGenerateRideQueryHandler(service.Object, currentTenant, currentCompany);

        var result = await handler.Handle(new GetOrGenerateRideQuery(SourceModule, SourceEntityId), CancellationToken.None);

        result.Value.Should().Be(ExpectedResult);
        service.Verify(
            s => s.GetOrGenerateAsync(TenantId, CompanyId, SourceModule, SourceEntityId, It.IsAny<CancellationToken>()),
            Times.Once);
        service.Verify(s => s.RegenerateAsync(
    ...
    {
        var service = new Mock<IRideDocumentService>();
        service.Setup(s => s.RegenerateAsync(TenantId, CompanyId, SourceModule, SourceEntityId, It.IsAny<CancellationToken>()))
            .ReturnsAsync(Result<RideGenerationResultDto>.Success(ExpectedResult));
        var currentTenant = new FixedCurrentTenant(TenantId);
        var currentCompany = new FixedCurrentCompany(CompanyId);
        var handler = new RegenerateRideCommandHandler(service.Object, currentTenant, currentCompany);

        var result = await handler.Handle(new RegenerateRideCommand(SourceModule, SourceEntityId), CancellationToken.None);

        result.Value.Should().Be(ExpectedResult);
        service.Verify(
            s => s.RegenerateAsync(TenantId, CompanyId, SourceModule, SourceEntityId, It.IsAny<CancellationToken>()),
            Times.Once);
        service.Verify(s => s.GetOrGenerateAsync(
            It.IsAny<Guid>(), It.IsAny<Guid>(), It.IsAny<string>(), It.IsAny<Guid>(), It.IsAny<CancellationToken>()), Times.Never);
    }
