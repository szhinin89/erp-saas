...
public sealed class CreateSystemUserHandlerTests
{
    ...
    private sealed class Fixture
    {
        ...
        public Fixture()
        {
            ...
            AccessRepo.Setup(r => r.AnyUserWithEmailAsync(It.IsAny<string>(), It.IsAny<CancellationToken>())).ReturnsAsync(false);
        }

        public CreateSystemUserHandler BuildHandler() => new(
            AccessRepo.Object, Hasher.Object, CurrentUser.Object, Mediator.Object, UnitOfWork.Object);
    }
    ...
    {
        var f = new Fixture();
        f.AccessRepo.Setup(r => r.AnyUserWithUsernameAsync(Username, It.IsAny<CancellationToken>())).ReturnsAsync(true);
        ...
        f.AccessRepo.Verify(r => r.AddUserAsync(It.IsAny<IdentityUser>(), It.IsAny<CancellationToken>()), Times.Never);
    ...
    public async Task Username_nuevo_crea_el_IdentityUser_con_password_hasheado_y_RequirePasswordReset_activo()
    {
        ...
        var f = new Fixture();
        f.AccessRepo.Setup(r => r.AnyUserWithUsernameAsync(Username, It.IsAny<CancellationToken>())).ReturnsAsync(false);
        ...
        f.AccessRepo.Setup(r => r.AddUserAsync(It.IsAny<IdentityUser>(), It.IsAny<CancellationToken>()))
        ...
        addedUser.RequirePasswordReset.Should().BeTrue("el usuario nuevo debe cambiar su contraseña en el primer login");
        f.Hasher.Verify(h => h.HashPassword("S3curePass!"), Times.Once);
        f.UnitOfWork.Verify(u => u.BeginTransactionAsync(It.IsAny<CancellationToken>()), Times.Once);
    ...
    public async Task Username_nuevo_delega_la_membership_en_UpsertCompanyUserMembershipCommand_sin_reimplementarla()
    {
        ...
        var handler = f.BuildHandler();
        var command = new CreateSystemUserCommand(tenantId, Username, "Ana", "Perez", Email, "S3curePass!", "Admin", profileId);
    ...
    public async Task Falla_de_membership_hace_Rollback_de_la_transaccion_y_nunca_Commit()
    {
        ...
        f.AccessRepo.Setup(r => r.AnyUserWithUsernameAsync(Username, It.IsAny<CancellationToken>())).ReturnsAsync(false);
        f.Mediator.Setup(m => m.Send(It.IsAny<UpsertCompanyUserMembershipCommand>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(Result<object>.Failure("El perfil no existe.", ApiResponseCodes.Common.ValidationError));
        ...
        f.AccessRepo.Verify(r => r.AddUserAsync(It.IsAny<IdentityUser>(), It.IsAny<CancellationToken>()), Times.Once);
        f.UnitOfWork.Verify(u => u.BeginTransactionAsync(It.IsAny<CancellationToken>()), Times.Once);
        f.UnitOfWork.Verify(u => u.RollbackAsync(It.IsAny<CancellationToken>()), Times.Once, "el IdentityUser ya se guardó en memoria pero la transacción externa nunca debe comitearse si falla la membership");
