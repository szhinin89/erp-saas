...
test.describe('Enterprise auth & limits', () => {
  ...
  test('happy path: login UI → select-company → dashboard', async ({ page }) => {
    ...
    if (page.url().includes('select-company')) {
      await expect(page.getByRole('heading', { name: /Empresa operativa/i })).toBeVisible();
  ...
  test('create company blocked at MAX_COMPANIES (403)', async ({ request }) => {
    ...
    if (!session.companyId && companies[0]) {
      const switched = await switchCompany(request, token, companies[0].companyId);
      token = switched.token;
    }

    const res = await request.post(`${API_BASE}/api/companies`, {
    ...
    if (raw.length > 0) {
      ...
      expect(message.toLowerCase()).toMatch(/límite|limit|empresa|companies|commercial/i);
    }
  });

  test('forbidden: switch a empresa inexistente o sin acceso', async ({ request }) => {
    const session = await login(request);

    const foreignSwitch = await request.post(`${API_BASE}/api/auth/switch-company`, {
    ...
    const foreignCompanyGet = await request.get(
      `${API_BASE}/api/companies/00000000-0000-0000-0000-000000000099`,
      { headers: { Authorization: `Bearer ${session.token}` } },
    );
