...
import { ZHCardSection, ZHGridRow, ZHInlineRowRight, ZHStack } from '../components/zh/ZHLayout';
import { formatApiRequestError } from '../modules/lib/apiError';
import './SuperAdminPlansPage.css';

const POLL_MS = 20_000;

function isModuleOrFormKind(kind: number): boolean {
...
function kindLabelKey(kind: number): string {
  switch (kind) {
    case 1:
      return 'superadmin.plansAdmin.kind.form';
    case 2:
      return 'superadmin.plansAdmin.kind.quota';
    case 3:
      return 'superadmin.plansAdmin.kind.integration';
    default:
      return 'superadmin.plansAdmin.kind.module';
  }
}
...
export function SuperAdminPlansPage() {
  ...
  const [newFeature, setNewFeature] = useState<CreateSaasFeatureDefinitionBody>({
    code: '',
    name: '',
    description: null,
    isMetered: false,
  ...
  /** Planes donde incluir la nueva feature (obligatorio si kind = módulo o formulario). */
  const [newFeaturePlanIds, setNewFeaturePlanIds] = useState<Record<string, boolean>>({});
  ...
  const saveNewFeature = async () => {
    ...
    setError('');
    try {
      const kindNum = Number(newFeature.kind);
      if (isModuleOrFormKind(kindNum)) {
        if (plans.length === 0) {
          setError(t('superadmin.plansAdmin.assignPlansNoPlans'));
          return;
        }
        const selectedPlanIds = plans.filter((p) => newFeaturePlanIds[p.id]).map((p) => p.id);
        if (selectedPlanIds.length === 0) {
          setError(t('superadmin.plansAdmin.assignPlansRequired'));
          return;
        }
      }

      const newId = await superAdminService.createSaasFeatureDefinition({
        ...newFeature,
        code: newFeature.code.trim(),
        name: newFeature.name.trim(),
        description: newFeature.description?.trim() ? newFeature.description.trim() : null,
        resourceRef: newFeature.resourceRef?.trim() ? newFeature.resourceRef.trim() : null,
      });

      if (isModuleOrFormKind(kindNum) && plans.length > 0) {
        const selectedPlanIds = plans.filter((p) => newFeaturePlanIds[p.id]).map((p) => p.id);
        for (const planId of selectedPlanIds) {
          const plan = plans.find((p) => p.id === planId);
          if (!plan) continue;
          const seen = new Set<string>();
          const rows: PlanFeatureAssignBody[] = [];
          ...
          if (!seen.has(newId)) {
            rows.push({ featureId: newId, isIncluded: true, limitPerPeriod: null });
          }
          await superAdminService.replaceSaasPlanFeatures(planId, rows);
        }
      }

      setFeatureCreateOpen(false);
      setNewFeaturePlanIds({});
      setNewFeature({
        code: '',
        name: '',
        description: null,
  ...
  if (!isSuperAdmin) {
    return (
      <PageShell kicker={t('app.nav.group.home')} title={t('superadmin.plansAdmin.title')} subtitle={t('common.noAccess')}>
        <TableCard>
  ...
  return (
    <PageShell
      ...
      title={t('superadmin.plansDashboard.title')}
      subtitle={t('superadmin.plansDashboard.pageSubtitle')}
    >
      <div className="sap-dash-head">
        <div className="sap-dash-headRow">
          ...
          <div className="sap-dash-actions">
            <NavLink className="zh-btn zh-btn--ghost zh-btn--md" to="/superadmin">
                  ...
                  {filteredTenantsForTable.map((tn) => {
                    ...
                    return (
                      <tr key={tn.id}>
                        ...
                        <td>
                          <NavLink className="zh-btn zh-btn--ghost zh-btn--sm" to={`/companies?subscription=${encodeURIComponent(tn.id)}`}>
                            {t('common.edit')}
                          </NavLink>
                        </td>
                      </tr>
      ...
      <TableCard>
        <ZHCardSection title={t('superadmin.plansAdmin.featureDefsTitle')}>
          <p className="subtle">{t('superadmin.plansAdmin.featureDefsHint')}</p>
          <div className="sap-defList">
            {features.map((f) => (
              <div key={f.id} className="sap-defRow">
                <span className="mono sap-defCode">{f.code}</span>
                <span className="sap-defName">{f.name}</span>
                <span className="sap-planPill">{t(kindLabelKey(f.kind))}</span>
                {f.resourceRef ? <span className="mono subtle">{f.resourceRef}</span> : null}
              </div>
            ))}
          </div>
          <ZHBtn
            ...
            type="button"
            onClick={() => {
              setFeatureCreateOpen((v) => {
                const next = !v;
                if (next) setNewFeaturePlanIds({});
                return next;
              });
            }}
          >
            {featureCreateOpen ? t('superadmin.plansAdmin.hideFeatureForm') : t('superadmin.plansAdmin.newFeature')}
          </ZHBtn>
          {featureCreateOpen ? (
            <ZHStack className="sap-newFeature">
              <ZHGridRow cols={2}>
                <ZHField label={t('superadmin.plansAdmin.field.code')}>
                  <input className="zh-input" value={newFeature.code} onChange={(e) => setNewFeature((s) => ({ ...s, code: e.target.value }))} />
                </ZHField>
                <ZHField label={t('superadmin.plansAdmin.field.name')}>
                  <input className="zh-input" value={newFeature.name} onChange={(e) => setNewFeature((s) => ({ ...s, name: e.target.value }))} />
                </ZHField>
              </ZHGridRow>
              <ZHField label={t('superadmin.plansAdmin.field.description')}>
                <input
                  className="zh-input"
                  value={newFeature.description ?? ''}
                  onChange={(e) => setNewFeature((s) => ({ ...s, description: e.target.value || null }))}
                />
              </ZHField>
              <ZHGridRow cols={2}>
                <ZHField label={t('superadmin.plansAdmin.field.kind')}>
                  <select
                    className="zh-input"
                    value={newFeature.kind}
                    onChange={(e) => setNewFeature((s) => ({ ...s, kind: Number(e.target.value) as SaasFeatureKind }))}
                  >
                ...
                <ZHField label={t('superadmin.plansAdmin.field.resourceRef')}>
                  <input
                    className="zh-input"
                    value={newFeature.resourceRef ?? ''}
                    onChange={(e) => setNewFeature((s) => ({ ...s, resourceRef: e.target.value || null }))}
              ...
              <label className="sap-check">
                <input
                  type="checkbox"
                  checked={newFeature.isMetered}
                  onChange={(e) => setNewFeature((s) => ({ ...s, isMetered: e.target.checked }))}
                />
                {t('superadmin.plansAdmin.field.metered')}
              </label>
              {isModuleOrFormKind(Number(newFeature.kind)) ? (
                <div className="sap-planPick">
                  <p className="subtle sap-planPick-title">{t('superadmin.plansAdmin.assignPlansTitle')}</p>
                  <p className="subtle sap-planPick-hint">
                    {Number(newFeature.kind) === 1
                      ? t('superadmin.plansAdmin.assignPlansHintForm')
                      : t('superadmin.plansAdmin.assignPlansHintModule')}
                  </p>
                  {plans.length === 0 ? (
                    <p className="sap-planPick-warn subtle">{t('superadmin.plansAdmin.assignPlansNoPlans')}</p>
                  ) : (
                    plans.map((p) => (
                      <label key={p.id} className="sap-check sap-check--plan">
                        <input
                          type="checkbox"
                          checked={!!newFeaturePlanIds[p.id]}
                          onChange={() =>
                            setNewFeaturePlanIds((m) => ({
                              ...m,
                              [p.id]: !m[p.id],
                            }))
                          }
                        />
                        <span>
                          <span className="mono">[{String(p.shortLabel ?? p.code).toUpperCase()}]</span> {p.name}
                        </span>
                      </label>
                    ))
                  )}
              ...
              <ZHBtn variant="primary" size="md" type="button" onClick={() => void saveNewFeature()} disabled={busy}>
