...
export function isPlanBuilderSessionMenu(rows: NavMenuGroupDto[] | undefined): boolean {
...
export function mapSessionMenuToNavGroups(dto: NavMenuGroupDto[], t: TranslateFn): NavGroup[] {
...
export function flattenAccessIntoSecurity(groups: NavGroup[]): NavGroup[] {
  ...
  const liftedDeduped = access.items.filter((it) => !existingInSecurity.has(it.to));
  const newSecurity: NavGroup = { ...security, items: [...liftedDeduped, ...security.items] };
  return rest.map((g) => (g.id === 'security' ? newSecurity : g));
}

/** Ensures ERP dashboard is the first item in the home group. */
export function ensureTenantHomeOverview(groups: NavGroup[], t: TranslateFn): NavGroup[] {
  // Id sintético fijo (no proviene del backend): prefijo `synthetic-` evita colisión con ids reales.
  const erpItem: NavItem = { id: 'synthetic-home-erp-dashboard', to: '/dashboard', label: t('app.nav.erpDashboard') };

  const homeIdx = groups.findIndex((g) => g.id === 'home');
  if (homeIdx < 0) {
    return sortNavGroupsForMainBar([
      { id: 'home', label: t('app.nav.group.home'), icon: '⊞', sortOrder: defaultBarRank('home') * 10, items: [erpItem] },
      ...groups,
    ]);
  }
