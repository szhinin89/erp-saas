...
import { ZHToggle, ZHField, ZHBtn, ZHFormActions } from '../../../components/zh/ZHForm';
...
export function SecuritySettingsPage() {
  ...
  const openPreferences = async (target: SecurityUser) => {
    ...
    try {
      ...
      const existing = await companyUserPreferencesService.get(target.companyUserMembershipId);
      resetPrefs({
        loginMode: existing?.loginMode ?? 'AskBranch',
        defaultBranchId: existing?.defaultBranchId ?? null,
      });
    } catch (e) {
      setPrefsSaveError(formatApiRequestError(e, { generic: t('security.preferences.error.load', 'No se pudieron cargar las preferencias.') }));
    } finally {
  ...
  const onSubmitPreferences = handlePrefsSubmit(async (values) => {
    ...
    setPrefsSaveError('');
    try {
      await companyUserPreferencesService.update(prefsTarget.companyUserMembershipId, {
        loginMode: values.loginMode,
        defaultBranchId: values.defaultBranchId || null,
      });
      message.success(t('security.preferences.success', 'Preferencias actualizadas correctamente.'));
      closePreferences();
    } catch (e) {
      const applied = applyServerErrors(e, setPrefsFieldError, (msg) => setPrefsSaveError(msg));
      if (!applied) {
        setPrefsSaveError(formatApiRequestError(e, { generic: t('security.preferences.error.save', 'No se pudieron guardar las preferencias.') }));
      }
    } finally {
  ...
  return (
    <PageShell kicker={t('app.nav.group.security')} title={t('security.title')} subtitle={t('security.subtitle')}>
            ...
            <table className="security-table">
              <thead>
                <tr>
                  ...
                  {canManagePreferences ? <th>{t('security.preferences.column', 'Preferencias')}</th> : null}
                </tr>
              </thead>
              <tbody>
                {rows.map((u) => {
                    ...
                    <tr key={u.id} className={!u.isActive ? 'row--inactive' : ''}>
                      <td className="sticky-col">
                        <div className="userCell">
                          ...
                          <div className="userMeta">
                            <span className="mono security-mono">{u.username}</span>
                            {u.email ? <span className="mono security-mono">{u.email}</span> : null}
                      ...
                      {canManagePreferences ? (
                        <td className="cell-center">
                          <ZHBtn variant="ghost" size="md" type="button" onClick={() => void openPreferences(u)}>
      ...
      <ZHModal
        ...
        size="md"
        title={t('security.preferences.title', 'Preferencias de inicio de sesión')}
        ...
        {prefsLoading ? (
          ...
          <form onSubmit={onSubmitPreferences}>
            ...
            <ZHField
              ...
              hint={prefsLoginMode === 'AskBranch' ? t('security.preferences.defaultBranch.optional', 'Opcional en este modo.') : null}
            >
              <select disabled={prefsSaving || branchesLoading} {...registerPrefs('defaultBranchId')}>
            ...
            {prefsSaveError ? (
              <ZHPageNotice variant="error" message={t('common.errorPrefix')} detail={prefsSaveError} />
            ) : null}
