...
export function ElectronicDocumentsDashboardCards({
  ...
  const cardCls = (active: boolean) =>
    `edm-kpi-card${active ? " edm-kpi-card--active" : ""}`;

  return (
    <>
      <ZHGrid cols={3}>
        <ZHCard
          title={t("electronicDocuments.monitor.dashboard.pending")}
          className={cardCls(activeState === PENDING_STATES)}
        >
          <button
            type="button"
            className="edm-kpi-btn"
        ...
        <ZHCard
          title={t("electronicDocuments.monitor.dashboard.inProgress")}
          className={cardCls(activeState === IN_PROGRESS_STATES)}
        >
          <button
            type="button"
            className="edm-kpi-btn"
          ...
          >
            <div className="edm-metric">
              {value(dashboard ? dashboard.signed + dashboard.sent : undefined)}
            </div>
          </button>
        </ZHCard>
        <ZHCard
          title={t("electronicDocuments.monitor.dashboard.authorized")}
          className={cardCls(activeState === "Authorized")}
        >
          <button
            type="button"
            className="edm-kpi-btn"
          ...
          >
            <div className="edm-metric edm-metric--success">
              {value(dashboard?.authorized)}
            </div>
          </button>
        </ZHCard>
      </ZHGrid>
      ...
      <ZHGrid cols={3}>
        <ZHCard
          title={t("electronicDocuments.monitor.dashboard.rejected")}
          className={cardCls(activeState === "Rejected")}
        >
          <button
            type="button"
            className="edm-kpi-btn"
          ...
          >
            <div className="edm-metric edm-metric--danger">
              {value(dashboard?.rejected)}
            </div>
          </button>
        </ZHCard>
        <ZHCard
          title={t("electronicDocuments.monitor.dashboard.error")}
          className={cardCls(activeState === ERROR_STATES)}
        >
          <button
            type="button"
            className="edm-kpi-btn"
        ...
        <ZHCard
          title={t("electronicDocuments.monitor.dashboard.totalToday")}
          className={cardCls(isTodayActive)}
        >
      ...
      <ZHGrid cols={3}>
        <ZHCard
          title={t("electronicDocuments.monitor.dashboard.authorizedToday")}
        >
          <div className="edm-metric edm-metric--success">
            {value(dashboard?.authorizedToday)}
          </div>
        </ZHCard>
        <ZHCard title={t("electronicDocuments.monitor.dashboard.errorsToday")}>
          <div className="edm-metric edm-metric--danger">
            {value(dashboard?.errorsToday)}
          </div>
        </ZHCard>
        <ZHCard
          title={t("electronicDocuments.monitor.dashboard.pendingRetries")}
        >
