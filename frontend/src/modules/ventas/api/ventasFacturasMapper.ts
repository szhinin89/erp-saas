import type { VentasFacturaDetailDto, VentasFacturaDto, VentasFacturaLineDto } from './ventasFacturasService';
...
export function mapSalesBillToVentasFactura(raw: ApiSalesBillDto): VentasFacturaDto {
  return {
    id: raw.id,
    clienteId: raw.businessPartnerId,
    clienteNombre: raw.customerName,
    businessPartnerId: raw.businessPartnerId,
    bodegaId: raw.warehouseId,
    ...
    fechaEmision: raw.issueDate,
    subtotal: raw.subtotal,
    impuesto: raw.vatTotal,
    total: raw.total,
    estado: raw.status,
...
function mapLine(raw: ApiSalesDetailDto): VentasFacturaLineDto {
  return {
    id: raw.id,
...
export function mapSalesBillDetailToVentasFactura(raw: ApiSalesBillDetailDto): VentasFacturaDetailDto {
...
export function invoiceNumber(row: Pick<VentasFacturaDto, 'establecimiento' | 'puntoEmision' | 'secuencial'>): string {
