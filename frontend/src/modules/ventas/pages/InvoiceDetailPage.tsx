...
export function InvoiceDetailPage() {
  ...
  const { data, loading, error, refetch } = useVentasFacturaDetail(id ?? null);
  const actions = useVentasFacturaActions(refetch);
  ...
  const handlePrint = async () => {
    if (!data) return;
    await openVentasFacturaPrint(data.id);
  };

  const handleRide = async () => {
    if (!data) return;
    const blob = await ventasFacturasService.downloadRide(data.id);
            ...
            <div className="pg-section-body">
              <div className="pg-form-grid pg-form-grid--4">
                <InfoItem label={t('ventas.facturas.col.status')}>
                  <span className={statusBadgeClass(data.estado)}>{data.estado}</span>
                </InfoItem>
