...
const mkLine = (qty: number, price: number, vatCode = '10', discountPct = 0, ctx?: any) => ({
  description: 'Test', quantity: qty, unitPrice: price, vatCode, discountPct, ...ctx && { context: ctx },
});
...
describe('flete', () => {
  it('flete se suma al total', () => {
    const s = calcSummary([mkLine(1, 100, '0', 0, { vatPercent: 0, icePercent: 0 })], 50, 0);
...
describe('otros costos', () => {
  it('otros costos se suman al total', () => {
    const s = calcSummary([mkLine(1, 100, '0', 0, { vatPercent: 0, icePercent: 0 })], 0, 30);
...
describe('múltiples líneas mixtas', () => {
  ...
  it('descuento total', () => {
    const s = calcSummary(lines, 0, 0);
    expect(s.discount).toBe(1000 * 0.05 + 500 * 0.10);
  });
