...
export function BranchesManagementSection() {
  ...
  return (
    <ConfigTabsLayout
      ...
      error={
        ctx.error
          ? <ZHPageNotice variant="error" message={ctx.t('common.errorPrefix')} detail={ctx.error} />
          : undefined
      }
      listContent={
        <BranchesListSection
      ...
      editorContent={
        ctx.panelOpen ? (
          ...
          <div className="cfg-tabs-empty">
            <span className="material-symbols-outlined cfg-empty-panel__icon">business</span>
            <p className="cfg-empty-panel__title">Seleccione o cree una sucursal</p>
            <p className="cfg-empty-panel__sub">
              Use el botón <strong>Nueva Sucursal</strong> en la pestaña Lista, o seleccione una para editar.
            </p>
          </div>
        )
      }
