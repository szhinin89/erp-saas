...
export function BranchConfigSection({ branchId }: Props) {
  ...
  useEffect(() => {
    if (!settingsState.data) { reset({ defaultWarehouseId: null }); return; }
    reset({ defaultWarehouseId: settingsState.data.defaultWarehouseId ?? null });
  ...
  const warehouseLabel  = warehouses.find(w => w.id === settingsState.data?.defaultWarehouseId);
          ...
          <div className="pg-section-header-left">
            ...
            <div>
              ...
              <p className="pg-section-desc">
                Parámetros operativos de esta sucursal. La bodega por defecto se aplica
                al crear facturas de venta originadas desde esta sucursal.
              </p>
            </div>
          </div>
          {canEdit && (
      ...
      <ZHModal
        ...
        footer={
          <div className="zh-modal-footer-actions">
            ...
            <ZHBtn variant="primary" size="md" type="submit" form="branch-config-form" disabled={saving || !canEdit}>
        ...
        {warehousesState.loading ? (
          ...
          <form id="branch-config-form" onSubmit={onSubmit}>
            <ZHField
              ...
              hint={warehouses.length === 0 ? 'No hay bodegas activas para esta sucursal.' : undefined}
              error={errors.defaultWarehouseId?.message}
            >
              <select disabled={saving || !canEdit} {...register('defaultWarehouseId')}>
