...
export type RideOutcome = 'Generated' | 'Cached' | 'PendingSource' | 'NotApplicable' | 'Failed';
...
export const rideService = {
  ...
  getOrGenerate(sourceModule: string, sourceEntityId: string): Promise<RideGenerationResultDto> {
    return apiGet<RideGenerationResultDto>(BASE, { params: { sourceModule, sourceEntityId } });
  },

  /** Fuerza la regeneración del RIDE aunque el cache siga siendo válido. */
  regenerate(sourceModule: string, sourceEntityId: string): Promise<RideGenerationResultDto> {
    return apiPost<RideGenerationResultDto>(`${BASE}/regenerate`, { sourceModule, sourceEntityId });
  },
   ...
   */
  async getContentBlob(sourceModule: string, sourceEntityId: string): Promise<Blob> {
