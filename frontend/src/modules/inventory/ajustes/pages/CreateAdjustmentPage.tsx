...
import { useAdjustmentActions } from '../hooks/useAdjustments';
import { useBodegas } from '../../transferencias/hooks/useTransfers';
import { adjustmentService, MOTIVOS_PREDEFINIDOS } from '../api/adjustmentService';
...
export function CrearAjustePage() {
  ...
  const [warehouseId,   setBodegaId]   = useState('');
  ...
  const { loading, error, crear } = useAdjustmentActions(() => navigate('/inventario/ajustes'));

  useEffect(() => {
    if (!warehouseId || !productId) { setStock(null); return; }
    adjustmentService.getStockDisponible(warehouseId, productId).then(setStock).catch(() => setStock(null));
  ...
  const validar = () => {
    if (!warehouseId)   return 'Selecciona la bodega.';
    if (!productId) return 'Selecciona el producto.';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLocalError(null);
    await crear({ warehouseId: warehouseId, productId: productId,
          ...
          <div className="pg-section-body">
            <div className="pg-form-grid pg-form-grid--2">
              <ZHField label="Bodega" required>
                <select className="zh-input" value={warehouseId}
              ...
              <ZHField label="Producto" required>
                <select className="zh-input" value={productId} disabled={loading}
