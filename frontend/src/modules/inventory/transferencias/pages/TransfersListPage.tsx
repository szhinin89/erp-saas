...
import { useBodegas, useTransfersList } from '../hooks/useTransfers';
import type { TransferStatus, TransfersFilter } from '../api/transferService';
...
export function TransferenciasListPage() {
  ...
  const [filter, setFilter] = useState<TransfersFilter>({ pageNumber: 1, pageSize: 20 });
  const { result, loading, error } = useTransfersList(filter);
        ...
        <div className="pg-table-controls">
          <div className="pg-table-controls-left">
            <select
              ...
              value={filter.status ?? ''}
              onChange={(e) =>
                setFilter((f) => ({ ...f, estado: (e.target.value as TransferStatus) || undefined, pageNumber: 1 }))
              }
            >
            ...
            <select
              className="zh-input"
              value={filter.sourceWarehouseId ?? ''}
              onChange={(e) =>
                setFilter((f) => ({ ...f, sourceWarehouseId: e.target.value || undefined, pageNumber: 1 }))
              }
            >
            ...
            <select
              className="zh-input"
              value={filter.targetWarehouseId ?? ''}
              onChange={(e) =>
                setFilter((f) => ({ ...f, targetWarehouseId: e.target.value || undefined, pageNumber: 1 }))
              }
            >
              <option value="">Todas las bodegas destino</option>
              {(bodegas ?? []).map((b) => <option key={b.id} value={b.id}>{b.name}</option>)}
            </select>
            <ZhDateInput value={filter.dateFrom ?? ''}
              onChange={(e) => setFilter((f) => ({ ...f, dateFrom: e.target.value || undefined, pageNumber: 1 }))} />
            <ZhDateInput value={filter.dateTo ?? ''}
