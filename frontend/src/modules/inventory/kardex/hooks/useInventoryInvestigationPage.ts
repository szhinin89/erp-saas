...
export function useInventoryInvestigationPage(
  ...
  const [searchMode, setSearchMode] = useState<SearchMode>(
    initialDocument ? "document" : "product",
  );
  ...
  // ── Modo Documento ───────────────────────────────────────────────────────
  const [docSubType, setDocSubType] =
    useState<DocumentSubType>("PurchaseInvoice");
  ...
  const [movementTypeFilter, setMovementTypeFilter] = useState<string>(
    initialFilters?.movementTypeFilter ?? "",
  );
  ...
  useEffect(() => {
    ...
    itemService
      ...
      .catch(() => {
        /* el Id sigue siendo válido para buscar aunque no se muestre el nombre */
      });
  }, [initialProductId]);
    ...
    resolve
      ...
      .catch(() =>
        setSelectedDoc({
          id: initialDocument.id,
          docType: initialDocument.docType,
          number: initialDocument.id,
          label: initialDocument.id,
        }),
      );
    ...
    try {
      if (docSubType === "PurchaseInvoice") {
        ...
        setDocMatches(
          res.items.map((i) => ({
            id: i.id,
            docType: "PurchaseInvoice" as const,
            number: i.invoiceNumber,
            label: `${i.invoiceNumber} — ${i.status}`,
          })),
        );
      } else {
  ...
  const runSearch = useCallback(async () => {
    ...
    try {
      if (searchMode === "product") {
        ...
        if (!selectedDoc) {
          message.error("Seleccione un documento.");
          return;
        }
        const list = await kardexService.getByDocument(
          selectedDoc.id,
          selectedDoc.docType,
        );
        setMovements(list);
  ...
  const lastMovement = useMemo(() => {
    ...
    return movements.reduce(
      (latest, m) => (m.createdAt > latest.createdAt ? m : latest),
      movements[0],
    );
  }, [movements]);
  ...
  const visibleMovements = useMemo(
    () =>
      movementTypeFilter
        ? movements.filter((m) => m.movementTypeName === movementTypeFilter)
        : movements,
