...
function movementBadgeVariant(typeName: string): string {
  if (typeName.includes('Entry') || typeName === 'PositiveAdjust' || typeName === 'SaleReturn') return 'pf-badge--success';
  if (typeName.includes('Exit') || typeName === 'NegativeAdjust' || typeName === 'PurchaseReturn') return 'pf-badge--danger';
...
export function KardexPage() {
  ...
  const initialDocument: InitialDocument | undefined =
    docId && (docTypeParam === 'PurchaseInvoice' || docTypeParam === 'SalesInvoice')
  ...
  const ctx = useInventoryInvestigationPage(initialProductId, initialDocument, initialFiltersRef.current);

  useEffect(() => { void ctx.loadWarehouses(); }, []); // eslint-disable-line react-hooks/exhaustive-deps
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (ctx.selectedWarehouseId) next.set('warehouseId', ctx.selectedWarehouseId);
    if (ctx.dateFrom) next.set('dateFrom', ctx.dateFrom);
    if (ctx.dateTo) next.set('dateTo', ctx.dateTo);
    if (ctx.movementTypeFilter) next.set('movementType', ctx.movementTypeFilter);
    setSearchParams(next, { replace: true });
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [ctx.searchMode, ctx.selectedProductId, ctx.selectedWarehouseId, ctx.dateFrom, ctx.dateTo, ctx.movementTypeFilter]);

  if (!canView) return <NoAccessPage title="Centro de Investigación de Inventario" />;
  ...
  return (
    ...
    >
      <ZHTabBar
        ...
        activeTab={ctx.tab}
        onChange={() => { /* única pestaña activa hoy */ }}
      ...
      <div className="prd-section">
        ...
        <div className="pf-mini-card" style={{ marginBottom: 16 }}>
          ...
          {ctx.searchMode === 'product' ? (
            <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', alignItems: 'flex-end' }}>
              <ZHField density="compact" label="Producto" style={{ minWidth: 280, position: 'relative' }}>
                <input
                  type="text"
                  placeholder="Buscar por SKU o nombre..."
                  value={ctx.selectedProduct ? `${ctx.selectedProduct.sku} — ${ctx.selectedProduct.shortName}` : ctx.productQuery}
                  onChange={e => { ctx.clearSelectedProduct(); void ctx.searchProducts(e.target.value); }}
                />
                {ctx.productResults.length > 0 && (
                  <div className="pf-picker-dropdown">
                    {ctx.productResults.map(item => (
                      <button key={item.id} type="button" className="pf-picker-item" onClick={() => ctx.selectProduct(item)}>
              ...
              <ZHField density="compact" label="Bodega (opcional)" style={{ width: 220 }}>
                <select value={ctx.selectedWarehouseId} onChange={e => ctx.setSelectedWarehouseId(e.target.value)}>
              ...
              <ZHField density="compact" label="Desde" style={{ width: 160 }}>
                <input type="date" value={ctx.dateFrom} onChange={e => ctx.setDateFrom(e.target.value)} />
              </ZHField>
              <ZHField density="compact" label="Hasta" style={{ width: 160 }}>
                <input type="date" value={ctx.dateTo} onChange={e => ctx.setDateTo(e.target.value)} />
              </ZHField>
              <button className="pf-btn pf-btn--primary" onClick={() => void ctx.runSearch()} disabled={!ctx.selectedProductId}>
                <span className="material-symbols-outlined pf-btn__icon">search</span> Buscar
              </button>
            </div>
          ) : (
            <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', alignItems: 'flex-end' }}>
              <ZHField density="compact" label="Tipo de documento" style={{ width: 200 }}>
                <select value={ctx.docSubType} onChange={e => ctx.setDocSubType(e.target.value as typeof ctx.docSubType)}>
                  <option value="PurchaseInvoice">Factura de Compra</option>
                  <option value="SalesInvoice">Factura de Venta</option>
                </select>
              </ZHField>
              <ZHField density="compact" label="Número de documento" style={{ minWidth: 260, position: 'relative' }}>
                <input
                  type="text"
                  placeholder="Ej. 001-001-000000123"
                  value={ctx.selectedDoc ? ctx.selectedDoc.label : ctx.docNumberQuery}
                  onChange={e => { ctx.clearSelectedDocument(); ctx.setDocNumberQuery(e.target.value); }}
                  onKeyDown={e => { if (e.key === 'Enter') void ctx.searchDocuments(); }}
                />
                {ctx.docMatches.length > 0 && (
                  <div className="pf-picker-dropdown">
                    {ctx.docMatches.map(d => (
                      <button key={d.id} type="button" className="pf-picker-item" onClick={() => ctx.selectDocument(d)}>
                        <div className="pf-picker-item__main">
              ...
              <button className="pf-btn" onClick={() => void ctx.searchDocuments()} disabled={ctx.docSearching || !ctx.docNumberQuery.trim()}>
                <span className="material-symbols-outlined pf-btn__icon">manage_search</span>
                {ctx.docSearching ? 'Buscando...' : 'Resolver documento'}
              </button>
              <button className="pf-btn pf-btn--primary" onClick={() => void ctx.runSearch()} disabled={!ctx.selectedDoc}>
                <span className="material-symbols-outlined pf-btn__icon">search</span> Ver movimientos
              </button>
            </div>
          )}
        </div>
        ...
        {ctx.searchMode === 'product' && ctx.summary && (
          <div className="pf-header-cards-row" style={{ marginBottom: 16 }}>
            <SummaryCard icon="inventory_2" label="Stock Actual" value={`${formatMoney(ctx.summary.quantity, qty)} und.`} />
            <SummaryCard icon="payments" label="Costo Promedio" value={formatMoneyWithSymbol(ctx.summary.averageCost, cost)} />
            <SummaryCard icon="account_balance_wallet" label="Valor de Inventario" value={formatMoneyWithSymbol(ctx.summary.totalStockValue, total)} />
            <SummaryCard
              icon="update"
              label="Último Movimiento"
              value={ctx.lastMovement ? `${MOVEMENT_TYPE_LABELS[ctx.lastMovement.movementTypeName] ?? ctx.lastMovement.movementTypeName} — ${formatDate(ctx.lastMovement.effectiveDate)}` : '—'}
            />
            <SummaryCard icon="format_list_numbered" label="Cantidad de Movimientos" value={`${ctx.movements.length}`} />
          </div>
        )}
        ...
        {ctx.movements.length > 0 && (
          <div style={{ display: 'flex', gap: 12, marginBottom: 12, alignItems: 'center', justifyContent: 'space-between' }}>
            <select value={ctx.movementTypeFilter} onChange={e => ctx.setMovementTypeFilter(e.target.value)}>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              <button className="pf-btn" disabled title="Próximamente">
                <span className="material-symbols-outlined pf-btn__icon">grid_on</span> Excel
              </button>
              <button className="pf-btn" disabled title="Próximamente">
                <span className="material-symbols-outlined pf-btn__icon">picture_as_pdf</span> PDF
              </button>
              <button className="pf-btn" disabled title="Próximamente">
                <span className="material-symbols-outlined pf-btn__icon">print</span> Imprimir
              </button>
            </div>
          </div>
        )}
        ...
        {/* ── Tabla principal ──────────────────────────────────────────── */}
        {ctx.movementsLoading ? <p>Cargando movimientos...</p> : (
          <table className="pf-table">
            <thead>
              <tr>
                <th>Seq.</th><th>Fecha Efectiva</th><th>Tipo</th><th>Documento</th>
            ...
            <tbody>
              {ctx.movements.map(m => (
                <tr key={m.id}>
                  <td style={{ fontFamily: 'monospace', fontWeight: 600 }}>#{m.sequenceNumber}</td>
                  <td>{formatDate(m.effectiveDate)}</td>
                  <td><span className={`pf-badge ${movementBadgeVariant(m.movementTypeName)}`}>{MOVEMENT_TYPE_LABELS[m.movementTypeName] ?? m.movementTypeName}</span></td>
                  <td style={{ fontSize: 12, color: 'var(--color-text-secondary)' }}>{m.reference ?? '—'}</td>
                  <td className="pf-td--num" style={{ color: 'var(--color-success)' }}>
                    {m.quantity > 0 ? formatMoney(m.quantity, qty) : '—'}
                  </td>
                  <td className="pf-td--num" style={{ color: 'var(--color-error)' }}>
                    {m.quantity < 0 ? formatMoney(-m.quantity, qty) : '—'}
                  </td>
                  <td className="pf-td--num" style={{ fontWeight: 600 }}>{formatMoney(m.resultQuantity, qty)}</td>
                  <td className="pf-td--num">{m.unitCost != null ? formatMoneyWithSymbol(m.unitCost, cost) : '—'}</td>
                  <td className="pf-td--num">{formatMoneyWithSymbol(m.runningAverageCost, cost)}</td>
                  <td className="pf-td--num">{formatMoneyWithSymbol(m.runningStockValue, total)}</td>
                  <td style={{ fontSize: 12 }}>{m.createdByName ?? '—'}</td>
                  <td className="pf-td--center">
                    <button className="pf-row-action" title="Ver expediente" onClick={() => void ctx.openDetail(m.id)}>
              ...
              {ctx.movements.length === 0 && (
                <tr><td colSpan={12} className="pf-table-empty">
                  {ctx.searchMode === 'product'
