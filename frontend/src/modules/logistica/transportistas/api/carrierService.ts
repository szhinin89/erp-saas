...
export const carrierService = {
  async getAll(search?: string, activeStatus = "all"): Promise<Carrier[]> {
    ...
    const raw = await api
      .get<ApiResponse<Carrier[]> | Record<string, unknown>>(
        `/api/v1/logistics/carriers?${q.toString()}`,
      )
  ...
  async create(payload: CreateCarrierRequest): Promise<Carrier> {
    return api
      .post<ApiResponse<Carrier> | Record<string, unknown>>(
  ...
  async update(id: string, payload: CreateCarrierRequest): Promise<Carrier> {
    return api
      .put<ApiResponse<Carrier> | Record<string, unknown>>(
        `/api/v1/logistics/carriers/${id}`,
  ...
  async disable(id: string): Promise<Carrier> {
    return api
      .patch<ApiResponse<Carrier> | Record<string, unknown>>(
        `/api/v1/logistics/carriers/${id}/disable`,
        {},
      )
      .then((r) => unwrap<Carrier>(r.data));
  },
  ...
  async enable(id: string): Promise<Carrier> {
    return api
      .patch<ApiResponse<Carrier> | Record<string, unknown>>(
        `/api/v1/logistics/carriers/${id}/enable`,
        {},
      )
      .then((r) => unwrap<Carrier>(r.data));
  },
