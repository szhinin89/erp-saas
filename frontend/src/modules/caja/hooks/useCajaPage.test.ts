...
const registers: CashRegisterDto[] = [
  { id: 'reg-1', branchId: 'branch-1', branchName: 'Matriz', branchCode: '001', emissionPointId: 'ep-1', establishmentCode: '001', emissionPointCode: '001', emissionPointName: null, code: 'CAJA-01', name: 'Caja Principal', notes: null, isActive: true, hasHistory: false, defaultWarehouseId: null, defaultWarehouseCode: null, defaultWarehouseName: null, defaultCustomerId: null, defaultCustomerName: null, createdAt: '2026-07-01T00:00:00Z', updatedAt: null },
  { id: 'reg-2', branchId: 'branch-1', branchName: 'Matriz', branchCode: '001', emissionPointId: 'ep-1', establishmentCode: '001', emissionPointCode: '001', emissionPointName: null, code: 'CAJA-02', name: 'Caja Secundaria', notes: null, isActive: true, hasHistory: false, defaultWarehouseId: null, defaultWarehouseCode: null, defaultWarehouseName: null, defaultCustomerId: null, defaultCustomerName: null, createdAt: '2026-07-01T00:00:00Z', updatedAt: null },
];

function buildSession(overrides: Partial<CashSessionDto> = {}): CashSessionDto {
  return {
    id: 'session-1', companyId: 'company-1', branchId: 'branch-1', userId: 'user-1',
    cashRegisterId: 'reg-1', cashRegisterCodeSnapshot: 'CAJA-01', cashRegisterNameSnapshot: 'Caja Principal',
    emissionPointId: 'ep-1', emissionPointCodeSnapshot: '001', emissionType: 'Electronic',
    defaultWarehouseId: null, defaultWarehouseName: null, defaultCustomerId: null, defaultCustomerName: null,
    openedAt: '2026-07-19T10:00:00Z', openingAmount: 100, status: 'Open', notes: null,
    closedAt: null, closedBy: null, closeNotes: null,
    expectedAmount: null, countedAmount: null, difference: null,
    totalIncome: 0, totalExpense: 0, currentBalance: 100,
    movements: [], closingCounts: [],
    createdAt: '2026-07-19T10:00:00Z', updatedAt: null,
    ...overrides,
  };
}
...
beforeEach(() => {
  vi.clearAllMocks();
  useActiveBranchStore.setState({ branch: { id: 'branch-1', name: 'Quito Norte', isMainBranch: true } });
  vi.mocked(cajaService.getCashRegisters).mockResolvedValue(registers);
  vi.mocked(cajaService.list).mockResolvedValue({ items: [], total: 0, page: 1, pageSize: 25 });
...
describe('useCajaPage', () => {
  it('carga las cajas disponibles de la sucursal activa al montar', async () => {
    ...
    await waitFor(() => expect(result.current.cashRegisters).toEqual(registers));
  ...
  it('selecciona automáticamente la primera caja disponible', async () => {
    ...
    await waitFor(() => expect(result.current.openForm.getValues('cashRegisterId')).toBe('reg-1'));
  });
  ...
  it('la apertura envía cashRegisterId y nunca emissionPointId', async () => {
    ...
    expect(payload).toEqual({ cashRegisterId: 'reg-1', openingAmount: 100, notes: 'Apertura de prueba' });
