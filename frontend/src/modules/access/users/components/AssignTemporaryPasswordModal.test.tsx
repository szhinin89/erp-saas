...
function renderModal(props: Partial<{ open: boolean; username: string; onClose: () => void }> = {}) {
  const onClose = props.onClose ?? vi.fn();
  const utils = render(
    <I18nProvider>
      <AssignTemporaryPasswordModal open={props.open ?? true} username={props.username ?? 'ana.perez'} onClose={onClose} />
    </I18nProvider>,
  );
...
async function fillAndSubmit(
  ...
  fireEvent.change(fieldByName(container, 'temporaryPassword'), { target: { value: values.password } });
  fireEvent.change(fieldByName(container, 'confirmPassword'), { target: { value: values.confirm } });
...
describe('AssignTemporaryPasswordModal — render', () => {
  ...
  it('muestra título, campos, aviso informativo y botones cuando open=true', () => {
    ...
    expect(screen.getByText(/Todas las sesiones activas serán cerradas automáticamente/)).toBeTruthy();
    expect(screen.getByRole('button', { name: 'Cancelar' })).toBeTruthy();
    expect(screen.getByRole('button', { name: 'Asignar contraseña' })).toBeTruthy();
  });
  ...
  it('nunca renderiza un <form> propio — este modal se monta dentro del <form> único de UserConfigPage, y un <form> anidado hace que el navegador recargue la página en vez de interceptar el submit', () => {
    ...
    expect((screen.getByRole('button', { name: 'Asignar contraseña' }) as HTMLButtonElement).type).toBe('button');
  });
});
...
describe('AssignTemporaryPasswordModal — validación', () => {
  it('contraseña que no cumple complejidad muestra error y no llama al servicio', async () => {
    ...
    await waitFor(() => expect(screen.getByText('La contraseña debe tener al menos 8 caracteres.')).toBeTruthy());
  ...
  it('confirmación distinta a la contraseña muestra error de coincidencia y no llama al servicio', async () => {
    ...
    await fillAndSubmit(container, { password: 'Temp0ral!', confirm: 'Otra0ral!' });
...
describe('AssignTemporaryPasswordModal — envío', () => {
  it('éxito: llama al servicio con username y password, muestra toast, cierra y limpia el formulario', async () => {
    vi.mocked(membershipService.assignTemporaryPassword).mockResolvedValue('Contraseña temporal asignada correctamente.');
    const onClose = vi.fn();
    const { container } = renderModal({ username: 'ana.perez', onClose });

    await fillAndSubmit(container, { password: 'Temp0ral!', confirm: 'Temp0ral!' });

    await waitFor(() => {
      expect(membershipService.assignTemporaryPassword).toHaveBeenCalledWith('ana.perez', 'Temp0ral!');
      expect(message.success).toHaveBeenCalledWith('Contraseña temporal asignada correctamente.');
  ...
  it('loading: deshabilita campos y botón mientras la petición está en curso', async () => {
    ...
    await waitFor(() => expect(membershipService.assignTemporaryPassword).toHaveBeenCalledTimes(1));
  });
  ...
  it('bloquea doble envío: dos clics rápidos solo invocan el servicio una vez', async () => {
    ...
    fireEvent.change(fieldByName(container, 'temporaryPassword'), { target: { value: 'Temp0ral!' } });
    fireEvent.change(fieldByName(container, 'confirmPassword'), { target: { value: 'Temp0ral!' } });
    const submitButton = () => screen.getByRole('button', { name: /Asignar contraseña|Guardando/ });
  ...
  it('cerrar (Cancelar) mientras hay un envío en curso no invoca onClose', async () => {
    ...
    await fillAndSubmit(container, { password: 'Temp0ral!', confirm: 'Temp0ral!' });
    await waitFor(() => expect((screen.getByRole('button', { name: 'Guardando...' }) as HTMLButtonElement).disabled).toBe(true));
  ...
  it('error 422 mapea el mensaje del servidor al campo correspondiente, sin cerrar el modal', async () => {
    const validationError = {
      ...
      response: {
        status: 422,
        data: { data: { errors: { temporaryPassword: ['El servidor rechazó la contraseña temporal.'] } } },
      },
    };
    vi.mocked(membershipService.assignTemporaryPassword).mockRejectedValue(validationError);
    const onClose = vi.fn();
    const { container } = renderModal({ onClose });

    await fillAndSubmit(container, { password: 'Temp0ral!', confirm: 'Temp0ral!' });

    await waitFor(() => expect(screen.getByText('El servidor rechazó la contraseña temporal.')).toBeTruthy());
    expect(onClose).not.toHaveBeenCalled();
  ...
  it('error genérico (no 422) muestra el mensaje estándar del API, sin cerrar el modal', async () => {
    ...
    vi.mocked(membershipService.assignTemporaryPassword).mockRejectedValue(networkError);
    const onClose = vi.fn();
    const { container } = renderModal({ onClose });

    await fillAndSubmit(container, { password: 'Temp0ral!', confirm: 'Temp0ral!' });

    await waitFor(() => expect(screen.getByText('No se pudo asignar la contraseña temporal.')).toBeTruthy());
    expect(onClose).not.toHaveBeenCalled();
  });
});
...
describe('AssignTemporaryPasswordModal — cierre', () => {
  it('Cancelar sin envío en curso invoca onClose y limpia el formulario', async () => {
    ...
    rerender(
      <I18nProvider>
        <AssignTemporaryPasswordModal open={true} username="ana.perez" onClose={onClose} />
      </I18nProvider>,
    );
    expect(fieldByName(container, 'temporaryPassword').value).toBe('');
  });
