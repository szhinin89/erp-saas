...
import { membershipService, type CompanyUserMembershipAdminDto } from '../api/membershipService';
...
function useMembershipExtras(membershipId: string) {
  const [branches, setBranches] = useState<{ authorized: number; total: number } | null>(null);
  ...
  useEffect(() => {
    ...
    Promise.allSettled([
      ...
      if (branchesResult.status === 'fulfilled') {
        ...
        setBranches({ authorized: list.filter((b) => b.authorized).length, total: list.length });
      }
...
function MembershipRow(props: {
  ...
  const { row, canManage, onOpen, onRevoke, onReactivate, actionPending } = props;
  const { t } = useI18n();
  const { branches, loginMode, loading } = useMembershipExtras(row.companyUserId);

  return (
    <tr className={!row.isActive ? 'usr-row--inactive' : ''}>
      <td>
        <button type="button" className="usr-user-cell usr-user-cell--link" onClick={() => onOpen(row)}>
      ...
      <td>
        {loading ? t('common.loading') : branches ? `${branches.authorized}/${branches.total}` : '—'}
      </td>
      <td>
        {loading
          ? t('common.loading')
      ...
      {canManage ? (
        <td>
          <div className="usr-row-actions">
            <ZHBtn variant="ghost" size="sm" type="button" onClick={() => onOpen(row)}>
            ...
            {row.isActive ? (
              ...
              <ZHBtn variant="secondary" size="sm" type="button" disabled={actionPending} onClick={() => onReactivate(row)}>
...
export function UsersPage() {
  ...
  const loadUsers = useCallback(async () => {
    ...
    try {
      ...
      setListError(formatApiRequestError(err, { generic: t('users.error.load', 'No se pudo cargar la lista de usuarios.') }));
    } finally {
  ...
  const filteredRows = useMemo(() => {
    ...
    if (!q) return rows;
    return rows.filter((r) => `${r.fullName} ${r.username} ${r.email ?? ''}`.toLowerCase().includes(q));
  }, [rows, search]);

  const openUser = (row: CompanyUserMembershipAdminDto) => {
    navigate(`/access/users/${row.companyUserId}`, { state: { from: listPath } });
  };
  ...
  const handleRevoke = async (row: CompanyUserMembershipAdminDto) => {
    ...
    try {
      ...
      message.success(t('users.revoke.success', 'Acceso revocado correctamente.'));
      await loadUsers();
    } catch (err) {
      message.error(formatApiRequestError(err, { generic: t('users.revoke.error', 'No se pudo revocar el acceso.') }));
    } finally {
  ...
  const handleReactivate = async (row: CompanyUserMembershipAdminDto) => {
    setActionPendingId(row.companyUserId);
    try {
      await membershipService.upsertMembership({ username: row.username, role: row.role, profileId: row.profileId });
      message.success(t('users.reactivate.success', 'Usuario reactivado correctamente.'));
      await loadUsers();
    } catch (err) {
      message.error(formatApiRequestError(err, { generic: t('users.reactivate.error', 'No se pudo reactivar el usuario.') }));
    } finally {
      setActionPendingId(null);
    }
  };
  ...
  return (
    <ErpPageTemplate
      ...
      subtitle={t('users.subtitle', 'Administra el acceso, rol, perfil y sucursales de los usuarios de esta empresa.')}
      action={
        canManage ? (
          <ZHBtn
            variant="primary"
            size="md"
            type="button"
            onClick={() => navigate('/access/users/new', { state: { from: listPath } })}
          >
            ...
            <table className="table">
              <thead>
                <tr>
                  ...
                  {canManage ? <th className="pg-th-actions">{t('common.actions')}</th> : null}
                </tr>
              </thead>
              <tbody>
