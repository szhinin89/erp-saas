...
const ASSIGN_TEMPORARY_PASSWORD_PERMISSION = 'access.identity_users.assign_temporary_password';
...
export function UserSecuritySection({ membership }: Props) {
  ...
  const canAssignTemporaryPassword = canShow(ASSIGN_TEMPORARY_PASSWORD_PERMISSION);
  ...
  return (
    <div>
      ...
      <ZHField label={t('users.membership.profile', 'Perfil')} readOnly>
        <span>{membership.profileName ?? t('users.table.noProfile', 'Sin perfil')}</span>
      </ZHField>

      {membership.profileId ? (
        <Link className="zh-btn zh-btn--ghost zh-btn--md" to="/admin/roles">
          {t('users.config.security.viewProfilePermissions', 'Ver permisos de este perfil')}
        </Link>
      ) : null}
        ...
        <div className="pg-section" style={{ marginTop: 'var(--space-5)' }}>
          <div className="pg-section-header">
            <div className="pg-section-header-left">
              <span className="material-symbols-outlined pg-section-icon" aria-hidden="true">password</span>
              <h3 className="pg-section-label">{t('users.config.security.card.title', 'Seguridad')}</h3>
            </div>
          </div>
          <div className="pg-section-body">
            <p className="subtle">
              {t('users.config.security.card.description', 'Administra la contraseña del usuario.')}
            </p>
            <ZHBtn type="button" variant="secondary" size="md" onClick={() => setModalOpen(true)}>
              {t('users.config.security.assignTemporaryPassword.action', 'Asignar contraseña temporal')}
            </ZHBtn>
          </div>
        </div>
      ) : null}
