...
export const profileService = {
  list: (onlyActive = true) =>
    api.get<ApiResponse<Profile[]>>('/api/v1/admin/iam/profiles', { params: { onlyActive } })
      .then((r) => r.data.data),

  create: (name: string, description?: string | null) =>
    api.post<ApiResponse<Profile>>('/api/v1/admin/iam/profiles', { name, description: description ?? null })
      .then((r) => r.data.data),

  update: (p: Profile) =>
    api.put<ApiResponse<Profile>>(`/api/v1/admin/iam/profiles/${p.id}`, {
      profileId: p.id,
      name: p.name,
  ...
  upsertPermissions: (profileId: string, items: { permissionKey: string; isAllowed: boolean }[]) =>
    api.put<ApiResponse<PermissionUpsertResult>>(`/api/v1/admin/iam/profiles/${profileId}/permissions`, {
      profileId,
      items,
    }).then((r) => r.data.data),
