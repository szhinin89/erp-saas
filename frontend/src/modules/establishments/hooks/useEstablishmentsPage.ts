...
export function useEstablishmentsPage() {
  ...
  const fetchList = useCallback(async () => {
    ...
    setLoading(true);
    try {
      setItems(await establishmentService.list(activeStatus, branchIdFilter, search || undefined));
    } catch {
  ...
  const toggleDisable = async (item: EstablishmentListItemDto) => {
    ...
    try {
      ...
      setError(formatApiRequestError(err, { generic: 'Error al cambiar el estado del establecimiento.' }));
    }
  };
...
export type EstablishmentsPageContext = ReturnType<typeof useEstablishmentsPage>;
