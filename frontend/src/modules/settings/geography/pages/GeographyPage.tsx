...
export function GeographyPage() {
  ...
  useEffect(() => {
    if (!countryId) { setProvinces([]); setProvinceId(''); return; }
    branchService.provinces(countryId).then(setProvinces).catch(() => setProvinces([]));
    setProvinceId('');
    setCantonId('');
  }, [countryId]);

  useEffect(() => {
    if (!provinceId) { setCantons([]); setCantonId(''); return; }
    branchService.cantons(provinceId).then(setCantons).catch(() => setCantons([]));
    setCantonId('');
  }, [provinceId]);

  useEffect(() => {
    if (!cantonId) { setParishes([]); return; }
    branchService.parishes(cantonId).then(setParishes).catch(() => setParishes([]));
  }, [cantonId]);

  if (!canView) return <NoAccessPage title={t('app.nav.item.settings.geography')} />;
  ...
  return (
    ...
    >
      {error && <ZHPageNotice variant="error" message="Error al cargar geografía" detail={error} />}

      <div className="pg-section">
        <div className="pg-table-controls">
          <div className="pg-table-controls-left">
            <select className="zh-input" value={countryId} disabled={loading} onChange={(e) => setCountryId(e.target.value)}>
              <option value="">País…</option>
              {countries.map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}
            </select>
            <select className="zh-input" value={provinceId} disabled={!countryId} onChange={(e) => setProvinceId(e.target.value)}>
              <option value="">Provincia…</option>
              {provinces.map((p) => <option key={p.id} value={p.id}>{p.name}</option>)}
            </select>
            <select className="zh-input" value={cantonId} disabled={!provinceId} onChange={(e) => setCantonId(e.target.value)}>
        ...
        {loading ? (
          ...
          <div className="pg-pad-40"><EmptyState message="No hay datos geográficos disponibles." /></div>
        ) : (
          <>
            ...
            {cantonId && parishes.length === 0 ? (
              ...
              <div className="pg-overflow-x">
                <table className="table">
                  ...
                  <tbody>
                    {(cantonId ? parishes : provinceId ? cantons : countryId ? provinces : countries).map((item) => (
                      <tr key={item.id}>
                        <td>{cantonId ? 'Parroquia' : provinceId ? 'Cantón' : countryId ? 'Provincia' : 'País'}</td>
