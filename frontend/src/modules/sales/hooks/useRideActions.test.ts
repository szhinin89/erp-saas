...
import type { RideGenerationResultDto, RideOutcome } from '../../ride/api/rideService';
...
function outcomeResult(outcome: RideOutcome): RideGenerationResultDto {
  return { outcome, storagePath: outcome === 'Generated' || outcome === 'Cached' ? 'ride/path.pdf' : null, metadata: null, reasonCode: null };
}

describe('useRideActions', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    Object.defineProperty(globalThis.URL, 'createObjectURL', { value: vi.fn(() => 'blob:mock'), writable: true });
  ...
  describe('handleViewRide', () => {
    it('Generated: obtiene el contenido y abre una pestaña nueva, sin mensajes', async () => {
      ...
      await act(async () => { await result.current.handleViewRide('doc-1'); });
    ...
    it('un error sin response ni message cae en el mensaje de fallback en español', async () => {
      ...
      expect(messageErrorMock).toHaveBeenCalledWith('No se pudo obtener el RIDE.');
    });

    it('usa el mensaje de negocio devuelto por el backend en un 422/400 en vez del fallback genérico', async () => {
      getOrGenerateMock.mockRejectedValue({ response: { data: { message: { user: 'Mensaje específico del backend.' } } } });
      const { result } = renderHook(() => useRideActions());

      await act(async () => { await result.current.handleViewRide('doc-1'); });

      expect(messageErrorMock).toHaveBeenCalledWith('Mensaje específico del backend.');
    });
  });
  ...
  describe('handleDownloadRide', () => {
    it('Generated: descarga el blob con un nombre de archivo basado en el número de factura', async () => {
      ...
      await act(async () => { await result.current.handleDownloadRide('doc-1', '001-001-000000123'); });

      expect(downloadBlobMock).toHaveBeenCalledWith(blob, 'RIDE-001-001-000000123.pdf');
    });
    ...
    it('NotApplicable: no descarga nada', async () => {
      ...
      await act(async () => { await result.current.handleDownloadRide('doc-1'); });
  ...
  describe('handleRegenerateRide', () => {
    it('Generated: muestra éxito y abre el PDF fresco', async () => {
      ...
      await act(async () => { await result.current.handleRegenerateRide('doc-1'); });
