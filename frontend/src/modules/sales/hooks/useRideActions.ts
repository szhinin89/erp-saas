...
import { rideService, type RideGenerationResultDto } from '../../ride/api/rideService';
...
function extractErrorMessage(e: unknown, fallback: string): string {
  const err = e as { response?: { data?: { message?: { user?: string }; data?: { errors?: string[] } } }; message?: string };
...
function ridePdfFileName(invoiceNumber: string | undefined, sourceEntityId: string): string {
...
function announceOutcome(result: RideGenerationResultDto): boolean {
  switch (result.outcome) {
    ...
    case 'Cached':
      return true;
    case 'PendingSource':
      message.warning('El documento electrónico aún no está autorizado por el SRI. Intente nuevamente en unos minutos.');
      return false;
    case 'NotApplicable':
      message.warning('Esta factura no tiene un documento electrónico autorizado (rechazada, anulada o inexistente) — no se puede generar el RIDE.');
      return false;
    case 'Failed':
      message.error('No se pudo generar el RIDE. Intente nuevamente o contacte soporte.');
      return false;
    default:
      return false;
  }
...
export function useRideActions() {
  ...
  const handleViewRide = useCallback(async (sourceEntityId: string) => {
    setRidePending(true);
    try {
      const result = await rideService.getOrGenerate(SOURCE_MODULE, sourceEntityId);
  ...
  const handleDownloadRide = useCallback(async (sourceEntityId: string, invoiceNumber?: string) => {
    setRidePending(true);
    try {
      const result = await rideService.getOrGenerate(SOURCE_MODULE, sourceEntityId);
      if (announceOutcome(result)) {
  ...
  const handleRegenerateRide = useCallback(async (sourceEntityId: string) => {
    setRidePending(true);
    try {
      const result = await rideService.regenerate(SOURCE_MODULE, sourceEntityId);
      if (announceOutcome(result)) {
  ...
  return { ridePending, handleViewRide, handleDownloadRide, handleRegenerateRide };
}
