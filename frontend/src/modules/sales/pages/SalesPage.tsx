...
export function SalesPage() {
  ...
  const statusLabel = (s: string) => s === 'Draft' ? 'Borrador' : s === 'Authorized' ? 'Autorizada' : 'Anulada';
  const statusBadgeClass = (s: string) => s === 'Draft' ? 'pf-badge--warning' : s === 'Authorized' ? 'pf-badge--success' : 'pf-badge--danger';
  ...
  return (
    <div style={{ padding: '8px 16px 0' }}>
      ...
      {ctx.hasCashSession === false && (
        <ZHPageNotice variant="warning" message="No tiene una caja abierta. Debe abrir una caja antes de autorizar facturas." />
      )}
      ...
      {ctx.tab === 'listado' && (
        <div className="prd-section">
          <div style={{ display: 'flex', gap: 12, marginBottom: 16, alignItems: 'center' }}>
            <button className="pf-btn pf-btn--primary" onClick={() => { ctx.resetForm(); ctx.setTab('nuevo'); }}>
              <span className="material-symbols-outlined pf-btn__icon">add</span>
              Nueva Factura
            </button>
            <div style={{ flex: 1 }} />
            <input type="text" placeholder="Buscar por número o cliente..." value={ctx.listSearch}
              onChange={e => ctx.setListSearch(e.target.value)} style={{ maxWidth: 300 }} />
          ...
          {ctx.listLoading ? <p>Cargando...</p> : (
            <table className="pf-table">
              <thead>
                <tr>
                  <th>Nro. Factura</th><th>Fecha</th><th>Cliente</th>
                  <th className="pf-th--right">Total</th>
                  <th className="pf-th--center">Líneas</th><th>Estado</th>
              ...
              <tbody>
                {ctx.listItems.map(inv => (
                  <tr key={inv.id}>
                    ...
                    <td className="pf-td--num">{formatMoneyWithSymbol(inv.grandTotal, getDecimalConfig().totalAmount)}</td>
                    <td className="pf-td--center">{inv.lineCount}</td>
                    <td><span className={`pf-badge ${statusBadgeClass(inv.status)}`}>{statusLabel(inv.status)}</span></td>
                    <td className="pf-td--center">
                      <button className="pf-row-action" onClick={() => void ctx.loadForEdit(inv.id)}
                        title={inv.status === 'Draft' ? 'Reintentar emisión' : 'Ver / Editar'}>
                      ...
                      {inv.status === 'Authorized' && (
                        <button className="pf-row-action" title="Ver RIDE" disabled={ride.ridePending}
                ...
                {ctx.listItems.length === 0 && <tr><td colSpan={7} className="pf-table-empty">Sin facturas registradas.</td></tr>}
              </tbody>
            </table>
          )}
        </div>
      ...
      {ctx.tab === 'nuevo' && (
        <div className="sf-layout">
          ...
          <div className="sf-sidebar">
            ...
            <div className="sf-tabs">
              <button className="sf-tab sf-tab--active">
                <span className="material-symbols-outlined sf-tab__icon">receipt_long</span>
                {ctx.editing ? 'Editar Factura' : 'Nueva Factura'}
              </button>
              <button className="sf-tab" onClick={() => { ctx.resetForm(); ctx.setTab('listado'); }}>
                <span className="material-symbols-outlined sf-tab__icon">history</span>
                Historial
              </button>
            </div>
            ...
            <div className="sf-sidebar__section">
              <div className="sf-sidebar__header">
                <span className="material-symbols-outlined sf-sidebar__header-icon">apartment</span>
                Datos de Emisión
              </div>
              <div className="sf-emission">
                ...
                {ctx.branchName && (
                  <div><span className="sf-emission__label">Sucursal: </span><span className="sf-emission__value">{ctx.branchName}</span></div>
                )}
                {ctx.myCashSession && (
                  <div><span className="sf-emission__label">Caja: </span><span className="sf-emission__value">{ctx.myCashSession.cashRegisterCodeSnapshot} — {ctx.myCashSession.cashRegisterNameSnapshot}</span></div>
                )}
                {ctx.myCashSession && (
                  <div><span className="sf-emission__label">Punto: </span><span className="sf-emission__value">{ctx.myCashSession.emissionPointCodeSnapshot}</span></div>
                )}
                ...
                {(ctx.myCashSession?.emissionType ?? ctx.editing?.emissionType) && (
                  <div><span className="sf-emission__label">Tipo Emisión: </span><span className="sf-emission__value">{(ctx.myCashSession?.emissionType ?? ctx.editing?.emissionType) === 'Electronic' ? '🟢 Electrónica' : '🔵 Física'}</span></div>
                )}
                <div>
                  <div className="sf-emission__label">Tipo Documento</div>
                  <select style={{ width: '100%', fontSize: 12, padding: '5px 8px', border: '1.5px solid var(--color-border)', borderRadius: 6, marginBottom: 4 }}
                    value={ctx.readOnly ? (ctx.editing?.docTypeCode ?? '') : ctx.formWatch.docTypeCode}
                    onChange={e => ctx.setValue('docTypeCode', e.target.value)} disabled={ctx.fieldDisabled}>
                ...
                <div>
                  ...
                  <select style={{ width: '100%', fontSize: 12, padding: '5px 8px', border: '1.5px solid var(--color-border)', borderRadius: 6, marginBottom: 4 }}
                    value={ctx.readOnly ? (ctx.editing?.sriPaymentMethodCode ?? '') : ctx.formWatch.sriPaymentMethodCode}
                    onChange={e => ctx.setValue('sriPaymentMethodCode', e.target.value)} disabled={ctx.fieldDisabled}>
                ...
                {ctx.editing && (
                  <div style={{ marginTop: 4 }}>
                    ...
                    <span className="sf-emission__value" style={{ fontFamily: 'monospace' }}>{ctx.editing.invoiceNumber}</span>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="sf-sidebar__section">
              <div className="sf-sidebar__header">
                <span className="material-symbols-outlined sf-sidebar__header-icon">person</span>
                Cliente
                <span className="material-symbols-outlined sf-sidebar__header-right" title="Nuevo cliente">person_add</span>
              </div>
              <ZHField density="compact" fieldError={ctx.errors.customerId?.message}>
                <CustomerPicker
              ...
              {ctx.customerProfile && (
                <div style={{ marginTop: 6, fontSize: 11, color: 'var(--color-text-secondary)', lineHeight: 1.6 }}>
                  ...
                  {!ctx.fieldDisabled && (
                    <button type="button" onClick={ctx.openEditCustomerModal}
                      style={{ marginTop: 4, background: 'none', border: 'none', cursor: 'pointer', color: 'var(--color-primary)', fontSize: 11, fontWeight: 600, padding: 0, display: 'flex', alignItems: 'center', gap: 3 }}>
            ...
            <div className="sf-sidebar__section" style={{ padding: '12px 16px' }}>
              <div className="sf-total-box">
                <table className="sf-tax-table">
                  <thead>
                    <tr><th>Impuesto</th><th>Base</th><th>Valor</th></tr>
                  </thead>
                  <tbody>
                    {ctx.taxBreakdown.map(e => (
                      <tr key={e.rate}>
                        <td>{e.label}</td>
                        <td>{formatMoneyWithSymbol(e.base, getDecimalConfig().totalAmount)}</td>
                        <td>{formatMoneyWithSymbol(e.tax, getDecimalConfig().totalAmount)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
                {ctx.totalDiscount > 0 && (
                  <div className="sf-summary__discount-total" style={{ marginTop: 8 }}>
                    <span>Descuento:</span>
                    <span>-{formatMoneyWithSymbol(ctx.totalDiscount, getDecimalConfig().totalAmount)}</span>
                  </div>
                )}
                <div className="sf-total-box__header" style={{ marginTop: 10 }}>
                  <span className="sf-total-box__label">Total a Cobrar</span>
                </div>
                <div className="sf-total-box__amount">{formatMoneyWithSymbol(ctx.grandTotal, getDecimalConfig().totalAmount)}</div>
              </div>
            </div>
          ...
          <div className="sf-bottombar">
            {ctx.isElectronic && (
              <div className="sf-bottombar__sri">
                <span className="sf-bottombar__sri-label">
                  <span className="material-symbols-outlined" style={{ fontSize: 14, verticalAlign: 'middle', marginRight: 4 }}>qr_code_2</span>
                  Clave de Acceso SRI
                </span>
                ...
                {ctx.editing && (
                  <button type="button" className="edm-icon-btn" onClick={() => setSriDiagnosticOpen(true)}
                    aria-label="Ver diagnóstico SRI" title="Ver diagnóstico SRI">
                    <span className="material-symbols-outlined zh-icon-sm">troubleshoot</span>
                  </button>
                )}
              </div>
            )}
            ...
            <button type="button" className="sf-bottombar__btn" onClick={() => void ctx.clearForm()}>
              <span className="material-symbols-outlined sf-bottombar__btn-icon">delete_sweep</span>
              Limpiar Todo
            </button>

            {ctx.isDraft && <EmitButton ctx={ctx} />}

            {ctx.editing && ctx.isElectronic && ctx.editing.status === 'Authorized' && ctx.editing.electronicStatus === 'None' && (
              <button className="sf-bottombar__btn" onClick={() => void ctx.handleGenerateElectronicDocument()} disabled={ctx.saving}
                title="Esta factura fue autorizada pero nunca generó su documento electrónico — regenera el registro en el Monitor.">
                <span className="material-symbols-outlined sf-bottombar__btn-icon">bolt</span>
                Generar documento electrónico
              </button>
            )}

            {ctx.editing && ctx.editing.status === 'Authorized' && (
              <button className="sf-bottombar__btn" disabled={ride.ridePending}
                ...
                <span className="material-symbols-outlined sf-bottombar__btn-icon">picture_as_pdf</span>
                Ver RIDE
              </button>
            )}
            ...
            {ctx.editing && ctx.editing.status === 'Authorized' && (
              <button className="sf-bottombar__btn" disabled={ride.ridePending}
                onClick={() => void ride.handleDownloadRide(ctx.editing!.id, ctx.editing!.invoiceNumber)}>
                <span className="material-symbols-outlined sf-bottombar__btn-icon">download</span>
                Descargar RIDE
              </button>
            )}
            ...
            {ctx.editing && ctx.editing.status === 'Authorized' && (
              <button className="sf-bottombar__btn" disabled={ride.ridePending}
                ...
                <span className="material-symbols-outlined sf-bottombar__btn-icon">refresh</span>
                Regenerar RIDE
              </button>
            )}

            {ctx.editing && ctx.editing.status === 'Authorized' && (
              <button className="sf-bottombar__btn" onClick={() => ctx.setModalCancelReason(true)}
                style={{ background: 'var(--color-error)', color: '#fff', borderColor: 'var(--color-error)' }}>
                <span className="material-symbols-outlined sf-bottombar__btn-icon">block</span>
                Anular
              </button>
            )}
          </div>
      ...
      <PaymentDetailModal
        ...
        available={Math.round((ctx.summary.total - ctx.payments.filter(p => p.paymentMethodId !== ctx.detailMethodId).reduce((s, p) => s + (p.amount || 0), 0)) * (10 ** getDecimalConfig().totalAmount)) / (10 ** getDecimalConfig().totalAmount)}
        onConfirm={(rows) => {
          ctx.setInvoicePayments(prev => {
            const without = prev.filter(p => p.paymentMethodId !== ctx.detailMethodId);
      ...
      <CreditSimulatorModal
        ...
        onRecalculate={() => ctx.setCreditRows(ctx.simulateCreditInstallments(ctx.creditAmount))}
        onConfirm={(totalAmount) => {
          const creditPm = ctx.paymentMethods.find(p => p.isCreditAllowed);
          if (creditPm) {
            ctx.setInvoicePayments(prev => {
              const exists = prev.find(p => p.paymentMethodId === creditPm.id);
              if (exists) return prev.map(p => p.paymentMethodId === creditPm.id ? { ...p, amount: totalAmount } : p);
              return [...prev, { _key: ctx.payKey, paymentMethodId: creditPm.id, amount: totalAmount, reference: null }];
            });
            const currentPayments = ctx.payments;
            if (!currentPayments.find(p => p.paymentMethodId === creditPm.id)) ctx.setPayKey(k => k + 1);
          }
      ...
      <SalesIssueModal
        ...
        onPrintRide={() => ctx.issueResult && void ride.handleViewRide(ctx.issueResult.id)}
        onDownloadPdf={() => ctx.issueResult && void ride.handleDownloadRide(ctx.issueResult.id, ctx.issueResult.invoiceNumber)}
      ...
      <ZHPromptModal open={ctx.modalCancelReason} variant="danger" title="Anular Factura"
        message={ctx.editing ? `¿ANULAR factura ${ctx.editing.invoiceNumber}? Esta acción NO se puede deshacer.` : ''}
...
function SalesFormChecklist({ ctx }: { ctx: SalesPageContext }) {
  ...
  const item = (label: string, status: ItemStatus) => (
    <div className="sf-checklist__item">
      <span className={`material-symbols-outlined sf-checklist__icon sf-checklist__icon--${status}`}>
        {status === 'ok' ? 'check_circle' : status === 'error' ? 'error' : 'radio_button_unchecked'}
      </span>
  ...
  return (
    <>
      <div className="sf-checklist">
        ...
        {item('Caja abierta', ctx.hasCashSession === true ? 'ok' : ctx.hasCashSession === false ? 'error' : 'missing')}
        {item(
          paymentExceeds ? 'Cobro excede el total' : 'Formas de cobro',
          paymentOk ? 'ok' : paymentExceeds ? 'error' : paid > 0 ? 'missing' : 'missing',
        )}
      </div>
      {nextStep && (
        <div className="sf-next-step">
          <span className="material-symbols-outlined sf-next-step__icon">arrow_forward</span>
          {nextStep}
        </div>
      )}
    </>
...
function EmitButton({ ctx }: { ctx: SalesPageContext }) {
  ...
  return (
    <div style={{ position: 'relative' }}>
      <button className="sf-bottombar__emit" onClick={ctx.openIssueFlow}
        disabled={!ctx.canEmit}
        title={reasons.length > 0 ? `No se puede emitir: ${reasons.join(', ')}` : undefined}>
        <span className="material-symbols-outlined sf-bottombar__emit-icon">play_arrow</span>
        {ctx.isElectronic ? 'Emitir Factura Electrónica (F8)' : 'Emitir Factura (F8)'}
      </button>
...
function PaymentMethodsSection({ ctx }: { ctx: ReturnType<typeof useSalesPage> }) {
  return (
    <div className="sf-sidebar__section">
      <div className="sf-sidebar__header">
        <span className="material-symbols-outlined sf-sidebar__header-icon">payments</span>
        Formas de Cobro
        {!ctx.readOnly && ctx.payments.length > 0 && (
          <span className="material-symbols-outlined sf-sidebar__header-right" title="Limpiar cobros"
            style={{ cursor: 'pointer', fontSize: 16 }}
            onClick={() => ctx.setInvoicePayments([])}>delete_sweep</span>
        )}
      </div>
      {ctx.readOnly ? (
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
          {(ctx.editing?.payments ?? []).filter(p => p.amount > 0).map(p => (
            <div key={p.id} style={{ padding: '6px 10px', borderRadius: 6, border: '1.5px solid var(--color-primary)', background: 'var(--color-primary)', color: '#fff', fontSize: 11, fontWeight: 700 }}>
              {p.paymentMethodName} <span style={{ fontFamily: 'monospace' }}>{formatMoneyWithSymbol(p.amount, getDecimalConfig().totalAmount)}</span>
            </div>
          ))}
        </div>
      ) : (
        <>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(130px, 1fr))', gap: 6 }}>
            {ctx.paymentMethods.map(pm => {
              const entries = ctx.payments.filter(ip => ip.paymentMethodId === pm.id);
              const entry = entries[0];
              const totalForMethod = entries.reduce((s, e) => s + (e.amount || 0), 0);
              const hasValue = totalForMethod > 0;
              const isCredit = pm.isCreditAllowed;
              const calcRemaining = () => {
                const factor = 10 ** getDecimalConfig().totalAmount;
                return Math.max(0, Math.round((ctx.summary.total - ctx.payments.filter(p => p.paymentMethodId !== pm.id).reduce((s, p) => s + (p.amount || 0), 0)) * factor) / factor);
              };

              return (
                <div key={pm.id} style={{
                  display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2,
                  padding: '8px 6px', borderRadius: 8, fontSize: 10, fontWeight: 700,
                  textTransform: 'uppercase', transition: 'all 0.15s',
                  border: hasValue ? '2px solid var(--color-primary)' : '1.5px solid var(--color-border)',
                  background: hasValue ? 'var(--color-primary)' : '#fff',
                  color: hasValue ? '#fff' : 'var(--color-text-primary)',
                }}>
                  <button type="button" disabled={ctx.fieldDisabled}
                    style={{
                      fontSize: 11, cursor: 'pointer', width: '100%', textAlign: 'center',
                      background: 'none', border: 'none', padding: 0, margin: 0,
                      font: 'inherit', fontWeight: 'inherit', textTransform: 'inherit', color: 'inherit',
                    }}
                    onClick={() => {
                      ...
                      if (isCredit) {
                        ...
                        const existing = ctx.payments.filter(p => p.paymentMethodId === pm.id);
                        if (existing.length > 0) {
                          ctx.setDetailRows(existing.map((e, i) => ({
                            _k: i + 1, amount: e.amount,
                            card: pm.detailType === 'Card' ? (e.cardDetail ?? {}) : undefined,
                            transfer: pm.detailType === 'Transfer' ? (e.transferDetail ?? {}) : undefined,
                            cheque: pm.detailType === 'Check' ? (e.chequeDetail ?? {}) : undefined,
                          })));
                        ...
                        const rem = calcRemaining();
                        if (rem > 0) {
                          ctx.setInvoicePayments(prev => [...prev, { _key: ctx.payKey, paymentMethodId: pm.id, amount: rem, reference: null }]);
                          ctx.setPayKey(k => k + 1);
                        }
                      }
                    }}>
                  ...
                  {hasValue && !isCredit && !pm.requiresReference && (
                    <div style={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                      ...
                      <ZhDecimalInput decimals={getDecimalConfig().totalAmount} positiveOnly
                        defaultValue={formatMoney(entry!.amount, getDecimalConfig().totalAmount)}
                        disabled={ctx.fieldDisabled}
                        onBlur={e => {
                          const val = Number(e.target.value) || 0;
                          if (val > 0) {
                            ctx.setInvoicePayments(prev => prev.map(p => p._key === entry!._key ? { ...p, amount: val } : p));
                          } else {
                            ctx.setInvoicePayments(prev => prev.filter(p => p._key !== entry!._key));
                          }
                        }}
                        style={{ width: 70, fontSize: 13, fontFamily: 'monospace', fontWeight: 800, textAlign: 'center', padding: '2px 4px', border: '1px solid rgba(255,255,255,0.4)', borderRadius: 4, background: 'rgba(255,255,255,0.15)', color: '#fff' }}
                      />
                      <button type="button" onClick={() => ctx.setInvoicePayments(prev => prev.filter(p => p._key !== entry!._key))}
                        style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'rgba(255,255,255,0.7)', fontSize: 14, padding: 0, lineHeight: 1 }}>×</button>
                    </div>
                  )}
                  {hasValue && pm.requiresReference && !isCredit && (
                    <span style={{ fontSize: 12, fontFamily: 'monospace', fontWeight: 800 }}>
                      {formatMoneyWithSymbol(totalForMethod, getDecimalConfig().totalAmount)} <span style={{ fontSize: 9, fontWeight: 400, opacity: 0.8 }}>({entries.length})</span>
                    </span>
                  )}
                  {hasValue && isCredit && (
                    <span style={{ fontSize: 13, fontFamily: 'monospace', fontWeight: 800, cursor: 'pointer' }}
                      onClick={() => {
                        ctx.setCreditAmount(entry!.amount);
                        ctx.setCreditRows(ctx.simulateCreditInstallments(entry!.amount));
                        ctx.setModalCredit(true);
                      }}>
                      {formatMoneyWithSymbol(entry!.amount, getDecimalConfig().totalAmount)}
                    </span>
                  )}
                </div>
              );
          ...
          {(() => {
            ...
            return (
              <div style={{ fontSize: 11, marginTop: 6, padding: '6px 8px', borderRadius: 6,
                background: diff === 0 ? '#f0fdf4' : exceeds ? '#fef2f2' : '#f8fafc',
                border: exceeds ? '1.5px solid #dc2626' : '1px solid var(--color-border)' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', color: 'var(--color-text-secondary)' }}>
                  <span>Total factura:</span><span style={{ fontFamily: 'monospace' }}>{formatMoneyWithSymbol(total, getDecimalConfig().totalAmount)}</span>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', color: 'var(--color-text-secondary)' }}>
                  <span>Total cobrado:</span><span style={{ fontFamily: 'monospace' }}>{formatMoneyWithSymbol(paid, getDecimalConfig().totalAmount)}</span>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', fontWeight: 700, marginTop: 2, paddingTop: 2, borderTop: '1px solid var(--color-border)',
                  color: diff === 0 ? '#16a34a' : exceeds ? '#dc2626' : 'var(--color-text-primary)' }}>
                  <span>{diff === 0 ? '✓ Cobro completo' : exceeds ? '✗ Excede' : 'Pendiente:'}</span>
                  {diff !== 0 && <span style={{ fontFamily: 'monospace' }}>{formatMoneyWithSymbol(Math.abs(diff), getDecimalConfig().totalAmount)}</span>}
                </div>
              </div>
            );
          })()}
