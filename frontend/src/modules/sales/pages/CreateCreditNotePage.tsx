...
export function CreateCreditNotePage() {
  ...
  const invoicesState  = useAsync(() => salesInvoicesService.list({ pageSize: 500, estado: 'Autorizado' }));
  ...
  const invoiceLabel = (inv: SalesInvoiceDto) =>
            ...
            {selectedInvoice && (
              <div className="pg-form-span-full">
                <div className="pg-summary-box">
                  ...
                  <span><strong>Fecha:</strong> {formatDate(selectedInvoice.issueDate)}</span>
                </div>
              </div>
            )}
          </div>
