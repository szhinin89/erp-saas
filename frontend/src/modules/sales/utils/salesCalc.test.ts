...
import { lineGross, lineDiscountAmt, lineNet, calcLineTax, calcSummary } from './salesCalc';
import type { SalesLineInput, SalesInvoiceDetailDto } from '../api/salesService';
...
export const mockBackendDetails: SalesInvoiceDetailDto[] = [
  {
    id: 'det-001', itemId: 'item-001', warehouseId: null, description: 'Resma de papel A4',
    snapshotSku: 'PAP-A4', snapshotItemName: 'Resma Papel A4 75gr',
    uomCode: 'UNIT', conversionFactor: 1, quantityInBaseUom: 10,
    quantity: 10, unitPrice: 5.00, discountPct: 0, discountAmount: 0,
    taxableBase: 50.00,
    vatCode: '10', vatRate: 15, vatAmount: 7.50, snapshotVatName: 'IVA 15%',
    iceCode: null, iceRate: 0, iceAmount: 0, snapshotIceName: null,
    taxInclusiveTotal: 57.50,
    notes: null, sortOrder: 1,
  },
  {
    id: 'det-002', itemId: 'item-002', warehouseId: null, description: 'Toner HP 85A',
    snapshotSku: 'TON-85A', snapshotItemName: 'Toner HP 85A Compatible',
    uomCode: 'UNIT', conversionFactor: 1, quantityInBaseUom: 2,
    quantity: 2, unitPrice: 45.00, discountPct: 10, discountAmount: 9.00,
    taxableBase: 81.00,
    vatCode: '10', vatRate: 15, vatAmount: 12.15, snapshotVatName: 'IVA 15%',
    iceCode: null, iceRate: 0, iceAmount: 0, snapshotIceName: null,
    taxInclusiveTotal: 93.15,
    notes: null, sortOrder: 2,
  },
  {
    id: 'det-003', itemId: null, warehouseId: null, description: 'Servicio de consultoría',
    snapshotSku: null, snapshotItemName: null,
    uomCode: 'UNIT', conversionFactor: 1, quantityInBaseUom: 1,
    quantity: 1, unitPrice: 200.00, discountPct: 0, discountAmount: 0,
    taxableBase: 200.00,
    vatCode: '0', vatRate: 0, vatAmount: 0, snapshotVatName: 'IVA 0%',
    iceCode: null, iceRate: 0, iceAmount: 0, snapshotIceName: null,
    taxInclusiveTotal: 200.00,
    notes: null, sortOrder: 3,
  },
];
  ...
  describe('mock backend data — structural integrity', () => {
    it('has consistent taxInclusiveTotal = taxableBase + vatAmount + iceAmount', () => {
      for (const d of mockBackendDetails) {
        expect(d.taxInclusiveTotal).toBeCloseTo(d.taxableBase + d.vatAmount + d.iceAmount, 2);
      }
    });

    it('has consistent taxableBase = (quantity * unitPrice) - discountAmount', () => {
      for (const d of mockBackendDetails) {
        expect(d.taxableBase).toBeCloseTo(d.quantity * d.unitPrice - d.discountAmount, 2);
      }
    });
