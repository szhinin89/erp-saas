...
import { expensesService, type GastoDto, type EstadoGasto } from '../api/expensesService';
...
export function GastosListPage() {
  ...
  const handleValidar = (id: string) => {
    setActionId(id);
    void doAction(() => expensesService.validate(id), 'Gasto validado.');
  };
  const handleAprobar = (id: string) => {
    setActionId(id);
    void doAction(() => expensesService.approve(id), 'Gasto aprobado. Se generó el asiento contable.');
  };
  const handleRechazar = async () => {
    if (!rejectId || !rejectText.trim()) return;
    setActionId(rejectId);
    await doAction(() => expensesService.reject(rejectId, rejectText.trim()), 'Gasto rechazado.');
    setRejectId(null);
    setRejectText('');
  };
