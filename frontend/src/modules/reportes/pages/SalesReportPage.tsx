  ...
  return (
    ...
    >
      <div className="pg-kpis">
        <ReportKpiCard icon="payments" tone="primary" badge="+12.4%" badgeTone="success" label="Total Ventas" value="$248,500.00" />
        <ReportKpiCard icon="analytics" tone="secondary" badge="Estable" badgeTone="warning" label="Margen Promedio" value="32.4%" />
        <ReportKpiCard icon="inventory" tone="tertiary" badge="+5%" badgeTone="success" label="Pedidos Procesados" value="1,429" unit="pedidos" />
        <ReportKpiCard icon="star" tone="tertiary" label="Top Producto" value="Servidor Xeon E5 v4" sub="42 unidades vendidas" />
      </div>
      ...
      <ReportChartPanel title="Tendencia de Ventas Diarias" period={period} onPeriod={setPeriod}>
        <svg viewBox="0 0 1000 280" preserveAspectRatio="none" aria-hidden>
          ...
          <path d="M0,280 L0,233 L100,204 L200,222 L300,140 L400,167 L500,74 L600,111 L700,56 L800,93 L900,37 L1000,65 L1000,280 Z" fill="url(#rpt-chart-fill)" />
          <polyline points="0,233 100,204 200,222 300,140 400,167 500,74 600,111 700,56 800,93 900,37 1000,65" fill="none" stroke="#3a5f84" strokeWidth="3" strokeLinejoin="round" />
          ...
          <tbody>
            {filtered.map((row) => (
              <tr key={row.id}>
                ...
                <ReportTd><ReportStatusBadge label={row.statusLabel} tone={row.status} /></ReportTd>
