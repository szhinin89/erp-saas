...
export function MasterDataBusinessPartnerDetailPage() {
  ...
  const { options: legalEntityTypes } = useLegalEntityTypes();
  ...
  const openLocEdit = async (loc: BpLocationDto) => {
    ...
    const purposeVal = loc.purposes.reduce(
      (acc, p) => acc | (purposeMap[p] ?? 0),
      0,
    );
    const typeVal =
      LocationTypeEnum[loc.locationType as keyof typeof LocationTypeEnum] ??
      LocationTypeEnum.Other;
    locForm.reset({
      name: loc.name,
      type: typeVal,
      purpose: purposeVal,
      ...
      parishCode: loc.parishCode ?? "",
      phone: loc.phone ?? "",
      email: loc.email ?? "",
      otherDescription: loc.otherDescription ?? "",
      isPrimary: false,
    });
    setActionError(null);
    await loadProvinces();
  ...
  const saveLoc = locForm.handleSubmit(async (values) => {
    ...
    try {
      const body = {
        ...
        parishCode: values.parishCode || null,
      ...
      if (locModal?.mode === "edit" && locModal.data) {
        ...
        await businessPartnerFacade.updateLocation(
          id,
          locModal.data.id,
  ...
  const conForm = useForm<ContactFormValues>({
    ...
    defaultValues: {
      firstName: "",
      role: ContactRoleEnum.Commercial,
      isPrimary: false,
    },
  });
  ...
  const openConEdit = (con: BpContactDto) => {
    const roleVal =
      ContactRoleEnum[con.contactRole as keyof typeof ContactRoleEnum] ??
      ContactRoleEnum.Other;
    conForm.reset({
      ...
      notes: con.notes ?? "",
      isPrimary: false,
      otherDescription: con.otherDescription ?? "",
    });
    setActionError(null);
  ...
  const saveCon = conForm.handleSubmit(async (values) => {
    ...
    try {
      ...
      if (conModal?.mode === "edit" && conModal.data) {
        const { isPrimary: _, ...updateBody } = body;
        await businessPartnerFacade.updateContact(
          id,
          conModal.data.id,
          updateBody as UpdateContactBody,
        );
        message.success("Contacto actualizado correctamente.");
      } else {
        await businessPartnerFacade.createContact(
          id,
      ...
      const applied = applyServerErrors(err, conForm.setError, (msg) =>
        setActionError(msg),
      );
            ...
            <section className="md-detail-section">
              {bp.roles.length === 0 ? (
                <p className="md-detail-empty-hint">
                  Este BP no tiene roles asignados.
                </p>
              ) : (
                <>
                    ...
                    <div className="md-roles-grid">
                      {activeRoles.map((r) => (
                        <div key={r.id} className="md-role-card">
                          ...
                          {r.notes && (
                            <p className="md-role-card__notes">{r.notes}</p>
                          )}
                          {r.customerConfig && (
                            <div className="md-role-card__config md-role-card__config--crm">
                              ...
                              {r.customerConfig.customerSegment && (
                                <small>
                                  Segmento: {r.customerConfig.customerSegment}
                                </small>
                              )}
                              {r.customerConfig.salesZone && (
                                <small>
                                  Zona: {r.customerConfig.salesZone}
                                </small>
                              )}
                              {r.customerConfig.creditRating && (
                                <small>
                                  Rating: {r.customerConfig.creditRating}
                                </small>
                              )}
                  ...
                  {filteredLocs.length === 0 ? (
                    <p className="md-detail-empty-hint">
                      {locations.length === 0
                        ...
                        : `No hay ubicaciones ${locFilter === "active" ? "activas" : "inactivas"}.`}
                    </p>
                  ) : (
        ...
        <div className="pg-section-body">
          <ZHGrid cols={2}>
            ...
            <ZHField
              label="Tipo *"
              required
              error={locForm.formState.errors.type?.message}
            >
              <select
                disabled={saving}
                {...locForm.register("type", { valueAsNumber: true })}
              >
          ...
          <ZHGrid cols={3}>
            ...
            >
              <ZhSelect
                ...
                onChange={(e) => {
                  locForm.setValue("provinceCode", e.target.value);
                  locForm.setValue("cantonCode", "");
                  locForm.setValue("parishCode", "");
            ...
            >
              <ZhSelect
                ...
                onChange={(e) => {
                  locForm.setValue("cantonCode", e.target.value);
                  locForm.setValue("parishCode", "");
                ...
                <label key={pf.flag} className="zh-checkbox-label">
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      locForm.setValue(
                        "purpose",
                        e.target.checked ? cur | pf.flag : cur & ~pf.flag,
                      );
                    }}
                    disabled={saving}
                  />
        ...
        <div className="pg-section-body">
          ...
          <ZHGrid cols={2}>
            <ZHField
              label="Rol *"
              required
              error={conForm.formState.errors.role?.message}
            >
              <select
                disabled={saving}
                {...conForm.register("role", { valueAsNumber: true })}
              >
                {CONTACT_ROLES.map((r) => (
