...
function CustomerConfigModal({
  ...
  return (
    <ZHModal open onClose={onClose} title="Perfil Cliente — CRM" subtitle={bpName}>
      <form onSubmit={handleSave}>
        ...
        <ZHGrid cols={2}>
          ...
          <ZHField label="Nivel de fidelización">
            <select value={loyalty} onChange={(e) => setLoyalty(e.target.value)} disabled={saving}>
              <option value="">— Sin nivel —</option>
              {LOYALTY_TIERS.map((v) => <option key={v} value={v}>{v}</option>)}
            </select>
          </ZHField>
          <ZHField label="Formato de factura">
            <select value={invoiceFormat} onChange={(e) => setInvoiceFormat(e.target.value)} disabled={saving}>
              <option value="">— Sin preferencia —</option>
              {INVOICE_FORMATS.map((v) => <option key={v} value={v}>{v}</option>)}
            </select>
          </ZHField>
          <ZHField label="Clasificación comercial">
            <select value={classification} onChange={(e) => setClassification(e.target.value)} disabled={saving}>
...
export function MasterDataCustomersPage() {
  ...
  const handleCreate = useCallback(
    async (body: CreateBusinessPartnerBody): Promise<void> => {
      ...
      message.success(t('masterdata.customers.created.success', 'Cliente creado correctamente.'));
      setActiveTab('listado');
    },
  ...
  const handleUpdate = useCallback(
    async (body: UpdateBusinessPartnerBody): Promise<void> => {
      ...
      message.success(t('masterdata.customers.updated.success', 'Cliente actualizado correctamente.'));
      cancelEdit();
    },
  ...
  const handleAssign = useCallback(
    async (id: string): Promise<void> => {
      ...
      message.success(t('masterdata.customers.assigned.success', 'Rol de cliente asignado correctamente.'));
      setActiveTab('listado');
    },
    [page, addActivity, setActiveTab, t],
  );
  ...
  const handleDisable = useCallback(
    async (id: string) => {
      ...
      const bp = page.customers.find((c) => c.id === id);
      if (bp) { addActivity(bp.legalName, 'disabled'); message.info('Cliente desactivado.'); }
    },
    [page, addActivity],
  );
  ...
  const handleActivate = useCallback(
    async (id: string) => {
      ...
      const bp = page.customers.find((c) => c.id === id);
      if (bp) { addActivity(bp.legalName, 'enabled'); message.info('Cliente activado.'); }
    },
    [page, addActivity],
  );
