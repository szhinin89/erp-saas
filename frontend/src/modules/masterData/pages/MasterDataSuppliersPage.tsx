...
import { ZHField, ZHGrid, ZHToggle, ZHFormActions } from '../../../components/zh/ZHForm';
    ...
    >
        <form onSubmit={(e) => {
          ...
          onSave({
            ...
            <ZHGrid cols={2}>
              ...
              <ZHField label="Condición de pago *" required>
                <select value={paymentTermId} onChange={(e) => setPaymentTermId(e.target.value)} disabled={saving} required>
                  <option value="">— Seleccionar condición de pago —</option>
                  {paymentTermsList.filter(pt => pt.isActive).map(pt => (
                    <option key={pt.id} value={pt.id}>{pt.code} — {pt.name} ({pt.summary})</option>
                  ))}
                </select>
                {selectedPt && (
                  <div style={{ marginTop: 6, fontSize: 12, color: 'var(--color-text-secondary)' }}>
                    Cuotas: <strong>{selectedPt.installments}</strong> · Dias entre cuotas: <strong>{selectedPt.daysBetweenInstallments}</strong> · Total: <strong>{selectedPt.totalDays} dias</strong>
                  </div>
                )}
              </ZHField>
              <ZHField label="Método de pago SRI">
                <select value={paymentMethodCode}
              ...
              <ZHField label="Tipo de Proveedor">
                <select value={refundProviderType}
                  onChange={(e) => setRefundProviderType(e.target.value)} disabled={saving}>
          ...
          <ZHFormActions
            ...
            disableSave={saving}
            labels={{ cancel: 'Cerrar', save: saving ? 'Guardando...' : 'Guardar' }}
          />
        </form>
    </ZHModal>
  );
          ...
          onSave({
            ...
            <ZHGrid cols={2}>
              ...
              <ZHField label="Riesgo">
                <select value={risk} onChange={(e) => setRisk(e.target.value)} disabled={saving}>
                  <option value="">— Sin asignar —</option>
                  {SUPPLIER_RISKS.map((v) => <option key={v} value={v}>{v}</option>)}
                </select>
              </ZHField>
              <ZHField label="Rating">
                <select value={rating} onChange={(e) => setRating(e.target.value)} disabled={saving}>
                  <option value="">— Sin calificar —</option>
                  {SUPPLIER_RATINGS.map((v) => <option key={v} value={v}>{v}</option>)}
                </select>
              </ZHField>
              <ZHField label="Tipo de bien">
                <select value={goodType} onChange={(e) => setGoodType(e.target.value)} disabled={saving}>
                  <option value="">— Sin asignar —</option>
                  {SUPPLIER_GOOD_TYPES.map((v) => <option key={v} value={v}>{v}</option>)}
                </select>
              </ZHField>
              <ZHField label="Segmento estratégico">
                <select value={segment} onChange={(e) => setSegment(e.target.value)} disabled={saving}>
                  <option value="">— Sin asignar —</option>
...
export function MasterDataSuppliersPage() {
  ...
  const handleCreate = useCallback(
    async (body: CreateBusinessPartnerBody, supplierConfig?: { refundProviderTypeCode: string; paymentTermId: string }): Promise<void> => {
  ...
  const handleDisable = useCallback(
    async (id: string) => {
      ...
      const bp = page.suppliers.find((s) => s.id === id);
      if (bp) { addActivity(bp.legalName, 'disabled'); message.info('Proveedor desactivado.'); }
    },
    [page, addActivity],
  );
  ...
  const handleActivate = useCallback(
    async (id: string) => {
      ...
      const bp = page.suppliers.find((s) => s.id === id);
      if (bp) { addActivity(bp.legalName, 'enabled'); message.info('Proveedor activado.'); }
    },
    [page, addActivity],
  );
