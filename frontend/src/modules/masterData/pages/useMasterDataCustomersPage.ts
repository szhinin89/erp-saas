...
export function useMasterDataCustomersPage() {
  ...
  const createCustomer = async (body: CreateBusinessPartnerBody): Promise<void> => {
    setSaving(true);
    clearModalError();
    try {
  ...
  const updateCustomer = async (id: string, body: UpdateBusinessPartnerBody): Promise<void> => {
    setSaving(true);
    clearModalError();
    try {
