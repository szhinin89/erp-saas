...
interface Props {
  ...
  onSubmitCreate:  (body: CreateBusinessPartnerBody, supplierConfig?: SupplierConfigAtCreation) => Promise<void>;
...
export function MasterDataPartnerWizard({
  role, draftKey, submitting, editingPartner,
  ...
  const form = useForm<BusinessPartnerFormValues>({
    ...
    defaultValues: {
      ...
      personType:           PersonTypeEnum[editingPartner?.personType as keyof typeof PersonTypeEnum] ?? PersonTypeEnum.Legal,
  ...
  const prefill = (q: string) => {
    const s = q.trim();
    if (/^\d+$/.test(s)) form.setValue('identificationNumber', s, { shouldValidate: false });
  ...
  const handleAssign = async (bp: BusinessPartnerSummaryDto) => {
    ...
    try {
      ...
      setSearchError(formatApiRequestError(err, { generic: 'Error al asignar el rol.' }));
    } finally {
      setAssigning(null);
    }
  };
  ...
  const onValidSubmit = async () => {
    ...
    try {
      if (isEdit) {
        ...
        const v = form.getValues();
        const supplierConfig = role === 'supplier'
          ? { refundProviderTypeCode: v.refundProviderTypeCode || '', paymentTermId: v.paymentTermId || '' }
          : undefined;
        await onSubmitCreate(buildCreateBody(), supplierConfig);
        clearDraft(draftKey);
      }
    } catch (err) {
      const applied = applyServerErrors(err, form.setError, (msg) => setBannerError(msg));
      if (!applied) setBannerError(formatApiRequestError(err, { generic: 'Error al guardar. Revisa los datos.' }));
            ...
            return (
              <button key={s.id} type="button"
                ...
                <span className="prd-wiz-step__num">
                  {done ? <span className="material-symbols-outlined zh-icon-md">check</span> : s.id}
                </span>
          ...
          <div className="prd-draft-banner">
            ...
            <ZHBtn type="button" variant="ghost" size="sm"
              onClick={() => {
                ...
                if (d) {
                  form.setValue('identificationNumber', d.identificationNumber, { shouldValidate: false });
                  form.setValue('legalName', d.legalName, { shouldValidate: false });
                  setDraftBanner(null);
                  setStep(2);
                }
              }}>
            ...
            <ZHBtn type="button" variant="ghost" size="sm"
              onClick={() => { clearDraft(draftKey); setDraftBanner(null); }}>Descartar</ZHBtn>
          </div>
        )}

        {bannerError && (
          <div className="prd-wiz-error-banner" role="alert">
            <span className="material-symbols-outlined">error</span> {bannerError}
          </div>
        )}

        <form className="prd-wiz-form" onSubmit={form.handleSubmit(onValidSubmit)}>
            ...
            <div className="prd-wiz-panel">
              ...
              <div className="prd-search-box zh-mb-16">
                ...
                <input ref={queryRef} className="prd-search-input"
                  ...
                  disabled={searching} />
                <ZHBtn type="button" variant="primary" size="md"
                  ...
                  {results.map((bp) => {
                    ...
                    return (
                      <li key={bp.id} className="md-search-result-item">
                        ...
                        <ZHBtn type="button" variant="primary" size="sm"
          ...
          {step === 2 && (
            <div className="prd-wiz-panel">
              ...
              <MasterDataBpFormFields section="identity" saving={submitting} usage={role} />
            </div>
          )}
          ...
          {step === 3 && (
            <div className="prd-wiz-panel">
              ...
              <MasterDataBpFormFields section="review" saving={submitting} usage={role} />
              <div className="prd-review-warning" role="note">
                <span className="material-symbols-outlined zh-icon-lg">info</span>
          ...
          <div className="prd-wiz-footer">
            <div className="prd-wiz-footer__left">
              {((!isEdit && step > 1) || (isEdit && step > 2)) && (
                <ZHBtn type="button" variant="ghost" size="md" onClick={goPrev}>
                  <span className="material-symbols-outlined zh-icon-md">arrow_back</span>
                  Anterior
                </ZHBtn>
              )}
            </div>
            <div className="prd-wiz-footer__right">
              ...
              {!isEdit && step === 1 && (
                <ZHBtn type="button" variant="secondary" size="md"
                  onClick={() => { prefill(query); setStep(2); }}>
                  + Crear sin buscar
                </ZHBtn>
              )}
              {step === 2 && (
                <ZHBtn type="button" variant="ghost" size="md"
                  onClick={() => {
                    const v = form.getValues();
                    saveDraft(draftKey, v.identificationNumber, v.legalName);
                    setDraftBanner(loadDraft(draftKey));
                  }}>
                  <span className="material-symbols-outlined zh-icon-md">save</span> Guardar borrador
                </ZHBtn>
              )}
              {step < 3 ? (
                <ZHBtn type="button" variant="primary" size="md"
                  ...
                  Siguiente <span className="material-symbols-outlined zh-icon-md">arrow_forward</span>
                </ZHBtn>
              ) : (
                <ZHBtn type="submit" variant="secondary" size="md" className="prd-btn--success" disabled={submitting}>
                  {submitting ? (
                    <><span className="prd-spinner" aria-hidden /> Guardando…</>
                  ) : (
                    <><span className="material-symbols-outlined zh-icon-md">check</span>
