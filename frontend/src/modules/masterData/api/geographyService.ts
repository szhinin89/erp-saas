...
export const geographyService = {
  ...
  provinces: (countryId: string) =>
    apiGet<unknown>(
      `${BASE}/provinces?countryId=${encodeURIComponent(countryId)}`,
    ).then(normalize),
  cantons: (provinceId: string) =>
    apiGet<unknown>(
      `${BASE}/cantons?provinceId=${encodeURIComponent(provinceId)}`,
    ).then(normalize),
  parishes: (cantonId: string) =>
    apiGet<unknown>(
      `${BASE}/parishes?cantonId=${encodeURIComponent(cantonId)}`,
    ).then(normalize),
};
