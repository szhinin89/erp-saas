...
const BASE = '/api/master/business-partners';
...
export const bpLocationService = {
  /** GET /{bpId}/locations */
  list: (bpId: string, onlyActive = true): Promise<BpLocationDto[]> =>
    apiGet<BpLocationDto[]>(`${BASE}/${enc(bpId)}/locations?onlyActive=${onlyActive}`),
...
export const bpContactService = {
  /** GET /{bpId}/contacts */
  list: (bpId: string, onlyActive = true): Promise<BpContactDto[]> =>
    apiGet<BpContactDto[]>(`${BASE}/${enc(bpId)}/contacts?onlyActive=${onlyActive}`),
...
export const bpTradingSettingsService = {
  ...
  get: (bpId: string): Promise<CompanyBpTradingSettingsDto | null> =>
    apiGet<CompanyBpTradingSettingsDto | null>(`${BASE}/${enc(bpId)}/trading-settings`),
