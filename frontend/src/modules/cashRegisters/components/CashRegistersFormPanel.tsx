        ...
        <div className="cfg-panel-body">
          ...
          <div className="pg-section">
            ...
            <div className="pg-section-body">
              <ZHGrid cols={2}>
                <ZHField label="Sucursal" required error={errors.branchId?.message}>
                  <select
                    className="zh-input"
          ...
          <div className="pg-section">
            <div className="pg-section-header">
              <div className="pg-section-header-left">
                ...
                <span className="pg-section-label">Configuración fiscal (SRI)</span>
              </div>
            </div>
            <div className="pg-section-body">
              <ZHGrid cols={2}>
                ...
                <ZHField label="Punto de emisión" required error={errors.emissionPointId?.message}>
                  <select
                    className="zh-input"
                    disabled={editingHasHistory || saving || loadingEmissionPoints || emissionPoints.length === 0}
                    title={editingHasHistory ? 'Caja con historial operativo: el punto de emisión no puede reasignarse.' : undefined}
              ...
              <ZHField label="Notas" error={errors.notes?.message}>
                <textarea className="zh-input" rows={2} disabled={saving} {...register('notes')} />
              </ZHField>
            </div>
          </div>
          ...
          <div className="pg-section">
            <div className="pg-section-header">
              <div className="pg-section-header-left">
                ...
                <span className="pg-section-label">Configuración operativa</span>
              </div>
            </div>
            <div className="pg-section-body">
              <ZHGrid cols={2}>
                <ZHField label="Bodega por defecto" error={errors.defaultWarehouseId?.message}>
                  <select
                    className="zh-input"
                  ...
                  >
                    <option value="">
                      {loadingWarehouses ? 'Cargando…' : '— sin bodega por defecto —'}
                    </option>
                    {warehouses.map((w) => (
                ...
                <ZHField label="Cliente por defecto" error={errors.defaultCustomerId?.message}>
                  <Controller
                    ...
                    render={({ field }) => (
                      <CustomerPicker
                        ...
                        onChange={(c) => setValue('defaultCustomerId', c?.id ?? '')}
                        disabled={saving}
                      />
                    )}
                  />
        ...
        <div className="pg-actions-bar">
          <div className="pg-actions-buttons">
            ...
            <ZHBtn variant="primary" type="submit" disabled={saving}>
              {saving ? 'Guardando…' : isEdit ? 'Guardar cambios' : 'Crear caja'}
            </ZHBtn>
          </div>
        </div>
      </form>
