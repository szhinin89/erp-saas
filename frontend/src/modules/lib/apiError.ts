...
function pickArrayString(record: Record<string, unknown>, ...keys: string[]): string | null {
...
function pickNestedString(o: Record<string, unknown>, container: string, field: string): string | null {
...
function pickNestedErrors(o: Record<string, unknown>, container: string, field: string): string | null {
  const nested = o[container];
  if (nested && typeof nested === 'object' && !Array.isArray(nested)) {
...
function pickNestedErrorMap(o: Record<string, unknown>, container: string, field: string): string | null {
  const nested = o[container];
...
function messageFromRecord(o: Record<string, unknown>): string | null {
  ...
  const dataErrors =
    pickNestedErrors(o, 'data', 'errors') ?? pickNestedErrors(o, 'Data', 'Errors')
    ?? pickNestedErrorMap(o, 'data', 'errors') ?? pickNestedErrorMap(o, 'Data', 'Errors');
  if (dataErrors) return dataErrors;

  // 2. message.user (o Message.User) — mensaje genérico del catálogo por code.
  const catalogMessage =
    pickNestedString(o, 'message', 'user') ?? pickNestedString(o, 'Message', 'User');
