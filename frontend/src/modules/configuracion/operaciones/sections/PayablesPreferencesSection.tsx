...
import { usePayablesPreferencesSection } from "./usePayablesPreferencesSection";

/** ZH-SUPPLIER-PAYMENT-UNAPPLIED-ADVANCE-02C — mismo layout que PurchasesPreferencesSection. */
export function PayablesPreferencesSection() {
  const { t } = useI18n();
  const page = usePayablesPreferencesSection();

  if (!page.canView)
    return <NoAccessPage title={t("settings.operations.payables.title")} />;
      ...
      <div className="pg-section">
        <div className="pg-section-header">
          <div className="pg-section-header-left">
            ...
            <p className="pg-section-label">{t("settings.operations.payables.title")}</p>
          </div>
        </div>
        <div className="pg-section-body">
          <ZHToggle
            label={t("settings.operations.payables.allowSupplierPaymentWithoutPayableLabel")}
            description={t("settings.operations.payables.allowSupplierPaymentWithoutPayableDesc")}
            value={page.allowSupplierPaymentWithoutPayableValue}
            onChange={(next) =>
              page.setValue("allowSupplierPaymentWithoutPayable", next, { shouldDirty: true })
            }
