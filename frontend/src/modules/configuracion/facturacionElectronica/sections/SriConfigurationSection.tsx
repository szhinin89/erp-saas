...
export function SriConfigurationSection() {
  ...
  if (!page.canView) return <NoAccessPage title={t('settings.electronicInvoicing.title')} />;
  ...
  return (
    <>
      ...
      {page.sriState.error && (
        <ZHPageNotice variant="error" message={t('settings.electronicInvoicing.sri.loadError')} detail={page.sriState.error} />
      )}
      {page.saveError && (
        <ZHPageNotice variant="error" message={t('settings.electronicInvoicing.sri.saveErrorTitle')} detail={page.saveError} />
      )}
      {page.saved && <ZHPageNotice variant="success" message={t('settings.electronicInvoicing.sri.saved')} />}
      {page.validationError && (
        <ZHPageNotice variant="error" message={t('settings.electronicInvoicing.sri.validateErrorTitle')} detail={page.validationError} />
      )}
        ...
        <div className="pg-section sri-section-mb">
          <div className="pg-section-header">
            <div className="pg-section-header-left">
              <span className="material-symbols-outlined pg-section-icon">fact_check</span>
              <p className="pg-section-label">{t('settings.electronicInvoicing.sri.validationResultsTitle')}</p>
            </div>
          </div>
          <div className="pg-section-body">
            <ZHPageNotice
              variant={page.validationResult.isValid ? 'success' : 'warning'}
              message={page.validationResult.isValid
              ...
              <dl className="sri-cert-info">
                {page.validationResult.certificate.subject && (
                  <>
                    ...
                    <dd className="mono">{page.validationResult.certificate.subject}</dd>
                  </>
                )}
                {page.validationResult.certificate.issuer && (
                  <>
                    <dt>{t('settings.electronicInvoicing.sri.certIssuer')}</dt>
                    <dd className="mono">{page.validationResult.certificate.issuer}</dd>
                  </>
                )}
                {page.validationResult.certificate.daysRemaining !== null && (
                  <>
                    <dt>{t('settings.electronicInvoicing.sri.certDaysRemaining')}</dt>
        ...
        <div className="pg-actions-bar">
          ...
          <div className="pg-actions-buttons">
            ...
            >
              ...
              {page.validating ? t('settings.electronicInvoicing.sri.validating') : t('settings.electronicInvoicing.sri.validateButton')}
            </ZHBtn>
            ...
            <ZHBtn variant="primary" size="md" type="submit" disabled={page.saving || !page.canEdit || !page.isDirty}>
              ...
              {page.saving ? t('common.saving') : t('settings.electronicInvoicing.sri.saveButton')}
            </ZHBtn>
          </div>
        </div>
      </form>
