...
import { SRI_WSDL_DEFAULTS, type SriConfigValues } from '../schemas/sriConfigurationSchema';
import type { SriCertificateInfoDto, SriConfigurationDto } from '../api/electronicInvoicingService';
      ...
      <div className="pg-section sri-section-mb">
        <div className="pg-section-header">
          <div className="pg-section-header-left">
            ...
            <p className="pg-section-label">{t('settings.electronicInvoicing.sri.sectionEnvironment')}</p>
          </div>
        </div>
        <div className="pg-section-body">
          <ZHPageNotice variant="warning" message={t('settings.electronicInvoicing.sri.envWarning')} />
      ...
      <div className="pg-section sri-section-mb">
        <div className="pg-section-header">
          <div className="pg-section-header-left">
            ...
            <p className="pg-section-label">{t('settings.electronicInvoicing.sri.sectionCertificate')}</p>
          </div>
        </div>
        <div className="pg-section-body">
          <ZHPageNotice variant="info" message={t('settings.electronicInvoicing.sri.certInfo')} />
          <p className="sri-hint-sm">{t('settings.electronicInvoicing.sri.certPasswordHint')}</p>

          {!hasExistingConfig ? (
            <div className="sri-block-mt">
              <ZHPageNotice variant="warning" message={t('settings.electronicInvoicing.sri.certRequiresConfigSaved')} />
            </div>
          ) : (
            <div className="sri-block-mt">
              <ZhFileUpload
                ...
                currentFile={sriConfig?.certFileName ? {
                  name: sriConfig.certFileName,
                  sizeBytes: sriConfig.certSizeBytes ?? 0,
                  uploadedAt: sriConfig.certUploadedAtUtc ? formatDateTime(sriConfig.certUploadedAtUtc) : '',
                } : null}
                selectLabel={t('settings.electronicInvoicing.sri.certSelectFile')}
                dropLabel={t('settings.electronicInvoicing.sri.certDropHint')}
                uploadingLabel={t('settings.electronicInvoicing.sri.certUploading')}
              ...
              <div className="sri-pass-wrap">
                <input
                  ...
                  placeholder={hasExistingConfig
                    ? t('settings.electronicInvoicing.sri.certPasswordPlaceholderKeep')
                    : t('settings.electronicInvoicing.sri.certPasswordPlaceholderNew')}
                  disabled={saving || !canEdit}
          ...
          {certInspecting && (
            <p className="sri-hint-sm">{t('settings.electronicInvoicing.sri.certInspecting')}</p>
          )}

          {!certInspecting && certInspection && (
            <div className="sri-block-mt">
              <ul className="sri-validation-list">
                <li className={`sri-validation-item ${certInspection.passwordCorrect ? 'sri-validation-item--pass' : 'sri-validation-item--fail'}`}>
                  ...
                  <span>
                    {certInspection.passwordCorrect
                      ...
                      : (certInspection.errorMessage ?? t('settings.electronicInvoicing.sri.certPasswordWrong'))}
                  </span>
                </li>
              </ul>
              {certInspection.passwordCorrect && (
                <dl className="sri-cert-info">
                  ...
                  {certInspection.notAfterUtc && (
                    <>
                      <dt>{t('settings.electronicInvoicing.sri.certExpiresAt')}</dt>
                      <dd>{formatDateTime(certInspection.notAfterUtc)}{certInspection.daysRemaining !== null ? ` (${certInspection.daysRemaining} ${t('settings.electronicInvoicing.sri.certDaysRemaining')})` : ''}</dd>
                    </>
                  )}
                </dl>
              )}
      ...
      <div className="pg-section sri-section-mb">
        <div className="pg-section-header">
          <div className="pg-section-header-left">
            ...
            <p className="pg-section-label">{t('settings.electronicInvoicing.sri.sectionWsdl')}</p>
          </div>
        </div>
        <div className="pg-section-body">
          <ZHGrid cols={1}>
            <ZHField label={t('settings.electronicInvoicing.sri.wsdlUrl')} required error={errors.wsdlUrl?.message}>
          ...
          <div className="sri-wsdl-hint">
            <strong>{t('settings.electronicInvoicing.sri.environmentTest')}:</strong> <span className="mono">{SRI_WSDL_DEFAULTS.pruebas}</span>
            <br />
            <strong>{t('settings.electronicInvoicing.sri.environmentProd')}:</strong> <span className="mono">{SRI_WSDL_DEFAULTS.produccion}</span>
          </div>
        </div>
      </div>
    </>
