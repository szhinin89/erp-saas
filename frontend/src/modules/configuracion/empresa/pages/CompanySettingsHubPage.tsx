...
import { companySettingsTabs, type CompanySettingsTabId } from '../companySettingsTabs';
...
function parseTab(raw: string | null): CompanySettingsTabId {
  if (raw && (VALID_TAB_IDS as string[]).includes(raw)) return raw as CompanySettingsTabId;
  return DEFAULT_TAB;
}
...
export function CompanySettingsHubPage() {
  ...
  const activeTab = companySettingsTabs.find((tab) => tab.id === activeTabId) ?? companySettingsTabs[0];
