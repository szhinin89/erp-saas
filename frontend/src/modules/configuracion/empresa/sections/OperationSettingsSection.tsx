...
export function OperationSettingsSection() {
  ...
  const { register, handleSubmit, reset, setError: setFieldError, formState: { isDirty } } = useForm<CompanyOperationValues>({
  ...
  useEffect(() => {
    ...
    if (!profile) return;
    reset({ languageCode: (profile.languageCode as CompanyOperationValues['languageCode']) ?? 'es' });
  ...
  const onSubmit = handleSubmit(async (values) => {
    ...
    setSaving(true);
    try {
      await companyProfileService.updateOperation({ languageCode: values.languageCode });
      setSaved(true);
    ...
    <>
      ...
      <div className="pg-section">
        <div className="pg-section-header">
          <div className="pg-section-header-left">
            ...
            <p className="pg-section-label">{t('settings.company.operation.title')}</p>
          </div>
        </div>
          ...
          <ZHGrid cols={2}>
            <ZHField label={t('settings.company.operation.companyStatus')}>
              <span className={`pg-kpi-badge ${profile?.isActive ? 'pg-kpi-badge--success' : 'pg-kpi-badge--neutral'}`}>
                {profile?.isActive ? t('settings.company.operation.statusActive') : t('settings.company.operation.statusInactive')}
              </span>
            </ZHField>

            <ZHField label={t('settings.company.operation.onboardingStatus')}>
              <span className={`pg-kpi-badge ${profile?.onboardingCompleted ? 'pg-kpi-badge--success' : 'pg-kpi-badge--neutral'}`}>
                {profile?.onboardingCompleted ? t('settings.company.operation.onboardingDone') : t('settings.company.operation.onboardingPending')}
              </span>
            </ZHField>
          </ZHGrid>
        </div>
      ...
      <form onSubmit={onSubmit}>
        <div className="pg-section">
          <div className="pg-section-header">
            <div className="pg-section-header-left">
              <span className="material-symbols-outlined pg-section-icon">translate</span>
              <p className="pg-section-label">{t('settings.company.operation.languageCode')}</p>
            </div>
          </div>
          <div className="pg-section-body">
            <ZHGrid cols={2}>
              <ZHField label={t('settings.company.operation.languageCode')}>
                <select disabled={saving || !canEdit} {...register('languageCode')}>
