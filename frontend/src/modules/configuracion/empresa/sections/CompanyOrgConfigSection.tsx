...
export function CompanyOrgConfigSection() {
  ...
  // ── Valores actuales ─────────────────────────────────────────────────────
  const settingsState = useAsync(() => orgConfigService.getCompanyInvoiceDefaults());

  const { register, handleSubmit, reset, setError: setFieldError, formState: { errors } } =
    useForm<FormValues>({
  ...
  useEffect(() => {
    ...
    reset({
      ...
      defaultSriPaymentMethodCode: settingsState.data.defaultSriPaymentMethodCode ?? null,
  ...
  const onSubmit = handleSubmit(async (values) => {
    ...
    try {
      ...
      if (!applied)
        setSaveError(formatApiRequestError(err, { offline: t('common.apiUnreachable'), generic: t('common.errorGeneric') }));
    } finally {
      setSaving(false);
    }
  });
  ...
  const catalogsLoading =
    docTypesState.loading || paymentMethodsState.loading || paymentTermsState.loading;
  ...
  const paymentTerms   = (paymentTermsState.data  ?? []).filter(pt => pt.isActive);

  const s                  = settingsState.data;
  const docTypeLabel       = docTypes.find(d => d.code === s?.defaultDocTypeCode);
  const paymentMethodLabel = paymentMethods.find(p => p.code === s?.defaultSriPaymentMethodCode);
  const paymentTermLabel   = paymentTerms.find(pt => pt.id === s?.defaultPaymentTermId);
          ...
          <div className="pg-section-header-left">
            ...
            <div>
              ...
              <p className="pg-section-desc">
                Parámetros de factura de venta por defecto configurados a nivel empresa.
                Las sucursales administran su propia bodega por defecto desde <em>Editar Sucursal → Configuraciones</em>.
              </p>
            </div>
          </div>
          {canEdit && (
            <div className="pg-section-header-right">
              <ZHBtn variant="secondary" size="md" type="button" onClick={openModal}>
      ...
      <ZHModal
        ...
        footer={
          <div className="zh-modal-footer-actions">
            ...
            <ZHBtn variant="primary" size="md" type="submit" form="company-org-config-form" disabled={saving || !canEdit}>
        ...
        {catalogsLoading ? (
          ...
          <form id="company-org-config-form" onSubmit={onSubmit}>
            <ZHGrid cols={1}>
              ...
              <ZHField label="Tipo de Documento" error={errors.defaultDocTypeCode?.message}>
                <select disabled={saving || !canEdit} {...register('defaultDocTypeCode')}>
              ...
              <ZHField label="Forma de Pago SRI" error={errors.defaultSriPaymentMethodCode?.message}>
                <select disabled={saving || !canEdit} {...register('defaultSriPaymentMethodCode')}>
                  <option value="">— Sin configurar —</option>
              ...
              <ZHField label="Condición de Pago" error={errors.defaultPaymentTermId?.message}>
                <select disabled={saving || !canEdit} {...register('defaultPaymentTermId')}>
                  <option value="">— Sin configurar —</option>
                  {paymentTerms.map(pt => (
