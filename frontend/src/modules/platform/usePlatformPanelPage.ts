...
export type UsePlatformPanelPageParams = {
...
export function usePlatformPanelPage({ embeddedTab, shellLayout }: UsePlatformPanelPageParams = {}) {
  ...
  const homeTab: PlatformHomeTab = embeddedTab
    ? embeddedTab
  ...
  const [subModalSubscriber, setSubModalSubscriber] = useState<PlatformSubscriber | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const list = await platformService.getPlansCatalog();
  ...
  const openSubscriptionModal = (subscriber: PlatformSubscriber) => {
  ...
  const saveCreateSubscriber = async () => {
    ...
    try {
      ...
      const session: SessionResponse = await platformService.createSubscriberWithAdmin({
        ...createForm,
        subscriberName,
        subscriberSlug,
        adminEmail,
  ...
  const saveSubscriptionModal = async () => {
    ...
    try {
      ...
      await platformService.updateSubscriberSubscription(subModalSubscriber.id, {
        planCode,
...
export type PlatformPanelPageState = ReturnType<typeof usePlatformPanelPage>;
