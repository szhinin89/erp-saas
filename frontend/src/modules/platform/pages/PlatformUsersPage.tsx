...
import { platformService, type PlatformUserRow } from '../api/platformService';
...
export function PlatformUsersPage() {
  ...
  const load = useCallback(() => {
    setLoading(true);
    setError(null);
    Promise.all([platformService.getPlatformUsers(), platformService.getPlatformImpersonationHistory()])
      .then(([page, events]) => {
        setUsers(page?.users ?? []);
  ...
  const revokePlatformSessions = async (userId: string) => {
    setBusyId(userId);
    try {
      await platformService.revokePlatformUserSessions(userId, '00000000-0000-0000-0000-000000000000');
      load();
    } catch {
  ...
  return (
    <PlatformCrudTemplate title="Platform users" subtitle={`Operadores globales · activos: ${activeCount}`}>
