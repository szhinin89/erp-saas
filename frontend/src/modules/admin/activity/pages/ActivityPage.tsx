...
function actionVerbI18nKey(action: string): string {
  ...
  switch (verb) {
    case 'create':  return 'audit.action.create';
    case 'update':  return 'audit.action.update';
    case 'enable':  return 'audit.action.enable';
    case 'disable': return 'audit.action.disable';
    default:        return 'audit.action.unknown';
  }
}
...
export function ActivityPage() {
  ...
  if (!canView) return <NoAccessPage title={t('app.nav.item.admin.activity')} />;
    ...
    >
      ...
      <div className="pg-section">
        ...
        {loading ? (
          ...
          <div className="pg-pad-40"><EmptyState message={t('audit.empty')} /></div>
        ) : (
        ...
        <div className="pg-table-footer">
          <div className="pg-pagination-controls">
            <button className="pg-pagination-btn" type="button" disabled={page <= 1 || loading}
              onClick={() => setPage((p) => p - 1)}>
              <span className="material-symbols-outlined">chevron_left</span>
            </button>
            <button className="pg-pagination-btn" type="button" disabled={rows.length < pageSize || loading}
              onClick={() => setPage((p) => p + 1)}>
