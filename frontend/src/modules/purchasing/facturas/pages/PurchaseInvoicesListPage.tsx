...
import { purchaseInvoicesService, type PurchaseInvoiceDto, type PurchaseInvoiceStatus } from '../api/purchaseInvoicesService';
...
function estadoBadge(e: PurchaseInvoiceStatus): BadgeInfo {
...
export function ComprasListPage() {
  ...
  const stats = useMemo(() => ({
    total:     rows.length,
    aprobadas: rows.filter((r) => r.status === 2).length,
  ...
  const handleValidar = (id: string) => {
    setActionId(id);
    void doAction(() => purchaseInvoicesService.validate(id), 'Compra validada.');
  };
  const handleAprobar = (id: string) => {
    setActionId(id);
    void doAction(() => purchaseInvoicesService.approve(id), 'Compra aprobada. Se generó el asiento contable.');
  };
  const handleRechazar = async () => {
    if (!rejectId || !rejectText.trim()) return;
    setActionId(rejectId);
    await doAction(
      () => purchaseInvoicesService.reject(rejectId, rejectText.trim()),
      'Compra rechazada.',
    );
    setRejectId(null);
    setRejectText('');
