...
import { ZHField, ZHFormAlert, ZHFormSection, ZHGrid } from '../../../../components/zh/ZHForm';
...
import { itemService, type ItemPricingSimulationRow } from '../../api/itemService';
...
export function PricingTab({ t, disabled, isEditMode, itemId }: Props) {
  const { register, watch, formState: { errors } } = useFormContext<CreateItemFormValues>();
  ...
  const priceListsState = useAsync(() => priceListService.list(true).catch(() => []));
  const defaultList = (priceListsState.data ?? []).find(pl => pl.isDefault);

  const companyProfileState = useAsync(() => companyProfileService.getProfile().catch(() => null));
  const currencyCode = defaultList?.currencyCode ?? companyProfileState.data?.currencyCode ?? '';
  ...
  return (
    <>
      <ZHFormSection
        ...
        description={t('items.pricing.sectionDesc', 'Precio base (PVP) sin impuestos y el descuento máximo que se le puede aplicar en una venta.')}
      >
        {!isEditMode && (
          <ZHFormAlert
            type="info"
            message={t('items.pricing.hint', 'Podrás marcar a qué listas pertenece este ítem después de guardarlo.')}
          />
        )}

        <ZHGrid cols={2}>
          <ZHField
            label={t('items.pricing.pvp', `PVP — Precio base sin IVA (${currencyCode})`)}
          ...
          >
            <div className="items-currency-input">
              ...
              <ZhDecimalInput
                ...
                {...register('baseSalePrice', { valueAsNumber: true, setValueAs: v => v === '' ? null : Number(v) })}
                disabled={disabled}
          ...
          <ZHField label={t('items.pricing.maxDiscount', 'Descuento máximo permitido (%)')}>
            <ZhDecimalInput decimals={dc.percentage} positiveOnly placeholder={t('items.pricing.maxDiscountPlaceholder', '0')}
              {...register('saleConfig.maxDiscountPercent', { valueAsNumber: true, setValueAs: v => v === '' ? null : Number(v) })}
              disabled={disabled} />
          </ZHField>
        </ZHGrid>
      </ZHFormSection>
 ...
 */
function PriceListSimulationTable({ t, itemId, disabled, basePrice, maxDiscount }: {
  t: (key: string, fallback?: string) => string;
  itemId?: string;
  disabled: boolean;
  ...
  const handleToggle = async (row: ItemPricingSimulationRow) => {
    if (!itemId) return;
    setToggleError('');
    const currentlyAssigned = rows.filter(r => r.isAssigned).map(r => r.priceListId);
  ...
  return (
    <ZHFormSection
      title={t('items.pricing.simulation.title', 'Simulación de listas de precios')}
      description={itemId
        ? t('items.pricing.simulation.desc', 'Marca a qué listas pertenece el ítem. El precio de cada lista se recalcula automáticamente — no hay nada que guardar salvo la pertenencia.')
        : t('items.pricing.simulation.descPreview', 'Así quedaría el precio en cada lista activa. Podrás marcar a cuáles pertenece el ítem después de guardarlo.')}
    >
      {toggleError && <ZHFormAlert type="error" message={toggleError} />}
      {simState.error && <ZHFormAlert type="error" message={simState.error} />}

      {!hasBasePrice ? (
        <p>{t('items.pricing.simulation.needsPrice', 'Ingresa un precio base para ver la simulación.')}</p>
      ) : simState.loading && rows.length === 0 ? (
        <p>{t('common.loading', 'Cargando...')}</p>
      ) : rows.length === 0 ? (
        <p>{t('items.pricing.simulation.empty', 'No hay listas de precios activas y vigentes. Créalas desde el módulo Precios.')}</p>
      ) : (
          ...
          <table className="table">
            <thead>
              <tr>
                ...
                <th>{t('items.pricing.simulation.col.rule', 'Regla aplicada')}</th>
                <th>{t('items.pricing.simulation.col.net', 'Precio sin IVA')}</th>
                <th>{t('items.pricing.simulation.col.maxDiscount', 'Descuento máximo')}</th>
                <th>{t('items.pricing.simulation.col.minAllowed', 'Precio mínimo permitido')}</th>
              </tr>
            </thead>
            <tbody>
              {rows.map(row => (
                <tr key={row.priceListId}>
                  <td>
                    <input
                      ...
                      disabled={!itemId || disabled || pendingIds.has(row.priceListId)}
                  ...
                  <td>
                    ...
                    {row.ruleSummary.source === 'Exception' && (
                      <>
                        {' '}
                        <a href="/pricing" title="Administrar en el módulo Precios">
                          {t('items.pricing.simulation.viewException', 'Ver excepción')}
                        </a>
                      </>
                    )}
                  </td>
                  <td>{row.currencyCode} {formatMoney(row.netPrice, dc.salesUnitPrice)}</td>
                  <td>{row.maxDiscountPercent != null ? `${row.maxDiscountPercent}%` : '—'}</td>
                  <td>{row.currencyCode} {formatMoney(row.minAllowedPrice, dc.salesUnitPrice)}</td>
                </tr>
              ))}
            </tbody>
          </table>
