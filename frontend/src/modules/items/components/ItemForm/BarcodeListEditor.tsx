...
export function BarcodeListEditor({ t, disabled, barcodeTypeOptions }: Props) {
  ...
  const listError =
    typeof barcodesError?.message === "string" ? barcodesError.message : null;
    ...
    >
      ...
      {fields.map((field, index) => (
        <ZHGrid cols={3} key={field.id}>
          ...
          >
            <input
              {...register(`barcodes.${index}.code`)}
              placeholder={t("items.barcodes.codePlaceholder", "7501234567890")}
              disabled={disabled}
            />
          </ZHField>
          <ZHField
            ...
            required
            fieldError={
              errors.barcodes?.[index]?.barcodeType?.message
                ? t(
                    errors.barcodes[index]!.barcodeType!.message!,
                    errors.barcodes[index]!.barcodeType!.message!,
                  )
      ...
      <ZHBtn
        ...
        disabled={disabled}
        onClick={() =>
          append({ code: "", barcodeType: "", isPrimary: fields.length === 0 })
        }
      >
