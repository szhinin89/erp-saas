...
export function GeneralTab({ t, disabled, isEditMode, categoryOptions, brandOptions, sriUomOptions, itemTypeOptions }: Props) {
  ...
  return (
    <ZHFormSection
      ...
      description={t('items.section.identityDesc', 'Datos que identifican y clasifican el ítem: cómo se llama, de qué tipo es, a qué marca/categoría pertenece, y su unidad de medida base.')}
    >
      <ZHGrid cols={3}>
        <ZHField label={t('items.form.sku', 'SKU')} required fieldError={fe(errors.sku?.message)}>
          <input {...register('sku')} placeholder={t('items.form.skuPlaceholder', 'CAMISA-ROJA')} disabled={disabled} className="zh-input--upper" />
        </ZHField>
        <ZHField label={t('items.form.shortName', 'Nombre corto')} required fieldError={fe(errors.shortName?.message)}>
          <input {...register('shortName')} placeholder={t('items.form.shortNamePlaceholder', 'Camisa Roja L')} disabled={disabled} />
        </ZHField>
        <ZHField label={t('items.form.itemType', 'Tipo')} required fieldError={fe(errors.itemTypeId?.message)}>
      ...
      <ZHField label={t('items.form.description', 'Descripción')} required fieldError={fe(errors.description?.message)}>
        <input {...register('description')} placeholder={t('items.form.descriptionPlaceholder', 'Descripción completa del ítem')} disabled={disabled} />
      </ZHField>
      <ZHGrid cols={3}>
        <ZHField label={t('items.form.brand', 'Marca')} required={!isEditMode} fieldError={fe((errors as any).brandId?.message)}>
        ...
        <ZHField label={t('items.form.category', 'Categoría')} required={!isEditMode} fieldError={fe((errors as any).categoryNodeId?.message)}>
