...
export interface UpdateItemRequest extends Omit<CreateItemRequest,
...
export const itemService = {
  ...
  getById: (id: string) =>
    apiGet<ItemDetailDto>(`/api/v1/items/${id}`),
  ...
  updateVariant: (itemId: string, variantId: string, request: AddVariantRequest) =>
    apiPut<ItemVariantDto>(`/api/v1/items/${itemId}/variants/${variantId}`, request),
  ...
  replaceImages: (itemId: string, images: { storageObjectId: string; altText?: string | null; isMain?: boolean; isEcommerce?: boolean; sortOrder?: number; variantId?: string | null }[]) =>
  ...
  replaceUnitConversions: (itemId: string, conversions: { fromUomCode: string; toUomCode: string; factor: number }[]) =>
    apiPut<ItemDetailDto>(`/api/v1/items/${itemId}/unit-conversions`, { conversions }),

  replaceSubstitutes: (itemId: string, substitutes: { substituteItemId: string; priority: number; note?: string | null }[]) =>
    apiPut<ItemDetailDto>(`/api/v1/items/${itemId}/substitutes`, { substitutes }),

  replacePackagingLevels: (itemId: string, levels: { name: string; level: number; baseQuantity: number; uomCode: string; barcode?: string | null; weight?: number | null; isBaseUnit?: boolean; isPurchaseDefault?: boolean; isSaleDefault?: boolean }[]) =>
    apiPut<ItemDetailDto>(`/api/v1/items/${itemId}/packaging-levels`, { levels }),

  addBarcode: (itemId: string, variantId: string, code: string, barcodeType: string) =>
    apiPost<{ id: string; code: string; barcodeType: string }>(`/api/v1/items/${itemId}/variants/${variantId}/barcodes`, { code, barcodeType }),

  disableBarcode: (itemId: string, variantId: string, barcodeId: string) =>
    apiPatch<boolean>(`/api/v1/items/${itemId}/variants/${variantId}/barcodes/${barcodeId}/disable`),
  ...
  simulatePrice: (itemId: string, newPvp: number) =>
    apiPost<PriceSimulationDto>(`/api/v1/items/${itemId}/simulate-price`, { newPvp }),
   ...
   * No persiste nada.
   */
  getPricingSimulation: (itemId: string, overrides: PricingSimulationOverrides = {}) =>
    apiPost<ItemPricingSimulationRow[]>(`/api/v1/items/${itemId}/pricing-simulation`, overrides),
  ...
  previewPricingSimulation: (overrides: PricingSimulationOverrides) =>
    apiPost<ItemPricingSimulationRow[]>('/api/v1/items/pricing-simulation-preview', overrides),
};
...
export type MarginStatusCode = 'SALUDABLE' | 'BAJO' | 'NEGATIVO' | 'CERO' | 'SIN_PRECIO';
