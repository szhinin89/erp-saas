...
export function ItemsPage() {
  ...
  const handleSubmit = async (values: CreateItemFormValues): Promise<void> => {
    ...
    const flatPayload = { ...base, ...normalizedTaxConfig, ...saleConfig, ...stockConfig };
    ...
    if (editingItemId) {
      ...
      const { barcodes: _barcodes, supplierCodes: _supplierCodes, ...updatePayload } = flatPayload;
      await updateItem({ id: editingItemId, ...updatePayload } as UpdateItemRequest);

      message.success(t('items.updated.success', 'Ítem actualizado correctamente.'));
      cancelEdit();
      refetch();
      return;
    }
  ...
  return (
    ...
    >
      {anyError && (
        <ZHPageNotice variant="error" message={anyError} />
      )}

      <ZHTabBar
        tabs={TABS.map(tab => ({ id: tab.id, label: t(tab.labelKey, tab.labelFb), icon: tab.icon }))}
        activeTab={activeTab}
      ...
      <div className="prd-tab-content">
          ...
          <div className="prd-fadein">
            ...
            <div className="prd-filters-bar">
              ...
              <select
                ...
                onChange={e => setFilterIsActive(e.target.value === '' ? undefined : e.target.value === 'true')}
              >
              ...
              <select
                ...
                onChange={e => setFilterItemTypeId(e.target.value || undefined)}
              >
        ...
        {/* Nuevo / Editar / Ver */}
        {activeTab === 'nuevo' && (canCreate || (editingItemId && (canEdit || readOnly))) && (
