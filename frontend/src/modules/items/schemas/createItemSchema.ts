...
const barcodeSchema = z.object({
  code: z
    .string()
    .trim()
    .min(1, "items.validation.barcodeCodeRequired")
    .max(100, "items.validation.barcodeCodeMax"),
  barcodeType: z.string().trim().min(1, "items.validation.barcodeTypeRequired"),
  isPrimary: z.boolean().default(false),
});

const barcodesArraySchema = z
  .array(barcodeSchema)
  .min(1, "items.validation.barcodesRequired")
  .refine((list) => list.filter((b) => b.isPrimary).length === 1, {
    message: "items.validation.barcodePrimaryRequired",
  })
  .refine(
    (list) =>
      new Set(list.map((b) => b.code.trim().toUpperCase())).size ===
      list.length,
...
const supplierCodesArraySchema = z
  .array(supplierCodeSchema)
  .default([])
  .refine((list) => list.filter((s) => s.isPrimary).length <= 1, {
...
export const createItemSchema = z.object({
  ...
  sku: z
    .string()
    .trim()
    .min(1, "items.validation.skuRequired")
    .max(50, "items.validation.skuMax")
    .regex(/^[A-Za-z0-9\-_.]+$/, "items.validation.skuFormat"),
  shortName: z
    .string()
    .trim()
    .min(1, "items.validation.shortNameRequired")
    .max(50, "items.validation.shortNameMax"),
  description: z
    .string()
    .trim()
  ...
  defaultUomCode: z
    .string()
    .trim()
    .min(1, "items.validation.uomRequired")
    .max(10),
