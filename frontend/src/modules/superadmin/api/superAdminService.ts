...
export const superAdminService = {
  ...
  getMetrics: async (): Promise<SuperAdminMetrics> => {
    const [metricsRes, subsRes] = await Promise.all([
      ...
      api.get<ApiResponse<SuperAdminSubscriber[]>>(PLATFORM_API.subscribers),
    ]);
  ...
  getPlansCatalog: () =>
    api.get<ApiResponse<{ plans: SuperAdminPlan[] }>>(PLATFORM_API.plans)
  ...
  recordLegacyUiRoute: (routeKey: string, referrer?: string) =>
    api.post(`${PLATFORM_API.observability}/legacy-ui`, { routeKey, referrer }).catch(() => undefined),

  recordLegacyMasterDataUsage: (sourceKey: string, detail?: string) =>
    api.post(`${PLATFORM_API.observability}/legacy-masterdata`, { sourceKey, detail }).catch(() => undefined),
