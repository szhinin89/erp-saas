...
export function useSubscriberDetailPage(subscriberId: string | undefined) {
  ...
  const reloadDetail = useCallback(async () => {
    ...
    try {
      ...
      const [resolved, globals, menu] = await Promise.all([
        ...
        superAdminService.getSubscriberResolvedMenu(subscriberId),
      ]);
  ...
  const loadEntitlements = useCallback(async () => {
    if (!subscriberId) return;
    const snap = await superAdminService.getSubscriberEntitlements(subscriberId);
    setEntitlements(snap ?? null);
  }, [subscriberId]);

  const loadTenantUsers = useCallback(async () => {
    if (!subscriberId) return;
    const rows = await superAdminService.getSubscriberTenantUsers(subscriberId);
    setTenantUsers(rows);
  }, [subscriberId]);
