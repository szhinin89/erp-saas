...
import { superAdminService, type LegacyEndpointUsageDashboard, type PlatformMetricsSnapshot } from '../api/superAdminService';
...
export function SuperAdminObservabilityPage() {
  ...
  const load = useCallback(() => {
    ...
    setError(null);
    Promise.all([
      superAdminService.getPlatformMetrics(),
      superAdminService.getLegacyEndpointUsage(),
      superAdminService.getObservabilityHealthIndex(),
    ])
      .then(([m, l, h]) => {
        setMetrics(m ?? null);
        setLegacy(l ?? null);
  ...
  return (
    <SuperAdminCrudTemplate title="Observability" subtitle="Métricas platform, health checks y uso de endpoints legacy">
