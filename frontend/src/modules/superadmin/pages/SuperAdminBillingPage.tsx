...
type BillingSummary = NonNullable<Awaited<ReturnType<typeof superAdminService.getPlatformBillingSummary>>>;
...
export function SuperAdminBillingPage() {
  ...
  useEffect(() => {
    Promise.all([
      superAdminService.getPlatformBillingSummary(),
      superAdminService.getPlatformBillingInvoices(30),
      superAdminService.getPlatformBillingOverdue(),
    ])
      .then(([s, inv, od]) => {
        setSummary(s ?? null);
        setInvoices(inv);
