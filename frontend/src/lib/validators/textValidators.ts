...
export function allowsLettersKey(
...
export function allowsAlphanumericKey(
  e: React.KeyboardEvent<HTMLInputElement>,
): boolean {
  const { key, ctrlKey, metaKey } = e;
  if (ctrlKey || metaKey) return true;
...
export function allowsPhoneDigitKey(
  e: React.KeyboardEvent<HTMLInputElement>,
): boolean {
  const { key, ctrlKey, metaKey } = e;
  if (ctrlKey || metaKey) return true;
...
export const zSkuCode = z
  ...
  .regex(
    /^[A-Z0-9\-_]*$/,
    "SKU: solo mayúsculas, números, guiones y guiones bajos.",
  );
