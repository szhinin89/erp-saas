 ...
 */
export function ZHFilterBar({ children, chips, onClear, clearLabel = 'Limpiar filtros', disabled }: ZHFilterBarProps) {
  return (
    <div className="zh-filterbar">
      {chips && <div className="zh-filterbar__chips">{chips}</div>}
      {children}
      {onClear && (
        <ZHBtn variant="ghost" size="sm" type="button" className="zh-filterbar__clear"
          onClick={onClear} disabled={disabled}>
          <span className="material-symbols-outlined zh-icon-sm">filter_alt_off</span>
          {clearLabel}
        </ZHBtn>
      )}
    </div>
