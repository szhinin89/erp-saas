...
export function ZHToast() {
  ...
  return (
    <div className="zh-toast-container" aria-live="assertive">
      {queue.map((item) => (
        <ZHToastItem key={item.id} id={item.id} type={item.type} message={item.message}
          persistent={item.persistent} createdAt={item.createdAt} onDismiss={dismiss} />
      ))}
    </div>
  );
}
...
function ZHToastItem(props: {
  ...
  persistent: boolean; createdAt: number; onDismiss: (id: string) => void;
}) {
  ...
  return (
    <div className={`zh-toast zh-toast--${type}`} role="alert">
      ...
      <button type="button" className="zh-toast__close" onClick={handleDismiss}
