...
export function ElectronicDocumentSriMessages({ messages }: Props) {
  ...
  if (messages.length === 0) {
    return <p className="edm-hint-sm">{t('electronicDocuments.diagnostic.messagesEmpty')}</p>;
  }
      ...
      {messages.map((m, idx) => (
        <div key={idx} className="edm-message-card">
          <div className="edm-message-header">
            <Badge variant={messageTypeBadgeVariant(m.messageType)} label={m.messageType} upper />
            {m.code && <span className="edm-message-code">#{m.code}</span>}
            <span className="edm-hint-sm">{formatDateTime(m.occurredAtUtc)}</span>
          </div>
          <div className="edm-message-text">{m.message}</div>
          {m.additionalInfo && <div className="edm-message-additional">{m.additionalInfo}</div>}
        </div>
      ))}
    </div>
  );
