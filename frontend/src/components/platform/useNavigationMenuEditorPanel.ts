...
import {
  ...
  findSiblingArray,
  moveInPlace,
} from '../../modules/platform/navigationMenuEditorModel';
...
export function useNavigationMenuEditorPanel(splitWorkspace: boolean) {
  ...
  const load = useCallback(async () => {
    setError('');
    setSuccess(false);
    const data = await platformService.getNavigationMenu();
  ...
  const handleCreateNavItemSubmit = async (e: FormEvent) => {
    ...
    setError('');
    try {
      await platformService.createNavigationMenuItem({
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await platformService.reorderNavigationGroups(orderedGroupIds);
      const levels: NavItemSiblingOrderLevel[] = [];
      for (const g of menu.groups) {
        collectItemLevels(g.id, g.rootItems, null, levels);
      }
      await platformService.reorderNavigationItemLevels(levels);
      setSuccess(true);
      await load();
    } catch (e) {
