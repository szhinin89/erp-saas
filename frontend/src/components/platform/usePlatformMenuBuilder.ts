...
import { usePlatformMenuBuilderActions } from './usePlatformMenuBuilderActions';
import { usePlatformMenuBuilderCrmEffects } from './usePlatformMenuBuilderCrmEffects';
import { usePlatformMenuBuilderSharedEffects } from './usePlatformMenuBuilderSharedEffects';

export function usePlatformMenuBuilder(crmWorkspace: boolean) {
  ...
  const reloadArbol = useCallback(async () => {
    try {
      const rows = await platformService.getFuncionalidadesArbol();
      setArbol(rows);
    } catch {
      setArbol([]);
    }
  ...
  usePlatformMenuBuilderCrmEffects({
    crmWorkspace,
    planId,
    setPlanId,
    setSub,
  ...
  usePlatformMenuBuilderSharedEffects({
    crmWorkspace,
    planId,
    plans,
    editorMainTab,
...
export type UsePlatformMenuBuilderReturn = ReturnType<typeof usePlatformMenuBuilder>;
