...
import type { WizardStep } from './PlatformMenuBuilderCrmModals';

export type PlatformMenuBuilderCrmWorkspaceHandlersParams = Pick<
...
export function usePlatformMenuBuilderCrmWorkspaceHandlers(params: PlatformMenuBuilderCrmWorkspaceHandlersParams) {
  const {
    t,
    planId,
    setPlanId,
  ...
  const createNewPlan = async () => {
    ...
    setErr('');
    try {
      const newPlanId = await platformService.createCommercialPlan({
        code,
        name: normalizedName,
        shortLabel,
        isActive: true,
      ...
      const nextPlans = await platformService.listCommercialPlansAdmin();
      setPlans(nextPlans);
  ...
  const inheritFromPreviousPlanNow = async () => {
    ...
    setErr('');
    try {
      const bundle = await platformService.getPlanMenu(previousPlanId);
