...
export function buildItemEditorSchema(mode: ItemEditorMode) {
  return z.object({
    ...
    barcode: mode === 'create'
      ? z.string().trim().min(1, 'El código de barras es obligatorio.').max(100, 'El código de barras no puede exceder 100 caracteres.')
      : z.string().optional(),
    barcodeType: mode === 'create'
      ? z.string().min(1, 'El tipo de código de barras es obligatorio.')
      : z.string().optional(),
    observations: z.string().trim().max(500, 'Las observaciones no pueden exceder 500 caracteres.').optional(),
...
export type ItemEditorFormValues = z.infer<ReturnType<typeof buildItemEditorSchema>>;
