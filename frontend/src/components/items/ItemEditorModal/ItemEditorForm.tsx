...
import { sriLookupService, type SriVatRateLookup } from '../../../modules/items/catalog/api/catalogService';
...
import type { CreateItemInitialData, ItemEditorMode, ItemCreatedResult } from './types';
...
export function ItemEditorForm({ mode, existingItem, initialData, onClose, onSaved }: Props) {
  const { register, handleSubmit, setError, watch, formState: { errors, isSubmitting } } = useFormContext<ItemEditorFormValues>();
  ...
  const vatRatesState = useAsync(() => sriLookupService.vatRates().catch(() => [] as SriVatRateLookup[]));
    ...
    try {
      if (isUpdate && existingItem) {
        const item = await itemService.update({
          ...
          baseSalePrice: values.updatePrice ? (values.salePrice ?? existingItem.baseSalePrice ?? 0) : (existingItem.baseSalePrice ?? 0),
        ...
        onSaved({ id: item.id, sku: item.sku, shortName: item.shortName, baseSalePrice: item.baseSalePrice });
        return;
      }
      ...
      if (!handledAsFieldErrors) {
        setError('root', { type: 'server', message: formatApiRequestError(err, { generic: isUpdate ? 'No se pudo actualizar el ítem.' : 'No se pudo crear el ítem.' }) });
      }
    }
  });
  ...
  return (
    <form onSubmit={onSubmit}>
        ...
        <ZHFormSection title="Actualizar Item existente">
          <ZHGrid cols={3}>
            ...
            <ZHField label="Estado" readOnly density="compact">
              <div><Badge variant={existingItem.isActive ? 'green' : 'gray'} label={existingItem.isActive ? 'Activo' : 'Inactivo'} /></div>
            </ZHField>
          </ZHGrid>
        </ZHFormSection>
      )}

      <ZHFormSection title="Información del Item" description="Datos del producto en el catálogo.">
        ...
        <ZHGrid cols={2}>
          <ZHField label="Tipo de ítem" required fieldError={errors.itemTypeId?.message}
            hint={isUpdate ? 'El tipo de ítem no se puede cambiar luego de creado.' : undefined} hintType="muted">
        ...
        <ZHField label="Observaciones" fieldError={errors.observations?.message}>
      ...
      <PriceAndProfitability
        ...
        xmlVatPercent={purchaseContext?.vatCode ? vatRateByCode.get(purchaseContext.vatCode)?.percentage : undefined}
        register={register}
      ...
      <ZHFormActions onCancel={onClose} hideDraft saveButtonType="submit" disableSave={isSubmitting}
        labels={{ save: isSubmitting ? (isUpdate ? 'Actualizando...' : 'Creando...') : (isUpdate ? 'Actualizar Item' : 'Crear Item') }} />
    </form>
  );
}

/** "¿Qué llegó del proveedor?" — solo lectura, nunca editable, igual en ambos modos. */
function PurchaseInfoReadOnly({ purchaseContext }: {
 ...
 */
function PriceAndProfitability({ purchaseContext, salePrice, updatePrice, saleVatCode, vatRateOptions, xmlVatPercent, register, fieldError, isUpdate }: {
  ...
  const dc = getDecimalConfig();
  const costFinal = purchaseContext ? purchaseContext.unitCost * (1 - (purchaseContext.discountPct ?? 0) / 100) : null;
  const hasCost = costFinal != null;
  const hasPrice = salePrice != null && salePrice > 0;
  const salePriceValue = salePrice ?? 0;
  const itemVatPercent = vatRateOptions.find(v => v.code === saleVatCode)?.percentage;
  const hasItemVat = itemVatPercent != null;
  const priceFinal = hasPrice && hasItemVat ? salePriceValue * (1 + itemVatPercent / 100) : null;
  const marginAmount = hasCost && hasPrice ? calcMarginAmount(costFinal, salePriceValue) : 0;
  const marginPct = hasCost && hasPrice ? calcMarginPercent(costFinal, salePriceValue) : 0;

  return (
    <ZHFormSection title="Precio y Rentabilidad" description="El precio y el IVA son editables en ambos modos — la simulación es solo una ayuda visual, no requiere guardarse.">
      <ZHGrid cols={2}>
        <ZHField label="IVA XML" readOnly density="compact">
          <input value={xmlVatPercent != null ? `${formatMoney(xmlVatPercent, dc.percentage)}%` : '—'} disabled />
        </ZHField>
        <ZHField label="IVA del Item">
          <select {...register('saleVatCode')}>
            <option value="">Sin IVA configurado</option>
            {vatRateOptions.map(v => <option key={v.code} value={v.code}>{v.name} ({formatMoney(v.percentage, dc.percentage)}%)</option>)}
          </select>
        </ZHField>
      </ZHGrid>
      ...
      <ZHGrid cols={2}>
        ...
        <ZHField label="Actualizar precio del Item">
          <label className="zh-checkbox-label">
            ...
            <span>{isUpdate ? 'Usar este precio como nuevo precio del Item' : 'Usar este precio como precio inicial del Item'}</span>
          </label>
        </ZHField>
      </ZHGrid>
      ...
      <div className="citm-margin-sim">
        <ZHGrid cols={3}>
          <div className="citm-margin-sim__item">
            ...
            <span className="citm-margin-sim__value">{hasCost ? formatMoneyWithSymbol(costFinal, dc.purchaseUnitPrice) : '—'}</span>
          </div>
          <div className="citm-margin-sim__item">
          ...
          <div className="citm-margin-sim__item">
            ...
            <span className="citm-margin-sim__value">{hasItemVat ? `${formatMoney(itemVatPercent, dc.percentage)}%` : '—'}</span>
          </div>
          <div className="citm-margin-sim__item">
            <span className="citm-margin-sim__label">Precio final</span>
            <span className="citm-margin-sim__value">{priceFinal != null ? formatMoneyWithSymbol(priceFinal, dc.salesUnitPrice) : '—'}</span>
          </div>
          <div className="citm-margin-sim__item">
            <span className="citm-margin-sim__label">Ganancia</span>
            <span className={`citm-margin-sim__value ${hasCost && hasPrice && marginAmount < 0 ? 'citm-margin-sim__value--neg' : ''}`}>
              {hasCost && hasPrice ? formatMoneyWithSymbol(marginAmount, dc.salesUnitPrice) : '—'}
            </span>
          </div>
          <div className="citm-margin-sim__item">
            <span className="citm-margin-sim__label">Margen</span>
            <span className={`citm-margin-sim__value citm-margin-sim__value--pct ${hasCost && hasPrice && marginPct < 0 ? 'citm-margin-sim__value--neg' : ''}`}>
              {hasCost && hasPrice ? `${formatMoney(marginPct, dc.percentage)}%` : '—'}
            </span>
          </div>
        </ZHGrid>
      </div>
      {!updatePrice && hasPrice && (
        <ZHFormAlert type="info" message={`El precio no se guardará: active "Actualizar precio del Item" para usarlo como ${isUpdate ? 'nuevo precio' : 'precio inicial'}.`} />
      )}
    </ZHFormSection>
  );
}
