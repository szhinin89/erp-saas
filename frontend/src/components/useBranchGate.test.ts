...
describe('useBranchGate', () => {
  it('no abre el gate cuando ya existe una sucursal activa', async () => {
    ...
    useActiveBranchStore.setState({ branch: { id: 'branch-1', name: 'Matriz', isMainBranch: true } });
  ...
  it('DirectToDefault con default autorizado ejecuta switch automático sin mostrar selector', async () => {
    ...
    vi.mocked(sessionService.switchBranch).mockResolvedValue({ id: 'branch-1', name: 'Matriz', isMainBranch: true });

    setCompany('company-1');
    const { result } = renderHook(() => useBranchGate());

    await waitFor(() => expect(sessionService.switchBranch).toHaveBeenCalledWith('branch-1'));
  ...
  it('error al cambiar de sucursal manualmente se expone en error y no cierra el gate', async () => {
    ...
    vi.mocked(sessionService.switchBranch).mockRejectedValue({ isAxiosError: true, response: undefined });
