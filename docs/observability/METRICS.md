...
| Métrica | Evento |
|---------|--------|
| `security.cross_company_denied` | Empresa fuera del subscriber |
...
`subscriber_id`, `company_id`, `endpoint`, `request_type`, `correlation_id`

**Prohibido en métricas/logs:** JWT, emails, passwords, secrets.

## Log scope (HTTP)

`SecurityCorrelationMiddleware`: SubscriberId, CompanyId, UserId, SessionId, RequestId.
