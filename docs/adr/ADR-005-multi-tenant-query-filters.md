...
Decisión original (ex `docs/decisions/ADR-004`, superseded): el modelo multi-tenant nació con `SubscriberId` en entidades de negocio, filtros globales en `ErpDbContext`, índices únicos compuestos `(SubscriberId, Code)` y JWT como fuente de tenant en la API. La consolidación `SubscriberId → TenantId` (FASE 4, ver [`docs/STATUS.md`](../STATUS.md)) y la eliminación de SuperAdmin/Platform (ver [`ERP_CORE_FREEZE.md`](../../ERP_CORE_FREEZE.md)) actualizaron la nomenclatura; los principios de aislamiento por filtros globales y JWT-as-source-of-tenant siguen vigentes en esta ADR-005.
