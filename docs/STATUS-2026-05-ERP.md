...
**Fecha de referencia:** 2 de mayo de 2026  
...
| **Clientes (maestro)** | Tabla `customers`; API `/api/customers` (CRUD + disable/enable); UI `/catalog/customers`; permisos `catalog.customers.*`; auditoría `UserActivity` (`entityType: Customer`). Migración **`20260502034035_AddCustomersTable`**. |
...
- **Backend:** .NET 10 (`net10.0`), SDK mínimo en `backend/src/global.json` (alineado a CI en GitHub), ASP.NET Core Web API, JWT, EF Core 10, PostgreSQL (Npgsql).
...
└── docs/STATUS-2026-05-ERP.md     # este documento (estado del proyecto)
```
...
- `20260502034035_AddCustomersTable` — maestro **clientes** por tenant (`tenant_id` + índice único tipo+número documento).
...
**Controladores (ruta base `/api/...`):** Auth, Access, Products, catálogos (brands, product types, units, tax rates, tariffs, categories, subcategories, product lines), Accounting (accounts, journal entries), Tenants, SuperAdmin, Security, **Branches**, **Geography** (solo lectura para combos), **Customers** (maestro clientes).
...
- `CustomersController` — listado (`activeStatus`, `search`), detalle, alta, actualización, `PATCH .../disable` y `.../enable`; permisos `perm:catalog.customers.*` (deshabilitar = `delete`, reactivar = `update`).
...
| `/dashboard` | Inicio |
| `/products`, `/catalog/*` | Productos y catálogos (incluye **`/catalog/customers`** — clientes) |
...
**Servicios clave:** `branchService.ts` (branches + geography con `normalizeGeographyList` y lectura tolerante de envelope `responseObject` / `ResponseObject`); **`customerService.ts`** (mismo patrón de envelope para `/api/customers`).
...
5. Abrir el ERP en el navegador y comprobar login + **Sucursales** y carga de países; si el perfil tiene permisos, **Clientes** en `/catalog/customers`.
...
- `dotnet test` en los cuatro proyectos `*.Tests` (Domain, Application, Infrastructure, API). Application incluye pruebas de **`CreateCustomerHandler`** (alta, duplicado, email inválido).
...
- **Perfiles:** asignar `catalog.customers.*` donde corresponda; seguir ampliando permisos para nuevas pantallas.
...
*Fin del informe. Archivo canónico: `docs/STATUS-2026-05-ERP.md`. Actualizá este archivo cuando cambie algo relevante (migraciones, puertos, flujos de auth o despliegue).*
