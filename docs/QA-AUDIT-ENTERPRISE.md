...
- EF global filters vía `EnterpriseQueryFilterConfigurator` + `ISubscriberScopedEntity`.
- `ICurrentSubscriber` lee JWT — no body.
- Sin auth → `SubscriberId = Guid.Empty` → cero filas.
...
- [ ] `/saas/companies` crear/editar empresa
