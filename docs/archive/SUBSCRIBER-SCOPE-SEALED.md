...
> ⚠️ **Archivado (2026-07-23).** Decisión de equipo tomada sobre la discrepancia señalada en auditoría de cierre 2026-06-08: [`docs/STATUS.md`](../STATUS.md) registra "FASE 1 — ERP Kernel Cleanup (2026-06-05): Eliminado Billing domain, Subscriptions domain, Platform entities, Commercial plans, Entitlements", excluidos **permanentemente** del ERP Core por [`ERP_CORE_FREEZE.md`](../../ERP_CORE_FREEZE.md). Las tablas/clases descritas abajo (`subscribers`, `subscriber_billing_profile`, `SubscriberBillingProfile`, `SubscriberSubscription`, etc.) **no existen** en el esquema ni en `ERP.Domain`/`ERP.Application`, y no forman parte del roadmap del ERP Core. Movido de `AI-RULES/SUBSCRIBER-SCOPE-SEALED.md` (dejó de ser normativo Nivel 2) a este archivo histórico. Se conserva como registro de un diseño SaaS ya descartado del repo actual — no usar para decisiones ni implementación. Arquitectura vigente: [`docs/ARCHITECTURE.md`](../ARCHITECTURE.md) (Nivel 1).

> **Estado histórico: era SEALED como Nivel 2 normativo hasta 2026-07-23.**
> Cualquier reactivación de este modelo requeriría un ADR nuevo y volver a promoverlo
> fuera de `docs/archive/`.
...
Referencia canónica (histórica): [AI-RULES/CORE-ARCHITECTURE.md](../../AI-RULES/CORE-ARCHITECTURE.md#subscriber-scope--modelo-canónico-sellado)
