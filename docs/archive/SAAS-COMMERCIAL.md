...
> ⚠️ **Documento desactualizado.** FASE 1 — ERP Kernel Cleanup (2026-06-05) eliminó por completo esta capa: Billing domain, Subscriptions domain, Commercial plans, Entitlements y sus servicios/controllers/jobs (ver [`docs/STATUS.md`](../STATUS.md)). Las tablas, interfaces y providers descritos abajo (`commercial_plans*`, `subscriber_subscriptions`, `ICommercialPlanLimitService`, `Max*LimitUsageProvider`, etc.) **no existen** en el código actual. Se conserva como referencia histórica de un diseño SaaS futuro; no usar para decisiones ni implementación. La arquitectura vigente vive en [`docs/ARCHITECTURE.md`](../ARCHITECTURE.md).
...
| Crear | `perm:erp.companies.create` + límite plan |
| Actualizar perfil | `perm:erp.companies.update` |

API: `GET/POST/PUT /api/companies/*`, `POST /api/auth/switch-company`.

Frontend: `/companies`, `/select-company`, `CompanySwitcher`.
