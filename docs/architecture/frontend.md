...
- **Inputs:** `ZhDecimalInput`/`ZhCurrencyInput` aceptan `precision` (prioridad `decimals` > `precision` > legacy 2). El input normaliza coma/paste a punto canónico y redondea con el mismo motor; entrar y salir sin editar no reescribe el valor ni emite `onChange`.
