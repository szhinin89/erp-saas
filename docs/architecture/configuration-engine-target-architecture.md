...
- **Qué tabla eliminar:** `GeneralParameter` (migrar sus 5 keys `decimal.*` a `org_settings` con scope `Company`, key `presentation.decimal.*`, registradas en `ConfigurationDefinition`). **Superado (ERP-PRECISION-POLICY-SSOT-CLEANUP-04):** las keys `presentation.decimal.*` fueron eliminadas; la precisión decimal vive solo en `CompanyPrecisionPolicy` (definición única `PrecisionPolicyDefinitions`).
...
| `CreditTerm` | **Frontera resuelta — ver [ADR-033](../decisions/ADR-033-payment-term-ssot-and-document-schedules.md)** | `PaymentTerm` es el SSOT operativo de plazos de pago/cobro; `CreditTerm` queda explícitamente fuera de alcance (infraestructura completa sin caso de uso transaccional conectado), sin fusionarse con `PaymentTerm`. Ya no es deuda abierta de este documento |
| `PaymentTerm` | **Mantener — SSOT operativo, ver [ADR-033](../decisions/ADR-033-payment-term-ssot-and-document-schedules.md)** | Catálogo tenant-wide, consumido por `CompanyBpTradingSettings.PaymentTermId` (default cliente) y por `CompanyBpPurchaseSettings.PaymentTermId` (default proveedor, company-scoped). Migración completa: `SupplierRoleConfig` ya no tiene campo de condición de pago propio |
...
- ~~Resolución de la frontera conceptual `CreditTerm` vs `PaymentTerm`~~ — resuelta por [ADR-033](../decisions/ADR-033-payment-term-ssot-and-document-schedules.md): `PaymentTerm` es el SSOT operativo, `CreditTerm` queda fuera de alcance. Ya no es deuda pendiente.
