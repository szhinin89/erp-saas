...
| `PaymentTerm` | **Mantener — SSOT operativo, ver [ADR-033](../decisions/ADR-033-payment-term-ssot-and-document-schedules.md)** | Catálogo tenant-wide, consumido por `CompanyBpTradingSettings.PaymentTermId` (default cliente) y por el default de proveedor (`SupplierRoleConfig.PaymentTermId`, pendiente migrar a company-scope por ADR-033) |
