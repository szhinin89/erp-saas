...
> ⚠️ **Vigencia sin verificar (Bloque 16B, 2026-08-07).** Este archivo se autodeclaraba "Authoritative — generated from controller scan" pero (1) no tiene fecha/timestamp de generación, (2) no existe ningún script en `tools/`/`scripts/` que lo regenere automáticamente, y (3) su "Ghost Endpoint Inventory" lista como no-implementados varios módulos que `STATUS.md`/`FEATURES.md` documentan hoy como **FROZEN/CLOSED** (Sales Invoice, Establecimientos, Puntos de Emisión, Pricing Engine v2, etc.). **`STATUS.md` y `FEATURES.md` son la fuente de verdad vigente sobre qué endpoints/módulos existen — no este archivo.** Antes de usar este registro para decidir si un endpoint existe, verificar contra el código real (`backend/src/ERP.API/Controllers/`) o contra `STATUS.md`/`FEATURES.md`. Pendiente: regenerar este archivo desde un scan real de controllers, con fecha de generación, o formalizar su automatización.

**Status:** Sin verificar — no autodeclarar "Authoritative" hasta regenerarse con fecha de generación explícita.
**Rule (si se revalida):** El frontend no debería llamar a un endpoint no listado aquí. Ghost calls deben guardarse o eliminarse — pero verificar primero que la lista de "ghost endpoints" de abajo sigue vigente.
