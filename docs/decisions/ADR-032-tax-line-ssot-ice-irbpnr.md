...
- **Pendiente — `ELECTRONIC-DOCUMENTS-IRBPNR-CATEGORY-01`** (registrado 2026-08-29, Subfase 5C): `SriTaxCategoryCodeResolver` (`ERP.Infrastructure/Services/ElectronicDocuments/`) hoy solo reconoce `VAT→"2"` e `ICE→"3"`. IRBPNR ya llega completo como `ElectronicDocumentDetailTax` desde `SalesInvoiceElectronicDocumentDataProvider` (Subfase 5C) — falta extender el resolver para `IRBPNR→"5"` en un ticket controlado de ElectronicDocuments (toca infraestructura FROZEN de ADR-023, requiere su propio proceso de evolución). No se tocó en 5C porque `InvoiceXmlBuilder`/`SriTaxCategoryCodeResolver` estaban explícitamente fuera de su alcance.
