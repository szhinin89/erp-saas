...
**Rediseñada (2026-09-03).** El detalle completo de arquitectura, agregado `RetentionDocument`, entidad `RetentionDocumentLine`, estados/transiciones, flujo desde Gastos, flujo futuro desde Compras, impacto en CxP/contabilidad, reglas de elegibilidad tributaria SRI, fases de implementación (`RETENTIONS-ELIGIBILITY-01` + E1-A a E1-E), riesgos y decisiones aprobadas está documentado en [`RETENTIONS-MODULE-DESIGN-01.md`](./RETENTIONS-MODULE-DESIGN-01.md) — no se repite aquí para evitar dos fuentes de verdad.

**Resumen:** Retenciones deja de plantearse como campos directos en `ExpenseLine`/`ExpenseDocument` y pasa a ser un módulo independiente (`ERP.Domain/Modules/Retentions`, etc.) con relación genérica a su documento origen (`SourceDocumentType`/`SourceDocumentId`). Antes de confirmar, valida que la empresa sea agente de retención (reutilizando `Company.WithholdsRenta`/`WithholdsVat`/`IsAccountingReq`/`SpecialTaxpayerNo`, ya existentes) y que el proveedor no esté exento (`SupplierRoleConfig.IsRetentionExempt`). Gastos es el primer consumidor implementado; Compras sigue usando `IssuedWithholding` sin cambios hasta una fase de migración separada. La emisión sigue siendo manual (el usuario decide si aplica), pero se integra dentro de la confirmación transaccional del gasto — no queda como acción posterior olvidable: confirmar, crear retención, aplicar en CxP y generar asiento ocurren todo o nada.
