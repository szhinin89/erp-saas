...
**Baseline:** Frontend Governance v1.0 (2026-05-21)  
...
- [ ] Usuario tenant Admin + SuperAdmin disponibles
...
| T7 | Banner impersonación SuperAdmin (si aplica) no rompe layout | | |
...
| Ruta | Checks |
|------|--------|
| `/configuracion/empresa` | Secciones, tabla SRI lectura |
| `/configuracion/sri` | Tab datos, ambiente, WSDL |
| `/configuracion/facturacion` | Logo, textarea footer |
...
| `/select-subscriber` | Excepción documentada |
...
- Menu builder: drag/drop intacto tras migración `smb-*` / `smp-*`.
- Impersonación: volver al panel global desde banner.
