...
- **Pendiente nuevo registrado** — `PURCHASE-RETURN-CONCURRENCY-TESTCONTAINERS-01`: tras los dos fixes anteriores, la suite Docker/Testcontainers de concurrencia de `PurchaseReturn` (`AuthorizePurchaseReturnConcurrencyTests`, `AuthorizePurchaseReturnSequenceConcurrencyTests`, `AuthorizePurchaseReturnLockAConcurrencyTests`) sigue fallando en runtime — `DbUpdateConcurrencyException` real dentro de `SaveChangesWithSequenceRetryAsync` (`AuthorizePurchaseReturnUseCases.cs`), reproducible incluso en aislamiento total (un solo test, sin paralelismo). Estaba oculto porque la suite nunca compiló desde 5D-1. Causa raíz aún sin diagnosticar — no es un guard de IRBPNR ni algo introducido por 5E; probablemente relacionado con RowVersion/xmin/tracking entre contextos EF distintos dentro del mismo test. Requiere ticket propio; no se tocó `AuthorizePurchaseReturnHandler` ni `StockRepository` para no exceder el alcance aprobado.
