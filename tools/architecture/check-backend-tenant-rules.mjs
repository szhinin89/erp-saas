...
export const CHECK_NAME = 'backend-tenant-rules';
...
export function runCheckBackendTenantRules() {
  const rules = loadConfig('architecture-rules.json');
  const cfg = rules.backend.tenant;
...
if (process.argv[1]?.endsWith('check-backend-tenant-rules.mjs')) {
  const { printCheckResult } = await import('./shared/report-utils.mjs');
  process.exit(printCheckResult(runCheckBackendTenantRules()) ? 0 : 1);
}
