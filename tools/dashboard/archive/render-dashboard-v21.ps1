# =============================================================================
# ZH Technologies
# Progress Dashboard v21
# Engineering Health Report Renderer
#
# Evoluciona v20 (sin modificarlo) agregando la seccion "Architecture Risk
# Map": por dominio, cuantos modulos/features/procesos tiene y cual es su
# nivel de riesgo agregado -- leido directamente de impact.json (calculado
# por analyze-impact.ps1). El renderer no deriva riesgo por su cuenta, solo
# agrega lo que el analizador ya calculo con evidencia real.
#
...
Write-Host " Dashboard Renderer v21"
Write-Host "==============================================" -ForegroundColor Cyan
Write-Host ""
...
function Get-RiskRankV21($level)
{
    switch($level)
    {
        "CRITICAL" { return 4 }
...
foreach($domainImpact in $impactDomains)
{
    ...
    foreach($module in $modules)
    {
        if((Get-RiskRankV21 $module.risk) -gt (Get-RiskRankV21 $domainRisk))
        {
...
# =============================================================================
# Architecture Risk Map HTML
# =============================================================================

$riskMapCardHtml = @"
<div class='card'>
<h2>Architecture Risk Map</h2>
<p>Engineering Risk Coverage: $($impactData.coverage.percentage)% ($($impactData.coverage.mappedFeaturePoints) / $($impactData.coverage.totalFeaturePoints) capability points mapped to a verified process)</p>
<table>
...
"<html>",
"<head>",
"<title>ZH Technologies ERP Engineering Dashboard v21</title>",
...
$technicalDebtTrendHtml,
$releaseRecommendationHtml,
"<h1>ZH Technologies ERP Engineering Dashboard v21</h1>",
...
Write-Host ""
Write-Host "Dashboard v21 generated successfully." -ForegroundColor Green
Write-Host $Output
