...
export function runValidateFrontendRoutes() {
  ...
  const platformRoutesExists = fs.existsSync(platformRoutesPath);

  for (const rel of ROUTER_FILES) {
    const file = path.join(REPO_ROOT, rel);
    if (!fs.existsSync(file)) {
      // platformRoutes.tsx is part of the Platform Control Plane shell, not the
      // ERP Core perimeter (see ERP_CORE_FREEZE.md — ERP never depends on Platform).
      // Its absence here is expected and must not fail the ERP-core build.
      if (rel === 'frontend/src/routes/platformRoutes.tsx') continue;
