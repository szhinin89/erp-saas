...
- **Permisos ≠ política de flujo documental**: los permisos definen quién puede ejecutar acciones; `DocumentFlowPolicy` define cómo se comporta cada tipo de documento por empresa; los estados definen en qué etapa está un documento concreto; los eventos de dominio definen qué efectos genera al avanzar. Nunca usar una política documental como permiso de usuario. Detalle: [docs/architecture/security.md § Permisos vs. política de flujo documental](docs/architecture/security.md#permisos-vs-política-de-flujo-documental-document-flow-policy-01).
