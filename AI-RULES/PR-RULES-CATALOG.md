...
### RULE
Las entidades de dominio no pueden cruzar boundaries de scope sin justificación arquitectónica documentada. **SUBSCRIBER** aquí es histórico — corresponde al Control Plane SaaS eliminado permanentemente del ERP Core (ver [`ERP_CORE_FREEZE.md`](../ERP_CORE_FREEZE.md)); el boundary se conserva como guardrail preventivo por si una futura Platform externa reintroduce ese dominio:
- **SUBSCRIBER** (histórico) no puede contener lógica ERP ni referenciar entidades COMPANY.
...
### RULE
Toda pantalla/componente de `frontend/src/modules/**`, `frontend/src/pages/**` y `frontend/src/templates/**` usa los componentes/clases únicos del Design System documentados en [`AI-RULES/FRONTEND-RULES.md`](FRONTEND-RULES.md#design-system-estándares-únicos-obligatorios): `ZHGrid` (grids de formulario), `ZHToggle` (checkboxes/switches), `.zh-icon-sm/md/lg/xl` (tamaño de íconos, nunca `style={{fontSize}}`), `ZHModalHeader` + `ZHFormActions` (headers/footers de modal), `.prd-tabs`/`.prd-tab-btn` (tabs), `.table` + `.prd-table-wrap` (tablas), `.prd-activity__*` (timeline de actividad). **Prohibido** reintroducir los patrones deprecados de esa tabla (`.pg-form-grid*`, `.zh-inline-check`, `.companies-checkbox-label`, `.toggle-ui`, `.md-page-check`, `.prd-icon-sm`, `.md-modal*`, `.prf-modal-header-main`, `.zh-form-tabs`, `.md-table*`, `.bod-activity__*`, `style={{fontSize}}` en íconos).
...
- Revisión contra [IDENTITY.md](../docs/IDENTITY.md).
...
- Cambios a reglas bloqueantes: PR dedicado + entrada en [STATUS.md](../docs/STATUS.md).
