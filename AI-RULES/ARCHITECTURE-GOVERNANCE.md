...
**Regla GLOBAL:** Sin `subscriber_id`. Sin `company_id`. Sin lógica de negocio.
Referencia: [SUBSCRIBER-SCOPE-SEALED.md](./SUBSCRIBER-SCOPE-SEALED.md)

---

### SUBSCRIBER (Control Plane SaaS)

**Propósito:** Identidad del tenant, billing SaaS, suscripción, seguridad.
Estado: SEALED — ver [SUBSCRIBER-SCOPE-SEALED.md](./SUBSCRIBER-SCOPE-SEALED.md)

| Entidad canónica | Tabla | Propósito único |
|---|---|---|
| `SubscriberBillingProfile` | `subscriber_billing_profile` | Perfil fiscal + recibos |
| `SubscriberBillingAccount` | `subscriber_billing_accounts` | Estado billing SaaS |
| `SubscriberSubscription` | `subscriber_subscriptions` | Plan activo |
| `SaasBillingInvoice` | `saas_billing_invoices` | Facturas SaaS |
| `IdentityUser` | `identity_users` | Usuarios del sistema |
| `RefreshToken` | `refresh_tokens` | Sesiones RTR |
| `CommercialPlan` | `commercial_plans` | Planes de precio |
...
| Área | Documento |
|---|---|
| Subscriber scope sellado | [SUBSCRIBER-SCOPE-SEALED.md](./SUBSCRIBER-SCOPE-SEALED.md) |
